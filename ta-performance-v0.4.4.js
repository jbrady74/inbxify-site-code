/* ============================================================
   ta-performance-v0.4.4.js
   INBXIFY — Performance tab (T-A)
   v0.4.4 (from v0.4.3):
     • NEW Slug column in By-Newsletter table, immediately right of
       Newsletter. Shows the NEWSLETTERS slug — the value the Nexstar
       ingest matches on (report link id 1 carries /nl/<slug>).
       Read from data-nl-slug on .newsletter-wrapper.
     • Table cardinal rules applied: no cell may wrap to a second
       line (th + td nowrap), horizontal padding reduced 18px→9px,
       explicit per-column widths proportioned to real content.
       Fixes v0.4.3 defect where Status + Send time stacked onto two
       lines while the numeric columns ran oversized.
   v0.4.0 (from v0.3.7):
...
(function () {
  ...
  function normalizeStats(j) {
    ...
    if (j.source === 'nexstar' && (j.links || j.items)) {
      return { v: 1, sources: { nexstar: [j] }, settings: { nexstarMultiplier: 1.0 } };
    }
    return j;
  }
  ...
  var TIPS = {
    ...
    clicks: 'Nexstar (\u00d7 your multiplier) + Mailchimp send + resend. Sources shown in the detail view.',
  ...
  function itemLabel(rec) {
    ...
    return esc(articleTitle(rec))
      + (TYPE_LABEL[rec.type] ? '<span class="perf-type">' + TYPE_LABEL[rec.type] + '</span>' : '');
  }
  ...
  function sourceRows(nl) {
    ...
    nexCampaigns(s).forEach(function (c, i) {
      ...
      rows += '<tr><td class="nlm-lbl">Nexstar \u00b7 ' + esc(suffix) + '</td>'
        + '<td class="num">' + fmt(c.delivered) + '</td><td class="num">' + fmt(c.opens) + '</td>'
    ...
    mcCampaigns(s).forEach(function (x) {
      rows += '<tr><td class="nlm-lbl">' + esc(x.label) + '</td>'
        ...
        + '<td class="num">' + fmt(x.c.clicksRaw) + '</td><td class="num"></td><td class="num">' + fmt(x.c.clicksRaw) + '</td><td class="num"></td></tr>';
    });
    if (!mcCampaigns(s).length) {
      rows += '<tr><td class="nlm-lbl">Mailchimp \u00b7 send</td><td class="num perf-dash" colspan="5">\u2014</td><td class="num">' + dash('not ingested') + '</td></tr>'
        + '<tr><td class="nlm-lbl">Mailchimp \u00b7 resend</td><td class="num perf-dash" colspan="5">\u2014</td><td class="num">' + dash('not ingested') + '</td></tr>';
    }
    rows += '<tr><td class="nlm-lbl">Site ' + iCircle(TIPS.site) + '</td>'
      + '<td class="num perf-dash" colspan="2">\u2014 pageviews</td>'
      + '<td class="num perf-dash" colspan="3">\u2014 avg engagement</td>'
  ...
  function contentRows(nl) {
    ...
    var keys = Object.keys(items).sort(function (a, b) { return (items[b].sawIt - items[a].sawIt) || (items[b].clicks - items[a].clicks); });
  ...
  function openNlModal(nl) {
    ...
    overlay.innerHTML =
      ...
      + '<tr><th>Source</th><th class="num">Delivered</th><th class="num">Opens</th><th class="num">Clicks raw</th><th class="num">Multiplier</th><th class="num">Clicks adj.</th><th class="num">Report</th></tr>'
      + sourceRows(nl)
      + '</table><div class="nlm-meta">Multiplier adjusts Nexstar clicks only \u00b7 raw values never change</div></div>'
      + '<div class="nlm-card nlm-content-card"><h4>Content</h4><table>'
      + '<tr><th>Item</th><th class="num">Clicks</th><th class="num">Saw it (est.) ' + iCircle(TIPS.sawit) + '</th><th class="num">On site ' + iCircle(TIPS.onsite) + '</th></tr>'
      + contentRows(nl)
  ...
  function saveMultiplier(nl, value, btn, done) {
    ...
    saveSetting(nl, 'set-multiplier', value, btn, function (ok) {
      if (ok) {
        nl.stats.settings = nl.stats.settings || {};
        nl.stats.settings.nexstarMultiplier = value;
      }
  ...
  var STATE = { channel: 'nl', drill: null };
  ...
  function render() {
    ...
    view.innerHTML = '<div class="ta-perf__card perf-nl-card"><table>'
      ...
      + (NLS.map(rowNl).join('') || '<tr><td colspan="11">' + dash('no newsletters found \u2014 is the newsletter-wrapper list on the page?') + '</td></tr>')
      + '</tbody></table></div>';
  }
