...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function saveEdit() {
    ...
    params.set('hidden-category-id', (item.productId || (slotData && slotData.catId) || ''));
    ...
    // Fire webhook — same pattern as PubPlan tile UI
    fetch(webhookUrl + '?' + params.toString(), { mode: 'no-cors' })
      .then(function () {
        console.log('[CONTENT-LIBRARY] Webhook sent for', item.name, '→', slotLabel);
      })
      .catch(function (err) {
        console.error('[CONTENT-LIBRARY] Webhook error:', err);
      });
      ...
      items.forEach(function (item) {
        ...
        if (isEd) {
          ...
          PUBPLANS.forEach(function (pp) {
            // Only show In Progress PubPlans (or if no status attribute yet, show all)
            if (pp.planningStatus && pp.planningStatus !== 'In Progress') return;
