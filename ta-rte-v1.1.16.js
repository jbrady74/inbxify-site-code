  ...
  class InbxRTE {
    ...
    bindImageEditClickHandler() {
      ...
      this.editorEl.addEventListener('click', (event) => {
        // Trix's attachment wrapper has interactive bits (resize handles,
        // attachment toolbar) — bail if click is on those.
        if (event.target.closest('.trix-button, [data-trix-action]')) return;
        ...
        if (!attachment) {
          console.warn('[RTE] click-to-edit: no Trix.Attachment matched for figure');
          return;
        }
