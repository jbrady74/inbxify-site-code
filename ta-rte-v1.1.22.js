...
(function () {
  ...
  class InbxRTE {
    ...
    _renderFindingCard(f, idx) {
      ...
      return `
        <div class="rte-assess-finding rte-assess-finding-${priority}">
          ...
          ${replacement ? `
            <div class="rte-assess-finding-block">
              <div class="rte-assess-finding-block-label">
                Drop-in replacement
                <button type="button" class="rte-assess-copy-btn" data-copy-idx="${idx}" aria-label="Copy replacement text">
                  <svg viewBox="0 0 16 16" width="11" height="11" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:4px;">
    ...
    _wireResultsCopyButtons() {
      ...
      this._assessPanel.querySelectorAll('.rte-assess-copy-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          const idx = parseInt(btn.getAttribute('data-copy-idx'), 10);
          const f = findings[idx];
          if (!f || !f.replacement_text) return;
          const text = f.replacement_text;
          // Copy to clipboard via the modern API; fallback to legacy execCommand
          const onSuccess = () => {
          ...
          if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(text).then(onSuccess).catch(() => {
              console.warn('[RTE v1.1.22] navigator.clipboard.writeText failed; falling back to execCommand');
    ...
    _legacyCopy(text, onSuccess) {
      ...
      try {
        ...
        console.error('[RTE v1.1.22] copy failed:', e);
      }
...
Target reissue date: ${this._assessTargetDate}
    ...
    injectStyles() {
      ...
      style.textContent = `
        ...
        .rte-assess-copy-btn:hover { opacity:0.85; }
