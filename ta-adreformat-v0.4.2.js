...
(function () {
  ...
  function injectStyles() {
    ...
    var css = [
      ...
      '.ix-arf-ltbar{display:flex;flex-direction:column;gap:0;margin-top:10px;max-width:760px;border:1px solid #e6e2d6;border-radius:8px;padding:4px 10px;background:#fff !important;}',
  ...
  function openEditor() {
    ...
    ed.innerHTML =
      ...
      'container to any shape. <b>\u2702 Crop from source</b> lets you draw a box on the original ad ' +
      'to grab exactly the part you want. <b>Lock ratio</b> holds the container shape; ' +
      '<b>Reset</b> restores the original. <b>Double-click text</b> to edit the words; ' +
      'the toolbar sets font, size, weight, color and alignment, or <b>Use original as image</b> ' +
      'to keep the ad\u2019s exact wording.</div>' +
  ...
  function renderEditor() {
    ...
    S.layers.forEach(function (L, i) {
      ...
      if (L.type === 'img') {
        ...
        node.appendChild(sp);
        // Double-click to edit the words inline on the banner. v0.4.1.
  ...
  function renderLayerToolbar() {
    ...
    function swatchRow(kind, current, list) {
      // kind: 'bg' | 'tc'
      var html = list.map(function (c) {
        var isT = c === 'transparent';
        var sel = (current || (kind === 'bg' ? 'transparent' : '')).toUpperCase() === c.toUpperCase() ? ' sel' : '';
        var style = isT
          ...
          : 'background:' + esc(c) + ';';
        return '<button class="ix-arf-sw" data-kind="' + kind + '" data-c="' + esc(c) + '" title="' + (isT ? 'Transparent' : esc(c)) + '" style="' + style + '"></button>';
      }).join('');
      var cid = kind === 'bg' ? 'ix-arf-bgcustom' : 'ix-arf-tcustom';
      var cval = kind === 'bg'
        ? (/^#/.test(L.bg || '') ? esc(L.bg) : '#ffffff')
        : (/^#/.test(L.color || '') ? esc(L.color) : '#1A3A3A');
      html += '<label class="ix-arf-ltcustom" title="Custom"><input type="color" id="' + cid + '" value="' + cval + '"></label>';
      return html;
    }

    var rows = '';

    // Background row (all layers)
    var bgList = ['transparent'].concat((S.result.brand_colors || []).slice(0, 5));
    rows += '<div class="ix-arf-trow"><span class="ix-arf-rowlbl">Background</span>' +
            '<div class="ix-arf-rowctl">' + swatchRow('bg', L.bg, bgList) + '</div></div>';

    if (L.type === 'img') {
      rows += '<div class="ix-arf-trow"><span class="ix-arf-rowlbl">Image</span><div class="ix-arf-rowctl">' +
        ...
        '<button class="ix-arf-ltbtn" id="ix-arf-layerreset">Reset</button>' +
        '</div></div>';
    }
    ...
    if (L.type === 'text') {
      var fontOpts = ARF_FONTS.map(function (fam) {
        ...
        var seld = (L.font || '') === stack ? ' selected' : '';
        return '<option value="' + esc(stack) + '"' + seld + ' style="font-family:' + esc(stack) + '">' + esc(fam) + '</option>';
      }).join('');
      // Type row: font, size, bold, align
      rows += '<div class="ix-arf-trow"><span class="ix-arf-rowlbl">Text</span><div class="ix-arf-rowctl">' +
        '<select id="ix-arf-tfont" class="ix-arf-tsel" title="Font">' + fontOpts + '</select>' +
        '<input type="number" id="ix-arf-tsize" class="ix-arf-tnum" min="6" max="120" value="' + (L.size || 13) + '" title="Size (px)"><span class="ix-arf-unit">px</span>' +
        '<button class="ix-arf-ltbtn' + ((L.weight || 400) >= 600 ? ' on' : '') + '" id="ix-arf-tbold" title="Bold">B</button>' +
        '<span class="ix-arf-talign">' +
          '<button class="ix-arf-ltbtn' + (L.align === 'left' || !L.align ? ' on' : '') + '" data-al="left" title="Left">L</button>' +
          '<button class="ix-arf-ltbtn' + (L.align === 'center' ? ' on' : '') + '" data-al="center" title="Center">C</button>' +
          '<button class="ix-arf-ltbtn' + (L.align === 'right' ? ' on' : '') + '" data-al="right" title="Right">R</button>' +
        '</span>' +
        '</div></div>';
      // Text color row (clearly separate from Background)
      var tcList = ['#1A3A3A', '#FFFFFF'].concat((S.result.brand_colors || []).slice(0, 4));
      rows += '<div class="ix-arf-trow"><span class="ix-arf-rowlbl">Text color</span>' +
              '<div class="ix-arf-rowctl">' + swatchRow('tc', L.color, tcList) + '</div></div>';
      // Actions row
      rows += '<div class="ix-arf-trow"><span class="ix-arf-rowlbl"></span><div class="ix-arf-rowctl">' +
        '<button class="ix-arf-ltbtn" id="ix-arf-torig" title="Replace with the ad\u2019s exact wording cropped as an image">Use original as image</button>' +
        '<button class="ix-arf-ltbtn" id="ix-arf-layerreset">Reset</button>' +
        '</div></div>';
    }
    ...
    // Unified swatches: data-kind distinguishes bg vs text-color. Discrete
    // click → full re-render is safe. This also FIXES the v0.4.1 bug where
    // background swatches "did nothing" (old handler only matched .ix-arf-bgsw
    // and the markup had drifted). v0.4.2.
    bar.querySelectorAll('.ix-arf-sw').forEach(function (b) {
      b.addEventListener('click', function () {
        var L2 = S.layers[S.selected];
        if (b.dataset.kind === 'tc') L2.color = b.dataset.c;
        else L2.bg = b.dataset.c;
        renderEditor();
      });
    });

    // Custom BACKGROUND color — live node update, no full rebuild.
    ...
    // Custom TEXT color — live node update, no full rebuild.
    var tcust = document.getElementById('ix-arf-tcustom');
    if (tcust) tcust.addEventListener('input', function () {
      var L2 = S.layers[S.selected]; L2.color = tcust.value;
      var n = document.querySelector('.ix-arf-layer[data-i="' + S.selected + '"] > div');
      if (n) n.style.color = tcust.value;
    });

    // Lock-ratio checkbox (image).
    ...
    // Reset (unique id ix-arf-layerreset; v0.3.5 dup-id fix). Restores
    // geometry + image + text style from the layer's snapshot.
