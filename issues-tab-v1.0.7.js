...
(function () {
        ...
        '<div class="ppm-field">' +
          ...
          '<div class="ppm-issue-row">' +
            ...
            '<input type="number" id="ppm-number" class="ppm-input ppm-number-input" ' +
              'data-pp-add-number value="' + esc(String(defaultNumber)) + '" min="1" step="1" autocomplete="off">' +
          '</div>' +
  ...
  function handleAddPubplanSubmit(context, backdrop) {
    ...
    // v1.0.6: Issue Number is numeric again. Edition + Name are
    // free-text. Nothing is required — fields submit as-is, blank
    // or filled.
    var issueNumber     = numberInput  ? (numberInput.value  || '').trim() : '';
    var edition         = editionInput ? (editionInput.value || '').trim() : '';
    var name            = nameInput    ? (nameInput.value    || '').trim() : '';
    var publicationDate = dateInput    ? (dateInput.value    || '').trim() : '';

    var issueName = abbreviation ? (abbreviation + '-' + issueNumber) : issueNumber;
