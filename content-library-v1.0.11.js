// ============================================================
// content-library-v1.0.11.js
// INBXIFY Content Library — T-A Page Tab
// Mounts into #content-library-mount on ContentLibrary tab
// Config from window.TA_CONFIG
//
// v1.0.11: Webhook waits for response before updating local state.
//          Tries cors first, falls back to no-cors.
//          Shows "Assigning…" state and error/retry on failure.
//          Added categoryId to article reader.
//          Fixed hidden-category-id to use categoryId field.
// v1.0.10: Version bump (Jeff)
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function saveEdit() {
    ...
    params.set('hidden-product-id', item.productId || '');
    params.set('hidden-category-id', item.categoryId || item.productId || (slotData && slotData.catId) || '');
    ...
    // Capture edit state before async call
    var savedPubplan = S.edit.pubplan;
    var savedSlot = S.edit.slot;
    ...
    fetch(webhookUrl + '?' + params.toString())
      .then(function (resp) {
        if (resp.ok) {
          ...
          item.pubplanId = savedPubplan || null;
          item.slot = savedSlot ? savedSlot.toUpperCase() : null;
          if (item.pubplanId) {
  ...
  function render() {
    ...
    if (!items.length) {
      ...
      items.forEach(function (item) {
        ...
        h += '<span class="cl-bdg ' + (isAssigned ? 'cl-bdg-asgn' : 'cl-bdg-unasgn') + '">' + (isAssigned ? (item.pubplanName ? item.pubplanName : 'assigned') : 'available') + '</span>';
        h += '</div></div>';
