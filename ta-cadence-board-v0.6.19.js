/* ════════════════════════════════════════════════════════════════
   ...
   THE BOARD TITLE IS WHITE, AND IT FAILS CONTRAST. White on
   --ix-gold measures 2.40:1 against a 4.5:1 floor (3:1 even if
   treated as large text). Requested deliberately as an aesthetic
   call, recorded here so it reads as a decision rather than a miss.
   contrast-audit.py will keep reporting it; that is correct and the
   report should not be silenced. Teal on the same gold measures
   5.11:1 if it is ever revisited, and white becomes legible if the
   bar itself darkens.
...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  function nextBar() {
    ...
    return '<div class="ixcb-nextbar">' +
             ...
             (src ? '<span class="ixcb-nb-src">' + esc(src) + '</span>' : '') +
