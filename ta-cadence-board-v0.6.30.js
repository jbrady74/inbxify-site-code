...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  function slateRows() {
    ...
    for (var i = 0; i < S.slate.length; i++) {
      ...
      shown++;
      var dv = rowDisp(r), tv = rowType(r);
      var popOpen = S.slatePop && S.slatePop.id === r.id;

      rows +=
        '<tr class="ixcb-sl-row' + (dv === 'not-for-newsletter' ? ' ixcb-sl-row--out' : '') + '">' +
          '<td class="ixcb-sl-pg">' + (r.page == null ? '\u2014' : r.page) + '</td>' +
          '<td class="ixcb-sl-sec" title="' + esc(r.section || '') + '">' +
            esc(r.section || '\u2014') + '</td>' +
          '<td>' + slateTypeCell(r, tv) + '</td>' +
          '<td class="ixcb-sl-ttl" title="' + esc(r.name) + '">' + esc(r.name) + '</td>' +
          '<td class="ixcb-sl-img">' + slateImgCell(r) + '</td>' +
          '<td class="ixcb-asn' + (popOpen && S.slatePop.kind === 'iss' ? ' ixcb-asn--open' : '') + '">' +
            slateIssueCell(r) + '</td>' +
          '<td class="ixcb-asn' + (popOpen && S.slatePop.kind === 'disp' ? ' ixcb-asn--open' : '') + '">' +
            slateCell(r, 'disp', SLATE_DISP, dv, 'Adjudicate',
                      'Undecided and Intended follow assignment on their own.') + '</td>' +
          '<td class="ixcb-sl-act">' +
            '<button type="button" class="ixcb-del" data-ixcb="slate-del" ' +
              'data-id="' + esc(r.id) + '" title="Delete this row">\u00d7</button></td>' +
        '</tr>';
    }
    if (!shown) {
      /* A filtered list must never read as an empty one. */
      rows = '<tr><td colspan="8" class="ixcb-sl-none">' +
  ...
  function overviewBody() {
    ...
    return '<div class="ixcb-sl">' +
      ...
      '<div class="ixcb-sl-tw"><table class="ixcb-sl-tbl">' +
        '<colgroup>' +
          '<col class="ixcb-c-pg"><col class="ixcb-c-sec"><col class="ixcb-c-typ">' +
          '<col class="ixcb-c-ttl"><col class="ixcb-c-img"><col class="ixcb-c-iss">' +
          '<col class="ixcb-c-disp"><col class="ixcb-c-act">' +
        '</colgroup>' +
        '<thead><tr>' +
          '<th class="ixcb-th-r">Pg</th><th>Print Section</th>' +
          '<th title="The parse\u2019s assessment when the page was read. Not editable here.">' +
            'Estimated Type</th><th>Title</th>' +
          '<th title="Photos arrived in the Drive drop over photos counted in print. ' +
            'Matching is not built, so arrived reads as a dash.">Images</th>' +
          '<th>PubPlan Assignment</th><th>Status</th><th></th>' +
