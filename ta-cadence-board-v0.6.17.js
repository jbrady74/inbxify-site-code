/* ════════════════════════════════════════════════════════════════
   ta-cadence-board-v0.6.17.js
   INBXIFY T-A — Cadence Board · SKELETON
   Companion: ta-cadence-board-v0.6.16.css — INDEPENDENTLY VERSIONED.
...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  function heroHtml() {
    ...
    var sIss = gateVal('nIss', '2');
    var sEd  = gateVal('nEd', edGuess ? String(edGuess) : '2');
    var sCad = gateVal('cad', '7');
    var sFst = gateVal('first', '');

    var sel = function (key, list, cur, derived) {
      var o = '';
      for (var i = 0; i < list.length; i++) {
        o += '<option value="' + esc(list[i][0]) + '"' +
             (list[i][0] === cur ? ' selected' : '') + '>' + esc(list[i][1]) + '</option>';
      }
      var changed = S.gate && S.gate[key] !== undefined && S.gate[key] !== null;
      return '<select class="ix-picker-input' + (changed ? ' ix-picker-input--changed' : '') +
             '" data-ixcb="gate" data-k="' + key + '"' +
             (derived ? ' title="' + esc(derived) + '"' : '') + '>' + o + '</select>';
    };

    var fstChanged = S.gate && S.gate.first !== undefined && S.gate.first !== null;

    return '<div class="ixcb-hero">' +
      '<div class="ixcb-cover">' +
        '<div class="ixcb-cover-ph">' +
          '<span>Cover</span>' +
          '<span class="ixcb-cover-note">No receptacle for the print cover yet</span>' +
        '</div>' +
      ...
      '<div class="ixcb-setup">' +
        '<div class="ixcb-gate">' +
          '<div class="ixcb-ref">' +
            (lastLbl
              ? 'Last sent <b>' + esc(lastLbl) + '</b>'
              : 'No plan on this title reads as sent, so nothing can be derived from history.') +
          '</div>' +
          '<h3 class="ixcb-gate-h">' +
            (nxt ? 'Set up issues \u2014 next is WLN-' + nxt : 'Set up issues') + '</h3>' +
          '<div class="ixcb-ctrls">' +
            '<label>Split into</label>' +
            sel('nIss', [['1','1 issue'],['2','2 issues'],['3','3 issues']], sIss, '') +
            '<label>Editions each</label>' +
            sel('nEd', [['1','1'],['2','2 (A/B)'],['3','3 (A\u2013C)'],['5','5 (A\u2013E)']], sEd,
                edGuess ? 'Suggested from ' + lastLbl + ', which ran ' + edGuess + '.' : '') +
            '<label>First send</label>' +
            '<input type="date" class="ix-picker-input' + (fstChanged ? ' ix-picker-input--changed' : '') +
              '" data-ixcb="gate" data-k="first" value="' + esc(sFst) + '">' +
            sel('cad', [['7','weekly'],['14','every 2 weeks']], sCad, '') +
          ...
          '</div>' +
          '<div class="ixcb-gate-ro">' +
            (nxt
              ? 'Numbering and editions are suggested from what actually shipped \u2014 never typed, ' +
                'always editable. Nothing is created until you press Create these.'
              : 'Bind data-issue-number and a planning status on pubplan-item, or nothing here ' +
                'can be suggested from history.') +
          '</div>' +
        '</div>' +
      '</div>' +
    '</div>';
  ...
  function onSearchInput(e) {
    ...
    if (el.getAttribute('data-ixcb') === 'gate') {
      S.gate[el.getAttribute('data-k')] = el.value;
      renderAll();
      return;
    }
