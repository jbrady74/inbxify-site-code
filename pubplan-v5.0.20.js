...
//            - Unified dropdown: LBP + 6 TXA categories in one select
//            - Section-level tfCategoryId/tfCategoryNm variables
//            - switchTfMode replaced by handleTfCategoryChange
//            - buildTfCategoryDropdown function added
//            - buildCategoryOptions group map updated for TF
//            - Submit sends tfCategoryId (section-level) not s.catId
//            - Resolves TD-008
...
(function () {
  ...
  // ═══ THE FIND (TF) — CATEGORY + TXA (5) or LBP (1) ═══
  // Architecture:
  //   1. Category dropdown (section-level) — always visible at top
  //   2. Below it: TXA 5-pack grid OR LBP single tile, based on selected category's section-code
  //   3. Category saved in CMS → shows as text + edit pencil (same pattern as FA/TS/BA)
  //   4. Category change warns if slots have data, clears them on confirm
  //   5. Separate TF-CAT POST for category, per-slot POSTs for customer assignments

  // ── Category display: view mode (text + pencil) or edit mode (dropdown) ──
  function renderTfCategory() {
    ...
    var showDropdown = !hasSaved || tfCategoryEditing || !hasCurrent;
    ...
    // After category display renders, update slot visibility
    renderTfSlots();
  }

  // ── Slot visibility: show TXA grid or LBP tile based on mode ──
  function renderTfSlots() {
  ...
  window.handleTfCategoryChange = function(sel) {
    ...
    // Check if existing slots have data that would be cleared
    var hasTxaData = [1,2,3,4,5].some(function(i) { var s = state['TXA-'+i]; return s && s.custId; });
    var hasLbpData = state['LBP-1'] && state['LBP-1'].custId;
    var hasSlotData = (currentTfMode === 'txa' && hasTxaData) || (currentTfMode === 'lbp' && hasLbpData);
    ...
    var doIt = function() {
      ...
      // Set new category + mode
      currentTfMode = newMode;
      tfCategoryId = newCatId;
      tfCategoryNm = newCatNm;
      tfCategoryDirty = (newCatId !== tfOriginalCategoryId);
      tfCategoryEditing = false;

      // Update TXA slot catIds
      for (var i = 1; i <= 5; i++) {
        var s = state['TXA-' + i];
        if (s) s.catId = newCatId;
      }

      // Mark occupied slots dirty for re-submit with new category
      if (newMode === 'txa') {
        for (var i = 1; i <= 5; i++) {
          var s = state['TXA-' + i];
    ...
    if (hasSlotData && newCatId !== tfCategoryId) {
      showInlineConfirm(sel, 'Changing category will clear current slot assignments.', doIt, function() {
  ...
  // ── TXA state + render (5 slots, customer-only) ──
  ...
  // ── LBP state + render (1 slot, customer-only) ──
  ...
  // ── TF progress: category dirty OR slot dirty activates Save ──
  ...
  window.submitSection = async function(section) {
    ...
    if (section === 'gr') {
      ...
      // Category-level POST (Route 1 in Make) — fires when category changed
      if (tfCategoryDirty) {
      ...
      // Per-slot POSTs (Routes 2-6 in Make) — fires for each dirty slot with customer data
      ...
      else{var s=state['LBP-1'];if(s&&s.dirty&&s.custId)slots.push({'hidden-pubplan-id':pubplanId,'hidden-slot-label':'LBP-1','hidden-titleadmin-id':s.titleadminId||'','hidden-section-code':s.secC,'hidden-category-group':s.section,'hidden-customer-id':s.custId,'hidden-customer-name':s.custNm,'hidden-category-id':tfCategoryId,'hidden-category-name':tfCategoryNm,'hidden-tf-mode':'lbp'});}
    }
  ...
  function init() {
    ...
    // FIX #14: Read initial TF mode, then populate unified category dropdown
    ...
    // FIX #14: Read catId from first TXA slot (section-level, all slots share it)
    var firstTxa = state['TXA-1'];
    tfCategoryId = (firstTxa && firstTxa.catId) ? firstTxa.catId : '';
    tfCategoryNm = '';
    if (tfCategoryId) {
      var catEl = document.querySelector('.products-wrapper[data-id="' + tfCategoryId + '"]');
      if (catEl) {
        tfCategoryNm = catEl.dataset.name || '';
        // Derive mode from product's section-code if not already set by CMS
    ...
    // FIX #14: Render category display (view or edit mode)
    // renderTfCategory() calls renderTfSlots() which handles TXA/LBP visibility + rendering
    renderTfCategory();

    renderGr(); renderEm(); renderAllFa(); renderAllTs(); renderAllBa();
