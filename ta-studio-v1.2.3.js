...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
      ...
      '<div class="std-root">' +
        '<div class="ix-hdr">' +
          '<div class="ix-hdr-left">' +
            ...
            '</div>' +
          '</div>' +
          '<span class="ix-badge">v1.2.3</span>' +
        '</div>' +
          ...
          onSave: function (editedHTML) {
            saveArticleBody(articleItemId, editedHTML)
              .then(function () {
                ...
                // v1.2.3: reload so the Article's body-bound DOM
                // (and bodyStatus = Edited) reflect the fresh state.
                studioReloadAfterCMSWrite();
              })
              .catch(function (err) {
    ...
    // refreshing. The delay lets the success toast register visually
    ...
    function studioReloadAfterCMSWrite(opts) {
      ...
      // Defensive: if for any reason the page is already in the middle of
      // unloading, don't queue another reload.
      if (window._studioReloadQueued) return;
      window._studioReloadQueued = true;
      setTimeout(function () {
        // location.reload() with no argument: browser may serve from
        // disk cache. We want a real network round-trip to pick up the
        // freshly-published Webflow HTML.
        // location.reload(true) is deprecated but still works in most
        // browsers. The safer modern equivalent: append a cache-busting
        // query string and replace location.
        try {
          var u = new URL(window.location.href);
          u.searchParams.set('_r', Date.now().toString());
          window.location.replace(u.toString());
        } catch (e) {
          // Fallback: plain reload if URL constructor unavailable
    ...
    function asmFireAttach(slot, mediaId, articleId, it) {
      ...
      if (!roleHash) {
        console.error('[TA-STUDIO v1.2.3] No componentRole hash for slot:', slot);
      ...
      .then(function (body) {
        ...
        showStudioToast('\u2713 ' + attachLabel + ' attached', { duration: 4000 });

        renderAssembler();
        // v1.2.3: reload so .articles-wrapper data-attributes reflect
        // the freshly-attached image (Main Image preview, Interior
        // Images gallery). Toast lingers ~1.5s before reload kicks in.
        studioReloadAfterCMSWrite();
      })
      .catch(function (err) {
        console.error('[TA-STUDIO v1.2.3] Asm Route 4 failed:', err);
    ...
    function getRoleHash(roleLabel) {
      ...
      if (!hash) {
        console.error('[TA-STUDIO v1.2.3] No TA_CONFIG.optionIds.componentRole.' + key + ' configured for role:', roleLabel);
        return '';
      }
      return hash;
    }
    ...
    function getRoleHashForSlot(slotKey) {
      ...
      if (!hash) {
        console.error('[TA-STUDIO v1.2.3] No TA_CONFIG.optionIds.componentRole.' + slotKey + ' configured for slot:', slotKey);
        return '';
      }
      return hash;
    }
    ...
    function cmpFireAttach(mediaId, articleId, it) {
      ...
      if (!roleHash) {
        ...
        console.error('[TA-STUDIO v1.2.3] No componentRole hash for role:', it.role);
        showStudioToast('Configuration error \u2014 unknown role.');
        return;
      }
      ...
      .then(function (body) {
        ...
        cmpRenderGrid();
        // v1.2.3: reload so .media-wrapper data-attributes reflect
        // the new Status=Attached / ARTICLE ref state, and any
        // open Assembler view picks up the change.
        studioReloadAfterCMSWrite();
      })
      .catch(function (err) {
        console.error('[TA-STUDIO v1.2.3] Scenario G Route 4 failed:', err);
    ...
    function cmpFireDelete(mediaId, mode, it) {
      ...
      .then(function (body) {
        ...
        if (mode === 'archive') {
          ...
          if (S.cmp.drawerOpenId === mediaId) cmpRenderDrawer();
          cmpRenderGrid();
          // v1.2.3: reload so the .media-wrapper Status data-attribute
          // reflects Archived. The local cache flip handles the
          // immediate UI; reload makes it durable across panel switches.
          studioReloadAfterCMSWrite();
        } else {
          ...
          cmpRenderGrid();
          // v1.2.3: reload so the deleted .media-wrapper element is
          // gone from the DOM (otherwise stale wrapper references
          // can persist until next page nav).
          studioReloadAfterCMSWrite();
        }
      })
      .catch(function (err) {
    ...
    // ── Init ──
    renderAssembler();
    console.log('\u29BE Studio v1.2.3 mounted \u2014 Components tab (read + attach + archive + hard-delete) + Assembler field-level Attach (Main Image, Interior Images) + auto-reload after CMS writes');
  });
