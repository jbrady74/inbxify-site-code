...
// v2.2.0: Choices.js searchable dropdowns (Customer + CMS Item)
//         Live DOM reads (fresh on every modal render)
//         Alphabetical sort on all dropdown options
// ============================================================
...
/* Choices.js overrides for Content Manager */
...
.cm-choices .choices__list--dropdown{font-family:'DM Sans',system-ui,sans-serif;font-size:12px;border:1.5px solid #ddd9c8;border-radius:3px}
...
<div class="cm-root">
  <div class="cm-hdr">
    ...
    <span class="cm-badge">v2.2.0</span>
  </div>
  ...
  function initChoicesDropdowns() {
    ...
    if (custSel) {
      const cc = new Choices(custSel, {
        ...
        position: 'bottom',
        classNames: { containerOuter: 'choices cm-choices' },
      });
  ...
  // ── Init ──
  loadFiles('DONE');
  console.log('\u{1F4C2} Content Manager v2.2.0 mounted');
});
