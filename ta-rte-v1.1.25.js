/* ============================================================
   ...
   v1.1.25 changes (4 UX fixes — link popover, sticky toolbar,
   caption editing/collapse, multi-level indent):

     1. LINK POPOVER (replaces Trix's native link dialog).
        Trix's built-in dialog gave no clear apply affordance and no
        confirmation of what was selected, so operators thought the
        Link button was broken. We now intercept the link toolbar
        button and render our own popover anchored under the toolbar:
          - Shows the current selection text ("Apply to: ...")
          - URL input with scheme auto-prefix: a bare domain like
            "example.com" becomes "https://example.com"; a leading
            "/" is treated as an internal path (left as-is);
            mailto:/tel: pass through untouched.
          - Apply link / Remove link / Cancel buttons.
          - DECISION (Jeff, v1.1.25): with NO text selected, Apply is
            DISABLED and the popover shows "Select text first, then
            add a link." (No auto-insert of URL-as-text.)
          - Remove link shows only when the selection already carries
            an href; strips the link, keeps the text.
          - Enter = Apply (when enabled), Esc = Cancel.
        Implementation: we keep Trix's native link button element but
        remove its data-trix-action so Trix's dialog never fires, then
        bind our own click handler. The actual link apply/remove uses
        the Trix editor API (activateAttribute('href', url) /
        deactivateAttribute('href')) against the saved selection range.

     2. STICKY TOOLBAR IN INLINE MODE.
        The toolbar was already position:sticky in fullscreen
        (.rte-fs-mount trix-toolbar). Inline mounts (ABE-in-page,
        Workbench) had a static toolbar that scrolled away. Added a
        sticky rule for the base trix-toolbar so content swims under
        a pinned toolbar in every mount. Sticky needs a scroll
        container; the host mount provides it. Header in inline mode
        is not sticky (no inline header-scroll expectation), so the
        toolbar pins to top:0.

     3. CAPTION EDITING + EMPTY COLLAPSE.
        - Inline-image captions are edited through the EXISTING image
          click-to-edit popover (v1.1.18): a new "Caption" text row
          sits below Size/Position. This is the only safe path —
          inserted images are Trix content attachments, so the caption
          must be written via the attachment's content (setAttribute
          'content', ...) the same way Size/Position already are. A raw
          contenteditable <figcaption> would fight Trix's model and the
          edit would NOT survive save. Editing follows the existing
          staged/dirty/Apply precedent; clearing the field and applying
          removes the <figcaption> node entirely.
        - Empty figcaption collapses to zero space everywhere via
          `figcaption:empty { display:none }` (editor + the saved-HTML
          CSS block). insertImage and applyEditPopover both omit
          <figcaption> when the caption is blank, so no empty node
          ships; the :empty rule is the render-side backstop.
        - Main-image caption is OUT OF SCOPE — tracked as TD-242
          (needs its own Webflow field; separate surface from body
          html-content).

     4. MULTI-LEVEL INDENT (up to 3 nest levels, distinct markers).
        Bullet lists cycle marker glyph by depth:
          level 1 disc -> level 2 circle -> level 3 square
        Ordered lists cycle numbering by depth:
          level 1 decimal -> level 2 lower-alpha -> level 3 lower-roman
        Trix's native increase/decreaseNestingLevel drives the nesting;
        CSS keys off ul/ol nesting depth. Hard cap at 3 (Jeff): the
        increase-indent button no-ops past depth 3. Same depth CSS is
        added to the saved-HTML block so production render matches.
...
(function () {
  ...
  class InbxRTE {
    ...
    customizeToolbar() {
      ...
      // v1.1.25 #4: indent / outdent buttons (block-tools group).
      // Trix ships no default indent buttons; nesting drives the
      // depth-styled markers (disc/circle/square, 1/a/i) via CSS.
      // Hard cap at depth 3 enforced in onIndentClick().
      if (blockGroup) {
        const outdentBtn = document.createElement('button');
        outdentBtn.type = 'button';
        outdentBtn.className = 'trix-button rte-indent-btn';
        outdentBtn.title = 'Decrease indent';
        outdentBtn.tabIndex = -1;
        outdentBtn.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"/><path d="M9 9l-3 3 3 3" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
        outdentBtn.addEventListener('click', (e) => { e.preventDefault(); this.onIndentClick(-1); });

        const indentBtn = document.createElement('button');
        indentBtn.type = 'button';
        indentBtn.className = 'trix-button rte-indent-btn';
        indentBtn.title = 'Increase indent (max 3 levels)';
        indentBtn.tabIndex = -1;
        indentBtn.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"/><path d="M6 9l3 3-3 3" stroke-linecap="round" stroke-linejoin="round"/></svg>`;
        indentBtn.addEventListener('click', (e) => { e.preventDefault(); this.onIndentClick(1); });

        blockGroup.appendChild(outdentBtn);
        blockGroup.appendChild(indentBtn);
      }

      // v1.1.25 #1: neutralize Trix's native link dialog and bind our
      // own popover. We keep the button element (so toolbar layout is
      // unchanged) but strip data-trix-action so Trix never opens its
      // confusing built-in "Enter a URL" dialog.
      const linkBtn = toolbar.querySelector('.trix-button--icon-link');
      if (linkBtn) {
        linkBtn.removeAttribute('data-trix-action');
        linkBtn.classList.add('rte-link-btn');
        linkBtn.title = 'Add or edit link (select text first)';
        linkBtn.addEventListener('click', (e) => {
          e.preventDefault();
          this.openLinkPopover();
        });
      }
      // Native link dialog markup, if Trix injected it, is hidden via CSS
      // (.trix-dialogs is display:none in v1.1.25). Defensive removal too:
      const nativeDialogs = toolbar.parentElement
        ? toolbar.parentElement.querySelector('.trix-dialogs')
        : null;
      if (nativeDialogs) nativeDialogs.style.display = 'none';
    ...
    /* ── v1.1.25 #1: Link popover ──
       Replaces Trix's native link dialog. Captures the current
       selection, shows it, lets the operator type a URL with
       scheme auto-prefix, and applies/removes the link via the
       Trix editor API. */
    openLinkPopover() {
      if (!this.trixEditor) return;

      // Capture the selection range NOW — opening the popover and
      // focusing its input will blur the editor and lose the range.
      const range = this.trixEditor.getSelectedRange();
      this._linkRange = range ? [range[0], range[1]] : null;
      const hasSelection = !!(this._linkRange && this._linkRange[1] > this._linkRange[0]);

      // Selected plain text (for the "Apply to:" line)
      let selText = '';
      if (hasSelection) {
        const doc = this.trixEditor.getDocument();
        selText = doc.toString().substring(this._linkRange[0], this._linkRange[1]).replace(/\n+$/, '');
      }

      // Existing href on the selection? (enables Remove link)
      let existingHref = '';
      if (hasSelection) {
        try {
          // Trix exposes current attributes on the editor itself.
          const attrs = (this.trixEditor.getCurrentAttributes
            ? this.trixEditor.getCurrentAttributes()
            : {}) || {};
          existingHref = attrs.href || '';
        } catch (e) { existingHref = ''; }
      }

      // Remove any prior popover instance
      this.closeLinkPopover();

      const pop = document.createElement('div');
      pop.className = 'rte-link-popover';
      pop.id = `${this.id}-link-popover`;
      pop.innerHTML = `
        <span class="rte-link-label">Link URL</span>
        <input type="text" class="rte-link-input" id="${this.id}-link-input"
               placeholder="example.com  or  /community-news"
               value="${esc(existingHref)}" ${hasSelection ? '' : 'disabled'}>
        <div class="rte-link-hint" id="${this.id}-link-hint"></div>
        <div class="rte-link-target ${hasSelection ? '' : 'rte-link-target-empty'}" id="${this.id}-link-target">
          ${hasSelection
            ? `Apply to: <b>${esc(selText.length > 60 ? selText.slice(0, 60) + '\u2026' : selText)}</b>`
            : 'Select text first, then add a link.'}
        </div>
        <div class="rte-link-actions">
          <button type="button" class="rte-link-btn rte-link-apply" id="${this.id}-link-apply" ${hasSelection ? '' : 'disabled'}>Apply link</button>
          ${existingHref ? `<button type="button" class="rte-link-btn rte-link-remove" id="${this.id}-link-remove">Remove link</button>` : ''}
          <button type="button" class="rte-link-cancel" id="${this.id}-link-cancel">Cancel</button>
        </div>
      `;

      // Anchor under the toolbar (button-row is position:relative)
      const toolbar = this.mount.querySelector('trix-toolbar');
      const host = (toolbar && toolbar.querySelector('.trix-button-row')) || toolbar || this.mount;
      host.appendChild(pop);
      this._linkPopover = pop;

      const input  = pop.querySelector(`#${this.id}-link-input`);
      const hint   = pop.querySelector(`#${this.id}-link-hint`);
      const apply  = pop.querySelector(`#${this.id}-link-apply`);
      const remove = pop.querySelector(`#${this.id}-link-remove`);
      const cancel = pop.querySelector(`#${this.id}-link-cancel`);

      const renderHint = () => {
        const v = (input.value || '').trim();
        if (!hasSelection) { hint.innerHTML = ''; return; }
        if (!v) { hint.innerHTML = `Type a domain — we'll add <b>https://</b> for you.`; return; }
        const n = this.normalizeUrl(v);
        if (/^\//.test(v))            hint.innerHTML = `Internal path: <b>${esc(n)}</b>`;
        else if (/^(mailto:|tel:)/i.test(v)) hint.innerHTML = `Will link to: <b>${esc(n)}</b>`;
        else                          hint.innerHTML = `Will link to: <b>${esc(n)}</b>`;
      };
      renderHint();
      ...
      if (input) {
        ...
        input.addEventListener('keydown', (e) => {
          if (e.key === 'Enter') { e.preventDefault(); if (!apply.disabled) this.applyLinkFromPopover(); }
          if (e.key === 'Escape') { e.preventDefault(); this.closeLinkPopover(); }
        });
        setTimeout(() => input.focus(), 10);
      }
      if (apply)  apply.addEventListener('click', () => this.applyLinkFromPopover());
      if (remove) remove.addEventListener('click', () => this.removeLinkFromPopover());
      if (cancel) cancel.addEventListener('click', () => this.closeLinkPopover());

      // Click-outside closes
      this._linkOutsideHandler = (e) => {
        if (this._linkPopover && !this._linkPopover.contains(e.target)
            && !e.target.closest('.rte-link-btn')) {
          this.closeLinkPopover();
        }
      };
      setTimeout(() => document.addEventListener('mousedown', this._linkOutsideHandler), 0);
    }

    normalizeUrl(v) {
      v = (v || '').trim();
      if (!v) return '';
      // already schemed, internal path, anchor, mail/tel → leave as-is
      if (/^(https?:\/\/|mailto:|tel:|\/|#)/i.test(v)) return v;
      return 'https://' + v;
    }

    applyLinkFromPopover() {
      const input = this._linkPopover && this._linkPopover.querySelector('.rte-link-input');
      if (!input) return;
      const url = this.normalizeUrl(input.value);
      if (!url) { this.closeLinkPopover(); return; }
      if (!this._linkRange) { this.closeLinkPopover(); return; }

      // Restore the saved selection, then apply href via Trix API.
      this.trixEditor.setSelectedRange(this._linkRange);
      this.trixEditor.activateAttribute('href', url);
      this.dirty = true;
      this.updateDirtyState && this.updateDirtyState();
      this.closeLinkPopover();
      this.editorEl && this.editorEl.focus();
    }
    ...
    closeLinkPopover() {
      if (this._linkOutsideHandler) {
        document.removeEventListener('mousedown', this._linkOutsideHandler);
        this._linkOutsideHandler = null;
      }
      if (this._linkPopover && this._linkPopover.parentElement) {
        this._linkPopover.parentElement.removeChild(this._linkPopover);
      }
      this._linkPopover = null;
    }

    /* ── v1.1.25 #4: Indent handling (hard cap 3 levels) ── */
    onIndentClick(dir) {
      if (!this.trixEditor) return;
      if (dir > 0) {
        if (this.currentNestingDepth() >= 3) {
          this.showRteToast && this.showRteToast('Maximum indent is 3 levels.', 'info');
          return;
        }
    ...
    // Count ul/ol ancestors at the cursor to know current nesting depth.
    currentNestingDepth() {
      try {
    ...
    _readAttachmentState(outerFig, attachment) {
      ...
      return {
        ...
        stagedCaption: (img && typeof img.caption === 'string') ? img.caption : ''
      };
    }
    ...
    renderEditPopover() {
      ...
      const captionDirty = (st.stagedCaption || '') !== (st.img.caption || '');
      const anyDirty   = sizeDirty || alignDirty || captionDirty;
      ...
      pop.innerHTML = `
        ...
        <div class="rte-img-edit-row">
          <span class="rte-img-edit-label">Caption</span>
          <input type="text" class="rte-img-edit-caption-input${captionDirty ? ' is-dirty' : ''}"
                 id="${this.id}-img-edit-caption"
                 value="${esc(st.stagedCaption || '')}"
                 placeholder="Optional — leave blank for no caption">
        </div>
      ...
      // v1.1.25 #3: caption input — stage on input (no re-render, keeps
      // focus). Apply button enabling is recomputed on next render; to
      // reflect dirty immediately we toggle the Apply disabled state here.
      const captionInput = pop.querySelector(`#${this.id}-img-edit-caption`);
      if (captionInput) {
        captionInput.addEventListener('input', () => {
          this._editState.stagedCaption = captionInput.value;
          const applyB = pop.querySelector(`#${this.id}-img-edit-apply`);
          if (applyB) applyB.disabled = false;
          captionInput.classList.toggle('is-dirty',
            (captionInput.value || '') !== (this._editState.img.caption || ''));
        });
        captionInput.addEventListener('keydown', (e) => {
          if (e.key === 'Enter') { e.preventDefault(); this.applyEditPopover(); }
        });
      }
    ...
    applyEditPopover() {
      ...
      const _caption = (st.stagedCaption || '').trim();
      const _style = buildFigureInlineStyle(_size, _align);

      const newHTML =
        `<figure class="rte-inserted-image" data-img-size="${esc(_size)}" data-img-align="${esc(_align)}" style="${esc(_style)}">`
        + `<img src="${esc(st.img.src)}" alt="${esc(_caption || st.img.alt)}" data-media-id="${esc(st.img.mediaId)}" data-component-role="${esc(st.img.componentRole)}">`
        + (_caption ? `<figcaption>${esc(_caption)}</figcaption>` : '')
        + `</figure>`;
...
Target reissue date: ${this._assessTargetDate}
    ...
    destroy() {
      ...
      // v1.1.25 #1: tear down link popover if still open
      this.closeLinkPopover();
      if (this._fsOverlay) {
    ...
    injectStyles() {
      ...
      style.textContent = `
        ...
        /* v1.1.25 #2: pin the toolbar in inline mode too. The host mount
           is the scroll container; content swims underneath. Fullscreen
           keeps its own sticky rule (.rte-fs-mount trix-toolbar, top:76px). */
        .rte-root trix-toolbar { position:sticky; top:0; z-index:25; background:white; padding:6px 0 !important; }
        /* popover anchors to the toolbar's button-row */
        trix-toolbar .trix-button-row { position:relative; }
        /* v1.1.25 #1: never show Trix's native link dialog — our popover replaces it. */
        trix-toolbar .trix-dialogs, .rte-root .trix-dialogs { display:none !important; }
        ...
        /* v1.1.25 #4: indent buttons match Trix's native button face. */
        trix-toolbar .rte-indent-btn { background:${CREAM} !important; min-width:32px; height:30px; border:none !important; color:${TEXT_MID}; cursor:pointer; display:inline-flex; align-items:center; justify-content:center; }
        trix-toolbar .rte-indent-btn:hover { background:${CREAM_ALT} !important; }
        trix-toolbar .rte-indent-btn svg { pointer-events:none; }

        /* v1.1.25 #1: Link popover — anchored under the toolbar. */
        .rte-link-popover { position:absolute; top:42px; left:0; z-index:60; width:430px; max-width:calc(100vw - 48px); background:white; border:1px solid ${BORDER}; border-radius:8px; box-shadow:0 10px 30px rgba(26,58,58,0.18); padding:14px; }
        .rte-link-label { display:block; font-family:'DM Mono',monospace; font-size:9px; font-weight:600; text-transform:uppercase; letter-spacing:0.07em; color:${TEXT_LIGHT}; margin-bottom:6px; }
        .rte-link-input { width:100%; font-family:'DM Sans',system-ui,sans-serif; font-size:14px; padding:9px 11px; border:1.5px solid ${BORDER}; border-radius:5px; color:${TEXT_DARK}; }
        .rte-link-input:focus { outline:none; border-color:${TEAL}; box-shadow:0 0 0 2px rgba(26,58,58,0.08); }
        .rte-link-input:disabled { background:${CREAM}; color:${TEXT_LIGHT}; cursor:not-allowed; }
        .rte-link-hint { font-family:'DM Mono',monospace; font-size:10px; color:${TEXT_LIGHT}; margin-top:7px; min-height:14px; line-height:1.4; }
        .rte-link-hint b { color:#854F0B; }
        .rte-link-target { font-family:'DM Mono',monospace; font-size:10px; color:${TEXT_MID}; background:${CREAM}; border:1px solid ${BORDER}; border-radius:4px; padding:6px 8px; margin-top:9px; line-height:1.4; }
        .rte-link-target b { color:${TEAL}; }
        .rte-link-target-empty { color:${TEXT_LIGHT}; font-style:italic; }
        .rte-link-actions { display:flex; gap:8px; margin-top:12px; align-items:center; }
        .rte-link-btn { font-family:'DM Mono',monospace; font-size:11px; font-weight:600; padding:8px 14px; border-radius:5px; cursor:pointer; border:1px solid transparent; letter-spacing:0.02em; }
        .rte-link-apply { background:${TEAL}; color:white; }
        .rte-link-apply:hover:not(:disabled) { background:#0f2929; }
        .rte-link-apply:disabled { background:${CREAM_ALT}; color:${TEXT_LIGHT}; cursor:not-allowed; }
        .rte-link-remove { background:white; color:#c0392b; border-color:#e8b5a8; }
        .rte-link-remove:hover { background:#fff3f0; }
        .rte-link-cancel { background:none; border:none; color:${TEXT_MID}; font-family:'DM Sans',system-ui,sans-serif; font-size:12px; text-decoration:underline; cursor:pointer; margin-left:auto; padding:6px; }
        .rte-link-cancel:hover { color:${TEXT_DARK}; }
        ...
        /* v1.1.25 #1: linked text inside the editor — teal w/ gold underline. */
        trix-editor.rte-trix-editor a { color:${TEAL}; text-decoration:underline; text-decoration-color:${GOLD}; text-underline-offset:2px; }

        /* v1.1.25 #4: multi-level list markers (3 levels, distinct). */
        trix-editor.rte-trix-editor ul { list-style:disc; }
        trix-editor.rte-trix-editor ul ul { list-style:circle; }
        trix-editor.rte-trix-editor ul ul ul { list-style:square; }
        trix-editor.rte-trix-editor ol { list-style:decimal; }
        trix-editor.rte-trix-editor ol ol { list-style:lower-alpha; }
        trix-editor.rte-trix-editor ol ol ol { list-style:lower-roman; }

        /* v1.1.25 #3: empty caption collapses to zero space (editor). */
        trix-editor.rte-trix-editor figure.rte-inserted-image figcaption:empty { display:none; padding:0; }
        ...
        /* ── Saved HTML CSS block ──
           ...
           // v1.1.25 #3 — empty caption never shows as a spacer:
           figure.rte-inserted-image figcaption:empty { display:none; padding:0; }

           // v1.1.25 #4 — multi-level list markers (match the editor):
           .post-body ul, .article-body ul { list-style:disc; }
           .post-body ul ul, .article-body ul ul { list-style:circle; }
           .post-body ul ul ul, .article-body ul ul ul { list-style:square; }
           .post-body ol, .article-body ol { list-style:decimal; }
           .post-body ol ol, .article-body ol ol { list-style:lower-alpha; }
           .post-body ol ol ol, .article-body ol ol ol { list-style:lower-roman; }
           // (adjust the .post-body / .article-body wrapper selector to
           //  whatever the render template uses for body html-content.)
        ── */
        ...
        /* v1.1.25 #3: caption input in the edit popover. Gold dirty
           border matches the platform editing-state convention. */
        .rte-img-edit-caption-input { flex:1; min-width:0; font-family:'DM Sans',system-ui,sans-serif; font-size:12px; padding:6px 9px; border:1.5px solid ${BORDER}; border-radius:4px; color:${TEXT_DARK}; }
        .rte-img-edit-caption-input:focus { outline:none; border-color:${TEAL}; box-shadow:0 0 0 2px rgba(26,58,58,0.08); }
        .rte-img-edit-caption-input.is-dirty { border-color:${GOLD}; }
