...
(function () {
  ...
  // Image layer = a FRAME (x,y,w,h on the banner) containing the image,
  // which is scaled (imgScale) and panned (imgOx/imgOy) INSIDE the frame;
  // overflow clipped. imgScale 1 = image's natural width fills frame width.
  // natAspect = the image's intrinsic w/h, used to aspect-lock frame resize.
  function mkImgLayer(part, x, y, w, h) {
    var L = {
      ...
      natAspect: part.canvas.width / part.canvas.height,
      imgScale: 1, imgOx: 0, imgOy: 0
    };
    L.initial = snapshot(L);
    return L;
  }

  // Shallow snapshot of geometry/style for the (future) per-layer reset.
  function snapshot(L) {
    return { x: L.x, y: L.y, w: L.w, h: L.h, bg: L.bg,
      imgScale: L.imgScale, imgOx: L.imgOx, imgOy: L.imgOy,
  ...
  function openEditor() {
    ...
    ed.innerHTML =
      ...
      '<div class="ix-arf-edhint">Click a part to select. Drag to move; corner to resize ' +
      '(images keep their aspect). For a photo: <b>Zoom</b> scales it inside its frame and ' +
      '<b>Pan photo</b> lets you drag the image within the frame \u2014 anything outside the frame is cropped.</div>';
    root.appendChild(ed);
    renderEditor();
  }
  ...
  function renderEditor() {
    ...
    S.layers.forEach(function (L, i) {
      ...
      node.dataset.i = i;
      if (L.type === 'img') {
        // The image sits INSIDE the frame, scaled by imgScale and panned by
        // imgOx/imgOy (banner-space px). At imgScale 1 the image's natural
        // width equals the frame width; the frame clips any overflow.
        var im = document.createElement('img');
        im.src = L.canvas.toDataURL('image/png');
        var frameWpx = L.w; // banner-space; CSS % handles display scaling
        var imgWpct = L.imgScale * 100;           // width as % of frame width
        var imgHpct = (L.imgScale * (L.w / L.natAspect) / L.h) * 100; // keep ratio
        im.style.cssText = 'position:absolute;display:block;pointer-events:none;' +
          'width:' + imgWpct + '%;height:' + imgHpct + '%;' +
          'left:' + (L.imgOx / L.w * 100) + '%;top:' + (L.imgOy / L.h * 100) + '%;';
        node.appendChild(im);
      } else {
  ...
  function renderLayerToolbar() {
    ...
    if (L.type === 'img') {
      imgControls =
        ...
        '<input type="range" id="ix-arf-zoom" min="0.2" max="4" step="0.02" value="' + (L.imgScale || 1) + '" class="ix-arf-zoom">' +
        '<button class="ix-arf-ltbtn' + (S.panMode ? ' on' : '') + '" id="ix-arf-pan" title="Drag the photo inside its frame">' +
    ...
    // Zoom slider: scale image inside frame, live, no full rebuild.
    var zoom = document.getElementById('ix-arf-zoom');
    if (zoom) zoom.addEventListener('input', function () {
      var L2 = S.layers[S.selected]; L2.imgScale = parseFloat(zoom.value);
      var n = document.querySelector('.ix-arf-layer[data-i="' + S.selected + '"]');
      var imEl = n && n.querySelector('img');
      if (imEl) {
        imEl.style.width = (L2.imgScale * 100) + '%';
        imEl.style.height = (L2.imgScale * (L2.w / L2.natAspect) / L2.h * 100) + '%';
      }
    });

    // Pan toggle: when on, dragging the photo body pans it inside the frame.
  ...
  function wireLayer(node, i, handle) {
    ...
    node.addEventListener('pointerdown', function (e) {
      ...
      // Image layer + pan mode (or shift held) → PAN the image inside frame.
      if (L.type === 'img' && (S.panMode || e.shiftKey)) {
        ...
        function panMv(ev) {
          ...
          if (imEl) { imEl.style.left = (L.imgOx / L.w * 100) + '%'; imEl.style.top = (L.imgOy / L.h * 100) + '%'; }
        }
      ...
      // Otherwise MOVE the frame on the banner.
      var ox = L.x, oy = L.y;
      function mv(ev) {
        var dx = (ev.clientX - startX) / sr.width * BANNER_W;
        var dy = (ev.clientY - startY) / sr.height * BANNER_H;
    ...
    handle.addEventListener('pointerdown', function (e) {
      ...
      // For TEXT layers the frame resizes freely. For IMAGE layers the
      // FRAME is aspect-locked to the image's natural ratio so the photo
      // never distorts (zoom/crop happens via the slider + pan, not by
      // squashing the frame). v0.3.2.
      var lockAspect = (L.type === 'img');
      var frameAspect = ow / oh;
      function mv(ev) {
        var dx = (ev.clientX - startX) / sr.width * BANNER_W;
        var dy = (ev.clientY - startY) / sr.height * BANNER_H;
        ...
        if (lockAspect) {
          // drive height from width using the CURRENT frame aspect
          nh = Math.round(nw / frameAspect);
        }
        nw = Math.min(nw, BANNER_W - L.x);
        nh = Math.min(nh, BANNER_H - L.y);
        if (lockAspect) nw = Math.round(nh * frameAspect); // re-fit after clamp
        L.w = nw; L.h = nh;
        n.style.width = (L.w / BANNER_W * 100) + '%';
        n.style.height = (L.h / BANNER_H * 100) + '%';
        // keep image scale visually consistent during frame resize
        var imEl = n.querySelector('img');
        if (imEl) {
          imEl.style.width = (L.imgScale * 100) + '%';
          imEl.style.height = (L.imgScale * (L.w / L.natAspect) / L.h * 100) + '%';
        }
      }
      function up() { document.removeEventListener('pointermove', mv); document.removeEventListener('pointerup', up); }
      document.addEventListener('pointermove', mv); document.addEventListener('pointerup', up);
