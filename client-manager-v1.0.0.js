...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ── Read DOM data ──
  var CUSTOMERS = [];
  document.querySelectorAll('.customers-wrapper .w-dyn-item').forEach(function (el) {
    var d = el.dataset;
    var rec = { id: d.id || '' };
    ALL_FIELDS.forEach(function (f) {
      var key = f.s;
      var attr = d[camelCase(key)];
      if (f.t === 'switch') {
  ...
  // ── Init ──
  render();
  console.log('\u{1F465} Client Manager v1.0.0 mounted — ' + CUSTOMERS.length + ' customers loaded');
});
