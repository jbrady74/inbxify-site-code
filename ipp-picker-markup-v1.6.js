/* ============================================================
   ipp-picker-markup-v1.6.js
   CHANGELOG
     v1.6 (from v1.5): canonical edit-state alignment (pairs v1.5.js/v1.7.css).
       Greeting Cancel (#ipp_grCancel) gains the canonical .ipp-cancel-link
       class ALONGSIDE its existing .ix-revert (look unchanged — ix-revert
       styling still applies; the added class just marks it as the same
       canonical affordance the rest of the picker uses). No other markup
       changes; all tile/dropdown demo markup untouched. The shared dirty
       border + dropdown selection persistence are behavior (JS) + styling
       (CSS), so no per-tile markup edits are required here.
     v1.5 (from v1.4): CORRECTION — greeting rebuilt for the GR-block-row
       contract (ipp webhook), replacing the abandoned 2-field gr-save path.
       THREE fields now: Title (30) + Paragraph (300) + Super-short (140),
       matching block-greeting-title / -paragraph / -supershort on NL-BLOCKS.
       Demo values removed (greeting hydrates live from the GR planning row).
       Save/Cancel affordances unchanged (per-card ix-revert + Save).
     v1.4 (from v1.3): greeting card gains edit-state affordances —
       per-card Save (#ipp_grSave) + Cancel (.ix-revert #ipp_grCancel)
       in the greeting cat-head. Conforms to ix-form-controls canon
       (ix-revert link). Save is disabled / Cancel hidden until a field
       is dirty (wired in ipp-picker-v1.2.js). No redesign of v33 —
       affordances sit in the existing cat-head action row.
