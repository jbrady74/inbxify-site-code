...
(function () {
  ...
  function buildLayers() {
    var layers = [];
    // Image parts: photo to the left, logo/others to the right.
    ...
    if (photo) {
      // left ~58% width, full height, cover
      layers.push({ type: 'img', canvas: photo.canvas, label: photo.label,
        x: 0, y: 0, w: Math.round(BANNER_W * 0.58), h: BANNER_H });
    }
    ...
    rest.forEach(function (p) {
      ...
      layers.push({ type: 'img', canvas: p.canvas, label: p.label,
        x: rx, y: ry, w: Math.round(pw * scale), h: Math.round(ph * scale) });
      ry += Math.round(ph * scale) + 8;
    });

    // Text parts: stack on the right, beneath logo.
  ...
  function renderEditor() {
    ...
    S.layers.forEach(function (L, i) {
      ...
      if (L.type === 'img') {
        ...
        sp.style.cssText = 'width:100%;height:100%;display:flex;align-items:center;overflow:hidden;' +
          'font:' + L.weight + ' ' + L.size + 'px/1.1 "DM Sans",sans-serif;color:' + L.color + ';pointer-events:none;';
        node.appendChild(sp);
      }
