/* ta-asf-v1.9.2.js
   ...
   v1.9.2 is JS only. Companion CSS addendum: ta-asf-layout-v1.8.2.css.
...
(function () {
  ...
  var VERSION = '1.9.2';   // v1.9.2 — realigned with the filename
  ...
  function render() {
    ...
    pickerHost.innerHTML = renderPicker() + renderNewCustomerModal();
    ...
    if (displayUrl) {
      ...
      cardHtml = '' +
        '<div class="asf-mainog-card empty needs-pick"' +
             ...
             ' title="Pick the Main image in Bundle images">' +
          '<div class="asf-mainog-empty-icon">📷</div>' +
          '<div class="asf-mainog-empty-prompt">Pick in Bundle images →</div>' +
  ...
  function renderSide(rtp) {
    ...
    return '' +
      ...
      renderSideSection('pictures',    'Pictures',                  renderMainOgZones() + renderSideS3Credits()) +
  ...
  function maybePulseRailOnOpen() {
    ...
    setTimeout(function () { focusBundleRail(true); }, 60);
  }
  ...
  function renderBundleRail() {
    ...
    var a = S.article || {};
    var mainId = a.mainImageMediaId || '';
    var ogId   = a.ogImageMediaId   || '';
    var collapsed = !!S.railCollapsed;   // v1.5.20 — chunk (b) collapse
    // v1.9.2 — measure against the opening snapshot, not against empty.
    var base    = imageChoiceBaseline();
    var pending = imageChoicePending();
    ...
    var cards = ids.map(function (id) {
      ...
      var isOg   = (id === ogId);
      // v1.9.2 — PENDING is a role this card holds that the form did not
      // open with. Gold inset border, the standing editing-state signal.
      var isPending = (isMain && id !== base.main) || (isOg && id !== base.og);
      ...
      return '' +
        '<div class="asf-rail-card' + (isMain ? ' is-main' : '') + (isOg ? ' is-og' : '') +
          ...
          '<div class="asf-rail-meta">' +
            '<div class="asf-rail-tags">' + (tags || '<span class="asf-rail-tag asf-rail-tag--none">attached when used</span>') + '</div>' +
          '</div>' +
          '<div class="asf-rail-toggles">' +
            '<label class="asf-rail-tog asf-rail-tog--main" title="Set as Main image">' +
              '<input type="checkbox" data-asf-action="rail-main" data-media-id="' + esc(id) + '"' + (isMain ? ' checked' : '') + '>' +
              '<span>Main</span>' +
            '</label>' +
            '<label class="asf-rail-tog asf-rail-tog--og" title="Set as OG / social image">' +
              '<input type="checkbox" data-asf-action="rail-og" data-media-id="' + esc(id) + '"' + (isOg ? ' checked' : '') + '>' +
              '<span>OG</span>' +
            '</label>' +
          '</div>' +
        '</div>';
    ...
    return '' +
      '<div class="asf-rail' + (collapsed ? ' is-collapsed' : '') +
        ...
        '<div class="asf-rail-head">' +
          '<span class="asf-rail-title">Bundle images</span>' +
          '<span class="asf-rail-count">' + ids.length + '</span>' +
          // v1.9.2 — standing rule: every edit gets a small cancel text
          // link back to the state it started in. Shown only when there
          // is something to undo.
          (pending
            ? '<button type="button" class="asf-field-cancel asf-rail-cancel"' +
          ...
          '</button>' +
        '</div>' +
        // v1.9.2 — nothing is chosen for the operator any more, so the rail
        // says so. Rendered outside the collapse so a collapsed rail still
        // shows the open decision.
        (owed
          ? '<div class="asf-rail-banner">No Main image chosen. ' +
            'Pick one from the ' + ids.length + ' candidate' + (ids.length === 1 ? '' : 's') +
            ' below \u2014 nothing is picked for you.</div>'
          : '') +
        (collapsed ? '' :
          '<div class="asf-rail-list">' + cards + '</div>' +
          '<div class="asf-rail-foot">Pick Main / OG here \u2014 this is the only picker. ' +
          'A gold border marks a pick this Save will write. ' +
          'Upload adds a new image to this set. Insert others inline from the body editor\u2019s image button.</div>') +
      '</div>';
  }
  ...
  function handleAction(action, el, e) {
    switch (action) {
      ...
      case 'focus-rail': {
        // v1.5.19 — "Change →" / empty-card click in rail contexts:
        // don't open a picker; direct the eye to the ONE picker (the
        // Bundle-images rail). Scroll it into view and pulse it.
        focusBundleRail();
        return;
      }
      case 'rail-collapse': {
  ...
  function publicOpen(params) {
    ...
    // v1.5.19 — auto-pulse (Jeff decision May 26): create-from-bundle
    // with images on the rail but NO Main set → pulse the rail once on
    // open so the operator's eye lands on the one picker. Prefill
    // defaults the first bundle image to Main, so this only fires when
    // that default didn't apply (text-only selection, Main cleared, etc).
    if (mode === 'create' && hasBundleRail() &&
        !(S.article && S.article.mainImageMediaId)) {
      setTimeout(focusBundleRail, 350);   // after mount paint settles
    }
