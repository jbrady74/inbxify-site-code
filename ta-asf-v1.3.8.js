...
(function () {
  ...
  function blankEV() {
    ...
    return {
      ...
      eventLocation:      '',     // relabeled "Venue Name" in the UI (v1.3.7)
      eventVenueAddress:  '',     // v1.3.7 — "Venue Address (Street, City)"
      eventStartDate:     '',
      eventEndDate:       '',
      eventStartTime:     '',
      eventEndTime:       '',
  ...
  function renderEventFoundImage() {
    ...
    return '' +
      '<div class="asf-found-image">' +
        '<div class="asf-found-image-head">Found this image on the page</div>' +
        '<div class="asf-found-image-body">' +
          '<div class="asf-found-image-thumb"><img src="' + esc(af.image) + '" alt="Image found on the event page"></div>' +
          '<div class="asf-found-image-meta">' +
            '<div class="asf-found-image-url">' + esc(af.image) + '</div>' +
            '<button type="button" class="asf-mini-btn" disabled title="Needs the Scenario B remote-URL route">Condition &amp; use as event image</button>' +
            '<div class="asf-found-image-note">Separate step \u2014 needs the Scenario B remote-URL route (not built yet).</div>' +
          '</div>' +
        '</div>' +
      '</div>';
  }
  ...
  function fetchEventFromUrl() {
    ...
    fetch(endpoint, {
      ...
      .then(function (j) {
        ...
        var f = j.fields;
        var keys = ['eventName','eventDescription','eventLocation','eventVenueAddress',
                    'eventStartDate','eventStartTime','eventEndDate','eventEndTime','eventRedirectLink'];
        if (!S.article) S.article = {};
        S.autoFill.fields = {};
        var n = 0;
        ...
        S.autoFill.image   = (j.imageUrl && /^https?:\/\//i.test(j.imageUrl)) ? j.imageUrl : null;
        S.autoFill.count   = n;
  ...
  function renderEVMain() {
    return '' +
      ...
      '<div class="asf-re-grid">' +
        ...
        renderField({
          label: 'Venue Address (Street, City)', field: 'eventVenueAddress',
          value: curValRead('eventVenueAddress'),
          placeholder: 'e.g. 12 Main St, Wyckoff'
        }) +
      '</div>' +
      renderField({
  ...
  function commitCreateEV() {
    ...
    var payload = {
      ...
      fields: {
        ...
        eventVenueAddress:  curVal('eventVenueAddress'),
