/* ============================================================
   ta-converter-v1.0.11.js
   INBXIFY File Converter — Standalone module

   v1.0.11 (Upscale: client-side polling — kills VENDOR_TIMEOUT):
     Upscale tool only. Reduce and SVG paths untouched.
     Requires upscaleinbxifycom Worker v1.1.0 (submit/poll modes).

     PROBLEM: the Worker's blocking flow polled WaveSpeed inside
     one request with a 25s ceiling (HC-UPS-006). A queued/slow
     vendor job returned VENDOR_TIMEOUT ("Polled 25000ms, last
     status: processing") even on a 660KB PNG — the job wasn't
     broken, the window was.

     FIX: two-phase call.
       1. POST { mode:"submit", source, ... } → Worker resolves
          the source, submits to WaveSpeed, returns
          vendor_request_id immediately.
       2. Browser polls POST { mode:"poll", vendor_request_id,
          context } every 1500 ms. Worker does ONE vendor status
          check per poll; on completion it finalizes (fetch bytes,
          Uploadcare re-upload) and returns the SAME success
          payload as before — downstream handling (alpha remask,
          result view, Save, Download) is unchanged.
     Status line shows live elapsed time ("Upscaling… 12s")
     instead of the fixed "(~13s)" estimate.

     Hardcoding (HC-UPS series):
       HC-UPS-015 — client poll cadence 1500 ms; runaway guard
                    180 s. Not a vendor timeout — a safety cap so
                    an orphaned job can't spin the UI forever. On
                    trip, error view names the vendor_request_id
                    for manual follow-up.

     Webflow head deploy:
       SWAP: ta-converter-v1.0.10.js → ta-converter-v1.0.11.js
       jsDelivr purge if @main loops:
         https://purge.jsdelivr.net/gh/jbrady74/inbxify-site-code@main/ta-converter-v1.0.11.js
       DEPLOY ORDER: Worker v1.1.0 FIRST, then this file. (The
       Worker is backward-compatible with v1.0.10; this file is
       NOT compatible with Worker v1.0.1 — mode:"submit" would be
       ignored and the legacy blocking flow would run with its
       response shape mismatching the submit handler.)

   ─────────────────────────────────────────────────────────────
   v1.0.10 (Upscale: paste intake + editable output filename):
     Two additive changes, Upscale tool only. No Worker, Make,
     or CMS changes. Reduce and SVG paths untouched.

     1. PASTE INTAKE (Upscale only) — HC-UPS-012:
        A document-level 'paste' listener, wired once per mount,
        fires only when (a) the active tool is Upscale, (b) no
        file is already staged, and (c) focus isn't in a text
        input/textarea (so it never steals a normal text paste,
        including into the new filename field). It grabs the first
        image/* clipboard item, wraps the blob in a named File via
        synthPasteName(), and routes it through the same
        validate → loadFile path as drop/browse. Detaches itself
        if the mount leaves the DOM (tab switch).
        Scoped to Upscale deliberately: Reduce wants JPEG and SVG
        wants SVG; neither arrives cleanly from a clipboard image.

     2. EDITABLE OUTPUT FILENAME — HC-UPS-013, HC-UPS-014:
        The result view now has a filename field (base name +
        locked extension suffix). It seeds from the derived
        {source}-upscaled name and drives BOTH Download and the
        Scenario B Save (originalFilename). This is the rename
        opportunity the operator previously lacked — important for
        pasted images, whose synthesized source name is a
        pasted-image-{timestamp} stub. The field locks once Save
        commits the name to the MEDIA record. sanitizeOutName()
        strips illegal characters and guarantees the extension.

     Hardcoding (HC-UPS series):
       HC-UPS-012 — paste intake scoped to the Upscale tool only.
       HC-UPS-013 — pasted-image synthesized name pattern:
                    pasted-image-{ISO-ts}.{ext}, ext from blob MIME.
       HC-UPS-014 — output filename sanitizer: [a-zA-Z0-9._-] only,
                    path separators → underscore, single forced ext.

     Self-contained <style> additions for the rename field (cream
     fill, !important on backgrounds per the publisher-wrapper
     universal-reset rule). No companion CSS file change.

     Webflow head deploy:
       SWAP: ta-converter-v1.0.9.js → ta-converter-v1.0.10.js
       jsDelivr purge if @main loops:
         https://purge.jsdelivr.net/gh/jbrady74/inbxify-site-code@main/ta-converter-v1.0.10.js

   ─────────────────────────────────────────────────────────────
   v1.0.9 (filename bump only — fresh jsDelivr path):
     Identical feature code to v1.0.7/v1.0.8 (single-panel flow +
     browser-side alpha preservation). v1.0.7 hit a jsDelivr edge
     ERR_TOO_MANY_REDIRECTS; v1.0.8 same on jsDelivr and Mixed-Content
     on statically (http). Neither was a code fault — the file never
     loaded. Fresh filename on jsDelivr (where prior versions loaded).
     If THIS filename also loops on jsDelivr, purge it:
       https://purge.jsdelivr.net/gh/jbrady74/inbxify-site-code@main/ta-converter-v1.0.9.js

     Webflow head deploy (back on jsDelivr, @main):
       <script src="https://cdn.jsdelivr.net/gh/jbrady74/inbxify-site-code@main/ta-converter-v1.0.9.js"></script>

   ─────────────────────────────────────────────────────────────
   v1.0.8 (filename bump only — dodge jsDelivr cached redirect loop):
     Identical code to v1.0.7. v1.0.7 hit net::ERR_TOO_MANY_REDIRECTS
     on jsDelivr @main (cache/propagation loop) and never loaded.
     Fresh filename sidesteps the cached path entirely.

     Webflow head deploy:
       SWAP: ta-converter-v1.0.6.js (or v1.0.7) → ta-converter-v1.0.9.js

   ─────────────────────────────────────────────────────────────
   v1.0.7 (Upscale: single-panel flow + browser-side alpha preserve):
     • Single panel that transforms in place — the result REPLACES
       the staged view (no more stacked duplicate source/toggle).
     • Transparency preserved for sources with a real alpha channel:
       after the Worker upscales the RGB (it flattens alpha), the
       browser re-applies the source alpha scaled to the upscaled
       dimensions, exports a transparent PNG, and uploads THAT to
       Uploadcare so both Download and Save use the transparent file.
       Opaque sources keep the Worker's JPEG unchanged.
     • Honesty: the source mask is original-resolution, so alpha
       EDGES are bilinear-upscaled (perfect for hard-edged logos,
       slightly soft for heavily feathered edges). Surfaced in UI.
     • Alpha detection on load (canvas scan). Re-mask failure falls
       back to the flattened Worker URL so the operator still gets a
       usable result.
     HC-UPS series unchanged.

     Webflow head deploy:
       SWAP: ta-converter-v1.0.6.js → ta-converter-v1.0.9.js

   ─────────────────────────────────────────────────────────────
   v1.0.6 (bg toggle fix):
     The v1.0.5 background toggle had no visible effect: the
     .cm-ups-bg background rules were beaten by the T-A page's
     section.publisher-wrapper * { background-color: var(--ix-white)
     !important; background-image: none !important } universal reset.
     All .cm-ups-bg background declarations + the active-button
     highlight now carry !important, matching the standing rule for
     styled elements inside publisher-wrapper.

     Webflow head deploy:
       SWAP: ta-converter-v1.0.5.js → ta-converter-v1.0.6.js
...
(function () {
  ...
  // HC-UPS-013 — pasted-image synthesized filename pattern.
  // Clipboard image blobs carry no filename. We synthesize one so the
  // staged view, Download, and the Scenario B originalFilename all have
  // a real name. Operator can override it post-conversion via the
  // editable filename field (v1.0.10). Pattern: pasted-image-{ts}.{ext}
  // where ext is derived from the blob MIME (png for screenshots).
  function synthPasteName(blob) {
  ...
  // HC-UPS-014 — output filename sanitizer. Used for both the Download
  // attribute and the Uploadcare object name. Strips path separators and
  // anything Uploadcare's object-name slot rejects; guarantees a single
  // image extension. Mirrors the inline regex uploadBlobToUploadcare
  // already used, lifted out so the editable-filename field shares it.
  function sanitizeOutName(name, fallbackExt) {
    let n = (name || '').trim().replace(/[\/\\]/g, '_').replace(/[^a-zA-Z0-9._-]/g, '_');
    if (!n) n = 'upscaled';
    const ext = fallbackExt || 'png';
    if (!new RegExp('\\.' + ext + '$', 'i').test(n)) {
      n = n.replace(/\.(jpe?g|png|webp)$/i, '') + '.' + ext;
    }
    return n;
  }
  ...
  function injectUpscaleStyles() {
    ...
    st.textContent = [
      ...
      '.cm-ups-pane .cm-ups-bg{position:absolute;inset:0;background-color:#f4f1ea !important;background-image:linear-gradient(45deg,#ccc 25%,transparent 25%),linear-gradient(-45deg,#ccc 25%,transparent 25%),linear-gradient(45deg,transparent 75%,#ccc 75%),linear-gradient(-45deg,transparent 75%,#ccc 75%) !important;background-size:16px 16px !important;background-position:0 0,0 8px,8px -8px,-8px 0 !important;}',
      '.cm-ups-pane.bg-dark .cm-ups-bg{background-image:none !important;background-color:#1a3a3a !important;}',
      '.cm-ups-pane.bg-light .cm-ups-bg{background-image:none !important;background-color:#ffffff !important;}',
      ...
      '.cm-ups-bgbtn.active{border:2px solid #5b7fff !important;background:#EEF2FF !important;color:#5b7fff !important;font-weight:600;padding:3px 9px;}',
      '.cm-ups-rename{margin:2px 0 12px;padding:10px 12px;border:1px solid #d4cfbf;border-radius:6px;background:#fdfcf8 !important;}',
      '.cm-ups-rename-lbl{display:block;font-family:\'DM Mono\',ui-monospace,monospace;font-size:9px;letter-spacing:0.06em;text-transform:uppercase;color:#8a8a7a;margin-bottom:5px;}',
      '.cm-ups-rename-row{display:flex;align-items:center;gap:6px;}',
      '.cm-ups-rename-input{flex:1;min-width:0;font-size:13px;padding:6px 8px;border:1px solid #d4cfbf;border-radius:4px;background:#ffffff !important;color:#1a3a3a;font-family:ui-monospace,monospace;}',
      '.cm-ups-rename-input:focus{outline:none;border:1.5px solid #5b7fff;}',
      '.cm-ups-rename-input:disabled{background:#f0eee6 !important;color:#8a8a7a;}',
      '.cm-ups-rename-ext{font-family:ui-monospace,monospace;font-size:12px;color:#8a8a7a;}',
      '.cm-ups-rename-hint{font-size:10px;color:#8a8a7a;line-height:1.4;margin-top:5px;}'
    ].join('');
  ...
  const TOOLS = {
    ...
    upscale: {
      ...
      hint: 'Drop an image, click to browse, or paste (\u2318V / Ctrl+V)',
  ...
  function renderConverter(targetEl, initialTool) {
    ...
    // ── Paste intake (Upscale only) ──
    // Listens on document while the Converter is mounted. Fires only when
    // the active tool is Upscale AND no file is already staged (so a paste
    // into a result view's filename input is never hijacked). Pulls the
    // first image/* clipboard item, wraps the blob in a named File, and
    // routes it through the same validate → loadFile path as drop/browse.
    // HC-UPS-012 — paste scoped to Upscale only. Reduce expects JPEG and
    // SVG expects SVG; neither arrives cleanly from a clipboard image, so
    // paste is deliberately inert on those tools.
    function wirePasteIntake() {
      // Detach any handler from a prior render of this mount so the new
      // closure (with the current activeTool) is the only one live.
      if (el._ixPasteHandler) {
        document.removeEventListener('paste', el._ixPasteHandler);
        el._ixPasteHandler = null;
      }
      const handler = (e) => {
        // Mount gone (tab switched away) → detach.
        if (!document.body.contains(el)) {
          document.removeEventListener('paste', handler);
          if (el._ixPasteHandler === handler) el._ixPasteHandler = null;
          return;
        }
        if (activeTool !== 'upscale') return;     // Upscale only
        if (convFile) return;                      // already staged
        const ae = document.activeElement;
        if (ae && /^(INPUT|TEXTAREA)$/.test(ae.tagName)) return; // don't steal text paste
        ...
        for (let i = 0; i < items.length; i++) {
          if (items[i].type && items[i].type.indexOf('image/') === 0) {
            ...
            if (!blob) continue;
            e.preventDefault();
            const named = new File([blob], synthPasteName(blob), { type: blob.type });
            if (TOOLS[activeTool].validate(named)) loadFile(named);
            return;
          }
        }
      };
      el._ixPasteHandler = handler;
    ...
    // ──────────────────────────────────────────────────────────
    // Upscale path — async Worker call + browser-side alpha preserve
    //
    // v1.0.7 flow:
    //   1. On load, detect whether the source has a real alpha channel
    //      (transparent pixels). Store srcHasAlpha + the source <img>.
    //   2. On Upscale: send RGB to the Worker, get back upscaled RGB
    //      (the Worker flattens alpha — we only use its color data).
    //   3. If the source had alpha: re-apply it. Draw the upscaled
    //      image to a canvas, then scale the SOURCE alpha mask up to
    //      the upscaled dimensions and multiply it into the canvas
    //      alpha channel. Export PNG (keeps transparency). Upload that
    //      transparent PNG to Uploadcare so BOTH Download and Save use
    //      the transparent version.
    //   4. If no alpha: use the Worker's URL directly (JPEG is fine).
    //
    //   Honesty note surfaced in the UI when alpha is detected: the
    //   source mask is only at original resolution, so transparency
    //   EDGES are upscaled by bilinear, not by ESRGAN. Hard-edged
    //   logos are effectively perfect; heavily-feathered edges are
    //   good but slightly softer than the RGB detail.
    // ──────────────────────────────────────────────────────────
    function loadFileUpscale(file) {
      ...
      reader.onload = ev => {
        const img = new Image();
        img.onload = () => {
          // Detect alpha by sampling the source onto a canvas.
          const hasAlpha = detectAlpha(img);
          const alphaNote = hasAlpha
            ? `<div class="cm-conv-info-row"><span class="cm-conv-info-label">Transparency</span><span class="cm-conv-info-val">Detected \u2014 will be preserved (PNG)</span></div>`
            : '';
          const overWarn = tooBig
            ...
            : '';
          ws.innerHTML = buildUpscaleStaged(ev.target.result, file, img, sizeMB, tooBig, alphaNote, overWarn);
          wireBgToggle(ws);
          const goBtn = document.getElementById('cm-conv-go');
          if (!tooBig) goBtn.addEventListener('click', () => processUpscale(ev.target.result, file, img, hasAlpha));
          document.getElementById('cm-conv-reset').addEventListener('click', () => resetTool());
        };
        img.onerror = () => {
    ...
    // Staged view — single panel. The result REPLACES this in place
    // (no stacking, no duplicate source image or toggle).
    function buildUpscaleStaged(dataUrl, file, img, sizeMB, tooBig, alphaNote, overWarn) {
      return `<div class="cm-conv-preview"><div style="flex:1">
        ${bgToggleRow()}
        <div class="cm-ups-col" style="max-width:320px;margin-bottom:14px"><div class="cm-ups-lbl">Source \u00B7 ${img.width} \u00D7 ${img.height}</div><div class="cm-ups-pane"><div class="cm-ups-bg"></div><img src="${dataUrl}" alt="Source preview"></div></div>
        <div class="cm-conv-info">
          <div class="cm-conv-info-row"><span class="cm-conv-info-label">Filename</span><span class="cm-conv-info-val">${esc(file.name)}</span></div>
          <div class="cm-conv-info-row"><span class="cm-conv-info-label">Original Size</span><span class="cm-conv-info-val ${tooBig ? 'bad' : 'ok'}">${sizeMB} MB</span></div>
          <div class="cm-conv-info-row"><span class="cm-conv-info-label">Target</span><span class="cm-conv-info-val">Real-ESRGAN ${UPSCALE_DEFAULT_SCALE}\u00D7 super-resolution</span></div>
          ${alphaNote}${overWarn}
          <div class="cm-conv-actions"><button class="cm-conv-go ix-btn ix-btn--primary" id="cm-conv-go"${tooBig ? ' disabled title="File exceeds 10 MB \u2014 use Reduce first"' : ''}>Upscale ${UPSCALE_DEFAULT_SCALE}\u00D7</button><button class="cm-conv-go ix-btn ix-btn--danger ix-btn--ghost" id="cm-conv-reset">\u2715 Clear</button><span class="cm-conv-status" id="cm-conv-status"></span></div>
          <div class="cm-conv-progress" id="cm-conv-progress" style="display:none"><div class="cm-conv-progress-bar" id="cm-conv-bar" style="width:0%"></div></div>
        </div>
      </div></div>`;
    }

    function bgToggleRow() {
      return `<div class="cm-ups-bgtoggle"><span class="lbl">Preview bg:</span><button class="cm-ups-bgbtn active" data-bg="checker">Checker</button><button class="cm-ups-bgbtn" data-bg="dark">Dark</button><button class="cm-ups-bgbtn" data-bg="light">Light</button></div>`;
    }

    // Detect a real alpha channel: draw to canvas, scan for any pixel
    // with alpha < 250. Sample-steps for speed on large images.
    function detectAlpha(img) {
      try {
        ...
        for (let i = 3; i < data.length; i += 4 * 7) { // step ~7px
          if (data[i] < 250) return true;
        }
        return false;
      } catch (e) {
        return false; // tainted / unreadable → assume opaque
      }
    }

    function processUpscale(dataUrl, file, srcImg, srcHasAlpha) {
      const statusEl = document.getElementById('cm-conv-status'),
            progEl   = document.getElementById('cm-conv-progress'),
            barEl    = document.getElementById('cm-conv-bar'),
            goBtn    = document.getElementById('cm-conv-go');
      ...
      // HC-UPS-015 — client poll cadence + runaway guard.
      const CLIENT_POLL_MS   = 1500;
      const CLIENT_POLL_CAP  = 180000; // 3 min safety cap, not a vendor timeout

      const upsContext = { source_surface: 'converter-upscale' };
      ...
      // Poll loop — one Worker round-trip per tick. Worker v1.1.0
      // finalizes on the tick that sees completion and returns the
      // full legacy-shaped success payload, so handleSuccess below
      // is identical to the old blocking-response handling.
      function pollJob(requestId) {
        const elapsed = Date.now() - tStart;
        if (elapsed > CLIENT_POLL_CAP) {
          clearInterval(creepTimer);
          failUpscale('Upscale is taking too long (>3 min). The vendor job may still complete \u2014 job id: ' + requestId + '. Re-run to retry.', goBtn);
          return;
        }
        statusEl.textContent = 'Upscaling\u2026 ' + Math.round(elapsed / 1000) + 's';

        fetch(UPSCALE_WORKER_URL, {
          method:  'POST',
          headers: { 'Content-Type': 'application/json' },
          body:    JSON.stringify({ mode: 'poll', vendor_request_id: requestId, context: upsContext })
        })
          ...
          .then(({ ok, j }) => {
            if (!ok || !j || j.ok !== true) {
              ...
              failUpscale(msg, goBtn);
              return;
            }
            if (j.phase === 'processing') {
              setTimeout(() => pollJob(requestId), CLIENT_POLL_MS);
              return;
            }
            // phase "completed" — full success payload.
            handleSuccess(j);
          })
          .catch(() => {
            // Transient network blip — keep polling within the cap.
            setTimeout(() => pollJob(requestId), CLIENT_POLL_MS);
          });
      }
      ...
      fetch(UPSCALE_WORKER_URL, {
        ...
        .then(({ ok, j }) => {
          ...
          if (j.phase === 'submitted' && j.vendor_request_id) {
            setTimeout(() => pollJob(j.vendor_request_id), CLIENT_POLL_MS);
            return;
          }
          if (j.upscaled) {
            // Legacy Worker (pre-v1.1.0) ignored mode and ran the old
            // blocking flow — its success payload is directly usable.
            handleSuccess(j);
            return;
          }
          clearInterval(creepTimer);
          failUpscale('Unexpected Worker response \u2014 check Worker version (GET the Worker URL).', goBtn);
        })
        .catch(err => {
          clearInterval(creepTimer);
          failUpscale((err && err.message) ? err.message : 'Network error contacting the upscale service.', goBtn);
        });

      function handleSuccess(j) {
          const up = j.upscaled || {};
          const workerUrl = up.url;

          if (!srcHasAlpha) {
            // No alpha — Worker JPEG is fine, use it directly.
            clearInterval(creepTimer);
            barEl.style.width = '100%';
            finishUpscale({
              file, dataUrl,
              outUrl:  workerUrl,
              outUuid: up.uploadcare_uuid || '',
              sizeBytes: up.size_bytes || 0,
              isPng: false,
          ...
          // Alpha path — re-mask the upscaled RGB with the source alpha,
          // export transparent PNG, upload that to Uploadcare so Save +
          // Download both use the transparent version.
          statusEl.textContent = 'Restoring transparency\u2026';
          remaskAlpha(workerUrl, srcImg)
            .then(pngBlob => uploadBlobToUploadcare(pngBlob, (file.name || 'image').replace(/\.[^.]+$/, '') + '-upscaled.png'))
            .then(uc => {
              clearInterval(creepTimer);
              barEl.style.width = '100%';
              finishUpscale({
            ...
            .catch(err => {
              // Re-masking failed — fall back to the flattened Worker URL
              // so the operator still gets a usable upscaled image.
              clearInterval(creepTimer);
              barEl.style.width = '100%';
              finishUpscale({
                file, dataUrl,
                ...
                isPng: false,
                alphaPreserved: false,
                alphaError: (err && err.message) ? err.message : 'transparency restore failed'
              });
            });
      }
    }

    function failUpscale(msg, goBtn) {
      const progEl = document.getElementById('cm-conv-progress');
      const statusEl = document.getElementById('cm-conv-status');
      const resultHost = document.getElementById('cm-conv-status').closest('.cm-conv-info') || document.getElementById('cm-conv-ws');
      if (statusEl) statusEl.textContent = '';
      if (progEl) progEl.style.display = 'none';
      // Render error into the workspace, preserving the staged layout's reset.
      const ws = document.getElementById('cm-conv-ws');
      ws.innerHTML = `<div class="cm-conv-preview"><div class="cm-conv-info" style="flex:1">${convErrorHtml('Upscale failed', msg)}</div></div>`;
      document.getElementById('cm-conv-reset').addEventListener('click', () => resetTool());
    }

    // Re-mask: draw upscaled RGB to canvas, multiply in the source alpha
    // (scaled up to match). Returns Promise<Blob> (PNG).
    function remaskAlpha(upscaledUrl, srcImg) {
      return new Promise((resolve, reject) => {
        const up = new Image();
        up.crossOrigin = 'anonymous'; // Uploadcare serves CORS-open
        up.onload = () => {
          try {
            const W = up.naturalWidth, H = up.naturalHeight;
            // 1. Upscaled RGB onto the main canvas.
            const canvas = document.createElement('canvas');
            canvas.width = W; canvas.height = H;
            const ctx = canvas.getContext('2d');
            ...
            // 2. Source alpha mask, scaled to W×H.
            const mc = document.createElement('canvas');
            mc.width = W; mc.height = H;
            const mctx = mc.getContext('2d');
            mctx.drawImage(srcImg, 0, 0, W, H); // bilinear upscale of the source (incl. its alpha)
            const mask = mctx.getImageData(0, 0, W, H);

            // 3. Multiply mask alpha into the upscaled RGB alpha.
            const rd = rgb.data, md = mask.data;
            for (let i = 3; i < rd.length; i += 4) {
              rd[i] = md[i]; // take source alpha directly
            }
        ...
        up.onerror = () => reject(new Error('could not load upscaled image for re-masking'));
        up.src = upscaledUrl;
      });
    }

    // Upload a blob to Uploadcare directly (browser → Uploadcare /base/).
    // Returns Promise<{uuid,url,size}>.
    function uploadBlobToUploadcare(blob, filename) {
      const cfg = window.TA_CONFIG || {};
      const key = cfg.uploadcarePublicKey || cfg.uploadcareKey;
      return new Promise((resolve, reject) => {
        if (!key) { reject(new Error('Uploadcare public key missing in TA_CONFIG')); return; }
        ...
        fetch('https://upload.uploadcare.com/base/', { method: 'POST', body: fd })
          .then(r => { if (!r.ok) throw new Error('Uploadcare HTTP ' + r.status); return r.json(); })
          .then(d => {
            if (!d || !d.file) { reject(new Error('Uploadcare returned no UUID')); return; }
            const safe = (filename || 'upscaled.png').replace(/[^a-zA-Z0-9._-]/g, '_');
            const base = (cfg.uploadcareBase || 'https://uyluucdnr2.ucarecd.net').replace(/\/$/, '');
            resolve({ uuid: d.file, url: base + '/' + d.file + '/' + safe, size: blob.size });
          })
          .catch(reject);
      });
    }

    // Render the result IN PLACE (replaces the staged workspace).
    function finishUpscale(r) {
      const ws = document.getElementById('cm-conv-ws');
      const newSize = r.sizeBytes ? (r.sizeBytes / 1024).toFixed(0) + ' KB' : '\u2014';
      const baseName = (r.file.name || 'image').replace(/\.(jpe?g|png|webp)$/i, '');
      const ext = r.isPng ? 'png' : 'jpg';
      const defaultBase = baseName + '-upscaled';
      const fmtLabel = r.isPng ? 'PNG (transparency preserved)' : 'JPEG';
      const alphaRow = r.alphaPreserved
        ? `<div class="cm-conv-info-row"><span class="cm-conv-info-label">Transparency</span><span class="cm-conv-info-val ok">Preserved</span></div>`
        : (r.alphaError
            ? `<div class="cm-conv-info-row"><span class="cm-conv-info-label">Transparency</span><span class="cm-conv-info-val bad">Could not preserve \u2014 flattened</span></div>`
            : '');

      ws.innerHTML = `<div class="cm-conv-preview"><div style="flex:1">
        <div class="cm-conv-result-title">\u2705 Upscaled ${UPSCALE_DEFAULT_SCALE}\u00D7</div>
        ${bgToggleRow()}
        <div class="cm-ups-panes">
          <div class="cm-ups-col"><div class="cm-ups-lbl">Original</div><div class="cm-ups-pane"><div class="cm-ups-bg"></div><img src="${r.dataUrl}" alt="Original"></div></div>
          <div class="cm-ups-arrow">\u2192</div>
          <div class="cm-ups-col"><div class="cm-ups-lbl is-out">Upscaled ${UPSCALE_DEFAULT_SCALE}\u00D7</div><div class="cm-ups-pane is-out"><div class="cm-ups-bg"></div><img src="${r.outUrl}" alt="Upscaled"></div></div>
        </div>
        <div class="cm-conv-info">
          <div class="cm-ups-rename">
            <label class="cm-ups-rename-lbl" for="cm-conv-fname">Filename</label>
            <div class="cm-ups-rename-row">
              <input type="text" id="cm-conv-fname" class="cm-ups-rename-input" value="${esc(defaultBase)}" spellcheck="false" autocomplete="off">
              <span class="cm-ups-rename-ext">.${ext}</span>
            </div>
            <div class="cm-ups-rename-hint">Used for both Download and the Media Library record. Edit before you save.</div>
          </div>
          <div class="cm-conv-info-row"><span class="cm-conv-info-label">New Size</span><span class="cm-conv-info-val ok">${newSize}</span></div>
          <div class="cm-conv-info-row"><span class="cm-conv-info-label">Format</span><span class="cm-conv-info-val">${fmtLabel}</span></div>
          ${alphaRow}
          <div class="cm-conv-info-row"><span class="cm-conv-info-label">Vendor</span><span class="cm-conv-info-val">Real-ESRGAN (WaveSpeed)</span></div>
          <div class="cm-conv-actions" style="margin-top:12px;border-top:1px solid #e8e4d8;padding-top:14px">
            <button class="ix-btn ix-btn--secondary" id="cm-conv-download">\u2B07 Download</button>
            <button class="ix-btn ix-btn--primary" id="cm-conv-save-media">\u25A3 Save to Media Library</button>
            <button class="ix-btn ix-btn--danger ix-btn--ghost" id="cm-conv-reset">\u2715 Upscale another</button>
            <span class="cm-conv-status" id="cm-conv-save-status"></span>
          </div>
        </div>
      </div></div>`;
      wireBgToggle(ws);

      // Resolve the operator's current filename: base from the input,
      // extension locked (the format is fixed by the alpha path). The
      // sanitizer strips anything illegal and guarantees the .ext suffix.
      const fnameInput = document.getElementById('cm-conv-fname');
      const resolveName = () => {
        const base = fnameInput ? fnameInput.value : defaultBase;
        return sanitizeOutName(base, ext);
      };

      document.getElementById('cm-conv-download').addEventListener('click', () => {
        const dl = document.getElementById('cm-conv-download');
        const downloadName = resolveName();
        dl.disabled = true; const orig = dl.textContent; dl.textContent = 'Downloading\u2026';
        fetch(r.outUrl).then(res => res.blob()).then(b => {
      ...
      document.getElementById('cm-conv-save-media').addEventListener('click', () => {
        if (fnameInput) fnameInput.disabled = true;  // lock name once committed to the record
        saveToMediaLibrary({
          uploadcareUuid: r.outUuid,
          uploadcareUrl:  r.outUrl,
          fileName:       resolveName(),
          mimeType:       r.isPng ? 'image/png' : 'image/jpeg',
  ...
  console.log('\u21BB InbxConverter v' + VERSION + ' loaded (Upscale paste intake + editable output filename)');
})();
