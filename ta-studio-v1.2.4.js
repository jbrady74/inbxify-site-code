  ...
  document.addEventListener('DOMContentLoaded', function () {
      ...
      '<div class="std-root">' +
        '<div class="ix-hdr">' +
          '<div class="ix-hdr-left">' +
            ...
            '</div>' +
          '</div>' +
          '<span class="ix-badge">v1.2.4</span>' +
        '</div>' +
    ...
    function studioReloadAfterCMSWrite(opts) {
      ...
      try {
        ...
        console.warn('[TA-STUDIO v1.2.4] could not persist restore state:', e);
      }
    ...
    // v1.2.4: best-effort outer-tab activation. The T-A page tab system
    // lives in body.js and uses class-based show/hide. We don't know
    // its exact selector convention, so we try a few common patterns:
    //   1. data-ta-tab="studio" (preferred convention)
    //   2. button/link with text content "Studio" inside a tab nav
    //   3. element with id="studio-tab" or class containing "studio-tab"
    // First match wins. Silent no-op if nothing found — Studio is still
    // mounted, the user just clicks the tab themselves.
    function studioActivateOuterTab() {
      ...
      // Pattern 3
      var p3 = document.querySelector('#studio-tab, .studio-tab, [class*="studio-tab"]');
      if (p3 && candidates.indexOf(p3) === -1) candidates.push(p3);
      // Pattern 2 — text content match within nav-like elements
      Array.prototype.forEach.call(
        document.querySelectorAll('nav a, nav button, [role="tab"], .tab, .ta-tab, .tabs a, .tabs button'),
        function (el) {
          var t = (el.textContent || '').trim().toLowerCase();
          if (t === 'studio' && candidates.indexOf(el) === -1) candidates.push(el);
        }
      );
      // Click the first candidate. If body.js binds click handlers, this
      // activates the tab; if it expects a different event or does its
      // own routing on load, this no-ops harmlessly.
    ...
    function asmFireAttach(slot, mediaId, articleId, it) {
      ...
      if (!roleHash) {
        console.error('[TA-STUDIO v1.2.4] No componentRole hash for slot:', slot);
      ...
      .catch(function (err) {
        console.error('[TA-STUDIO v1.2.4] Asm Route 4 failed:', err);
    ...
    function getRoleHash(roleLabel) {
      ...
      if (!hash) {
        console.error('[TA-STUDIO v1.2.4] No TA_CONFIG.optionIds.componentRole.' + key + ' configured for role:', roleLabel);
        return '';
      }
      return hash;
    }
    ...
    function getRoleHashForSlot(slotKey) {
      ...
      if (!hash) {
        console.error('[TA-STUDIO v1.2.4] No TA_CONFIG.optionIds.componentRole.' + slotKey + ' configured for slot:', slotKey);
        return '';
      }
      return hash;
    }
    ...
    function cmpFireAttach(mediaId, articleId, it) {
      ...
      if (!roleHash) {
        ...
        console.error('[TA-STUDIO v1.2.4] No componentRole hash for role:', it.role);
        showStudioToast('Configuration error \u2014 unknown role.');
        return;
      }
      ...
      .catch(function (err) {
        console.error('[TA-STUDIO v1.2.4] Scenario G Route 4 failed:', err);
    ...
    console.log('\u29BE Studio v1.2.4 mounted \u2014 Components tab (read + attach + archive + hard-delete) + Assembler field-level Attach (Main Image, Interior Images) + auto-reload + state restore');
  });
