/* ============================================================
   ta-components-tab-v1.0.10.js
   INBXIFY TA Studio — Components tab
   ============================================================

   v1.0.10 — Selection circle blue + button readability + drawer 20%

   Three fixes:
   1. Selection circle → corporate blue #5b7fff (was invisible white)
   2. .cmp-btn secondary buttons → #f4f1ea bg (never white on cream)
   3. Drawer → 20vw width, right-justified (no full-screen traversal)

   CSS companion: ta-studio-components-v1.0.15.css

   Webflow head deploy:
     SWAP: ta-components-tab-v1.0.9.js  → ta-components-tab-v1.0.10.js
     SWAP: ta-studio-components-v1.0.14.css → ta-studio-components-v1.0.15.css

   Two layout changes:

   1. FILTER PANEL → ICON + POPOVER
      Filter controls move out of the always-visible vertical
      column into a popover triggered by a filter icon button
      to the right of the search input. Search + filter icon
      sit on one row. Active-filter count badge on the icon
      shows how many filters are non-default at a glance.
      Popover closes on outside click or Escape.
      Gold dirty-border contract preserved on changed selects.

   2. CARD GRID → LIST ROWS
      renderCard() now renders a full-width list row:
        [ thumb 80×56 ] [ name + meta ] [ actions inline ]
      .cmp-grid changes from auto-fill grid to single column.
      Thumbnail is fixed-size (80×56, object-fit:cover).
      Name, status dot + meta, and the three action buttons
      are all on the same horizontal line.
      Attach inline panel expands below the row (full width).

   CSS companion: ta-studio-components-v1.0.14.css

   Webflow head deploy:
     SWAP: ta-components-tab-v1.0.8.js → ta-components-tab-v1.0.9.js
     SWAP: ta-studio-components-v1.0.13.css → ta-studio-components-v1.0.14.css

   Two changes:

   1. CARD ACTIONS REDESIGNED
      renderCardActions() was: [ Details… ] [ Status… ]
      Now: [ Attach ▾ ] [ Status… ] [ Details… ]

      "Attach ▾" is a disclosure trigger. Click opens an inline
      attach panel (same cmp-inline-edit pattern, same gold-border
      dirty state, same Cancel revert) with four asset-type buttons:
        Article | Ad | Event | RE Listing
      Click one → asset picker appears inline, reading from the
      hidden DOM collections (.articles-wrapper, .ads-wrapper,
      [data-events-wrapper], [data-re-wrapper]).
      Select an asset → Save activates → fires Scenario K Route A.

      Customer assign removed from card face. Lives in drawer only.

   2. SAVE TO MEDIA LIBRARY (shared helper)
      window.InbxComponentsTab.saveToMediaLibrary(mediaId)
      Called by ta-converter-v1.1.0.js after a successful
      conversion. Reads the MEDIA item from S.items (already in
      memory if Components tab has been activated) and fires
      Scenario B createMediaFromBrowserUpload. If the Components
      tab hasn't been activated yet this session, it falls back
      gracefully (caller must handle the response themselves).
      Exposed on the public API so Converter can reach it without
      coupling to ta-components-tab internals.

   State additions:
     S.inlineEdit now accepts field === 'attach', with:
       { mediaId, field:'attach', assetType, assetId, assetName,
         snapshot: null }
     assetType: 'article'|'ad'|'event'|'re' (null = type not yet chosen)
     assetId/assetName: null until picker selection made

   Scenario K Route A payload (per platform-data-reference §14b):
     mode: 'attach'
     assetType, assetId, assetCollId (from ASSET_COLLS map)
     mediaIds: [mediaId]
     mediaCollId: '69990992411206c574331aed'
     tenantId: TA_CONFIG.taItemId
     titleSlug: TA_CONFIG.titleSlug
     mediaStatusAttached: TA_CONFIG.optionIds.mediaStatus.attached
     forceReplace: false

   Hardcoding:
     HC-CMP-001 — SCENARIO_K_URL hardcoded; move to TA_CONFIG.makeAttach
                  when that key is added to the page config.
     HC-CMP-002 — MEDIA collection ID hardcoded (69990992411206c574331aed);
                  same value used by ta-bundles. Move to TA_CONFIG.mediaCollId
                  when available.
     HC-CMP-003 — Per-type asset collection IDs hardcoded in ASSET_COLLS;
                  same values in platform-data-reference §6. Move to
                  TA_CONFIG.wfColl* when those keys are added to page config.

   CSS companion: ta-studio-components-v1.0.13.css (new §ATTACH-INLINE block)
   No Make scenario changes — Scenario K Route A already handles all four
   asset types. No CMS schema changes.

   Webflow head deploy:
     SWAP: ta-components-tab-v1.0.7.js → ta-components-tab-v1.0.8.js
     SWAP: ta-studio-components-v1.0.12.css → ta-studio-components-v1.0.13.css
   ============================================================ */

(function () {
  'use strict';

  var FILE_VERSION = '1.0.10';
  var DEBUG = false;
  ...
  function renderCard(it) {
    ...
    var mainRow =
      '<article class="' + classes.join(' ') + '" data-cmp-card="' + escAttr(it.mediaId) + '">' +
        '<div class="cmp-row-select-wrap">' +
          '<span class="cmp-row-select-circle"></span>' +
        '</div>' +
        thumb +
  ...
  function renderAttachPanel(it, e) {
    ...
    var actionRow = (
      '<div class="cmp-inline-actions">' +
        ...
        '<button type="button" class="ix-btn ix-btn--ghost" data-cmp-card-action="cancel-edit"' +
          (saving ? ' disabled' : '') + '>Cancel</button>' +
      '</div>'
    );

    return (
      '<div class="cmp-inline-edit cmp-inline-edit--attach">' +
        '<div class="cmp-attach-inline-label">Attach to\u2026</div>' +
        typeRow +
        pickerRow +
        errorBar +
        actionRow +
    ...
    host.innerHTML =
      ...
      '<aside class="cmp-detail-drawer" role="dialog" aria-modal="true" aria-label="Component details">' +
        ...
        '<div class="cmp-detail-body">' +
          ...
          '</dl>' +
          // v1.0.8: Customer assign lives here (moved from card face)
