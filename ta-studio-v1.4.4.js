...
//
// ──────────────────────────────────────────────────────────
// v1.4.4 (Generator panel now hosts tools):
//   The Generator panel was a permanent .std-placeholder shell
//   ("Coming later…"). Replaced its body with a tool mount point:
//     <div class="std-placeholder-title">Generator</div>
//     <div id="std-generator-mount"></div>
//   The "Generator" heading is KEPT (class .std-placeholder-title)
//   so the v1.4.3 ⓘ info popover still attaches — that class is in
//   INFO_HEADING_SELECTOR. The Ad Reformatter tool (ta-adreformat,
//   separate file) self-mounts into #std-generator-mount on the
//   'std:panel:generator' event, mirroring how the Converter tool
//   mounts into #std-converter-mount. Ad Reformatter is the first
//   real Generator tool; future Generator tools mount the same way.
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    mount.innerHTML =
      '<div class="std-root">' +
        ...
        '<div class="std-panel" data-std-panel-body="generator">' +
          '<div class="std-placeholder-title" style="margin-bottom:16px">Generator</div>' +
          '<div id="std-generator-mount"></div>' +
        '</div>' +
      '</div>';
    ...
    var TAB_INFO = {
      input: {
        title: 'Inputs',
        desc: 'Monitor-only surface showing intake channel health (Drive, Email, Form). Displays file counts across processing states. No actions — triage happens in Components.',
        scenarios: [
          { name: 'Scenario C — Inbox Reader', route: 'folderCounts', webhook: 'TA_CONFIG.scenarioC', notes: 'Read-only folder health poll. Fires on first activation and on manual refresh.' }
        ]
      },
      components: {
        title: 'Components',
      ...
      generator: {
        title: 'Generator',
        desc: 'AI tools that create new assets from inputs. First tool: Ad Reformatter — rebuild an existing print ad into web ad creatives (700x235 banner; 480x400 sidebar later). More Generator tools (e.g. derive teaser/summary from body text) mount here over time.',
        scenarios: [
          { name: 'ix-adreformat Worker', route: 'extract', webhook: 'TA_CONFIG.adReformatWorker', notes: 'Claude vision — decomposes a print ad into parts (text, images, brand colors). Deployed.' },
          { name: 'Scenario B — file conditioner', route: 'createMediaFromBrowserUpload', webhook: 'TA_CONFIG.makeConditioner', notes: 'Ad Reformatter saves the banner creative (sourceChannel=converter route #124).' }
        ]
      }
    };
    ...
    function renderInputsPanel() {
      ...
      el.innerHTML =
        '<div class="inp-root">' +
          ...
          '<div class="inp-section">' +
            '<div class="inp-section-label">Channels</div>' +
            '<div class="inp-channel-strip">' + channelPills + '</div>' +
          '</div>' +

          // ── Section 2: Drive folder health ──
          '<div class="inp-section">' +
            '<div class="inp-section-header">' +
              '<div class="inp-section-label">Drive Folder Health</div>' +
              '<button class="ix-btn ix-btn--ghost inp-refresh-btn"' +
                refreshBtnAttr +
                ' data-inp-refresh' +
                ' title="Refresh folder counts">' +
                '<span' + refreshSpinStyle + '>↻</span>' +
              '</button>' +
            '</div>' +
            errHtml +
            '<div class="inp-drive-tabs' + loadingClass + '">' + driveTabBadges + '</div>' +
            '<div class="inp-drive-note">Counts reflect Drive folders via Scenario C. PDF Queue is managed via the banner above.</div>' +
          '</div>' +

          // ── Section 3: Context note ──
          '<div class="inp-section inp-section--note">' +
            '<div class="inp-monitor-note">' +
              '<strong>Monitor only.</strong> File management and assignment are in the Components tab ' +
              'and the Uploads Processor during this parallel-run window. ' +
              'Full Inputs migration completes at S14 close.' +
            '</div>' +
          '</div>' +
      ...
      // Load counts on first render (lazy — not on Studio mount)
      if (!INP_COUNTS_LOADED && !INP_COUNTS_LOADING) {
    ...
    loadStudioPdfCount();
    console.log('\u29BE Studio v1.4.4 mounted \u2014 Generator panel now hosts tools via #std-generator-mount (Ad Reformatter is first; mounts on std:panel:generator like the Converter). \u24d8 info popover unchanged from v1.4.3.');
  });
