...
(function () {
  ...
  function readTaContext() {
    ...
    return { taId: taId, publisherId: publisherId, titleSlug: titleSlug };
  }

  // Parse abbreviation + most-recent number from existing issue IDs.
  // Returns { abbreviation: 'WLN', nextNumber: 111 } when at least
  // one issue parses cleanly. Otherwise returns null and the modal
  // falls back to manual abbreviation entry.
  function parseIssueContext() {
    ...
    return { abbreviation: bestAbbr, nextNumber: maxNum + 1 };
  }

  // Build modal HTML. Two layouts:
  //   - Auto-fill mode: prefix shown as static label left of number input
  //   - Manual mode (no existing issues): editable abbreviation field
  function renderModalHtml(context, issueCtx) {
      ...
      issueRow =
        '<div class="ppm-field">' +
          ...
          '<div class="ppm-issue-row">' +
            '<span class="ppm-prefix" data-pp-add-prefix>' + esc(issueCtx.abbreviation) + '-</span>' +
            '<input type="number" id="ppm-number" class="ppm-input ppm-number-input" ' +
              'data-pp-add-number value="' + issueCtx.nextNumber + '" min="1" step="1" autocomplete="off">' +
          '</div>' +
          '<div class="ppm-preview" data-pp-add-preview>Preview: ' +
            esc(issueCtx.abbreviation) + '-' + issueCtx.nextNumber +
          '</div>' +
        '</div>';
    } else {
      // Manual mode (HC-011 fallback path — no existing issues to parse)
      issueRow =
        '<div class="ppm-field">' +
        ...
        '<div class="ppm-field">' +
          ...
          '<input type="number" id="ppm-number" class="ppm-input" ' +
            'data-pp-add-number value="1" min="1" step="1" autocomplete="off">' +
  ...
  function openAddPubplanModal() {
    ...
    var issueCtx = parseIssueContext();
    var html = renderModalHtml(context, issueCtx);
    ...
    var prefixSpan  = backdrop.querySelector('[data-pp-add-prefix]');
    var previewEl   = backdrop.querySelector('[data-pp-add-preview]');

    function updatePreview() {
      var abbr = '';
      if (issueCtx) {
        abbr = issueCtx.abbreviation;
      } else if (abbrInput) {
      ...
      // Also update the static prefix label if the user is in auto-fill mode
      // (no-op — abbr is fixed in that mode — kept for symmetry)
    }
    ...
    if (submitBtn) {
      submitBtn.addEventListener('click', function () {
        handleAddPubplanSubmit(context, issueCtx, backdrop);
      });
    }
  ...
  function handleAddPubplanSubmit(context, issueCtx, backdrop) {
    ...
    var abbreviation = issueCtx
      ? issueCtx.abbreviation
    ...
    // Build payload — Make webhook expects JSON with these field names.
    // Multi-tenant IDs are non-negotiable: every record Make creates
    // gets bound to taId / publisherId for tenant isolation.
    var payload = {
      taId:            context.taId,
    ...
    fetch(MAKE_ADD_PUBPLAN_URL, {
      ...
      .then(function (res) {
        ...
        // Make returns 'Accepted' or JSON; we don't need to parse.
        return res.text();
      })
      .then(function () {
        // Success: render persistent banner-style toast, reload after 1s
        // so the new tile appears in the overview. Make publishes the
        // CMS item which triggers a re-render at next page load.
