...
document.addEventListener('DOMContentLoaded', function () {
  ...
  style.textContent = [
    ...
    '.cl-thead{display:grid;grid-template-columns:400px 300px 180px 180px 180px 170px;gap:0;border-bottom:2px solid #1a3a3a;padding:6px 0;align-items:end}',
    ...
    '.cl-card-main{display:grid;grid-template-columns:400px 300px 180px 180px 180px 170px;gap:0;align-items:center;padding:11px 0;cursor:pointer}',
  ...
  function render() {
    ...
    if (!items.length) {
      ...
      items.forEach(function (item) {
        ...
        // Col 3: STATUS — the asset's own state. In-use if assigned anywhere
        // (PubPlan or Newsletter), else Available; plus draft/live dot already
        // on the title. Reads the asset's real condition, not derived guesswork.
        h += '<div class="cl-cell cl-status">';
        if (item.newsletterId) {
          h += '<span class="cl-status-pill cl-status-inuse">In&nbsp;use</span>';
        } else if (item.pubplanId || S.armed[item.id]) {
          h += '<span class="cl-status-pill cl-status-planned">Planned</span>';
        } else {
          h += '<span class="cl-status-pill cl-status-avail">Available</span>';
        }
        h += '</div>';

        // Col 4: PUBPLAN (planning-stage assignment). '-' or the PubPlan name.
        ...
        if (item.pubplanName || item.pubplanId) {
          ...
          h += '<div class="cl-assign-mnls" style="color:#c4a35a">' + esc(S.armed[item.id].blockType) + ' · unsaved</div>';
        } else {
  ...
  function init() {
    ...
    console.log('[CONTENT-LIBRARY v1.0.57] Init:', {
