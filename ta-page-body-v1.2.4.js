/* ============================================================
   ...
   v1.2.4: step 3 breadcrumb — left-to-right orange sweep animation
           processing messages — 4-dot progress indicator, each dot
           lights up as each Claude stage completes
   ============================================================ */
...
<div class="sct-root">
  ...
  <div class="sct-hidden" id="sct-proc-wrap">
    ...
    <div class="sct-proc-stages">
      <div class="sct-proc-stage" id="sct-stage-0">
        <div class="sct-proc-dot"></div>
        <span>Sending to Claude</span>
      </div>
      <div class="sct-proc-stage" id="sct-stage-1">
        <div class="sct-proc-dot"></div>
        <span>Reading structure</span>
      </div>
      <div class="sct-proc-stage" id="sct-stage-2">
        <div class="sct-proc-dot"></div>
        <span>Generating summaries</span>
      </div>
      <div class="sct-proc-stage" id="sct-stage-3">
        <div class="sct-proc-dot"></div>
        <span>Formatting body</span>
      </div>
    </div>
  </div>
...
/* Processing stages — 4-dot progress */
.sct-proc-stages {
  display: flex;
  align-items: center;
...
.sct-proc-stage.active .sct-proc-dot {
  background: #e8a030;
  box-shadow: 0 0 0 3px rgba(232,160,48,.25);
  animation: sct-dot-pulse .9s ease-in-out infinite;
}
.sct-proc-stage.done .sct-proc-dot {
  background: #27ae60;
  box-shadow: none;
  animation: none;
}
@keyframes sct-dot-pulse {
  0%, 100% { box-shadow: 0 0 0 2px rgba(232,160,48,.25); }
  50%       { box-shadow: 0 0 0 5px rgba(232,160,48,.15); }
}
  `;
  ...
  window.sctDoTranscribe = async () => {
    ...
    // Reset all dots
    [0,1,2,3].forEach(i => {
      const stage = document.getElementById('sct-stage-'+i);
      if (stage) stage.classList.remove('active','done');
    });
    // Light up first dot immediately
    const firstStage = document.getElementById('sct-stage-0');
    if (firstStage) firstStage.classList.add('active');

    const ticker = setInterval(() => {
      // Mark current as done, activate next
      const curr = document.getElementById('sct-stage-'+mi);
      if (curr) { curr.classList.remove('active'); curr.classList.add('done'); }
      mi++;
      if (mi < 4) {
        const next = document.getElementById('sct-stage-'+mi);
