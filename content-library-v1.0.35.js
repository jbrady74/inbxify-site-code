...
document.addEventListener('DOMContentLoaded', function () {
  ...
  style.textContent = [
    ...
    '.cl-tabrow{display:flex;align-items:center;justify-content:space-between;gap:20px;margin-bottom:16px;padding-bottom:0}',
    '.cl-tabrow .cl-mtabs{border-bottom:none;margin-bottom:0;flex:0 0 auto;display:inline-flex;gap:6px;background:#f3f0e7;padding:5px;border-radius:11px}',
    '.cl-searchbox{position:relative;flex:0 1 380px;margin-bottom:0}',
    '.cl-searchbox .cl-search-ico{position:absolute;left:14px;top:50%;transform:translateY(-50%);width:15px;height:15px;opacity:.45;pointer-events:none}',
    '.cl-search-input{width:100%;font-family:"DM Sans",system-ui,sans-serif;font-size:14px;padding:11px 14px 11px 40px;border:1.5px solid #e3dfd2;border-radius:10px;background:#fff;color:#1a3a3a;outline:none;transition:border-color .15s,box-shadow .15s}',
    '.cl-search-input:focus{border-color:#1a3a3a;box-shadow:0 0 0 3px rgba(26,58,58,.07)}',
    ...
    '.cl-seg{display:inline-flex;border:1.5px solid #e3dfd2;border-radius:6px;overflow:hidden;background:#fff}',
    '.cl-seg button{font-family:"DM Mono",monospace;font-size:10px;letter-spacing:.03em;padding:6px 12px;border:none;background:transparent;color:#5a6a5a;cursor:pointer;transition:all .12s;border-right:1px solid #e3dfd2;white-space:nowrap;display:inline-flex;align-items:center;justify-content:center}',
    '.cl-seg button:last-child{border-right:none}',
    '.cl-seg button:hover{background:#f0ede4;color:#1a3a3a}',
    '.cl-seg button.on{background:#1a3a3a;color:#fff}',
    '.cl-seg button .sgc{font-size:9px;opacity:.6;margin-left:4px}',
    '.cl-seg button.on .sgc{opacity:.8}',
  ...
  function setMain(tab) { S.mainTab = tab; S.subTab = 'all'; S.q = ''; S.editingId = null; render(); }
  ...
  function render() {
    ...
    h += '<button class="cl-mtab ' + (S.mainTab === 'events' ? 'on' : '') + '" onclick="_clSetMain(\'events\')">Events<span class="tc">' + evCt + '</span></button>';
    h += '<button class="cl-mtab ' + (S.mainTab === 'realestate' ? 'on' : '') + '" onclick="_clSetMain(\'realestate\')">Real Estate<span class="tc">' + reCt + '</span></button>';
    h += '</div>';
    // inline search (no longer its own row)
    var searchLabel = S.mainTab === 'realestate' ? 'real estate' : S.mainTab;
    h += '<div class="cl-searchbox">';
    h += '<svg class="cl-search-ico" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="7" cy="7" r="5" stroke="#1a3a3a" stroke-width="1.6"/><path d="M11 11l3.5 3.5" stroke="#1a3a3a" stroke-width="1.6" stroke-linecap="round"/></svg>';
    ...
    // ── Filter strip (v1.0.34): labeled segmented controls, always-labeled ──
    h += '<div class="cl-fstrip">';

    // Section group (sub-tabs) — only meaningful for articles/ads with MNLS
    var tabItems = itemsForTab(S.mainTab);
    if (allMnls.length) {
      h += '<div class="cl-fgroup"><span class="cl-flabel">Section</span><div class="cl-seg">';
      h += '<button class="' + (S.subTab === 'all' ? 'on' : '') + '" onclick="_clSetSub(\'all\')">All<span class="sgc">' + tabItems.length + '</span></button>';
      allMnls.forEach(function (m) {
        var ct = tabItems.filter(function (i) { var im = findMnlsForItem(i); return im && im.id === m.id; }).length;
        h += '<button class="' + (S.subTab === m.id ? 'on' : '') + '" onclick="_clSetSub(\'' + m.id + '\')">' + esc(m.abbr) + '<span class="sgc">' + ct + '</span></button>';
      });
      h += '</div></div>';
    }
    ...
    if (relProds.length) {
      h += '<div class="cl-fgroup"><span class="cl-flabel">Product</span>';
      h += '<select class="cl-sort" onchange="_clSetSub(this.value)"><option value="all">All products</option>';
      allMnls.forEach(function (m) {
    ...
    // Content status — now WITH an explicit All segment (fixes invisible label)
    h += '<div class="cl-fgroup"><span class="cl-flabel">Status</span><div class="cl-seg">';
    h += '<button class="' + (S.contentStatus === 'all' ? 'on' : '') + '" onclick="_clSetCS(\'all\')">All</button>';
    h += '<button class="' + (S.contentStatus === 'draft' ? 'on' : '') + '" onclick="_clSetCS(\'draft\')">Draft</button>';
    h += '<button class="' + (S.contentStatus === 'live' ? 'on' : '') + '" onclick="_clSetCS(\'live\')">Live</button>';
    h += '</div></div>';

    // Assignment — explicit All segment too
    h += '<div class="cl-fgroup"><span class="cl-flabel">Assignment</span><div class="cl-seg">';
    h += '<button class="' + (S.assignStatus === 'all' ? 'on' : '') + '" onclick="_clSetAS(\'all\')">All</button>';
    h += '<button class="' + (S.assignStatus === 'assigned' ? 'on' : '') + '" onclick="_clSetAS(\'assigned\')">Assigned</button>';
    h += '<button class="' + (S.assignStatus === 'available' ? 'on' : '') + '" onclick="_clSetAS(\'available\')">Available</button>';
    h += '</div></div>';
    ...
    if (!items.length) {
      ...
      items.forEach(function (item) {
        ...
        if (isAssigned && item.newsletterName) {
        ...
        } else if (item.pubplanName && item.slot) {
          ...
          h += ' &nbsp;|&nbsp; <span class="cl-assign-slot">' + esc(item.slot) + '</span></div>';
  ...
  function init() {
    ...
    var ads = readAds();
    var events = readEvents();      // v1.0.33
    var realestate = readRE();      // v1.0.33 (empty until list bound)
    ALL_ITEMS = articles.concat(ads).concat(events).concat(realestate);
