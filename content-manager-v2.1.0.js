...
// v2.1.0: Step 1 = Collection (Articles/Ads/Events/RE)
//         Step 2 = Customer (optional, filters items)
//         Step 3 = CMS Item (filtered by collection + customer)
//         Step 4 = Tag each file's element type
//         No Product Library dependency — collection drives everything
//         Customer + Item appear simultaneously after collection pick
// ============================================================
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ── Customers from hidden DOM ──
  const CUSTOMERS = Array.from(document.querySelectorAll('.customers-wrapper[data-item]')).map(el => ({
    id: el.getAttribute('data-id') || '', name: el.getAttribute('data-name') || '',
  })).filter(c => c.id);

  // ── CMS items from hidden DOM (same selectors as PubPlan code) ──
  const ARTICLES = Array.from(document.querySelectorAll('.articles-wrapper')).map(el => ({
  ...
  const ADS = Array.from(document.querySelectorAll('.ads-wrapper')).map(el => ({
    id: el.dataset.adId || '',
    name: el.dataset.adName || el.dataset.adTitle || el.dataset.name || '',
...
<div class="cm-root">
  <div class="cm-hdr">
    ...
    <span class="cm-badge">v2.1.0</span>
  </div>
  ...
  function renderModal() {
      ...
      <div class="cm-modal">
        ...
        <div class="cm-modal-body">
          ...
          <div class="cm-fr"><div class="cm-ff">
            <label class="cm-fl">Assign To <span class="req">*</span></label>
    ...
    if (s1) {
      ...
      h += `<div class="cm-fr"><div class="cm-ff">
        ...
        <select class="cm-fsel${chg(A.customerId)}" data-cm-field="customerId">
      ...
      if (A.isNew) {
        ...
        h += `<div class="cm-fr"><div class="cm-ff">
          ...
          <select class="cm-fsel${chg(A.cmsItemId)}" data-cm-field="cmsItemId">
  ...
  // ── Modal event binding (runs after every renderModal) ──
  ...
  // ── Init ──
  loadFiles('DONE');
  console.log('\u{1F4C2} Content Manager v2.1.0 mounted');
});
