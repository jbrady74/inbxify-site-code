// pubplan-v4.0.31.js
// CHANGES FROM v4.0.30:
// - [FIX #1]  changed in BA only: Line 1410 (BA): 'hidden-ba-picker-id' → 'hidden-ad-id'
...
function getTitleadminId() {
  const el = document.querySelector('[data-titleadmin-id]');
...
function initFaState() {
  ...
  for (let i = 1; i <= 4; i++) {
    const code = `fa-${i}`;
    if (!state[code]) {
      state[code] = { sc: code, secC: 'fa', slotNum: i, section: 'Feature Article', catId: '', catNm: '', artId: '', artNm: '', custId: '', custNm: '', sponsorId: '', sponNm: '', artAdId: '', artAdName: '', noSponsor: false, catChanged: false, dirty: false };
    }
  }
}
...
function initTsState() {
  ...
  for (let i = 1; i <= 4; i++) {
    const code = `ts-${i}`;
    if (!state[code]) {
      state[code] = { sc: code, secC: 'ts', slotNum: i, section: 'Themed Spotlight', catId: '', catNm: '', artId: '', artNm: '', sponsorId: '', sponNm: '', artAdId: '', artAdName: '', noSponsor: false, catChanged: false, dirty: false };
    }
  }
}
...
function initBaState() {
  ...
  for (let i = 1; i <= 12; i++) {
    const code = `ba-${i}`;
    if (!state[code]) {
      state[code] = { sc: code, secC: 'ba', slotNum: i, section: 'Banner Ad', catId: '', catNm: '', custId: '', custNm: '', adId: '', adName: '', dirty: false };
    }
  }
}
