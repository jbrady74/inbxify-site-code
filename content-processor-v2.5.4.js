...
/* ── Transcriber reskin — hide standalone header when inside Content Processor tab ── */
...
<div class="cm-root">
  ...
  function renderChannels() {
    const gdCount = FILES.length ? ` (${FILES.filter(f => f.status==='DONE' && !f.assigned).length})` : '';
    let h = '';
    CHANNELS.forEach(ch => {
      const isActive = ch.id === CHANNEL;
      const countHtml = ch.id === 'gdrive' && gdCount ? `<span style="font-family:\'DM Mono\',monospace;font-size:9px;opacity:.6">${gdCount}</span>` : '';
      h += `<button class="cm-channel ${isActive?'active':''}" data-cm-channel="${ch.id}">${esc(ch.label)}${countHtml}</button>`;
    });
  ...
  function renderBreadcrumb() {
    ...
    const folders = {
      'pdf-queue':  { label: 'Pre-processing Folder',       id: window.TA_CONFIG?.workingFolderId || '' },
      'ready':      { label: 'Ready for Assignment Folder',  id: window.TA_CONFIG?.processedFolderId || '' },
      'assigned':   { label: 'Ready for Assignment Folder',  id: window.TA_CONFIG?.processedFolderId || '' },
      'archived':   { label: 'Ready for Assignment Folder',  id: window.TA_CONFIG?.processedFolderId || '' },
      'hidden':     { label: 'Pre-processing Folder',       id: window.TA_CONFIG?.workingFolderId || '' },
    };
