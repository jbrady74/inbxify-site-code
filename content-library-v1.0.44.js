...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ── v1.0.44: fetch one page of articles from the list webhook ──
  function fetchArticlesPage(offset) {
    if (!LIST_WEBHOOK) return Promise.resolve(null);
    var url = LIST_WEBHOOK + (LIST_WEBHOOK.indexOf('?') === -1 ? '?' : '&') +
      'assetType=article&taItemId=' + encodeURIComponent(TA_ITEM_ID) +
    ...
    return fetch(url, { method: 'GET' })
      ...
      .catch(function (e) { console.warn('[CL] article page fetch failed', e); return null; });
  }

  // ── v1.0.44: load articles (page 0), append more on demand ──
  function loadArticles(initial) {
    if (artPaging.loading || artPaging.done) return Promise.resolve();
    artPaging.loading = true;
    var off = artPaging.offset;
    return fetchArticlesPage(off).then(function (resp) {
      artPaging.loading = false;
      if (!resp) { artPaging.done = true; return; }
      // resp.items may be {items:[...],pagination} OR a bare array, depending
      // on how the Response module is mapped. Handle both.
      ...
      artPaging.total = total;
      // map + multi-tenant filter (defensive — server may already be scoped)
      var mapped = arr.map(mapWebhookArticle).filter(function (a) {
        return !TA_ITEM_ID || !a.associatedTitle || a.associatedTitle === TA_ITEM_ID;
      });
      // append to ALL_ITEMS (articles only; keep other collections intact)
      ALL_ITEMS = ALL_ITEMS.concat(mapped);
      enrichItems(mapped);
      artPaging.offset = off + arr.length;
      if (!arr.length || (total != null && artPaging.offset >= total)) artPaging.done = true;
      render();
    });
  }
  ...
  var PAGE_SIZE    = 100;
  var artPaging = { offset: 0, total: null, loading: false, done: false };
  var PUBPLANS = [];
  ...
  function render() {
    ...
    h += '<div class="cl-searchrail"><div class="cl-searchbox">';
    ...
    if (allMnls.length) {
      h += '<div class="cl-fgroup"><span class="cl-flabel">Section</span><div class="cl-pillrow">';
      h += pill(S.subTab === 'all', "_clSetSub('all')", 'All', tabItems.length);
      allMnls.forEach(function (m) {
        var ct = tabItems.filter(function (i) { var im = findMnlsForItem(i); return im && im.id === m.id; }).length;
        h += pill(S.subTab === m.id, "_clSetSub('" + m.id + "')", esc(m.abbr), ct);
      });
      h += '</div></div>';
    }
    ...
    h += '<div class="cl-fgroup"><span class="cl-flabel">Status</span><div class="cl-pillrow">';
    h += pill(S.contentStatus === 'all',   "_clSetCS('all')",   'All');
    h += pill(S.contentStatus === 'draft', "_clSetCS('draft')", 'Draft');
    h += pill(S.contentStatus === 'live',  "_clSetCS('live')",  'Live');
    h += '</div></div>';

    // Assignment — explicit All segment too
    h += '<div class="cl-fgroup"><span class="cl-flabel">Assignment</span><div class="cl-pillrow">';
    h += pill(S.assignStatus === 'all',       "_clSetAS('all')",       'All');
    h += pill(S.assignStatus === 'assigned',  "_clSetAS('assigned')",  'Assigned');
    h += pill(S.assignStatus === 'available', "_clSetAS('available')", 'Available');
    h += '</div></div>';
  ...
  // page bottom, fetch the next page. Throttled by artPaging.loading/done.
  var _scrollWired = false;
  function wireLazyScroll() {
    if (_scrollWired) return;
    _scrollWired = true;
    window.addEventListener('scroll', function () {
      if (S.mainTab !== 'articles') return;
      if (artPaging.loading || artPaging.done) return;
      var nearBottom = (window.innerHeight + window.scrollY) >= (document.body.offsetHeight - 600);
      if (nearBottom) loadArticles(false);
    }, { passive: true });
  }
  ...
  function init() {
    ...
    // v1.0.44: articles come from the webhook (paged). Other types: DOM.
    var ads = readAds();
    var events = readEvents();
    var realestate = readRE();
    ALL_ITEMS = ads.concat(events).concat(realestate);
    enrichItems(ALL_ITEMS);
    // kick off article loading (page 0) — appends + re-renders when it lands
    loadArticles(true);
    wireLazyScroll();

    console.log('[CONTENT-LIBRARY v1.0.44] Init:', {
