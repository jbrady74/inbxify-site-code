...
(function () {
  ...
  // The date the blast actually went out, per the report. Falls back to the
  // record's own publish date when nothing has been ingested yet.
  function sendDate(nl) {
    var camps = nl.stats ? nexCampaigns(nl.stats) : [];
    for (var i = 0; i < camps.length; i++) {
      var d = parseBroadcast(camps[i].broadcastDate);
      if (d) return { d: d, src: 'report' };
    }
    return nl.date ? { d: nl.date, src: 'record' } : null;
  }
  ...
  function rowNl(nl, idx) {
    ...
    var sdCell = dash();
    if (sd) {
      var drift = sd.src === 'report' && nl.date && !sameDay(sd.d, nl.date);
      sdCell = '<span class="perf-senddate' + (drift ? ' perf-senddate--drift' : '') + '"'
        + (drift ? ' title="Report says ' + esc(dLong(sd.d)) + ', record says ' + esc(dLong(nl.date)) + '"' : '')
        + '>' + esc(dShort(sd.d)) + (drift ? '\u00a0\u2303' : '') + '</span>';
    }
    return '<tr class="perf-row' + (wait ? ' perf-row--wait' : '') + '" data-perf-nl="' + idx + '">'
      ...
      + '<td class="perf-senddate-cell">' + sdCell + '</td>'
      + '<td>' + status + '</td>'
  ...
  function render() {
    ...
    view.innerHTML = '<div class="ta-perf__card perf-nl-card"><table>'
      ...
      + '<th>Send date</th><th>Status</th><th>Send time</th>'
      ...
      + (NLS.map(rowNl).join('') || '<tr><td colspan="12">' + dash('no newsletters found \u2014 is the newsletter-wrapper list on the page?') + '</td></tr>')
      + '</tbody></table></div>';
  }
