// ix-success-toast-v1.0.1.js
// ============================================================
// ix-success-toast-v1.0.1.js
//
// v1.0.1 — A RETRY CLEARS ITS OWN OLD RESULT
//
//   Failure and "not confirmed" messages stay until dismissed, on
//   purpose. But a later attempt at the SAME action left the old red
//   message on screen beside the new result. Pub Plans showed
//   "Set as Next failed for WLN-121A" next to a tile that had just
//   been confirmed as Next (2026-09-25).
//
//   New optional `key` on submit() and show(). Every message carries
//   its key. When submit() starts with a key, it first dismisses any
//   open message holding the same key: the new attempt supersedes
//   the old result, whatever that result was.
//
//   Callers choose a key per action AND per record, e.g.
//     key: 'issues-tab:set-next:' + pubplanId
//   so a retry on WLN-121A never clears a message about WLN-122A.
//
//   New public: IxToast.dismissKey(key).
//   No key given = v1.0.0 behaviour exactly. Nothing else changed.
//
// ============================================================
//
// ── v1.0.0 header follows ──
//
//
...
(function () {
  ...
  // v1.0.1 — close every open message that carries `key`.
  function dismissKey(key) {
    if (!key) return;
    var el = document.getElementById(RAIL_ID);
    if (!el) return;
    Array.prototype.slice.call(el.children).forEach(function (c) {
      if (c.getAttribute('data-ix-key') === String(key)) dismiss(c);
    });
  }

  // opts: { detail, fields, sticky, timeout, key }
  function render(msg, variant, opts) {
    ...
    if (opts.key) card.setAttribute('data-ix-key', String(opts.key));
  ...
  function submit(opts) {
    ...
    // v1.0.1 — a new attempt supersedes any earlier result for the
    // same action and record.
    if (opts.key) dismissKey(opts.key);

    var btnState = beginButton(opts.button, opts.busyLabel);
    var busyToast = opts.busyToast ? render(opts.busyToast, 'busy', { key: opts.key }) : null;
    ...
    function finish(result) {
      ...
      if (opts.silent !== true) {
        if (verified) {
          render(msgs.success || opts.success || 'Saved', 'ok', { key: opts.key });
  ...
  window.IxToast.dismissKey = dismissKey;
