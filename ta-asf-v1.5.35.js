...
(function () {
  ...
  var VERSION = '1.5.35';   // v1.5.31 — was stuck at 1.5.26 through .27-.30
    ...
    window.InbxGenerate.fromBody(body, S.sourceMetadata || null).then(function (out) {
      ...
      if (out && Array.isArray(out.altered)) {
        ...
        for (var ai = 0; ai < out.altered.length; ai++) {
          ...
          if (af === 'subtitle') S.aiFilled.subtitle = true;
          else if (af === 'title') S.aiFilled.name = true;
          else if (af) S.aiFilled[af] = true;
        }
      }
  ...
  function applySourceMetadata(meta, resolvedFields) {
    ...
    if (rawTitle) {
      if (!lim.title || rawTitle.length <= lim.title) put('name', rawTitle);
      else meta.overLimit = (meta.overLimit || {}), meta.overLimit.title = rawTitle;
    }
    if (rawSub) {
      if (!lim.subtitle || rawSub.length <= lim.subtitle) put('subtitle', rawSub);
      else meta.overLimit = (meta.overLimit || {}), meta.overLimit.subtitle = rawSub;
    }
  ...
  function applyTextIdentity(bodyHtml, resolvedFields) {
    ...
    for (var k in res.fields) {
      ...
      // Over-limit values are parked, not truncated — same rule the
      // metadata path uses. AI-assist compresses them if it runs.
      if (lim[k === 'name' ? 'title' : k] && v.length > lim[k === 'name' ? 'title' : k]) {
        log('textParse: ' + k + ' exceeds its limit (' + v.length + ') — left for review');
        continue;
      }
      tgt[k] = v;
      S.textFilled[k] = true;
  ...
  function renderSourceMetaPanel() {
    ...
    if (meta.overLimit) {
      ...
      if (bits.length) {
        overLine =
          '<div class="asf-meta-note asf-meta-note--warn">' +
            bits.join(' and ') + ' exceeded the field limit, so the AI ' +
            'shortened it using only the author&rsquo;s own words. The full ' +
            'original is shown below &mdash; copy it in if you prefer to trim it yourself.' +
          '</div>';
      }
    }
    return '' +
