/* ============================================================
   ta-adreformat-v1.0.2.js
   INBXIFY TA Studio — Ad Reformatter (Generator tab tool)

   v1.0.2 — FIX: staged source image now renders. v1.0.1 set the
     <img src> by interpolating the base64 data URL into the HTML
     string, which silently failed to paint (boxes drew, image
     blank). Now the src is assigned as a property after the node
     exists, plus the img is guarded against the publisher-wrapper
     white-background reset and zero-height collapse. No other
     change; compositing (Pieces 3+4) still pending.

   v1.0.1 — version bump only (fresh deploy slot over v1.0.0).
...
(function () {
  ...
  function injectStyles() {
    ...
    var css = [
      ...
      '.ix-arf-src{display:block;width:100%;height:auto;min-height:40px;background:transparent !important;}',
    ...
    root.innerHTML =
      '<div class="ix-arf-stage">' +
        '<div class="ix-arf-srcwrap">' +
          '<img class="ix-arf-src" id="ix-arf-srcimg" alt="Source ad">' +
      ...
      '</div>';
    // Set the data-URL via property, NOT innerHTML interpolation. A long
    // base64 string inside a quoted HTML attribute is where the source
    // silently failed to paint (v1.0.1). Property assignment is robust.
    var srcImg = document.getElementById('ix-arf-srcimg');
    if (srcImg) srcImg.src = S.dataUrl;
