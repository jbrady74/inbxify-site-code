/*
   ...
   DEPLOY:
     ...
     • Add: ta-intake-manager-v1.0.1.js (new filename — clean cache).
...
(function () {
  ...
  /* ── HC · documented fallbacks (override via TA_CONFIG) ──
     HC-INTK-1 · SCENARIO_I_FALLBACK  — Bundles_Read responder (hook 2718766).
       NOTE: this MUST be the URL that returns the JSON payload
       {ok, media[], ...} from the WebhookRespond module — NOT a
       hook that answers "Accepted". The legacy ta-bundles-v1.2.1
       used fkqas4u7... as its working reader; we match it here.
  ...
  function fetchAndRender(force) {
    ...
    .then(function (txt) {
      ...
      // Make's default webhook ack is the literal text "Accepted" — that means
      // the scenario answered immediately instead of running to its
      // WebhookRespond module. Surface a clear, actionable message.
      if (trimmed === '' || /^accepted$/i.test(trimmed)) {
        throw new Error('Reader returned "' + (trimmed || 'empty') +
          '" instead of JSON — the Scenario I webhook is not the Bundles_Read responder, ' +
          'or that scenario is set to immediate-response. Check TA_CONFIG.scenarioI / hook 2718766.');
      }
      var data;
      try { data = JSON.parse(trimmed); }
      catch (e) { throw new Error('Reader response was not valid JSON: ' + trimmed.slice(0, 80)); }
