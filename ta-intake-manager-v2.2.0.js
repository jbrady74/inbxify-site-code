...
(function () {
  ...
  function renderRowThumb(mediaItem, mime) {
    ...
    var rawType = (fd['media-type'] || '').toString().toLowerCase();
    var isText = rawType.indexOf('text') !== -1 || (mime && mime.indexOf('text/') === 0);
    var isImage = !!imgUrl || (mime && mime.indexOf('image/') === 0);

    if (isImage && imgUrl) {
      // Uploadcare: append a small resize transform for a light thumbnail.
      var thumb = imgUrl;
      if (imgUrl.indexOf('ucarecd') !== -1 && imgUrl.indexOf('/-/') === -1) {
        thumb = imgUrl.replace(/\/?$/, '/') + '-/resize/72x/-/format/auto/-/quality/lighter/';
      }
