/* ta-intake-manager-v3.0.5.js
   ============================================================
   ta-intake-manager-v3.0.5.js
   ============================================================
   Studio · Intake tab — bundle/loose-file management.

   ── v3.0.5 — hand the ASF the upload's metadata table (24 Sept) ──
   BUG (Jeff, WLN "Wyckoff Centennial"): the upload's own table said
   Title / Subtitle / By Line: By Lynn Bruggemann, and the Asset Form
   opened with no writer, no subtitle, and the first body paragraph
   as the headline.

   CAUSE: the html-clean Worker lifts that table into the text MEDIA
   row's source-metadata field. buildPrefilledMedia() passed the ASF
   id / mediaType / imageUrl / htmlContent and NOT source-metadata,
   so the ASF fell back to reading it off the page's MEDIA list — and
   a MEDIA row created since the last site publish is not on the page.
   ta-asf's resolver already prefers rm.sourceMetadata (v1.5.28 note:
   "if the array later starts carrying sourceMetadata, the lookup
   never fires").

   FIX: buildPrefilledMedia() also passes sourceMetadata from the
   record intake already holds (Scenario I payload). Nothing else
   changed.

   ── v3.0.3 — row pill honors MEDIA.status (link-missing fix) ────
   BUG (Jeff, screenshot, bundle 001 "One Fish Two Fish Wyckoff
   Fish"): 7 files. One image resolved "-> One Fish, Two Fish,
   Wyckoff Fish". The text body MEDIA rendered "BODY - LOCKED"
   (which only fires when status === attached) yet ALSO rendered
   "AWAITING CREATE". Five other images stayed "AWAITING CREATE".
   Bundle rolled up "PARTIALLY ASSIGNED".

   ROOT CAUSE, CLIENT SIDE: renderFileRow was the only surface in
   this file that ignored MEDIA.status. It keyed the pill purely
   off getMediaAssetRefs().length. computeBundleStatus() and
   looseBucketOf() both already treat attached/archived as
   resolved. So one MEDIA could be counted assigned at the bundle
   level and simultaneously read "Awaiting Create" on its own row.
   The text row is the proof: the re-attach guard requires
   status === attached, so status WAS attached, and the pill still
   said Awaiting Create.

   FIX HERE: row pill precedence is refs > status. Named refs win
   when present. With zero refs, status === attached renders
   "ASSIGNED - LINK MISSING" and status === archived renders
   "ARCHIVED". Only a genuinely unresolved row still reads
   "AWAITING CREATE". Only renderFileRow changed.

   NOT FIXED HERE (Make side): the five images with no ref AND no
   attached status are a real write gap. Image 6 resolved to the
   article's real NAME, not an id fallback, which proves the
   ARTICLE record IS in the Scenario I payload — so the read is
   fine. The created ARTICLE holds one MEDIA id in its media-items
   MRF instead of six, and the remaining images never received an
   `article` ref on MEDIA. That is a Scenario 104 / Scenario K
   mapping defect, same failure shape as TD-104-MEDIA-NULL. This
   build stops the UI from lying about the rows it CAN account
   for; it cannot invent links Make never wrote.

   New CSS selectors (pair with ta-intake-manager-v3.0.9.css):
     .bdl-pill--assigned   amber, "link missing" state
     .bdl-pill--archived   slate, was referenced by renderBundleCard
                           since v1.3.0 but never had a style rule

   ── v3.0.2 — image thumbnail render fix (TD-245) ────────────────
   BUG: image-type row thumbnails rendered as a solid blue-gray tile
   with NO image, even though image-url was valid (loaded fine in a
   standalone tab) and the inline style attribute was correctly set.
   Root cause is the T-A page chrome, not the data or this file's
   logic: title-admin-page-design ships a wildcard
     section.publisher-wrapper * {
       background-color: #fff !important;
       background-image: none  !important;
     }
   The `background-image: none !important` (selector-level longhand)
   wiped our inline `background-image:url(...)` because the inline
   value carried NO !important — a plain inline declaration loses to
   an !important selector declaration. This is the SAME cascade bug
   the Ad Reformatter fought at v0.4.5/0.4.6, and the architecture
   note's rule applies: injected elements need inline !important and
   must target the specific longhand.
   FIX: renderRowThumb() now emits the inline background-image WITH
   !important. An !important longhand inline beats an !important
   longhand in a selector, so the thumbnail survives the wildcard.
   Pairs with ta-intake-manager-v3.0.3.css (which chained
   .bdl-thumb.bdl-thumb--img / --text selectors to win the
   background-COLOR side of the same wildcard). Only renderRowThumb
   changed here.

   ── v3.0.1 — Continue-to-Asset-Form populate fix (WLN-114) ──────
   BUG: "Continue to Asset Form" opened ASF in create mode but the
   form came up EMPTY. Root cause is in ASF, not here: ASF's
   resolvePrefilledMediaIds resolved the passed ids by scraping
   .media-wrapper[data-item] nodes that ASF only paints on the
   ARTICLE EDIT page. From the fresh Bundles cascade those nodes
   aren't present → every id missed → "MEDIA not on page for id …"
   → blank form.
   FIX (paired with ta-asf v1.5.18, Option B): intake already holds
   the full MEDIA objects (it rendered the selected rows from the
   Scenario I payload), so we now ALSO pass the resolved MEDIA in
   the handoff as `prefilledMedia` — an array of lightweight shapes
   { id, mediaType, imageUrl, htmlContent } built straight from
   fieldData. ASF v1.5.18 reads prefilledMedia first and falls back
   to the DOM scrape when absent (older-caller-safe). `prefilledMediaIds`
   is still sent unchanged for back-compat.
   No new fetch, no Make scenario — this path is a pure client
   handoff; "Continue to Asset Form" never called a scenario.
   Field keys confirmed from platform-data-reference v1.30 §1:
   MEDIA.media-type, MEDIA.image-url, MEDIA.html-content.
   Only doCreateOpen changed.
...
(function () {
  ...
  function init() {
    ...
    if (window.console && console.log) console.log('[Intake] v' + FILE_VERSION + ' loaded (ta-intake-manager) — image thumb inline !important fix (TD-245); row thumbnails + TEXT tiles, bigger refresh, stronger dot; loose-first, activity dots, auto-refresh; subtabs + archive/restore + attach/create cascade');
  }
  ...
  // v3.0.1 (WLN-114) · Resolve selected ids into the lightweight MEDIA
  // shape ASF's resolver expects, sourced from the records intake already
  // loaded (Scenario I payload). Order follows the selection order so the
  // first image becomes Main and text becomes the body, matching ASF's
  // slot logic. Items not found are skipped (ASF still has the id and can
  // fall back to its DOM scrape).
  function buildPrefilledMedia(ids) {
    var out = [];
    for (var i = 0; i < ids.length; i++) {
      var rec = _findMediaById(ids[i]);
      if (!rec) continue;
      var fd = rec.fieldData || {};
      out.push({
        id:          ids[i],
        mediaType:   mediaTypeOf(fd),                                  // 'text'|'image'|'video'|'other'
        imageUrl:    fd['image-url'] || fd.imageUrl || fd.image || '',
        htmlContent: fd['html-content'] || fd.htmlContent || '',
        // v3.0.5 — the upload's metadata table (Title / Subtitle / By Line …),
        // lifted out by the html-clean Worker. ta-asf reads this first.
        sourceMetadata: fd['source-metadata'] || fd.sourceMetadata || ''
      });
    }
    return out;
  }
  ...
  function doCreateOpen(c) {
    ...
    // v3.0.1 (WLN-114): resolve the selected ids into full MEDIA shapes
    // HERE — intake already has the records from the Scenario I payload —
    // and pass them so ASF doesn't have to scrape page DOM that isn't
    // present on the fresh Bundles cascade. Keys mirror ASF's resolver:
    // id / mediaType ('text'|'image'|…) / imageUrl / htmlContent.
    var prefilledMedia = buildPrefilledMedia(mediaIds);
    try {
      window.InbxASF.open({
        mode:              'create',
        assetType:         c.type,
        prefilledMediaIds: mediaIds,        // back-compat (DOM-scrape fallback)
        prefilledMedia:    prefilledMedia,  // v3.0.1 — resolved payload (preferred)
        tenantId:          S.tenantId
      });
  ...
  function renderFileRow(mediaItem, lookup) {
    ...
    // v3.0.3 · row pill now honors MEDIA.status as a fallback signal.
    // Prior behavior: the pill was driven ONLY by refs.length, so a MEDIA
    // row whose status had already flipped to Attached still rendered
    // "Awaiting Create" whenever the asset link was missing from the read
    // payload. That made renderFileRow the only surface in this file that
    // ignored status — computeBundleStatus and looseBucketOf both treat
    // attached/archived as resolved. Rows now agree with the rollup.
    //
    // Precedence: named refs > status. A resolved ref is strictly more
    // informative than a bare status, so it always wins when present.
    var rowStatus = readMediaStatus(mediaItem);

    var refsHtml;
    if (refs.length === 0) {
      if (rowStatus === 'attached') {
        // Status says attached but no asset ref resolved in this payload.
        // Either the link write is missing on the Make side, or the asset
        // is absent from the Scenario I read. Surfaced honestly rather
        // than mislabeled as un-created work.
        refsHtml = '<span class="bdl-pill bdl-pill--assigned"' +
          ' title="MEDIA.status = Attached, but no asset link resolved in this read.' +
          ' Either the create/attach did not write the link, or the asset is not in the Scenario I payload.">' +
          'Assigned \u2014 link missing</span>';
      } else if (rowStatus === 'archived') {
        refsHtml = '<span class="bdl-pill bdl-pill--archived">Archived</span>';
      } else {
  ...
  /* v3.0.4 — split the selection into bundle-resident vs loose so the
     drawer can say WHERE the files are. Now that selections survive tab
     changes, a bare count is not enough: three files checked in Bundles
     are invisible while the operator is on Loose, and they would still
     ride into Create. */
  function selectionBreakdown() {
    var out = { bundle: 0, loose: 0, total: 0 };
    var all = mergeMedia(S.data) || [];
    var byId = {}, i;
    ...
    for (i = 0; i < ids.length; i++) {
      var m = byId[ids[i]];
      out.total++;
      if (!m) { out.loose++; continue; }   // unknown → count as loose, never drop
      var fd = m.fieldData || {};
      if (fd['bundle-id']) out.bundle++; else out.loose++;
    }
    return out;
  }
  ...
  function renderActionBar() {
    ...
    // Only show the breakdown when the selection actually spans both —
    // a single-source selection reads cleaner as a plain count.
    var where = (bd.bundle && bd.loose)
      ? ' <span class="bdl-action-bar-where">(' + bd.bundle + ' in Bundles, ' +
        bd.loose + ' loose)</span>'
      : '';
    ...
    return (
      '<div class="bdl-action-bar">' +
        '<div class="bdl-action-bar-count">' +
          '<strong>' + n + '</strong> file' + (n === 1 ? '' : 's') + ' selected' + where +
        '</div>' +
  ...
  document.addEventListener('click', function (e) {
    ...
    if (l1btn) {
      ...
      // v3.0.4 — the selection SURVIVES navigation. Removing this wipe is
      // the whole fix: v1.1.0 shipped "multi-select across bundle AND
      // loose rows", then v1.3.0 added these tabs and cleared everything
      // on every switch, so picking files in a Bundle and crossing to
      // Loose to add more silently emptied the drawer. Nothing needed
      // clearing here — pruneStaleSelections() already drops ids that
      // have genuinely gone away, and it checks against mergeMedia(),
      // which spans bundles and loose together.
      render();
      return;
    }
    ...
    if (l2b) {
      ...
      // v3.0.4 — filter chips keep the selection too (same reasoning).
      render();
      return;
    }
  ...
  function renderRowThumb(mediaItem, mime) {
    ...
    if (imgUrl) {
      ...
      // v3.0.2 (TD-245): inline background-image MUST carry !important.
      // The T-A page's `section.publisher-wrapper * { background-image: none
      // !important }` wildcard (title-admin-page-design §"publisher wrapper")
      // otherwise wipes the thumbnail — same root cause the Ad Reformatter hit
      // at v0.4.5/0.4.6. An !important longhand INLINE beats an !important
      // longhand in a selector, so the image now survives.
      return '<span class="bdl-thumb bdl-thumb--img" style="background-image:url(\'' + escAttr(thumb) + '\') !important"></span>';
    }
