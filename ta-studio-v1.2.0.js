...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
      ...
      '<div class="std-root">' +
        '<div class="ix-hdr">' +
          '<div class="ix-hdr-left">' +
            ...
            '</div>' +
          '</div>' +
          '<span class="ix-badge">v1.2.0</span>' +
        '</div>' +
    ...
    function asmGetPickerCandidates(slot, article) {
      // Returns Component items where status=Available, role matches slot,
      // and customer/product narrow filters match. Items with status !==
    ...
    function renderAsmPicker(article, slot) {
      ...
      h +=   '</label>';
      h +=   '<label class="asm-pick-filter">';
      h +=     '<span class="asm-pick-filter-lbl">Product</span>';
      h +=     '<select data-asm-pick-product>';
      h +=       '<option value="All"' + (p.productId === 'All' ? ' selected' : '') + '>All Products</option>';
      products.forEach(function (pr) {
        h +=     '<option value="' + esc(pr.id) + '"' + (p.productId === pr.id ? ' selected' : '') + '>' + esc(pr.name) + '</option>';
      });
      h +=     '</select>';
      h +=   '</label>';
      h += '</div>';
      ...
      if (!candidates.length) {
        h +=   '<div class="asm-pick-empty">' +
                 'No Available ' + esc(roleLabel) + ' Components match. ' +
    ...
    function asmFireAttach(slot, mediaId, articleId, it) {
      ...
      if (!roleHash) {
        console.error('[TA-STUDIO v1.2.0] No componentRole hash for role:', it.role);
      ...
      .catch(function (err) {
        console.error('[TA-STUDIO v1.2.0] Asm Route 4 failed:', err);
    ...
    function getRoleHash(roleLabel) {
      ...
      if (!hash) {
        console.error('[TA-STUDIO v1.2.0] No TA_CONFIG.optionIds.componentRole.' + key + ' configured for role:', roleLabel);
        return '';
      }
      return hash;
    }
    ...
    function cmpFireAttach(mediaId, articleId, it) {
      ...
      .catch(function (err) {
        console.error('[TA-STUDIO v1.2.0] Scenario G Route 4 failed:', err);
    ...
    // ── Init ──
    renderAssembler();
    console.log('\u29BE Studio v1.2.0 mounted \u2014 Components tab (read + attach + archive + hard-delete) + Assembler field-level Attach (Main Image, Interior Images)');
  });
