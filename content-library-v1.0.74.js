...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function saveBatch() {
    ...
    var done = 0, failed = 0;
    // v1.0.58: assign a distinct sentinel position per row in this batch so four
    // rapid assigns never collide. UNPLACED_BASE + index → they queue at the far
    // end of the PubPlan ("unplaced / to-do"). Index across the whole batch.
    ids.forEach(function (id, idx) {
      ...
      // v1.0.58: LOOSE STAGE — block-type OMITTED. Per Integration Scoping K2/K7
      // a PubPlan article is untyped until Promote resolves FA-vs-TS. We send
      // blockType/blockTypeHash as EMPTY so scenario 124 writes no Option value
      // (Webflow Option fields take an omitted/empty key more reliably than a
      // bogus hash). 124's row Name uses {{20.blockType}} → falls back gracefully.
      params.set('blockType', '');
      params.set('blockTypeHash', '');
      params.set('titleAdminId', CFG.taItemId || TA_ITEM_ID || '');
      params.set('stage', 'planning');
      params.set('position', String(UNPLACED_BASE + idx));   // HC-CL-UNPLACED sentinel
      params.set('source', 'asset-library');

      // v1.0.61: ALSO stamp the ARTICLE's own refs via Scenario 104 (op=edit).
      // This is what makes the PubPlan COLUMN reflect the assignment (native ref)
      // and replaces the Scenario-105 picker's job of setting the article's home
      // PubPlan. Fired alongside the 124 post; logged, non-blocking.
      (function postArticleEdit() {
        ...
        ap.set('op', 'edit');                                   // 104 edit route
        ap.set('itemId', item.id);                              // ArticleID → item_id
        ap.set('pubplanId', sel.pubplan);                       // → publication-plan ref
        ap.set('pubplanName', pp ? pp.label : '');              // carried through
        ap.set('taItemId', CFG.taItemId || TA_ITEM_ID || '');  // → associated-title ref
        ap.set('source', 'asset-library');
        fetch(aurl + '?' + ap.toString())
          .then(function (r) { if (!r.ok) console.warn('[ASSET-LIBRARY] 104 edit non-ok', item.name); })
          .catch(function () {
            fetch(aurl + '?' + ap.toString(), { mode: 'no-cors' })
              .catch(function (e) { console.error('[ASSET-LIBRARY] 104 edit failed', item.name, e); });
          });
      })();
