/* ============================================================
   ta-components-tab-v1.0.8.js
   INBXIFY TA Studio — Components tab
   ============================================================

   v1.0.8 — Primary attach actions + Save to Media Library hook
...
(function () {
  ...
  function bindShellEvents(root) {
    root.addEventListener('click', function (e) {
      ...
      if (t.closest && t.closest('[data-cmp-action="reset-filters"]')) {
        ...
        renderSearchRow(); renderFilterBar(); renderGrid();
        return;
      }
    ...
    root.addEventListener('change', function (e) {
      ...
      if (t.matches && t.matches('[data-cmp-filter]')) {
        ...
        renderFilterBar(); renderGrid(); return;
      }
  ...
  function renderFilterBar() {
    ...
    host.innerHTML =
      '<div class="cmp-filter-row">' +
        ...
        (filtersAreDefault() ? '' : '<button type="button" class="ix-btn ix-btn--ghost cmp-filter-reset" data-cmp-action="reset-filters">Reset</button>') +
      '</div>';
  }
  ...
  function renderCard(it) {
    ...
    if (it.imageUrl) {
      thumb = '<div class="cmp-thumb ix-expand-icon-host">' +
                '<img class="cmp-thumb-img" src="' + escAttr(it.imageUrl) + '" alt="' + escAttr(it.name) + '" loading="lazy" />' +
                '<button type="button" class="ix-expand-icon" data-cmp-card-action="expand-preview" aria-label="Preview at full size">' +
      ...
      thumb = '<div class="cmp-thumb"><span class="cmp-thumb-empty">' + escHtml(it.mediaType || 'Component') + '</span></div>';
    }
  ...
  function refresh() {
    ...
    S.assetCache = {}; // invalidate asset cache on refresh
    renderSearchRow();
    renderFilterBar();
    renderBulkBar();
    renderGrid();
