/* ============================================================
   ta-asset-workbench-v0.8.1.js
   INBXIFY TA Studio — Asset Workbench (detail/management surface)
   Companion stylesheet: ta-asset-workbench-v0.8.1.css
   Shared stylesheet:    ix-internal-notes-v1_0_0.css (self-loaded)
...
(function () {
  ...
  function ensureStylesLoaded() {
    ...
    // both derived from this script's own src (same directory / CDN ref).
    var me = document.currentScript;
    if (!me || !me.src) return;
    var wants = [
      'ta-asset-workbench-v' + VERSION.replace(/\./g, '.') + '.css',
      IX_NOTES_CSS
    ];
    wants.forEach(function (want) {
      var href = me.src.replace(/ta-asset-workbench-v[\d.]+\.js.*$/, want);
  ...
  function renderTypeSection(a) {
    ...
    // article (default)
    // v0.8.1 — read the CONFIRMED Articles slugs from the 120 fieldData
    // (§19b: associated-business-coc / product / associated-newsletter /
    // associated-title). resolved-*-name fields are the server-side
    // resolver targets (pending 120 edit — same pattern as RE agent);
    // until 120 resolves, a connected ref shows its bare ID (mono,
    // subtle) instead of falsely reading as unreferenced.
    return sbSection('References', [
      sbRefField('Customer',
        fld(a, ['resolved-customer-name', 'customerName', 'associated-business-coc']),
        fld(a, ['customer'])),
      sbRefField('Product',
        fld(a, ['resolved-product-name', 'productName']),
        fld(a, ['product', 'productId'])),
      sbRefField('Newsletter',
        fld(a, ['resolved-newsletter-name', 'newsletterName']),
        fld(a, ['associated-newsletter', 'newsletterId'])),
      sbRefField('Title',
        fld(a, ['resolved-title-name', 'titleName']),
        fld(a, ['associated-title', 'titleSlug']))
    ]);
  }
