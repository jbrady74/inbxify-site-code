/* ============================================================
   ...
   Companion stylesheet: ta-asf-v1.2.1.css
   Reference spec:       v1.2 ASF — assetType branching; Article +
                         Real Estate + Event variants (Ad to follow)
...
(function () {
  ...
  function blankEV() {
    ...
    return {
      ...
      mainImageMediaId:   '',
      mainImageAlt:       '',
      // Event Flyer — reuses generic og-image keys ('og' picker mode)
      ogImageSrc:         '',
      ogImageMediaId:     '',
      // Refs
      customerId:         '',
  ...
  // Two image zones — Event Image (hero, 'main' mode) + Event Flyer
  // ('og' mode). Modeled on renderREImageZone.
  function renderEVImageZones() {
    ...
    return '' +
      '<div class="asf-ev-images">' +
        ...
        zone({
          title: 'Event flyer', url: a.ogImageSrc, icon: '\uD83D\uDCC4',
          prompt: 'Click to attach flyer', sub: 'optional · second image',
  ...
  // EV create commit → Scenario 104 (assetType:'event'). The payload
  // maps ogImage* → eventFlyer* so Scenario 104 sees semantic flyer
  // fields rather than OG fields.
  function commitCreateEV() {
    ...
    var payload = {
      ...
      fields: {
        ...
        mainImageSrc:       a.mainImageSrc     || '',
        mainImageAlt:       curVal('mainImageAlt') || '',
        // Event flyer — semantic names (sourced from og-image keys)
        eventFlyerMediaId:  a.ogImageMediaId || '',
        eventFlyerSrc:      a.ogImageSrc     || '',
        // Refs
        customerId:         a.customerId   || '',
        customerName:       a.customerName || '',
        newsletterId:       a.newsletterId   || '',
  ...
  function renderInlinePicker() {
    ...
    switch (pk.mode) {
      ...
      case 'og':     headerLabel = isEvent ? 'Pick event flyer' : 'Pick OG image';      headerIcon = isEvent ? '📄' : '🔗'; break;
  ...
  function publicOpen(params) {
    ...
    if (assetType !== 'article' && assetType !== 'realestate' && assetType !== 'event') {
