...
//       .articles-wrapper, .ads-wrapper, .customers-wrapper,
...
//
// HARDCODE DECISIONS:
//   HC-CL-001: MNLS section mapping derived from data-group
//              attribute on products-wrapper items. If a new
//              section is added that doesn't match existing
//              group names, the JS derivation needs updating.
//   HC-CL-002: Collection type (articles/ads) derived from
//              data-group: "Articles" → articles, else → ads.
//              Events/RE not yet shown in Content Library.
// ============================================================
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function readMnlsSections() {
    ...
    items.forEach(function (el) {
      ...
      if (!groups[group]) {
        groups[group] = {
          id: 'mnls-' + group.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-'),
          label: group,
          abbr: group.split(' ').map(function (w) { return w[0]; }).join('').toUpperCase(),
          color: deriveColor(group),
          collection: deriveCollection(group),
          slots: [],
        };
      }
    });
  ...
  function deriveColor(group) {
    var g = group.toLowerCase();
    if (g.indexOf('feature') !== -1) return '#2d6a4f';
    if (g.indexOf('themed') !== -1 || g.indexOf('spotlight') !== -1) return '#7b2d8b';
    if (g.indexOf('banner') !== -1) return '#b8860b';
    if (g.indexOf('sidebar') !== -1) return '#8b6914';
    if (g.indexOf('find') !== -1) return '#996633';
    if (g.indexOf('event') !== -1) return '#1a5276';
    if (g.indexOf('real estate') !== -1 || g.indexOf('listing') !== -1) return '#8b4513';
    return '#5a6a5a';
  }

  // HC-CL-002: Collection derivation
  function deriveCollection(group) {
    var g = group.toLowerCase();
    if (g.indexOf('article') !== -1 || g.indexOf('spotlight') !== -1 || g.indexOf('feature') !== -1) return 'articles';
    if (g.indexOf('event') !== -1) return 'events';
    if (g.indexOf('real estate') !== -1 || g.indexOf('listing') !== -1) return 're';
    return 'ads';
  }
  ...
  function readProducts() {
    ...
    items.forEach(function (el) {
      ...
      if (!id || !name) return;
      var mnlsId = 'mnls-' + group.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-');
      out.push({ id: id, name: name, group: group, mnlsId: mnlsId, reqCust: reqCust });
    });
    return out;
  }
  ...
  function readArticles() {
    ...
    items.forEach(function (el) {
      out.push(readCmsItem(el, 'articles'));
    });
    return out;
  }
  ...
  function readAds() {
    var out = [];
    var items = document.querySelectorAll('.ads-wrapper [data-item="true"]');
    items.forEach(function (el) {
      out.push(readCmsItem(el, 'ads'));
    });
    return out;
  }

  // ── Shared CMS item reader ──
  function readCmsItem(el, collection) {
    return {
      id: (el.dataset.id || '').trim(),
      name: (el.dataset.name || '').trim(),
      collection: collection,
      status: (el.dataset.status || 'draft').trim().toLowerCase(),
      customerId: (el.dataset.customerId || '').trim() || null,
      customerName: (el.dataset.customerName || '').trim() || null,
      productId: (el.dataset.productId || '').trim() || null,
      productName: (el.dataset.productName || '').trim() || null,
      mnlsGroup: (el.dataset.group || el.dataset.mnlsGroup || '').trim(),
      pubplanId: (el.dataset.pubplanId || '').trim() || null,
      pubplanName: (el.dataset.pubplanName || '').trim() || null,
      slot: (el.dataset.slot || '').trim() || null,
      created: (el.dataset.created || '').trim() || null,
      // Assets count from data attribute if available
      assetCount: parseInt(el.dataset.assetCount || '0', 10),
    };
  }
  ...
  var S = {
    ...
    subTab: 'all',         // all | mnls-feature-article | etc.
    contentStatus: 'all',  // all | draft | live
    assignStatus: 'all',   // all | assigned | unassigned
  ...
  function mnlsForTab(tab) {
    ...
    Object.keys(MNLS_SECTIONS).forEach(function (key) {
      ...
      if (tab === 'articles' && m.collection === 'articles') out.push(m);
      if (tab === 'ads' && m.collection === 'ads') out.push(m);
    });
    return out;
  }

  function findMnlsByGroup(group) {
    var id = 'mnls-' + group.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-');
    return MNLS_SECTIONS[group] || Object.values(MNLS_SECTIONS).find(function (m) { return m.id === id; }) || null;
  }

  function itemsForTab(tab) {
    return ALL_ITEMS.filter(function (i) { return i.collection === (tab === 'articles' ? 'articles' : 'ads'); });
  }

  function getFiltered() {
    var items = itemsForTab(S.mainTab);

    // Sub-tab filter by MNLS
    if (S.subTab !== 'all') {
      items = items.filter(function (i) {
        var m = findMnlsByGroup(i.mnlsGroup);
    ...
    if (S.assignStatus === 'assigned') {
      items = items.filter(function (i) { return i.pubplanId && i.slot; });
  ...
  function render() {
    ...
    allMnls.forEach(function (m) {
      var ct = tabItems.filter(function (i) {
        var im = findMnlsByGroup(i.mnlsGroup);
    ...
    h += '<button class="cl-sp ' + (S.assignStatus === 'unassigned' ? 'on-unasgn' : '') + '" onclick="_clSetAS(\'unassigned\')">Unassigned</button>';
    ...
    if (!items.length) {
      ...
      items.forEach(function (item) {
        var mnls = findMnlsByGroup(item.mnlsGroup);
        var isEd = S.editingId === item.id;
        var isAssigned = item.pubplanId && item.slot;
        ...
        h += '<span class="cl-bdg ' + (isAssigned ? 'cl-bdg-asgn' : 'cl-bdg-unasgn') + '">' + (isAssigned ? 'assigned' : 'unassigned') + '</span>';
        h += '</div></div>';
  ...
  function init() {
    ...
    // Assign MNLS info to each item
    ALL_ITEMS.forEach(function (item) {
      if (!item.mnlsGroup && item.productId) {
        var prod = PRODUCTS.find(function (p) { return p.id === item.productId; });
        if (prod) item.mnlsGroup = prod.group;
      }
    });
  ...
  function waitForData() {
    ...
    var hasAds = document.querySelectorAll('.ads-wrapper [data-item="true"]').length > 0;
