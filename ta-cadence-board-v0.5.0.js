/* ════════════════════════════════════════════════════════════════
   ta-cadence-board-v0.5.0.js
   INBXIFY T-A — Cadence Board · SKELETON
   Companion: ta-cadence-board-v0.5.0.css (matched pair — load both)

   ── v0.5.0 — CONTROLS COLLAPSE TO ICONS ──
    ...
    var out =
      '<div class="ixcb-r ixcb-r-fold' + (allOn ? ' ixcb-r-sel' : '') + (openB ? ' ixcb-open' : '') +
        ...
        '<span class="ixcb-n"><span class="ixcb-n-t">' + esc(bun.label) + '</span>' +
          '<span class="ixcb-n-m">' + bun.items.length + ' files \u00b7 ' + esc(mix) + '</span></span>' +
      '</div>';
    if (openB) {
    ...
    try {
      window.localStorage.setItem(VIEW_KEY, JSON.stringify({
        ...
        /* q and typeFilter are deliberately NOT persisted — see the
           v0.5.0 note. A question asked last session must not silently
           filter this one. */
      }));
    } catch (e) {}
  }
