...
<div class="cm-root">
  <div class="cm-hdr">
    ...
    <span class="cm-badge">v2.3.2</span>
  </div>
  ...
  function renderFilters() {
    ...
    const hidden = FILES.filter(f => f.status==='HIDDEN').length;
    document.getElementById('cm-filters').innerHTML =
      ...
      `<button class="cm-pill ${FILTER==='hidden'?'active':''}" data-cm-filter="hidden">Hidden ${hidden}</button>`+
  ...
  function renderFileList() {
    ...
    if (FILTER==='assigned') vis = FILES.filter(f => f.status==='ASSIGNED' || f.assigned);
    if (FILTER==='decision') vis = FILES.filter(f => f.status==='AWAITING_DECISION');
    if (FILTER==='hidden')   vis = FILES.filter(f => f.status==='HIDDEN');
    if (!vis.length) {
      const msgs = { ready:'No conditioned files ready', all:'No files', assigned:'No files assigned yet', decision:'No PDFs awaiting decision', hidden:'No hidden files' };
    ...
    el.innerHTML = vis.map(f => {
      ...
      const badge = isAssigned ? 'assigned' : isHidden ? 'decision' : f.status==='AWAITING_DECISION' ? 'decision' : 'ready';
      const label = isAssigned ? 'Assigned' : isHidden ? 'Hidden' : f.status==='AWAITING_DECISION' ? 'Needs Decision' : 'Ready';
      const uc    = f.uuid ? `<span style="color:#1a3a3a">UC:${f.uuid.substring(0,8)}\u2026</span>` : '';
      const disabled = isAssigned || isHidden;
      return `<div class="cm-frow ${f.selected?'sel':''} ${isAssigned?'assigned':''}" data-cm-fid="${f.id}">
        <div class="cm-frow-main">
          <input type="checkbox" class="cm-fcheck" ${f.selected?'checked':''} ${disabled?'disabled':''}>
  ...
  function getSelected() { return FILES.filter(f => f.selected && !f.assigned && f.status!=='ASSIGNED' && f.status!=='HIDDEN'); }
  ...
  function setFilter(f) {
    ...
    A = freshA();
    const statusMap = { ready:'DONE', all:'DONE', assigned:'ASSIGNED', decision:'AWAITING_DECISION', hidden:'HIDDEN' };
  ...
  function hideSelected() {
    ...
    // Update Google Sheet status to HIDDEN via Scenario C or a dedicated webhook
    // For now, mark locally and send status update to Make
    const hideUrl = CFG.scenarioCUrl; // reuse Scenario C webhook for status updates
    sel.forEach(f => {
      f.status = 'HIDDEN';
      f.selected = false;
      f.elementType = '';
      // Fire-and-forget status update to sheet
      if (CFG.makeAssembly) {
  ...
  function bindModalEvents() {
    ...
    _modalClickHandler = function(e) {
      ...
      // Element type chip — save scroll position before re-render
  ...
  function handleFieldChange(field, value) {
    if (field === 'collection') {
      ...
      // Capture MNLS ID from the selected option's data-mnls attribute
  ...
  // ══════════════════════════════════════════════
  // SUBMIT — single webhook call, files as JSON array
  // Make scenario uses Iterator to process each file
  // ══════════════════════════════════════════════
  function submitAssignment() {
    ...
    // Build single payload with shared fields + files array
    const payload = {
    ...
    if (CFG.makeAssembly) {
      ...
      }).then(done).catch(err => { console.error('[CM] Submit error:', err); done(); });
    } else {
      done();
    }
  }
  ...
  // ── Init ──
  loadFiles('DONE');
  console.log('\u{1F4C2} Content Manager v2.3.2 mounted');
});
