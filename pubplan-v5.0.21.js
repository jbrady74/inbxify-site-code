...
(function () {
  ...
  function renderGr() {
    ...
    var isEd = s.dirty || !!originalState['GR-1'], hasC = s.grTit || s.grMsg || s.grSS, fh;
    if (isEd) {
      fh = '<div class="pc" style="flex:1;"><span class="ppt-col-label">Greeting Title</span><input type="text" class="pi" data-field="grTit" maxlength="'+GR_LIMITS.grTit+'" value="'+(s.grTit||'').replace(/"/g,'&quot;')+'" placeholder="title...">'+charCountHtml(GR_LIMITS.grTit,s.grTit)+'</div><div class="pc" style="flex:2;"><span class="ppt-col-label">Greeting Message</span><textarea class="ppt-textarea" data-field="grMsg" maxlength="'+GR_LIMITS.grMsg+'" placeholder="message...">'+(s.grMsg||'')+'</textarea>'+charCountHtml(GR_LIMITS.grMsg,s.grMsg)+'</div><div class="pc" style="flex:1;"><span class="ppt-col-label">Super Short Greeting</span><input type="text" class="pi" data-field="grSS" maxlength="'+GR_LIMITS.grSS+'" value="'+(s.grSS||'').replace(/"/g,'&quot;')+'" placeholder="140 char max...">'+charCountHtml(GR_LIMITS.grSS,s.grSS)+'</div>';
    } else if (hasC) {
      fh = '<div class="pc" style="flex:1;"><span class="ppt-col-label">Greeting Title</span><span class="pcv">'+(s.grTit||'—')+'</span></div><div class="pc" style="flex:2;"><span class="ppt-col-label">Greeting Message</span><span class="pcv">'+(s.grMsg||'—')+'</span></div><div class="pc" style="flex:1;"><span class="ppt-col-label">Super Short</span><span class="pcv">'+(s.grSS||'—')+'</span></div>';
    } else {
