// ============================================================
// PUBPLAN TILE UI — v5.0.21
// Changes from v5.0.20:
//   FIX #22: Read section-level TF category from
//            .txa-picker-wrapper[data-cat-id] instead of
//            state['TXA-1'].catId — TXA-1 only has catId after
//            a per-slot save, not after a TF-CAT category-only save
...
(function () {
  ...
  function init() {
    ...
    // FIX #22: Read section-level TF category from .txa-picker-wrapper[data-cat-id]
    // instead of state['TXA-1'].catId — the picker wrapper always has the
    // section-level category, even after a TF-CAT-only save where per-slot
    // wrappers may not have it
    var txaPicker = document.querySelector('.txa-picker-wrapper');
    tfCategoryId = (txaPicker && txaPicker.dataset.catId) ? txaPicker.dataset.catId : '';
    tfCategoryNm = '';
    if (tfCategoryId) {
      var catEl = document.querySelector('.products-wrapper[data-id="' + tfCategoryId + '"]');
      if (catEl) {
