// ============================================================
// PUBPLAN TILE UI — v5.0.29
// Base: v5.0.27 (deployed, confirmed working). v5.0.28 was NOT
// deployed and is superseded — its multi-attribute lookup became
// moot once data-article-created was bound in Webflow.
// Changes from v5.0.27:
//   FIX #29: FA article sort direction reversed — NEWEST created
//            first (was oldest first). Undated articles still sink
//            to the bottom, A-Z among themselves. A-Z remains the
//            whole-list fallback only when no article is dated.
//            Single comparator change; nothing else touched.
// Changes from v5.0.26:
//   FIX #26: FA customer is now OPTIONAL on paid categories.
//            - Article dropdown no longer gated on a customer
//              being chosen; it unlocks as soon as a category is.
//            - Paid category + NO customer  -> article list is the
//              full FA library (no category narrowing), per spec.
//            - Paid category + customer     -> unchanged (filtered
//              to that customer's articles).
//            - Non-paid category            -> unchanged (filtered
//              by category).
//            - Customer placeholder relabelled "Optional".
//            - Empty hidden-customer-id now legitimately posts on
//              FA slots. Make must key off "!= emptystring".
//   FIX #27: FA article lists filter to Available status and sort
//            newest-created-first (see FIX #29). Reads data-article-status and
//            data-article-created off .articles-wrapper. BOTH ARE
//            FAIL-OPEN: if the attribute is absent the filter/sort
//            is skipped and a one-time console warning fires, so
//            this ships safely before the Webflow bindings exist.
//            The currently-saved article is NEVER filtered out,
//            regardless of status.
//            TS (Themed Spotlight) intentionally UNCHANGED.
...
(function () {
  ...
  // ── Article availability + recency helpers (v5.0.27) ────────
  // HARDCODE NOTE (log in INBXIFY-Hardcoding-Tracker):
  //   ART_STATUS_ATTR / ART_CREATED_ATTR are DOM attribute names,
  //   ART_AVAILABLE is a CMS option value compared lower-cased.
  //   All three are single-point constants, not scattered literals.
  //   Multi-tenant safe: no publisher/title value is baked in.
  var ART_STATUS_ATTR  = 'articleStatus';    // data-article-status
  var ART_CREATED_ATTR = 'articleCreated';   // data-article-created
  var ART_AVAILABLE    = 'available';
  var _artStatusWarned = false, _artCreatedWarned = false;

  function articleIsAvailable(el) {
    var v = el.dataset[ART_STATUS_ATTR];
    if (v === undefined || v === '') {
      if (!_artStatusWarned) {
        _artStatusWarned = true;
        console.warn('[PubPlan v5.0.27] .articles-wrapper is missing data-article-status — Available filter skipped. Bind ARTICLES > Status to that attribute in Webflow.');
      }
      return true; // fail open
    }
    return v.trim().toLowerCase() === ART_AVAILABLE;
  }

  function articleCreatedAt(el) {
    var v = el.dataset[ART_CREATED_ATTR];
    if (!v) {
      if (!_artCreatedWarned) {
        _artCreatedWarned = true;
        console.warn('[PubPlan v5.0.27] .articles-wrapper is missing data-article-created — falling back to A-Z sort. Bind ARTICLES > Created On to that attribute in Webflow.');
      }
      return null;
    }
    var t = Date.parse(v);
  ...
  // v5.0.29: NEWEST created first. Undated items sink to the bottom,
  // A-Z among themselves. If NOTHING is dated, degrade to plain A-Z.
  function sortArticleItems(items) {
    var anyDated = items.some(function(a){ return a.created !== null; });
    if (!anyDated) { items.sort(function(a,b){ return a.name.localeCompare(b.name); }); return items; }
    items.sort(function(a,b){
      if (a.created === null && b.created === null) return a.name.localeCompare(b.name);
      if (a.created === null) return 1;
      if (b.created === null) return -1;
      if (a.created !== b.created) return b.created - a.created;   // descending
      return a.name.localeCompare(b.name);
    });
    return items;
  }

  // showAll=true  -> ignore the category narrowing entirely.
  //                  Used for paid categories with NO customer chosen.
  function buildFaArticleOptions(custId, catId, selId, showAll) {
    var all = document.querySelectorAll('.articles-wrapper'), items = [];
    all.forEach(function(el) {
      if (el.dataset.articleCategoryCode !== 'fa') return;
      var id=el.dataset.articleId||'', nm=el.dataset.articleTitle||'';
      if (!id || !nm) return;
      var isSel = (id === selId);
      if (custId) { if (el.dataset.articleCustomerId !== custId && !isSel) return; }
      else if (catId && !showAll) { if (el.dataset.categoryId !== catId && !isSel) return; }
      // Saved selection always survives the status filter.
      if (!isSel && !articleIsAvailable(el)) return;
      items.push({ id:id, name:nm, created:articleCreatedAt(el) });
    });
    if (!items.length) return '<option value="" disabled selected>No articles</option>';
    sortArticleItems(items);
  ...
  function renderFa(sc) {
    ...
    if (isPaid) {
      custCol = (!isEd && s.custId)
        ...
        : '<div class="pc"><span class="ppt-col-label">Customer</span><select class="pd'+(s.custId?' hs':'')+'" data-dropdown="fa-cust" data-sc="'+sc+'"'+(!s.catId?' disabled':'')+'>' + buildCustomerOptions(s.custId,'Optional — select customer') + '</select></div>';
    }

    // v5.0.27: customer is optional — article unlocks on category alone.
    var artDis = !s.catId ? ' disabled' : '';
    var faShowAll = isPaid && !s.custId;
    var artCol = (!isEd && s.artId)
      ? '<div class="pc"><span class="ppt-col-label">Article</span><span class="pcv">'+s.artNm+'</span></div>'
      : '<div class="pc"><span class="ppt-col-label">Article</span><select class="pd'+(s.artId?' hs':'')+'" data-dropdown="fa-art" data-sc="'+sc+'"'+artDis+'>'+buildFaArticleOptions(isPaid?s.custId:'',s.catId,s.artId,faShowAll)+'</select></div>';
  ...
  // v5.0.27: nid may legitimately be '' (customer cleared / never set).
  // Clearing still drops the article, since the list it came from changes.
  function handleFaCustChange(sc,sel){var s=state[sc];if(!s)return;var opt=sel.options[sel.selectedIndex],nid=opt?opt.value||'':'';if(nid===s.custId)return;var doIt=function(){if(!originalState[sc])originalState[sc]=Object.assign({},s);s.custId=nid;s.custNm=nid?(opt?opt.textContent||'':''):'';s.artId='';s.artNm='';s.dirty=true;renderFa(sc);updateFaProgress();};if(s.artId){showInlineConfirm(sel,nid?'Changing customer will clear article selection.':'Clearing customer will clear article selection.',doIt,function(){sel.value=s.custId||'';});}else{doIt();}}
  ...
  function initEvState() {
    ...
    slotEls.forEach(function(el) {
      ...
      if (evId && !evNm) {
        ...
        if (lib) evNm = lib.dataset.eventName || '';
      }
      state[code] = {
  ...
  function buildEventOptions(selId) {
    ...
    els.forEach(function(el) {
      ...
      if (id) all.push({ id: id, name: el.dataset.eventName || '(untitled)', month: el.dataset.eventMonth || '', day: el.dataset.eventDay || '', venue: el.dataset.eventLocation || '' });
    });
