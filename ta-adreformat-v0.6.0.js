/* ============================================================
   ta-adreformat-v0.6.0.js
   INBXIFY TA Studio — Ad Reformatter (Generator tab tool)

   CANVAS-FIRST REWRITE. Supersedes the v1.0.x line.

   v0.6.0 — WORKBENCH RESTRUCTURE (locked from workbench mockup v0.1,
     2026-07-08). The tool is a workbench around one source, not a
     pipeline — no extract gate:
     • GRAB BAR (Box / Point / Text + Isolate / Copy + Run) is live the
       moment a source loads, directly above the preview. Draw a box
       and isolate immediately; nothing is gated on extraction.
     • ASSET TRAY: every grab — isolated cut-out, plain copy, or
       AI-extracted part — lands in the tray as a peer card with its
       own actions: Add to Canvas · Save as MEDIA Item · Save &
       Download · Remove (✕).
     • EXTRACT PARTS demoted to an optional bulk auto-grab: its image
       regions flow into the same tray; detected text renders as
       click-to-copy chips under the source (copy → paste into any
       text layer); brand colors/business name kept.
     • BUILD BANNER is a downstream consumer: always available; opens
       the editor with auto-layout from extracted parts, or Add to
       Canvas from the tray opens an empty editor on demand.
     • Per-asset save runs the customer-assignment step (gold dirty
       border + cancel link + tenant-scoped list → Uploadcare →
       Scenario B Route 4). componentRole = ISOLATED_ROLE_HASH for
       isolated assets, blank for copies/parts.
     • The v0.5.0 in-editor Isolate overlay is REMOVED (superseded by
       the grab bar). v0.5.0 was never deployed.
...
(function () {
  ...
  var S = {
    ...
    assets: [],          // tray: [{ id, kind:'iso'|'copy'|'part', label, canvas }]
  ...
  function injectStyles() {
    ...
    var css = [
      ...
      // workbench + asset tray (v0.6.0)
      '.ix-arf-wbgrid{display:grid;grid-template-columns:minmax(380px,1.35fr) minmax(300px,1fr);gap:18px;align-items:start;}',
      '@media(max-width:980px){.ix-arf-wbgrid{grid-template-columns:1fr;}}',
      '.ix-arf-wbbar{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:10px;}',
      '.ix-arf-traycount{font:500 10px/1 "DM Mono",monospace;background:#C4A35A;color:#122B2B;border-radius:4px;padding:2px 7px;margin-left:6px;letter-spacing:.08em;}',
      '.ix-arf-trayempty{border:1.5px dashed #d4cfbf;border-radius:8px;padding:26px 16px;text-align:center;color:#8a8a7a;font:400 12.5px/1.6 "DM Sans",sans-serif;}',
      '.ix-arf-tray{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px;}',
      '@media(max-width:1240px){.ix-arf-tray{grid-template-columns:1fr;}}',
      '.ix-arf-asset{position:relative;border:1px solid #c9c3b2;border-radius:8px;overflow:hidden;}',
      '.ix-arf-asset-x{position:absolute;top:4px;right:4px;z-index:2;width:20px;height:20px;border:1px solid #c9c3b2;border-radius:50%;background:#fff !important;color:#B0483C;font:700 10px/1 "DM Sans",sans-serif;cursor:pointer;}',
      '.ix-arf-asset-x:hover{border-color:#B0483C;}',
      '.ix-arf-asset-th{display:flex;align-items:center;justify-content:center;min-height:80px;max-height:130px;overflow:hidden;}',
      '.ix-arf-asset-th canvas{display:block;max-width:100%;max-height:130px;width:auto;height:auto;}',
      '.ix-arf-asset-meta{font:500 9.5px/1.4 "DM Mono",monospace;color:#8a8a7a;padding:7px 9px 0;letter-spacing:.04em;}',
      '.ix-arf-kind{display:inline-block;font:600 8.5px/1 "DM Mono",monospace;letter-spacing:.1em;border-radius:4px;padding:3px 6px;margin-right:6px;text-transform:uppercase;}',
      '.ix-arf-kind--iso{background:#1A3A3A !important;color:#fff;}',
      '.ix-arf-kind--copy{background:#e7e2d5 !important;color:#22312F;}',
      '.ix-arf-kind--part{background:#C4A35A !important;color:#122B2B;}',
      '.ix-arf-asset-acts{display:flex;gap:4px;padding:8px 9px 10px;flex-wrap:wrap;}',
      '.ix-arf-mini{font:600 10.5px/1 "DM Sans",sans-serif;border:1px solid #c9c3b2;background:#fff !important;color:#1A3A3A;border-radius:6px;padding:6px 8px;cursor:pointer;}',
      '.ix-arf-mini:hover{border-color:#1A3A3A;}',
      '.ix-arf-mini--gold{background:#C4A35A !important;border-color:#C4A35A;color:#122B2B;}',
      '.ix-arf-chip--copy{cursor:pointer;border:1px solid #c9c3b2;background:#fff !important;}',
      '.ix-arf-chip--copy:hover{border-color:#C4A35A;}',
      '.ix-arf-chip--copy.copied{background:#C4A35A !important;border-color:#C4A35A;}',
      '.ix-arf-chip--copy.copied .val{color:#122B2B;}'
    ].join('');
  ...
  function loadFile(f) {
    ...
    reader.onload = function () {
      var img = new Image();
      img.onload = function () { S.img = img; renderWorkbench(); };
  ...
  // ── Workbench: grab bar + source + optional extract + asset tray ──
  // v0.6.0: the grab tools are live the moment the source paints. The
  // preview stays in the ISOLATED IFRAME (page CSS cannot reach inside;
  // see v0.2.3 note below) with pointer-events off; gestures land on the
  // wrapper, and the gold marquee/dot are wrapper-level overlays.
  function renderWorkbench() {
    var root = document.getElementById('ix-arf-root'); if (!root) return;
    root.innerHTML =
      '<div class="ix-arf-wbgrid">' +
        '<div class="ix-arf-wbsrc">' +
          '<div class="ix-arf-wbbar">' +
            '<span class="ix-arf-iso-seg" id="ix-arf-wb-seg">' +
              '<button class="on" data-tool="box">Box</button>' +
              '<button data-tool="point">Point</button>' +
              '<button data-tool="text">Text</button>' +
            '</span>' +
            '<select class="ix-arf-iso-mode" id="ix-arf-wb-mode">' +
              '<option value="isolate">Isolate \u2014 cut onto transparency</option>' +
              '<option value="copy">Copy \u2014 plain crop, all layers</option>' +
            '</select>' +
            '<button class="ix-btn ix-btn--primary" id="ix-arf-wb-run" disabled>' +
              '<span class="ix-arf-spin" id="ix-arf-wb-spin" style="display:none"></span>' +
              '<span id="ix-arf-wb-runlbl">Isolate \u2192 Tray</span></button>' +
            '<span class="ix-arf-status" id="ix-arf-wb-status"></span>' +
          '</div>' +
          '<div class="ix-arf-wbbar" id="ix-arf-wb-txtrow" style="display:none">' +
            '<input type="text" class="ix-arf-iso-txt" id="ix-arf-wb-txtin" ' +
              'placeholder=\'Describe the target \u2014 e.g. "the truck"\'>' +
          '</div>' +
          '<div class="ix-arf-srcwrap" id="ix-arf-srcwrap" style="position:relative;touch-action:none;overflow:hidden;line-height:0;">' +
            '<iframe id="ix-arf-frame" title="Ad preview" ' +
              'style="display:block;width:100%;border:0;background:#fff;pointer-events:none;"></iframe>' +
            '<div class="ix-arf-iso-rect" id="ix-arf-wb-marquee" style="display:none;border-style:dashed;"></div>' +
            '<div class="ix-arf-iso-dot" id="ix-arf-wb-dot" style="display:none"></div>' +
          '</div>' +
          '<div class="ix-arf-actions">' +
            '<button class="ix-btn ix-btn--ghost" id="ix-arf-extract">Extract parts (AI) \u2014 bulk auto-grab</button>' +
            '<button class="ix-btn ix-btn--secondary" id="ix-arf-build">Build banner \u2192</button>' +
            '<button class="ix-btn ix-btn--ghost" id="ix-arf-reset">Use a different ad</button>' +
            '<span class="ix-arf-status" id="ix-arf-status"></span>' +
          '</div>' +
          '<div class="ix-arf-bin" id="ix-arf-bin"></div>' +
        '</div>' +
        '<div class="ix-arf-wbtray">' +
          '<div class="ix-arf-lbl">Asset tray <span class="ix-arf-traycount" id="ix-arf-traycount">0 assets</span></div>' +
          '<div class="ix-arf-trayempty" id="ix-arf-trayempty">Nothing grabbed yet.<br>' +
            'Draw a box on the source and run <b>Isolate</b> or <b>Copy</b> \u2014 or run <b>Extract parts</b> to bulk-fill.</div>' +
          '<div class="ix-arf-tray" id="ix-arf-tray"></div>' +
          '<div class="ix-arf-iso-custrow" id="ix-arf-tray-saverow">' +
            '<label class="ix-arf-ltlbl" for="ix-arf-tray-cust">Assign to customer</label>' +
            '<select id="ix-arf-tray-cust" class="ix-arf-adv"></select>' +
            '<button class="ix-btn ix-btn--primary" id="ix-arf-tray-confirm" disabled>' +
              '<span class="ix-arf-spin" id="ix-arf-tray-savespin" style="display:none"></span>' +
              '<span id="ix-arf-tray-confirmlbl">Save \u2192</span></button>' +
            '<button class="ix-arf-iso-clear" id="ix-arf-tray-custcancel">cancel</button>' +
            '<span class="ix-arf-status" id="ix-arf-tray-savestatus"></span>' +
          '</div>' +
        '</div>' +
      '</div>';
    paintFrame([]);   // initial paint, no boxes yet
    document.getElementById('ix-arf-extract').addEventListener('click', extract);
    document.getElementById('ix-arf-build').addEventListener('click', openEditor);
    document.getElementById('ix-arf-reset').addEventListener('click', function () {
      S.assets = []; S.parts = []; S.result = null; S.layers = [];
      renderIntake();
    });
    wireGrabBar();
    wireGrabGestures();
    ...
    var go = function (base64, mediaType) {
      fetch(worker, {
        ...
        .then(function (data) {
          ...
          cropParts(S.result);
          renderExtractInfo(S.result);
          pushPartsToTray();   // extracted parts land in the tray as peers. v0.6.0
  ...
  // Extract info under the source: business name, brand colors, and the
  // detected text as CLICK-TO-COPY chips (copy \u2192 paste into any text
  // layer). Image parts do NOT render here \u2014 they land in the tray. v0.6.0
  function renderExtractInfo(p) {
    ...
    var txts = p.texts || [];
    html += '<div class="ix-arf-lbl">Text (' + txts.length + ') \u2014 click to copy</div><div class="ix-arf-chips" id="ix-arf-txtchips">';
    ...
    txts.forEach(function (t, i) {
      ...
      html += '<button class="ix-arf-chip ix-arf-chip--copy" data-copy="' + esc(t.value || '') + '">' +
              '<span class="r">' + esc(roleLabel(t.role) || 'text') + '</span>' + v +
              '<span class="val" title="' + esc(t.value) + '">' + esc(t.value || '') + '</span></button>';
    });
    html += '</div>';
    bin.innerHTML = html;

    bin.querySelectorAll('.ix-arf-chip--copy').forEach(function (b) {
  ...
  // ════════════════════════════════════════════════════════════
  //  WORKBENCH (v0.6.0) — grab tools + asset tray. The tool is a
  //  workbench around one source, not a pipeline: grab tools are live
  //  the moment the source loads (no extract gate), everything grabbed
  //  (isolated / copy / extracted part) lands in the tray as a peer,
  //  and every tray asset carries its own Add-to-Canvas / Save-as-MEDIA
  //  / Save-&-Download / Remove decisions.
  // ════════════════════════════════════════════════════════════
  ...
  // ── grab state (module-level; survives re-renders of the bin) ──
  var G = { tool: 'box', mode: 'isolate', box: null, pt: null, busy: false, hasRun: false };

  function wbStatus(kind, msg) {
    var s = document.getElementById('ix-arf-wb-status');
    if (s) { s.className = 'ix-arf-status ' + (kind || ''); s.textContent = msg || ''; }
  }
  function wbSyncRun() {
    var run = document.getElementById('ix-arf-wb-run');
    var lbl = document.getElementById('ix-arf-wb-runlbl');
    var txt = document.getElementById('ix-arf-wb-txtin');
    var ready = (G.tool === 'text') ? !!(txt && txt.value.trim())
              : (G.tool === 'point') ? !!G.pt : !!G.box;
    if (run) run.disabled = !ready || G.busy;
    if (lbl) lbl.textContent = (G.mode === 'copy') ? 'Copy \u2192 Tray' : 'Isolate \u2192 Tray';
  }
  function wbClearSel() {
    G.box = null; G.pt = null;
    var m = document.getElementById('ix-arf-wb-marquee'); if (m) m.style.display = 'none';
    var d = document.getElementById('ix-arf-wb-dot'); if (d) d.style.display = 'none';
    wbSyncRun();
  }

  // ── gesture wiring on the source wrap (over the display iframe) ──
  function wireGrabGestures() {
    var wrap = document.getElementById('ix-arf-srcwrap');
    var marquee = document.getElementById('ix-arf-wb-marquee');
    var dot = document.getElementById('ix-arf-wb-dot');
    if (!wrap || !marquee || !dot) return;
    // page rule whites/kills backgrounds; force. v0.4.6 lesson.
    marquee.style.setProperty('background-color', 'rgba(196,163,90,.14)', 'important');
    dot.style.setProperty('background-color', 'rgba(196,163,90,.35)', 'important');

    function W() { return S.img ? (S.img.naturalWidth || S.img.width) : 1; }
    function H() { return S.img ? (S.img.naturalHeight || S.img.height) : 1; }
    function toSrc(cx, cy) {
      var r = wrap.getBoundingClientRect();
      return { x: (cx - r.left) / r.width * W(), y: (cy - r.top) / r.height * H() };
    }
    function paintBox() {
      if (!G.box) { marquee.style.display = 'none'; return; }
      marquee.style.display = 'block';
      marquee.style.left = (G.box.x / W() * 100) + '%';
      marquee.style.top = (G.box.y / H() * 100) + '%';
      marquee.style.width = (G.box.w / W() * 100) + '%';
      marquee.style.height = (G.box.h / H() * 100) + '%';
    }
    function paintDot() {
      if (!G.pt) { dot.style.display = 'none'; return; }
      dot.style.display = 'block';
      dot.style.left = (G.pt.x / W() * 100) + '%';
      dot.style.top = (G.pt.y / H() * 100) + '%';
    }

    wrap.addEventListener('pointerdown', function (e) {
      if (!S.img || G.tool === 'text') return;
      e.preventDefault();
      var start = toSrc(e.clientX, e.clientY);
      if (G.tool === 'point') { G.pt = { x: start.x, y: start.y }; paintDot(); wbSyncRun(); return; }
      G.box = { x: start.x, y: start.y, w: 0, h: 0 };
      function mv(ev) {
        var p = toSrc(ev.clientX, ev.clientY);
        G.box.x = Math.max(0, Math.min(start.x, p.x));
        G.box.y = Math.max(0, Math.min(start.y, p.y));
        G.box.w = Math.min(W() - G.box.x, Math.abs(p.x - start.x));
        G.box.h = Math.min(H() - G.box.y, Math.abs(p.y - start.y));
        paintBox();
      }
      function up() {
        document.removeEventListener('pointermove', mv);
        document.removeEventListener('pointerup', up);
        if (G.box && (G.box.w < 4 || G.box.h < 4)) { G.box = null; paintBox(); }
        wbSyncRun();
      }
  ...
  function wireGrabBar() {
    var seg = document.getElementById('ix-arf-wb-seg');
    if (seg) seg.querySelectorAll('button').forEach(function (b) {
      b.addEventListener('click', function () {
        seg.querySelectorAll('button').forEach(function (x) { x.classList.remove('on'); });
        b.classList.add('on'); G.tool = b.getAttribute('data-tool');
        var tr = document.getElementById('ix-arf-wb-txtrow');
        if (tr) tr.style.display = (G.tool === 'text') ? '' : 'none';
        wbClearSel();
      });
    });
    var mode = document.getElementById('ix-arf-wb-mode');
    if (mode) mode.addEventListener('change', function () {
      G.mode = mode.value;
      // Copy = plain crop: needs a box (a point/text can't define a crop)
      if (G.mode === 'copy' && G.tool !== 'box') {
        var bb = document.querySelector('#ix-arf-wb-seg [data-tool="box"]');
        if (bb) bb.click();
        wbStatus('', 'Copy uses a box selection.');
      }
      wbSyncRun();
    });
    var txt = document.getElementById('ix-arf-wb-txtin');
    if (txt) txt.addEventListener('input', wbSyncRun);
    var run = document.getElementById('ix-arf-wb-run');
    if (run) run.addEventListener('click', runGrab);
  }

  // ── crop helpers (source px) ──
  function cropFromSource(b) {
    var c = document.createElement('canvas');
  ...
  // ── run a grab ──
  function runGrab() {
    if (G.busy || !S.img) return;
    if (G.mode === 'copy') {
      if (!G.box) return;
      var c = cropFromSource(G.box);
      addAsset('copy', c);
      wbStatus('ok', 'Region copied to tray.');
      wbClearSel();
      return;
    }
    var worker = CFG().objectIsolatorWorker;
    if (!worker) { wbStatus('err', 'No objectIsolatorWorker configured in TA_CONFIG.'); return; }

    var payload = { return_base64: true };
    var txtEl = document.getElementById('ix-arf-wb-txtin');
    var txtVal = (txtEl && txtEl.value || '').trim();
    if (G.tool === 'box') {
      if (!G.box) return;
      // two-stage (locked 2026-07-08): send ONLY the crop; the full-frame
      // box prompt targets the crop's dominant object at full mask res.
      var crop = cropFromSource(G.box);
      payload.image = crop.toDataURL('image/png'); // png: lossless crop edges
      payload.box_prompts = [[0, 0, crop.width, crop.height]];
      if (txtVal) payload.prompt = txtVal;
    } else if (G.tool === 'point') {
      if (!G.pt) return;
      payload.image = fullSourceDataUrl();
      payload.point_prompts = [[Math.round(G.pt.x), Math.round(G.pt.y)]];
    } else {
      if (!txtVal) return;
      payload.image = fullSourceDataUrl();
    ...
    G.busy = true; wbSyncRun();
    var spin = document.getElementById('ix-arf-wb-spin');
    var lbl = document.getElementById('ix-arf-wb-runlbl');
    if (spin) spin.style.display = 'inline-block';
    if (lbl) lbl.textContent = 'Isolating\u2026';
    wbStatus('work', (G.tool === 'box' ? 'crop \u2192 ' : '') + 'worker \u2192 SAM3\u2026');
    ...
    fetch(worker, {
      ...
      .then(function (img) {
        ...
        c.getContext('2d').drawImage(img, 0, 0);
        addAsset('iso', c);
        wbStatus('ok', 'Isolated \u2192 tray. Not right? Reselect and run again.');
        G.hasRun = true;
        wbClearSel();
      })
      .catch(function (err) { wbStatus('err', err.message || 'Isolation failed.'); })
      .finally(function () {
        G.busy = false;
  ...
  // ── asset tray ──
  var assetSeq = 0;
  function addAsset(kind, canvas, label) {
    S.assets.push({ id: ++assetSeq, kind: kind, canvas: canvas, label: label || kind });
    renderTray();
  }
  ...
  function pushPartsToTray() {
    (S.parts || []).forEach(function (p) {
      S.assets.push({ id: ++assetSeq, kind: 'part', canvas: p.canvas, label: roleLabel(p.kind) || p.label || 'part' });
    });
    renderTray();
  }

  function kindLabel(k) { return k === 'iso' ? 'isolated' : k === 'copy' ? 'copy' : 'part'; }

  function renderTray() {
    var tray = document.getElementById('ix-arf-tray');
    var empty = document.getElementById('ix-arf-trayempty');
    var count = document.getElementById('ix-arf-traycount');
    if (!tray) return;
    hideAssetSave(); // re-render invalidates any open save binding
    if (count) count.textContent = S.assets.length + ' asset' + (S.assets.length === 1 ? '' : 's');
    if (empty) empty.style.display = S.assets.length ? 'none' : '';
    tray.innerHTML = '';
    S.assets.forEach(function (a) {
      var card = document.createElement('div');
      card.className = 'ix-arf-asset';
      card.style.setProperty('background-color', '#fff', 'important');

      var x = document.createElement('button');
      x.className = 'ix-arf-asset-x'; x.title = 'Remove from tray'; x.textContent = '\u2715';
      x.addEventListener('click', function () { removeAsset(a.id); });
      card.appendChild(x);

      var th = document.createElement('div');
      th.className = 'ix-arf-asset-th';
      // transparency checkerboard — page CSS kills background-image via
      // stylesheet, so inline !important. v0.4.6 lesson.
      th.style.setProperty('background-image',
        'repeating-conic-gradient(#e8e4da 0% 25%, #f7f5ef 0% 50%)', 'important');
      th.style.setProperty('background-size', '14px 14px', 'important');
      ...
      meta.className = 'ix-arf-asset-meta';
      var badge = document.createElement('span');
      badge.className = 'ix-arf-kind ix-arf-kind--' + a.kind;
      badge.textContent = kindLabel(a.kind);
      meta.appendChild(badge);
      meta.appendChild(document.createTextNode(a.canvas.width + '\u00d7' + a.canvas.height + 'px'));
      card.appendChild(meta);

      var acts = document.createElement('div');
      acts.className = 'ix-arf-asset-acts';
      var bAdd = document.createElement('button');
      bAdd.className = 'ix-arf-mini'; bAdd.textContent = 'Add to Canvas';
      bAdd.addEventListener('click', function () { assetToCanvas(a); });
      var bSave = document.createElement('button');
      bSave.className = 'ix-arf-mini'; bSave.textContent = 'Save as MEDIA Item';
      bSave.addEventListener('click', function () { openAssetSave(a.id, false); });
      var bSaveDl = document.createElement('button');
      bSaveDl.className = 'ix-arf-mini ix-arf-mini--gold'; bSaveDl.textContent = 'Save & Download';
      bSaveDl.addEventListener('click', function () { openAssetSave(a.id, true); });
      acts.appendChild(bAdd); acts.appendChild(bSave); acts.appendChild(bSaveDl);
  ...
  // ── Add to Canvas: ensure the editor exists, then place as a layer ──
  function ensureEditor() {
    if (!document.getElementById('ix-arf-editor')) {
      if (!S.layers) S.layers = [];
      buildEditorShell();
    }
  }
  function assetToCanvas(a) {
    ensureEditor();
    var part = { label: a.label || kindLabel(a.kind), kind: a.kind === 'iso' ? 'isolated-object' : (a.kind || 'part'), bbox: null, canvas: a.canvas };
    var scale = Math.min((BANNER_W * 0.4) / a.canvas.width, (BANNER_H * 0.9) / a.canvas.height, 1);
    var L = mkImgLayer(part, 20, 20,
      Math.max(24, Math.round(a.canvas.width * scale)),
      Math.max(24, Math.round(a.canvas.height * scale)));
    S.layers.push(L);
    S.selected = S.layers.length - 1;
    renderEditor();
    var ed = document.getElementById('ix-arf-editor');
    if (ed && ed.scrollIntoView) ed.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
  }

  // ── per-asset save: the arc's first CMS-writing dropdown — gold dirty
  //    border while a selection is pending + cancel link (edit-state
  //    invariants), tenant-scoped customer list, Scenario B Route 4. ──
  var saveCtx = { assetId: null, withDownload: false };

  function custSel() { return document.getElementById('ix-arf-tray-cust'); }
  function hideAssetSave() {
    var row = document.getElementById('ix-arf-tray-saverow');
    if (row) row.style.display = 'none';
    saveCtx.assetId = null; saveCtx.withDownload = false;
    var sel = custSel();
    if (sel) {
      sel.value = '';
    ...
    var st = document.getElementById('ix-arf-tray-savestatus');
    if (st) { st.className = 'ix-arf-status'; st.textContent = ''; }
    var cf = document.getElementById('ix-arf-tray-confirm');
    if (cf) cf.disabled = true;
    var spin = document.getElementById('ix-arf-tray-savespin');
    if (spin) spin.style.display = 'none';
    var cl = document.getElementById('ix-arf-tray-confirmlbl');
    if (cl) cl.textContent = 'Save \u2192';
  }
  function openAssetSave(assetId, withDownload) {
    var row = document.getElementById('ix-arf-tray-saverow');
    var sel = custSel();
    if (!row || !sel) return;
    hideAssetSave();
    saveCtx.assetId = assetId; saveCtx.withDownload = !!withDownload;
    var custs = readCustomers();
    ...
    var cl = document.getElementById('ix-arf-tray-confirmlbl');
    if (cl) cl.textContent = saveCtx.withDownload ? 'Save & Download \u2192' : 'Save \u2192';
    if (row.scrollIntoView) row.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
  }
  function wireAssetSaveRow() {
    var sel = custSel();
    if (sel) sel.addEventListener('change', function () {
      ...
      var cf = document.getElementById('ix-arf-tray-confirm');
      if (cf) cf.disabled = !dirty;
    });
    var cancel = document.getElementById('ix-arf-tray-custcancel');
    if (cancel) cancel.addEventListener('click', hideAssetSave);
    var confirm = document.getElementById('ix-arf-tray-confirm');
    if (confirm) confirm.addEventListener('click', saveAssetConfirm);
  }
  ...
  function saveAssetConfirm() {
    var cfg = CFG();
    var sel = custSel();
    var btn = document.getElementById('ix-arf-tray-confirm');
    var spin = document.getElementById('ix-arf-tray-savespin');
    var lbl = document.getElementById('ix-arf-tray-confirmlbl');
    var st = document.getElementById('ix-arf-tray-savestatus');
    function status(kind, msg) { if (st) { st.className = 'ix-arf-status ' + (kind || ''); st.textContent = msg || ''; } }

    var asset = null;
    S.assets.forEach(function (a) { if (a.id === saveCtx.assetId) asset = a; });
    var customerId = sel ? sel.value : '';
    var customerName = (sel && sel.selectedOptions[0]) ? (sel.selectedOptions[0].getAttribute('data-name') || '') : '';
    if (!customerId || !asset) { status('err', 'Pick a customer first.'); return; }
    ...
    if (saveCtx.withDownload) downloadCanvas(asset.canvas, fname);
    ...
    asset.canvas.toBlob(function (blob) {
      uploadToUploadcare(blob, function (up) {
        status('work', 'Creating MEDIA item\u2026');
        // Scenario B — Route 4 (sourceChannel:'reformatter'). Customer
        // reference + isolated-object role make this MEDIA row part of the
        // customer's component library. DELIVERABLE path.
        var bPayload = {
          action: 'createMediaFromBrowserUpload',
          uploadcareUuid: up.uuid,
          uploadcareUrl: up.cdnUrl,
          ...
          titleSlug: cfg.titleSlug || '',
          taItemId: cfg.taItemId || '',
          componentRole: (asset.kind === 'iso') ? ISOLATED_ROLE_HASH : '',
          customerId: customerId,
          customerName: customerName,
          autoAttach: false,
          sourceChannel: 'reformatter',
        ...
        fetch(cfg.makeConditioner, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(bPayload) })
          ...
          .then(function (res) {
            ...
            if (spin) spin.style.display = 'none';
            // selection committed: clear the dirty edit-state.
            sel.classList.remove('ix-select--dirty');
            sel.style.removeProperty('border-color');
            sel.style.removeProperty('box-shadow');
          })
          .catch(function (err) {
            ...
            if (spin) spin.style.display = 'none';
            if (lbl) lbl.textContent = saveCtx.withDownload ? 'Save & Download \u2192' : 'Save \u2192';
          });
      }, function (msg) {
        status('err', msg);
        if (btn) btn.disabled = false;
