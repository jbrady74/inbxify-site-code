...
/* ============================================================
   ...
   NAVIGATION:
     Card click → /nl/{data-nl-slug}

   CHANGELOG:
     v1.0.3 — FIX: card click routed to /newsletters/ (404). Correct Webflow
              collection-page slug is /nl/. Route extracted to NL_URL_PREFIX
              constant so it is no longer a loose string literal.
   ============================================================ */
...
(function () {
  ...
  var INITIAL_SHOW = 4;
  var BATCH_SIZE = 4;
  var MOUNT_SEL = '#newsletters-mount';
  var SOURCE_SEL = '.newsletter-source .w-dyn-items';
  var NL_URL_PREFIX = '/nl/';   // Webflow NEWSLETTERS collection page slug (site-level, not tenant-specific)
  ...
  function render() {
    ...
    if (!newsletters.length) {
      root.innerHTML =
        ((window.IxHeader && IxHeader.render)
          ? IxHeader.render({ icon: '\u2709\uFE0F', title: 'Newsletters' })
          : '<div class="ix-hdr"><div class="ix-hdr-left"><div class="ix-hdr-icon">\u2709\uFE0F</div>' +
            '<div><h3>Newsletters</h3></div></div><div class="ix-hdr-right"></div></div>') +
    ...
    // Header
    // v1.0.2: header via the shared component (ix-header-v1.0.1.js).
    //   Was a bespoke .nl-tab-header copy. Subtitle is the live TITLE
    //   name, same as every other tab - the old issue-count line is
    //   dropped so all tabs read identically.
    html += (window.IxHeader && IxHeader.render)
      ? IxHeader.render({ icon: '\u2709\uFE0F', title: 'Newsletters' })
      : '<div class="ix-hdr"><div class="ix-hdr-left"><div class="ix-hdr-icon">\u2709\uFE0F</div>' +
        '<div><h3>Newsletters</h3></div></div><div class="ix-hdr-right"></div></div>';
  ...
  function handleCardClick(e) {
    ...
    if (slug) {
      window.location.href = NL_URL_PREFIX + slug;
    }
  }
