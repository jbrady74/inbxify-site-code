...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function render() {
    ...
    // ── Search box (v1.0.33) — filters current tab by name + customer ──
    h += '<div class="cl-search-row" style="padding:8px 0 0">';
    h += '<input id="cl-search" class="cl-search-input" type="text" placeholder="Search ' +
         esc(S.mainTab === 'realestate' ? 'real estate' : S.mainTab) + ' by name or customer\u2026" ' +
         'value="' + esc(S.q || '') + '" ' +
         'oninput="_clSetQ(this.value)" ' +
         'style="width:100%;max-width:360px;font-family:\'DM Sans\',sans-serif;font-size:13px;padding:7px 11px;border:1px solid #e8e4d8;border-radius:7px;background:#fff;color:#1a3a3a;outline:none">';
    h += '</div>';
    ...
    if (relProds.length) {
      ...
      h += '<select class="cl-sort" style="min-width:72px" onchange="_clSetSub(this.value)">';
