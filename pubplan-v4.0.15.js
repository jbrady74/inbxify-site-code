...
function renFa(sc) {
  ...
  // ── Category column: dropdown if no selection, locked pill + ✕ if selected ──
  let catCol;
  if (!s.catId || isEd) {
    if (s.catId) {
      catCol = `<div class="pc"><span class="pcl">Category</span><div class="ppt-pill-wrap"><span class="pcp cfa hs">${s.catNm}</span><span class="ppt-x" onclick="onFaCatClear('${sc}')" title="Change">✕</span></div></div>`;
    } else {
    ...
    catCol = `<div class="pc"><span class="pcl">Category</span><span class="pcp cfa">${s.catNm || '—'}</span></div>`;
  }
...
function renTs(sc) {
  ...
  if (!s.catId || isEd) {
    if (s.catId) {
      catCol = `<div class="pc"><span class="pcl">Category</span><div class="ppt-pill-wrap"><span class="pcp cts hs">${s.catNm}</span><span class="ppt-x" onclick="onTsCatClear('${sc}')" title="Change">✕</span></div></div>`;
    } else {
    ...
    catCol = `<div class="pc"><span class="pcl">Category</span><span class="pcp cts">${s.catNm || '—'}</span></div>`;
  }
...
function initTxaState() {
  ...
  for (let i = 1; i <= 5; i++) {
    const code = `txa-${i}`;
    if (!state[code]) {
      state[code] = { sc: code, secC: 'txa', slotNum: i, pubplanId: '', titleadminId: '', custId: '', custNm: '', dirty: false };
    }
  }
}
