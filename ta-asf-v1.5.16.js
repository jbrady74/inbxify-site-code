/* ============================================================
   ...
   Companion stylesheet: ta-asf-v1.5.16.css (paired bump)
...
(function () {
  ...
  function extractInlineMediaIds(html) {
    ...
    // v1.5.15: Trix stores inline images as attachments whose data-media-id
    // is HTML-entity-encoded inside the data-trix-attachment JSON, e.g.
    //   data-media-id=&quot;6a3ba2...&quot;
    // The old regex only matched literal/backslash-escaped quotes, so every
    // inlined image was missed → inlineMediaIds came through empty → Scenario
    // 104 never flipped those MEDIA to In Use. We now match THREE forms:
    //   1. plain or backslash-escaped quotes:  data-media-id="x"  / =\"x\"
    //   2. entity-encoded quotes:              data-media-id=&quot;x&quot;
    //   3. single-quote entity (rare):         data-media-id=&#39;x&#39;
    var rePlain = /data-media-id\s*=\s*\\?["']([^"'\\]+)\\?["']/g;
    var reQuot  = /data-media-id\s*=\s*&quot;([^&]+)&quot;/g;
    var reApos  = /data-media-id\s*=\s*&#39;([^&]+)&#39;/g;
    var m;
    while ((m = rePlain.exec(html)) !== null) add(m[1]);
    while ((m = reQuot.exec(html))  !== null) add(m[1]);
    while ((m = reApos.exec(html))  !== null) add(m[1]);
    return ids;
  }
