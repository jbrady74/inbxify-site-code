/* ============================================================
   ...
   5. The returned UUID flows through the SAME path an upload uses:
      window.InbxAsf.attachUploadcareAsMain(uuid, url, ...) which calls
      Scenario B 'createMediaFromBrowserUpload' → MEDIA row + autoAttach.
      No MEDIA-invariant bypass.

   ── CONFIG (TA_CONFIG) ──
     anthropicProxy   (already present — used by ta-rte / ta-generate)
     fluxGen          NEW — the Worker URL. Add to TA_CONFIG:
                      fluxGen: 'https://fluxgen.jeff-2cd.workers.dev/'
     (uploadcare + makeConditioner are read by ASF's own attach path)
...
(function () {
  ...
  /* ── Attach via the existing ASF Scenario B path ──
     The fluxgen Worker already returns a permanent Uploadcare UUID,
     which is exactly what an upload produces. So we replay the SAME
     createMediaFromBrowserUpload POST that ASF's notifyScenarioB sends,
     using the live tenant config + article id from InbxASF._internal.
     This lands a MEDIA row + autoAttach with no new Make work and no
     MEDIA-invariant bypass. */
  function attachAsMain(j, prompt) {
    ...
    var url = tenant.makeConditioner();
    if (!url) return Promise.resolve({ fallback: true, url: j.url });

    var a = S.article || {};
    var roleKey  = (CFG.pickerRoleMap && CFG.pickerRoleMap['main-image']) || 'mainImage';
    var roleHash = tenant.roleHash(roleKey);
    var payload = {
      action:           'createMediaFromBrowserUpload',
      uploadcareUuid:   j.uuid,
      uploadcareUrl:    j.url,
      originalFilename: 'flux-' + String(j.uuid).slice(0, 8) + '.jpg',
      mimeType:         'image/jpeg',
      size:             0,
      ...
      autoAttach:       true,
      sourceChannel:    'asf-image-gen',     // distinct channel for GA4/telemetry
      imageSource:      'ai-flux-2-pro',      // future provenance field (HC-IMG plan)
      generationPrompt: prompt,               // future provenance field
    ...
    return fetch(url, {
      ...
      var ok = false;
      try { var p = JSON.parse(body); if (p && p.ok === true) ok = true; } catch (e) {}
      if (!ok) throw new Error('Scenario B did not confirm (createMediaFromBrowserUpload route).');
      // Re-hydrate media + re-render so the new main image shows immediately.
      try { S.media = I.hydrateMedia(a.id); I.render(); } catch (e) {}
      return { ok: true };
    });
  }
  ...
  function render() {
    ...
    if (ta && !busy) {
      ta.addEventListener('input', function () { s.prompt = ta.value; render(); ta.focus();
  ...
  function onModalClick(e) {
    ...
    if (act === 'cancel') {
      if (s.prompt !== s.drafted) { s.prompt = s.drafted; render(); }   // revert
      else close();
      return;
    }
    ...
    if (act === 'use') {
      var btn = t; btn.disabled = true; btn.innerHTML = '<span class="ixig-spin"></span>Attaching…';
      attachAsMain(s.result, s.prompt).then(function (res) {
        if (res && res.fallback) {
          asfToast('Image generated. Bridge not wired — copy the URL and use Replace.', 'info');
        } else {
          asfToast('Generated image attached as main image.', 'success');
        }
        close();
      }).catch(function (err) {
        s.error = 'Attach failed: ' + ((err && err.message) || 'unknown'); s.phase = 'done'; render();
      });
      return;
    }
  }
