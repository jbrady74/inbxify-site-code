/* ============================================================
   ...
   Companion stylesheet: ta-asf-v1.1.8.css
   Reference spec:       v1.1 ASF layout + unified picker + inline
                         image size/align (parity with ta-rte v1.1.24)
...
(function () {
  ...
  function renderInlinePicker() {
    ...
    var headerIcon;
    switch (pk.mode) {
      case 'main':   headerLabel = 'Pick main image';   headerIcon = '🖼'; break;
      case 'og':     headerLabel = 'Pick OG image';     headerIcon = '🔗'; break;
    ...
    return '' +
      ...
      // v1.1.8 — size + align controls (inline mode only). The
      // selected size/align are applied to the figure when a
      // thumbnail is clicked. Matches the RTE's control set.
      (pk.mode === 'inline' ? renderPickerImageControls(pk) : '') +
  ...
  // v1.1.8 — Size (33/50/75/100%) + Align (L/C/R) control strip.
  // Inline mode only. Full size disables the align strip (100%
  // width can't be aligned), matching the RTE.
  function renderPickerImageControls(pk) {
    ...
    var alignDisabledStyle = isFull ? ' style="opacity:0.35;pointer-events:none;"' : '';
    ...
    return '' +
      '<div class="asf-picker-imgctl">' +
        ...
        '</div>' +
        '<div class="asf-picker-imgctl-group"' + alignDisabledStyle + '>' +
      ...
      case 'picker-insert-image':
        ...
        switch (pmMode) {
          ...
          case 'og':
            ...
            break;
          default:
            pmOk    = insertImageIntoBody(picked);
            pmLabel = 'Inserted: ';
        }
        if (pmOk) {
          if (S.inlinePicker) S.inlinePicker.open = false;
  ...
  log('mounted · v' + VERSION + ' · inline image size (33/50/75/100%) + align (L/C/R) restored to RTE parity — figure.rte-inserted-image content attachment.');
})();
