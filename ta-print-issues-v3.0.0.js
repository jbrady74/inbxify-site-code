/* ta-print-issues-v3.0.0.js
   ════════════════════════════════════════════════════════════════
   INBXIFY T-A — Print Issues
   Companion: ta-print-issues-v3.0.0.css
   ...
   `print-issue-plan-json` on TITLES-ADMIN, read from
   data-issue-plan on .ta-item. An array of entries, each with a
...
(function () {
  ...
  function readPlan() {
    ...
    if (!raw) {
      console.warn(TAG, 'no data-issue-plan on .ta-item \u2014 no schedule to show. ' +
                        'Add print-issue-plan-json to TITLES-ADMIN.');
    ...
    S.root.innerHTML =
      ...
      (S.orphans.length
        ? '<div class="tpi-orph"><span class="tpi-orph-h">Not on the calendar</span>' +
          '<span class="tpi-orph-w">The name matches no planned issue in ' + S.year +
            '. Check the name against print-issue-plan-json.</span>' +
  ...
  function renderNoPlan() {
    S.root.innerHTML =
      ...
      '<div class="tpi-noplan">' +
        ...
        '<span>Set <code>print-issue-plan-json</code> on TITLES-ADMIN and bind it to ' +
          '<code>data-issue-plan</code> on <code>.ta-item</code>:</span>' +
        '<pre>[\n' +
