// ============================================================
// content-manager-v1.1.0.js
// INBXIFY Content Manager — System 2 UI
// v1.1.0: fix dropdown flash (defer render on select changes)
...
<div class="cm-root">
  ...
  function getCMSItems(col, custId, productTypeId) {
    ...
    // Filter articles by product type (FA shows only FA articles, TS shows only TS articles)
    if (col==='articles' && productTypeId) {
      items = items.filter(i => !i.productTypeId || i.productTypeId === productTypeId);
    }
    // Filter by customer — show items for this customer + unassigned items
    if (custId && custId !== '__none__') {
      items = items.filter(i => !i.customerId || i.customerId === custId);
    }
    return items;
  }

  // ── Actions exposed to inline handlers ──
  // Trap keyboard events inside modal — prevent Webflow tab handler intercepting spacebar
  function trapKeys(e) {
    e.stopPropagation();
  }
  ...
  window.cmSetPT = val => {
    A = { ...A, productType:val, customerId:'', cmsItemId:'', isNew:false, newItemName:'', nestedOpen:false, nestedName:'' };
    const pt = getPT(val);
  ...
  window.cmSetCust = val => {
    if (val==='__new__') { A.nestedOpen=true; A.customerId=''; deferRender(); return; }
  ...
  window.cmSetRevenueType = val => { A.newRevenueType = val; deferRender(); };

  window.cmSetItem = val => { A.cmsItemId=val; deferRender(); };
  window.cmStartNew = () => { A.isNew=true; A.newItemName=''; deferRender(); };
  window.cmBackToExisting = () => { A.isNew=false; A.newItemName=''; deferRender(); };
