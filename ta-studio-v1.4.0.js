...
//     • Article  → window.InbxASF.open({ mode: 'create' })
//     • Ad       → toast "Coming soon"
//     • RE       → toast "Coming soon"
//     • Event    → toast "Coming soon"
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
      ...
      '<div class="std-root">' +
        '<div class="ix-hdr">' +
          ...
          '<div class="ix-hdr-right">' +
            ...
            '<span class="ix-badge">v1.3.9</span>' +
          '</div>' +
        '</div>' +
        ...
        '<div class="std-create-menu" data-std-create-menu' +
          ...
          '<button class="std-create-item" data-std-create-type="ad"' +
            ' style="display:block;width:100%;padding:10px 16px;border:0;background:transparent;' +
                    'text-align:left;font-size:14px;color:#666;cursor:pointer;">' +
            'Ad <span style="font-size:9px;font-family:\'DM Mono\',monospace;color:#999;letter-spacing:0.1em;margin-left:6px;">SOON</span>' +
          '</button>' +
          '<button class="std-create-item" data-std-create-type="re"' +
            ' style="display:block;width:100%;padding:10px 16px;border:0;background:transparent;' +
                    'text-align:left;font-size:14px;color:#666;cursor:pointer;">' +
            'RE Listing <span style="font-size:9px;font-family:\'DM Mono\',monospace;color:#999;letter-spacing:0.1em;margin-left:6px;">SOON</span>' +
          '</button>' +
          '<button class="std-create-item" data-std-create-type="event"' +
            ' style="display:block;width:100%;padding:10px 16px;border:0;background:transparent;' +
                    'text-align:left;font-size:14px;color:#666;cursor:pointer;">' +
            'Event <span style="font-size:9px;font-family:\'DM Mono\',monospace;color:#999;letter-spacing:0.1em;margin-left:6px;">SOON</span>' +
          '</button>' +
        '</div>' +
    ...
    // v1.3.8: route create-type picks to the right handler
    function handleCreateType(type) {
      if (type === 'article') {
        if (!window.InbxASF || typeof window.InbxASF.open !== 'function') {
          showStudioToast('ASF not loaded on this page — cannot create article');
          console.warn('[ta-studio v1.4.0] window.InbxASF is not available');
          return;
        }
        try {
          window.InbxASF.open({ mode: 'create' });
        } catch (err) {
          console.error('[ta-studio v1.3.8] InbxASF.open failed:', err);
          showStudioToast('Could not open ASF — see console');
        }
        return;
      }
      // Ad / RE / Event — not yet wired
      var labels = { ad: 'Ad', re: 'RE Listing', event: 'Event' };
      showStudioToast('Create ' + (labels[type] || type) + ' — coming soon');
    }
    ...
    function openBodyEditorLocal(params) {
      if (!window.InbxRTE || typeof window.InbxRTE.openFullscreen !== 'function') {
        console.error('[TA-STUDIO v1.4.0] openBodyEditorLocal: window.InbxRTE.openFullscreen not available.');
        alert('Body editor is not loaded. Check the ta-rte script reference in the T-A page head, and look for ta-rte errors in the browser console.');
        return;
      }
      console.log('[TA-STUDIO v1.4.0] openBodyEditorLocal:', { initialBodyLen: (params.initialBody || '').length });
      ...
      window._studioRteInstance = window.InbxRTE.openFullscreen({
        ...
        onSave: function (editedHTML) {
          ...
          try { params.onSave(editedHTML); } catch (e) {
            console.error('[TA-STUDIO v1.4.0] local-mode onSave threw:', e);
          }
        },
        onClose: function () {
          window._studioRteInstance = null;
    ...
    loadStudioPdfCount();
    console.log('\u29BE Studio v1.4.0 mounted \u2014 NEW: window.InbxStudioBodyEditor accepts {mode:"local"} for callers (ASF create mode) that need to edit body content WITHOUT going through Scenario G. Local mode skips fetch/save/reload — caller passes initialBody and onSave callback. Existing edit-mode contract unchanged. v1.3.9 launcher (Create dropdown) retained. Companion: ta-asf-v1.0.15+.');
  });
