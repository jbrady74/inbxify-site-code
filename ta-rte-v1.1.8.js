  ...
  class InbxRTE {
    ...
    customizeToolbar() {
      ...
      if (blockGroup) {
        ...
        // v1.1.7: Exclusive heading selection.
        // Trix toggles attributes additively — clicking H2 while H3 is active
        // stacks them instead of replacing. We intercept both buttons, remove
        // the sibling heading attribute first, then apply the desired one.
        // This also ensures the correct injectStyles rule fires (H2 rule was
        // not applying because H3 was still active on the block).
        const applyExclusiveHeading = (applyAttr, removeAttr) => {
          const editor = this.trixEditor;
          if (!editor) return;
          const doc = editor.getDocument();
          const sel = editor.getSelectedRange();
          // Check if removeAttr is active on current block and deactivate it
          const selectedDoc = doc.getDocumentAtRange(sel);
          let needsRemove = false;
          selectedDoc.getBlocks().forEach(block => {
            if (block.getAttributeLevel() > 0 || block.hasAttribute(removeAttr)) {
              needsRemove = true;
            }
          });
          if (needsRemove) {
            editor.deactivateAttribute(removeAttr);
          }
          // Now toggle the desired heading
          editor.activateAttribute(applyAttr);
        };

        h2Btn.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          applyExclusiveHeading('heading2', 'heading3');
        });

        h3Btn.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          applyExclusiveHeading('heading3', 'heading2');
        });
    ...
    insertImage() {
      ...
      const figureHTML = `<figure class="rte-inserted-image" data-img-size="${esc(this.selectedImgSize || 'large')}" data-img-align="${esc(this.selectedImgAlign || 'center')}">`
    ...
    swapPlaceholderForImage(placeholderId, opts) {
      ...
      try {
        ...
        const finalHTML =
          `<figure class="rte-inserted-image" data-img-size="${esc(this.selectedImgSize||'large')}" data-img-align="${esc(this.selectedImgAlign||'center')}">` +
    ...
    injectStyles() {
      ...
      style.textContent = `
        ...
        trix-editor.rte-trix-editor h2 { font-family:'Tenor Sans',serif; font-size:24px; font-weight:400; line-height:1.25; color:#1A3A3A; margin:32px 0 12px; }
        trix-editor.rte-trix-editor h3 { font-family:'Tenor Sans',serif; font-size:19px; font-weight:400; line-height:1.3; color:#C4A35A; margin:24px 0 10px; }
        ...
        .rte-fs-mount .rte-header { position:sticky; top:0; background:white; z-index:20; padding-bottom:10px; border-bottom:1px solid ${BORDER}; margin-bottom:0; }
        .rte-fs-mount trix-toolbar { position:sticky; top:52px; background:white; z-index:19; padding:6px 0; border-bottom:1px solid ${BORDER}; margin-bottom:8px; }
