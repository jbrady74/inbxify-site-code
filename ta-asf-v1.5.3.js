...
(function () {
  ...
  function publicOpen(params) {
    ...
    if (mode === 'edit') {
      // v1.5.3 (Decision A): ALL types — including article — hydrate from
      // the live Workbench Read webhook (DB-fresh), not the page DOM.
    ...
    if (mode === 'create') {
      ...
      // v1.5.3: article edit no longer reaches here (edit is async above).
      // Retained as a fallback only; DOM hydrate is stale-prone (see A).
