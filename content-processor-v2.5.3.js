...
// v2.5.2: TRANSCRIBE REDIRECT + TWO-LEVEL TAB STRUCTURE
// v2.4.2: PDF DECISION UI
// v2.4.1: ARCHIVE FEATURE
// v2.4.0: LAYOUT OVERHAUL
// v2.3.1: Batch submit, Hide button, readProducts()
// v2.3.0: Product Library dropdown
// v2.2.x: Choices.js, live DOM reads
// v2.0.0: Clean rewrite
// ============================================================
...
.cm-channels{display:flex;gap:0;margin-bottom:0;border-bottom:2px solid #1a3a3a;align-items:flex-end}
.cm-channel{font-family:'DM Sans',system-ui,sans-serif;font-size:11px;font-weight:600;padding:10px 18px 8px;border:none;background:transparent;color:#8a8a7a;cursor:pointer;transition:all .15s;border-bottom:3px solid transparent;margin-bottom:-2px}
...
<div class="cm-root">
  <div class="cm-hdr">
    ...
    <span class="cm-badge">v2.5.3</span>
  </div>
  ...
  // RENDER
  // ══════════════════════════════════════════════
  function render() { renderChannels(); const gdriveEl=document.getElementById('cm-gdrive-panel'),emailEl=document.getElementById('cm-email-panel'),formEl=document.getElementById('cm-form-panel'),transEl=document.getElementById('screenshot-transcriber-mount'); gdriveEl.style.display=CHANNEL==='gdrive'?'':'none'; emailEl.style.display=CHANNEL==='email'?'':'none'; formEl.style.display=CHANNEL==='form'?'':'none'; if(transEl)transEl.style.display=CHANNEL==='transcriber'?'':'none'; if(CHANNEL==='gdrive'){renderMonitor();renderTabs();renderError();renderSelBar();renderFileList();}else if(CHANNEL==='email'){renderEmailPlaceholder();}else if(CHANNEL==='form'){renderFormPlaceholder();} }

  function renderChannels() { const gdCount=FILES.length?` (${FILES.filter(f=>f.status==='DONE'&&!f.assigned).length})`:''; let h=''; CHANNELS.forEach(ch=>{const isActive=ch.id===CHANNEL;const countHtml=ch.id==='gdrive'&&gdCount?`<span style="font-family:'DM Mono',monospace;font-size:9px;opacity:.6">${gdCount}</span>`:'';h+=`<button class="cm-channel ${isActive?'active':''}" data-cm-channel="${ch.id}">${esc(ch.label)}${countHtml}</button>`;}); document.getElementById('cm-channels').innerHTML=h; }

  function renderEmailPlaceholder(){document.getElementById('cm-email-panel').innerHTML=`<div class="cm-placeholder"><div class="cm-placeholder-icon">\u{1F4E7}</div><div class="cm-placeholder-title">Email Intake</div><div class="cm-placeholder-text">Accept files from publishers via email. Configure a dedicated email address and files will appear here automatically.</div></div>`;}

  function renderFormPlaceholder(){document.getElementById('cm-form-panel').innerHTML=`<div class="cm-placeholder"><div class="cm-placeholder-icon">\u{1F4CB}</div><div class="cm-placeholder-title">Form Submission</div><div class="cm-placeholder-text">Accept content submissions via web form. Placeholder for future intake channel.</div></div>`;}

  function renderMonitor(){const u=MONITOR.uploadFolder,p=FILES.filter(f=>f.status==='AWAITING').length;const uCls=u>5?'alert':u>0?'warn':'',pCls=p>5?'alert':p>0?'warn':'';document.getElementById('cm-monitor').innerHTML=`<div class="cm-monitor"><span class="cm-monitor-item">Publisher Upload Folder: <span class="cm-monitor-count ${uCls}">${u}</span></span><span class="cm-monitor-item">Pre-processing: <span class="cm-monitor-count ${pCls}">${p}</span></span></div>`;}

  function renderTabs(){const c=getTabCounts();let h='';TABS.forEach(t=>{const isActive=t.id===TAB;const countHtml=t.status?`<span class="cm-tab-count">${c[t.id]}</span>`:'';h+=`<button class="cm-tab ${isActive?'active':''}" data-cm-tab="${t.id}">${esc(t.label)}${countHtml}</button>`;});h+=`<span class="cm-tab-spacer"></span>`;h+=`<button class="cm-refresh" data-cm-refresh>\u21BB Refresh</button>`;document.getElementById('cm-tabs').innerHTML=h;}

  function renderError(){const el=document.getElementById('cm-error');if(!LOAD_ERR){el.innerHTML='';return;}el.innerHTML=`<div class="cm-error"><span>${esc(LOAD_ERR)}</span><button data-cm-refresh>Retry</button></div>`;}

  function renderSelBar(){const sel=getSelected();const el=document.getElementById('cm-sel-bar');if(!sel.length){el.innerHTML='';return;}if(TAB==='pdf-queue'){el.innerHTML=`<div class="cm-sel-bar"><span class="cm-sel-count">${sel.length} PDF${sel.length>1?'s':''} selected</span><div class="cm-sel-spacer"></div><button class="cm-desel" data-cm-desel>\u2715 clear</button><button class="cm-pdf-btn" data-cm-pdf-jpeg>\u{1F5BC}\uFE0F Convert to JPEG</button><button class="cm-pdf-btn cm-pdf-btn-transcribe" data-cm-pdf-transcribe>\u{1F4DD} Transcribe &amp; Summarize</button></div>`;}else{el.innerHTML=`<div class="cm-sel-bar"><span class="cm-sel-count">${sel.length} file${sel.length>1?'s':''} selected</span><div class="cm-sel-spacer"></div><button class="cm-desel" data-cm-desel>\u2715 clear</button><button class="cm-desel" data-cm-hide-sel>\u{1F6AB} hide</button><button class="cm-archive-btn" data-cm-archive-sel>\u{1F4E6} Archive</button><button class="cm-assign-btn" data-cm-open-modal>Assign ${sel.length} File${sel.length>1?'s':''} \u2192</button></div>`;}}

  function renderFileList(){const el=document.getElementById('cm-flist');if(LOADING){el.innerHTML=`<div class="cm-loading"><div class="cm-loading-spin">\u27F3</div><div>Loading files\u2026</div></div>`;return;}const tab=TABS.find(t=>t.id===TAB);let vis=FILES;if(TAB==='ready')vis=FILES.filter(f=>f.status==='DONE'&&!f.assigned);if(TAB==='assigned')vis=FILES.filter(f=>f.status==='ASSIGNED'||f.assigned);if(TAB==='pdf-queue')vis=FILES.filter(f=>f.status==='AWAITING_DECISION');if(TAB==='archived')vis=FILES.filter(f=>f.status==='ARCHIVED');if(TAB==='hidden')vis=FILES.filter(f=>f.status==='HIDDEN');if(!vis.length){const msgs={'pdf-queue':'No PDFs awaiting decision',ready:'No files ready to assign',assigned:'No files assigned yet',archived:'No archived files',hidden:'No hidden files'};el.innerHTML=`<div class="cm-empty"><div class="cm-empty-icon">${TAB==='ready'?'\u2705':'\u{1F4ED}'}</div><div class="cm-empty-text">${msgs[TAB]||'No files'}</div></div>`;return;}el.innerHTML=vis.map(f=>{const icon=FILE_ICONS[f.mime]||'\u{1F4C4}';const isAssigned=f.assigned||f.status==='ASSIGNED';const isArchived=f.status==='ARCHIVED';const isHidden=f.status==='HIDDEN';const isPdf=f.status==='AWAITING_DECISION';const badge=isAssigned?'assigned':isArchived?'archived':isHidden?'hidden':isPdf?'decision':'ready';const label=isAssigned?'Assigned':isArchived?'Archived':isHidden?'Hidden':isPdf?'PDF Queue':'Ready';const uc=f.uuid?`<span style="color:#1a3a3a">UC:${f.uuid.substring(0,8)}\u2026</span>`:'';const disabled=isAssigned||isArchived||isHidden||(isPdf&&TAB!=='pdf-queue');if(tab&&tab.selectable&&!disabled){return`<div class="cm-frow ${f.selected?'sel':''}" data-cm-fid="${f.id}"><div class="cm-frow-main"><input type="checkbox" class="cm-fcheck" ${f.selected?'checked':''}><span class="cm-ficon">${icon}</span><div class="cm-finfo"><div class="cm-fname">${esc(f.name)}</div><div class="cm-fmeta">${f.size?`<span>${fmtSize(f.size)}</span>`:''} ${f.arrived?`<span>${f.arrived.substring(0,10)}</span>`:''} ${uc}</div></div><div class="cm-fright"><span class="cm-fbadge ${badge}">${label}</span></div></div></div>`;}return`<div class="cm-frow ${isAssigned?'assigned':''}"><div class="cm-frow-main no-select"><span class="cm-ficon">${icon}</span><div class="cm-finfo"><div class="cm-fname">${esc(f.name)}</div><div class="cm-fmeta">${f.size?`<span>${fmtSize(f.size)}</span>`:''} ${f.arrived?`<span>${f.arrived.substring(0,10)}</span>`:''} ${uc}</div></div><div class="cm-fright"><span class="cm-fbadge ${badge}">${label}</span></div></div></div>`;}).join('');}

  function renderTranscriberNotice(){const el=document.getElementById('screenshot-transcriber-mount');if(!el)return;const existing=el.querySelector('.cm-transcribe-notice');if(existing)existing.remove();if(TRANSCRIBE_PDFS.length){const fileList=TRANSCRIBE_PDFS.map(n=>`<span class="cm-transcribe-file">\u{1F4D5} ${esc(n)}</span>`).join('');const notice=`<div class="cm-transcribe-notice"><div class="cm-transcribe-notice-hdr">\u{1F4DD} Download these PDFs from Google Drive, then drop them here:</div><div class="cm-transcribe-notice-files">${fileList}</div><button class="cm-transcribe-notice-dismiss" onclick="this.closest('.cm-transcribe-notice').remove()">dismiss</button></div>`;el.insertAdjacentHTML('afterbegin',notice);}}

  function getSelected(){if(TAB==='pdf-queue')return FILES.filter(f=>f.selected&&f.status==='AWAITING_DECISION');return FILES.filter(f=>f.selected&&!f.assigned&&f.status!=='ASSIGNED'&&f.status!=='ARCHIVED'&&f.status!=='HIDDEN');}

  // ══════════════════════════════════════════════
  // EVENT DELEGATION
  // ══════════════════════════════════════════════
  mount.addEventListener('click',function(e){const chBtn=e.target.closest('[data-cm-channel]');if(chBtn){setChannel(chBtn.dataset.cmChannel);return;}const tabBtn=e.target.closest('[data-cm-tab]');if(tabBtn){setTab(tabBtn.dataset.cmTab);return;}if(e.target.closest('[data-cm-refresh]')){reload();return;}if(e.target.closest('[data-cm-desel]')){deselectAll();return;}if(e.target.closest('[data-cm-hide-sel]')){hideSelected();return;}if(e.target.closest('[data-cm-archive-sel]')){archiveSelected();return;}if(e.target.closest('[data-cm-pdf-jpeg]')){processPdfs('convert_to_jpeg');return;}if(e.target.closest('[data-cm-pdf-transcribe]')){redirectToTranscriber();return;}if(e.target.closest('[data-cm-open-modal]')){openModal();return;}const frow=e.target.closest('[data-cm-fid]');if(frow){toggleSel(frow.dataset.cmFid);return;}});

  function setChannel(id){CHANNEL=id;FILES.forEach(x=>{x.selected=false;x.elementType='';});A=freshA();if(id==='gdrive')TAB='ready';if(id!=='transcriber')TRANSCRIBE_PDFS=[];render();}
  function setTab(id){TAB=id;FILES.forEach(x=>{x.selected=false;x.elementType='';});A=freshA();render();}
  function reload(){loadFiles();}
  function toggleSel(id){const f=FILES.find(x=>x.id===id);if(!f||f.assigned||f.status==='ASSIGNED'||f.status==='ARCHIVED')return;if(f.status==='AWAITING_DECISION'&&TAB!=='pdf-queue')return;f.selected=!f.selected;if(!f.selected)f.elementType='';renderSelBar();renderFileList();}
  function deselectAll(){FILES.forEach(f=>{f.selected=false;f.elementType='';});A=freshA();closeModal();render();}

  // ── v2.5.3: Persistent status via Scenario C Route 3 ──
  ...
  function processPdfs(action){const sel=getSelected();if(!sel.length)return;if(!CFG.makeConditioner){console.error('[CP] No makeConditioner URL configured');return;}const actionLabel=action==='convert_to_jpeg'?'Convert to JPEG':'Transcribe & Summarize';console.log('[CP] PDF Decision:',actionLabel,'for',sel.length,'file(s)');sel.forEach(f=>{f.selected=false;f._processing=true;});render();const promises=sel.map(f=>fetch(CFG.makeConditioner,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({titleSlug:CFG.titleSlug,fileId:f.fileId,fileName:f.name,mimeType:f.mime,action:action}),}).then(res=>{if(!res.ok)throw new Error('Scenario B returned '+res.status+' for '+f.name);f.status='DONE';delete f._processing;console.log('[CP] PDF processed:',f.name,'\u2192',actionLabel);}).catch(err=>{console.error('[CP] PDF process error:',f.name,err);delete f._processing;}));Promise.all(promises).then(()=>render());}

  function redirectToTranscriber(){const sel=getSelected();if(!sel.length)return;TRANSCRIBE_PDFS=sel.map(f=>f.name);sel.forEach(f=>{f.selected=false;});CHANNEL='transcriber';A=freshA();render();renderTranscriberNotice();console.log('[CP] Redirected to Transcriber for:',TRANSCRIBE_PDFS.join(', '));}
