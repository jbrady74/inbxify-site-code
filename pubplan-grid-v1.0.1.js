...
(function () {
  ...
  var VERSION = '1.0.1';
  // v1.0.1: read path pivoted off Webflow nesting (NEWSLETTERS is at the 10-ref limit, can't
  // add an NL-BLOCKS multi-ref). Editions still come from a DOM list; BLOCKS now come from a
  // flat source — a [data-nlblocks-flat] DOM list (hand-authored embed for smoke-test, or any
  // scoped Webflow list) OR a fetch to the read endpoint — then grouped by newsletter id.
  ...
  var PP_BLOCKS_READ_FALLBACK = '';       // CORS read endpoint (Cloudflare Worker). Empty until wired.
  ...
  // ---- data adapter (v1.0.1): editions from DOM list, blocks flat (DOM list or fetch), grouped by newsletter id ----

  // editions metadata only — the NEWSLETTERS Collection List (filtered to Current PubPlan). No nested blocks.
  ...
  function blockFromAttrs(b) {
    var pos = parseFloat(b.getAttribute('data-nlb-position'));
    return {
      id: b.getAttribute('data-nlb-id') || null,
      newsletterId: b.getAttribute('data-nlb-newsletter') || null,   // for grouping into editions
  ...
  function blockFromJson(r) {
    var pos = parseFloat(r.position != null ? r.position : r.pos);
    return {
      id: r.id || null,
      newsletterId: r.newsletterId || r.newsletter || null,
      slotKey: r.slotKey || r['slot-key'] || '',
      pos: isNaN(pos) ? 0 : pos,
      type: r.type || r['block-type'] || '',
      asset: r.asset || null,
      assetId: r.assetId || r['asset-id'] || null,
      company: r.company || '',
      sponsored: r.sponsored === true || r.sponsored === 'true',
      bannerSize: r.bannerSize || r['banner-size'] || '235'
    };
  }

  // a flat NL-BLOCKS list in the DOM (each [data-nlb] carries data-nlb-newsletter). Returns null if absent.
  function readFlatBlocksDOM(root) {
    var wrap = root.querySelector('[data-nlblocks-flat]') || document.querySelector('[data-nlblocks-flat]');
    if (!wrap) return null;
    return Array.prototype.map.call(wrap.querySelectorAll('[data-nlb]'), blockFromAttrs);
  }

  // group a flat block array onto editions by matching newsletter id
  function attachBlocks(eds, blocks) {
    var byEd = {};
    eds.forEach(function (e) { e.blocks = []; byEd[e.id] = e; });
    blocks.forEach(function (b) { var e = byEd[b.newsletterId]; if (e) e.blocks.push(b); });
    return eds;
  }

  function blocksReadUrl() {
    var c = (window.TA_CONFIG && window.TA_CONFIG.ppWebhooks) || {};
    return c.blocksRead || (window.TA_CONFIG && window.TA_CONFIG.ppBlocksRead) || PP_BLOCKS_READ_FALLBACK;
  }
  ...
  function Grid(root) {
    ...
    this.demo = !eds;
    this.editions = (eds && eds.length) ? eds : demoEditions();   // demo editions carry their own blocks
    this.newSlot = 0; this.newEd = 0; this.dragKey = null;
  }

  // acquire blocks (flat DOM list -> sync; else fetch -> async; demo already has them), then build the model.
  Grid.prototype.load = function (done) {
    var self = this;
    function finish() { self.buildModel(); self.snapshot(); if (done) done(); }
    if (this.demo) { finish(); return; }                      // demo path
    var dom = readFlatBlocksDOM(this.root);
    if (dom) { attachBlocks(this.editions, dom); finish(); return; }   // flat DOM list path
    var url = blocksReadUrl();
    if (!url) {                                               // no source wired
      this.mountMsg('No block source. Add a <code>[data-nlblocks-flat]</code> list to the page, or set <code>TA_CONFIG.ppBlocksRead</code>.', true);
      console.warn(LOG, 'no block source (no [data-nlblocks-flat] and no read endpoint).');
      return;
    }
    this.mountMsg('Loading blocks\u2026', false);              // fetch path
    this.fetchBlocks(url).then(function (blocks) {
      attachBlocks(self.editions, blocks); finish();
    }).catch(function (err) {
      console.error(LOG, 'block read failed:', err);
      self.mountMsg('Couldn\u2019t load blocks (' + esc(err.message) + '). Check the read endpoint / CORS.', true);
    });
  };

  Grid.prototype.fetchBlocks = function (url) {
    var ids = this.editions.map(function (e) { return e.id; }).filter(Boolean);
    return fetch(url, {
      method: 'POST', mode: 'cors', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ pubplanId: this.pubplanId, titleAdminId: this.titleAdminId, editionIds: ids })
    }).then(function (r) {
      if (!r.ok) throw new Error('HTTP ' + r.status);
      return r.json();
    }).then(function (data) {
      var arr = (data && (data.blocks || data.items)) || (Array.isArray(data) ? data : []);
      return arr.map(blockFromJson);
    });
  };

  Grid.prototype.mountMsg = function (html, isErr) {
    this.root.innerHTML = '<div class="ppg-demo"' + (isErr ? ' style="border-left-color:#c0563f"' : '') + '>' + html + '</div>';
  };
  ...
  Grid.prototype.mount = function () {
    var demoNote = this.demo
      ? '<div class="ppg-demo">Demo data — no <code>[data-editions-wrapper]</code> on the page. Add the NEWSLETTERS editions list, plus a flat blocks source (a <code>[data-nlblocks-flat]</code> list or <code>TA_CONFIG.ppBlocksRead</code>), to read live.</div>'
      : '';
  ...
  function diagnose(root) {
    ...
    var flat = root.querySelector('[data-nlblocks-flat]') || document.querySelector('[data-nlblocks-flat]');
    var flatRows = flat ? flat.querySelectorAll('[data-nlb]').length : 0;
    var ep = blocksReadUrl();
    ...
    console.info('block source      :', flat ? ('flat DOM list (' + flatRows + ' rows)')
      : (ep ? ('fetch \u2192 ' + ep) : 'NONE (add [data-nlblocks-flat] or set TA_CONFIG.ppBlocksRead)'));
  ...
  function init() {
    ...
    diagnose(root);
    // uses-legacy / nl-strategy gating is handled by Webflow conditional visibility on the root,
    // so if this script is running, the root is the correct UI for this issue.
    var g = new Grid(root);
    g.load(function () {
      g.mount();
      console.info(LOG, 'mounted', g.demo ? '(demo data)' : '(live)', '\u00b7', g.editions.length, 'editions \u00d7', g.slots.length, 'slots');
    });
    window.InbxPubPlanGrid = { version: VERSION, instance: g,
      reload: function () {
        g.editions = readEditions(root) || demoEditions();
        g.demo = !root.querySelector('[data-editions-wrapper]');
        g.load(function () { g.mount(); });
      } };
  }
