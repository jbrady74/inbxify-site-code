...
<div class="cm-root">
  <div class="cm-hdr">
    ...
    <span class="cm-badge">v1.0.6</span>
  </div>
  ...
  // ══════════════════════════════════════════════
  // RTE — EDIT ARTICLE BODY (standalone, called from any UI)
  // ══════════════════════════════════════════════
  // Usage: window.InbxEditBody('article-item-id-here', 'Article Title')
  // Fetches body from CMS via Scenario F, opens fullscreen RTE,
  // saves edited body + updates Body Status to Edited.
  async function editArticleBody(articleItemId, articleTitle) {
    ...
    // Fetch current body from CMS via Scenario F
    ...
    window._cpRteInstance = InbxRTE.openFullscreen({
      ...
      onSave: function(editedHTML) {
        // POST edited body back to CMS via Scenario F
        if (CFG.makeAssembly) {
          fetch(CFG.makeAssembly, {
            ...
            // Update DOM data attribute if visible
  ...
  // Expose editArticleBody globally so Content Library and other tools can call it
  ...
  loadFiles();
  loadFolderCounts();
  console.log('\u{1F4C2} Uploads Processor v1.0.6 mounted');
});
