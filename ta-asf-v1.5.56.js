...
(function () {
  ...
  var VERSION = '1.5.56';   // v1.5.31 — was stuck at 1.5.26 through .27-.30
  ...
  var ARTICLE_HYDRATE_MAP = {
    ...
    slug:             'slug',
    subtitle:         'sub-title',
    subTitleNA:       'sub-title-na-2',
    bannerStatement:  'banner-statement',
    bannerStatementNA:'banner-statement-na-2',
    teaser:           'post-summary',
  ...
  function hydrateArticle(articleId) {
    ...
    /* ── v1.5.56 · sentinels must be read as VISIBLE, not merely present ──
       Webflow does NOT remove a conditionally-hidden element from the
       published DOM. It leaves it and adds w-condition-invisible. So a
       bare presence check is true whether the switch is on or off.
       flagOn() requires present AND not invisible. Note this only works
       once the sentinel actually has conditional visibility bound to its
       switch in the Designer; without that binding it renders
       unconditionally and still reads true. */
    function flagOn(cls) {
      var n = el.querySelector('.' + cls + ':not(.w-condition-invisible)');
      if (!n) return false;
