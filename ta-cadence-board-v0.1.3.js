/* ════════════════════════════════════════════════════════════════
   ta-cadence-board-v0.1.3.js
   INBXIFY T-A — Cadence Board · SKELETON
   Companion: ta-cadence-board-v0.1.3.css (matched pair — load both)
...
(function () {
  ...
  /* ── Lane controls ──
     The search box is an UNCONTROLLED input: its value is read from
     the DOM on input and the lane body alone is redrawn. Re-rendering
     the whole board on each keystroke would destroy focus and caret
     position mid-word — the same rule ta-image-gen follows for its
     prompt box. */
  var SORT_OPTS = [
    { v: 'new', l: 'Newest' },
    { v: 'old', l: 'Oldest' },
  ...
  function controlsHtml(laneId) {
    ...
    var ph = (laneId === 'assets')
      ? 'Search name, customer, or month'
      : 'Search filename or month';

    var sorts = '';
    for (var i = 0; i < SORT_OPTS.length; i++) {
      var o = SORT_OPTS[i];
      sorts += '<button type="button" class="ixcb-chip' +
               (S.sort[laneId] === o.v ? ' is-on' : '') + '" ' +
    ...
    var types = '';
    if (laneId === 'assets') {
      types += '<button type="button" class="ixcb-chip' + (!S.typeFilter ? ' is-on' : '') +
               '" data-ixcb="type" data-v="">All</button>';
      for (var j = 0; j < ASSET_KEYS.length; j++) {
        var k = ASSET_KEYS[j];
        types += '<button type="button" class="ixcb-chip' +
                 (S.typeFilter === k.type ? ' is-on' : '') +
                 '" data-ixcb="type" data-v="' + k.type + '">' + esc(k.label) + '</button>';
      }
    }

    return '<div class="ixcb-ctrls">' +
             '<div class="ixcb-search">' +
               '<input type="text" class="ixcb-q' + (q ? ' has' : '') + '" ' +
             ...
             '</div>' +
             (types ? '<div class="ixcb-chips">' + types + '</div>' : '') +
             '<div class="ixcb-chips">' + sorts + '</div>' +
           '</div>';
  }
  ...
  /* Redraw one lane body only. Used by search so the input keeps
     focus and caret; a full renderAll() would replace the element the
     operator is typing into. */
  function renderLane(laneId) {
    if (!S.root) return;
    var col = S.root.querySelector('.ixcb-col--' + laneId);
    if (!col) return;
    var lane = col.querySelector('.ixcb-lane');
    if (lane) lane.innerHTML = (laneId === 'components') ? componentsBody() : assetsBody();
    var cnt = col.querySelector('.ixcb-count');
    if (cnt) cnt.textContent = colCount({ id: laneId });
    var x = col.querySelector('.ixcb-qx');
    var has = !!S.q[laneId];
    if (has && !x) {
      var box = col.querySelector('.ixcb-search');
      if (box) {
        var b = document.createElement('button');
        b.type = 'button'; b.className = 'ixcb-qx';
        b.setAttribute('data-ixcb', 'clearq'); b.setAttribute('data-lane', laneId);
        b.title = 'Clear search'; b.innerHTML = '\u00d7';
        box.appendChild(b);
      }
    } else if (!has && x) {
      x.parentNode.removeChild(x);
    }
    var inp = col.querySelector('.ixcb-q');
    if (inp) inp.classList.toggle('has', has);
  }
