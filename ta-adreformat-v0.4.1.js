...
(function () {
  ...
  function injectStyles() {
    ...
    var css = [
      ...
      '.ix-arf-tsel{font:500 12px/1 "DM Sans",sans-serif;padding:5px 6px;border:1px solid #c9c3b2;border-radius:5px;background:#fff !important;color:#1A3A3A;}',
      '.ix-arf-tnum{width:48px;font:500 12px/1 "DM Sans",sans-serif;padding:5px 6px;border:1px solid #c9c3b2;border-radius:5px;background:#fff !important;color:#1A3A3A;}',
  ...
  function composeBanner(cb) {
    ...
    // banner background = first brand color (or white)
    ...
    S.layers.forEach(function (L) {
      ...
      if (L.type === 'img' && L.canvas) {
        ...
        var g = coverGeom(L);   // container-space px
  ...
  function renderLayerToolbar() {
    ...
    if (L.type === 'img') {
      imgControls =
        ...
        '<button class="ix-arf-ltbtn" id="ix-arf-layerreset" title="Restore the original crop & position">Reset</button>';
    }

    var textControls = '';
    if (L.type === 'text') {
      var fontOpts = [["'DM Sans', sans-serif", 'DM Sans'], ["'Fraunces', serif", 'Fraunces'], ["'DM Mono', monospace", 'DM Mono']]
        .map(function (f) { return '<option value="' + esc(f[0]) + '"' + ((L.font || '') === f[0] ? ' selected' : '') + '>' + esc(f[1]) + '</option>'; }).join('');
      var tcolors = (S.result.brand_colors || []).slice(0, 4);
      var tsw = ['#1A3A3A', '#FFFFFF'].concat(tcolors).map(function (c) {
        var sel = (L.color || '').toUpperCase() === c.toUpperCase() ? ' sel' : '';
        return '<button class="ix-arf-bgsw' + sel + '" data-tc="' + esc(c) + '" title="' + esc(c) + '" style="background:' + esc(c) + ';"></button>';
      }).join('');
      textControls =
        '<span class="ix-arf-ltsep"></span>' +
        '<select id="ix-arf-tfont" class="ix-arf-tsel" title="Font">' + fontOpts + '</select>' +
        '<input type="number" id="ix-arf-tsize" class="ix-arf-tnum" min="6" max="80" value="' + (L.size || 13) + '" title="Size (px)">' +
        '<button class="ix-arf-ltbtn' + ((L.weight || 400) >= 600 ? ' on' : '') + '" id="ix-arf-tbold" title="Bold">B</button>' +
        '<span class="ix-arf-tcolors">' + tsw +
          '<label class="ix-arf-ltcustom" title="Custom text color"><input type="color" id="ix-arf-tcustom" value="' + (/^#/.test(L.color || '') ? esc(L.color) : '#1A3A3A') + '"></label>' +
        '</span>' +
        '<span class="ix-arf-talign">' +
          '<button class="ix-arf-ltbtn' + (L.align === 'left' || !L.align ? ' on' : '') + '" data-al="left" title="Left">\u2630</button>' +
          '<button class="ix-arf-ltbtn' + (L.align === 'center' ? ' on' : '') + '" data-al="center" title="Center">\u2632</button>' +
          '<button class="ix-arf-ltbtn' + (L.align === 'right' ? ' on' : '') + '" data-al="right" title="Right">\u2630</button>' +
        '</span>' +
        '<button class="ix-arf-ltbtn" id="ix-arf-torig" title="Replace this text with the original wording cropped from the ad as an image">Use original as image</button>' +
        '<button class="ix-arf-ltbtn" id="ix-arf-layerreset" title="Restore original text & style">Reset</button>';
    }
    ...
    bar.innerHTML = '<span class="ix-arf-ltlbl">Background</span>' + sw +
      ...
      (/^#/.test(L.bg || '') ? esc(L.bg) : '#ffffff') + '"></label>' + imgControls + textControls;
    ed.appendChild(bar);
    ...
    // Reset: restore the layer's original geometry + image from snapshot.
    // (Unique id ix-arf-layerreset — the staged view also has an
    // ix-arf-reset "Use a different ad" button; the duplicate id meant
    // getElementById returned the wrong node and Reset never fired. v0.3.5)
    ...
    if (tsize) tsize.addEventListener('input', function () {
      var v = parseInt(tsize.value, 10); if (v >= 6 && v <= 80) { S.layers[S.selected].size = v; renderEditor(); }
    });
    ...
    bar.querySelectorAll('.ix-arf-bgsw[data-tc]').forEach(function (b) {
      b.addEventListener('click', function () { S.layers[S.selected].color = b.dataset.tc; renderEditor(); });
    });
    ...
    // Original as image: crop the original wording from the source ad and
    // replace this text layer with an IMAGE layer (pixel-exact original). v0.4.1.
    var torig = document.getElementById('ix-arf-torig');
    if (torig) torig.addEventListener('click', function () { openCropTool(S.selected, /*asTextReplace=*/true); });
  }
