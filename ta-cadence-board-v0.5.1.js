/* ════════════════════════════════════════════════════════════════
   ta-cadence-board-v0.5.1.js
   INBXIFY T-A — Cadence Board · SKELETON
   Companion: ta-cadence-board-v0.5.1.css (matched pair — load both)

   ── v0.5.1 — CONTROLS COLLAPSE TO ICONS ──
...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  function colBody(col) {
    if (col.id === 'components') return statusChips() + '<div class="ixcb-lane">' + componentsBody() + '</div>' + trayHtml();
    ...
    try {
      window.localStorage.setItem(VIEW_KEY, JSON.stringify({
        ...
        /* q and typeFilter are deliberately NOT persisted — see the
           v0.5.1 note. A question asked last session must not silently
           filter this one. */
      }));
    } catch (e) {}
  }
