/* ============================================================================
   ta-default-layout-v1.0.2.js
   INBXIFY · T-A Studio · Default Layout tab  (Workstream K-2)

   v1.0.2 — Save is LIVE. STUB_SAVE=false. saveRoute() reads PP_WEBHOOKS.dl
            (preferred) or TA_CONFIG.makeDefaultLayout (fallback). Real write
            is no-cors POST URLSearchParams (titleAdminId + defaultLayoutJson)
            to the Default Layout Save scenario.
   v1.0.1 — mount pattern corrected. There is NO Studio sub-tab strip on the
            T-A page; tabs are native Webflow tabs (.w-tab-link[role="tab"]).
            The tab now self-mounts when its native Webflow pane becomes
            current (delegated [data-w-tab] click + initial-load check),
            matching the platform convention used by Transcriber et al.
            Mount div is #default-layout-mount (was #std-default-layout-mount).
            TAB_NAME must match the tab's data-w-tab value in Webflow.
...
(function () {
  ...
  function mount() {
    ...
    render();
    S.mounted = true;
    console.log('[tdl] Default Layout tab mounted \u00b7 v1.0.2 \u00b7 blocks:', S.blocks.length,
