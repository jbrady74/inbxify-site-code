...
(function () {
  ...
  // v0.8.7 — has this asset ever been published? A slug alone is not
  // proof: Webflow assigns one to drafts too, so buildPublishedUrl()
  // happily returns a URL for a page that 404s. Only a publish
  // timestamp means a page actually exists to frame.
  function hasBeenPublished(a) {
    if (!a) return false;
    return !!(a.lastPublished || a.publishedOn ||
              fld(a, ['publish-date', 'published-date', 'published-on']));
  }
  ...
  function renderRenderPane(url, previewUrl) {
    // v0.8.7 — article frames its REAL published page so the modal
    // tracks the template with no code edit. Only an article that has
    // never been published falls back to the standalone render, since
    // there is no page to frame yet.
    if (S.assetType === 'article') {
      if (url && previewUrl && hasBeenPublished(S.asset || {})) {
        // fall through to the iframe pane below
      } else {
        return (
          '<div class="awb-render awb-render--standalone">' +
            '<div class="awb-render-note awb-render-note--draft">' +
              'Not published yet — showing a local render. Publish the ' +
              'article to preview the real page.' +
            '</div>' +
  ...
  ensureStylesLoaded();
  log('mounted · v' + VERSION + ' · article+ad iframe pane (proxy) · event/RE standalone · draft article falls back to standalone');
})();
