...
//         article form. Payload sends newProductType (Product Library
//         Item ID) and newMajorNlSection (MNLS Item ID from
//         data-collection-id). readProducts() live DOM reader added.
// ============================================================
...
<div class="cm-root">
  <div class="cm-hdr">
    ...
    <span class="cm-badge">v2.3.0</span>
  </div>
  ...
  function renderFileList() {
    ...
    el.innerHTML = vis.map(f => {
      ...
      const badge = (f.assigned || f.status==='ASSIGNED') ? 'assigned' : f.status==='AWAITING_DECISION' ? 'decision' : 'ready';
      const label = (f.assigned || f.status==='ASSIGNED') ? 'Assigned' : f.status==='AWAITING_DECISION' ? 'Needs Decision' : 'Ready';
      ...
      return `<div class="cm-frow ${f.selected?'sel':''} ${isAssigned?'assigned':''}" data-cm-fid="${f.id}">
        <div class="cm-frow-main">
          <input type="checkbox" class="cm-fcheck" ${f.selected?'checked':''} ${isAssigned?'disabled':''}>
  ...
  function getSelected() { return FILES.filter(f => f.selected && !f.assigned && f.status!=='ASSIGNED'); }
  ...
  // ── Init ──
  loadFiles('DONE');
  console.log('\u{1F4C2} Content Manager v2.3.0 mounted');
});
