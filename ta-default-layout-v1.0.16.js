 /* ============================================================================
   ta-default-layout-v1.0.16.js
   INBXIFY · T-A Studio · Default Layout tab  (Workstream K-2)

   v1.0.15 — K9 REVERSED: Photo Essay flag/switch REMOVED entirely (markup, handler,
            data keys, PHOTO_ESSAY_TYPES constant). Workstream I infers multi-photo
            from photo count downstream. JSON no longer carries photoEssay.
   v1.0.14 — Header title now TRULY centered (equal-thirds flex: No. left-justified,
            Title centered, Date right-justified). Pairs w/ CSS v1.0.14 (sticky
            side columns + cream newsletter border).
   v1.0.13 — fixed Header corrections: gold = #f4a127 (the title's header gold),
            HORIZONTAL row (No. | Title | Date), child divs forced transparent
            !important (page rule was painting white boxes), tooltip line removed.
   v1.0.12 — fixed gold Header at slot #1 (initial; restyled in v1.0.13).
   v1.0.11 — masthead background + color now carry !important inline. The div
            was rendering at correct size/position (confirmed via DOM) but a
            page-level !important rule in one of the ~14 loaded stylesheets was
            suppressing its background/text. Inline !important wins the tie.
   v1.0.10 — masthead styles moved INLINE onto the div (JS/CSS version drift kept
            the .tdl-fixed-mast CSS rule off the page → div invisible). Now self-
            contained: renders regardless of which CSS version the head links.
            Same spec: "Wyckoff Living Now", 65% width, 80px, #142e2e, white
            Fraunces, top corners 5px. HARDCODED (HC-TDL-MAST).
   v1.0.9 — fixed HARDCODED masthead band (HC-TDL-MAST) at top of center column:
            "Wyckoff Living Now", 65% width, 80px, #142e2e, white Fraunces, top
            corners rounded 5px. CMS color/logo binding never resolved on the
            page; this is a hardcoded stopgap (make per-title later). The old
            CMS banner removed. titleColor still drives tile accent if present.
   v1.0.8 — branded section banner replaces the logo masthead (logo Link field
            wouldn't bind cleanly). A primary-color band (TA_CONFIG.titleColor)
            carries the title name + "Newsletter Layout" label — signals the
            workspace. Color still drives --tdl-brand for tile accents. Logo
            accessor retained but unused.
   v1.0.7 — per-title branding (logo masthead + brand color). Superseded banner.
   v1.0.6 — Local Weather is now a SELECTABLE SINGLETON BLOCK (reverses K10
            "weather = switch"). Added to palette (icon WX), removed from the
            Default Switches panel (now empty → hidden). At most one allowed:
            palette item disables while present, add/drop/dup all guarded.
            Legacy switches.localWeather:true migrates to a Local Weather block
            on load. NOTE: strip POSITION in the rendered newsletter still needs
            a Scenario 106 rebuild (WEATHER-AS-BLOCK spec) — this is the UI half.
   ...
   PRODUCTION RECONCILIATIONS (locks confirmed 2026-06-08) — enforced in CODE:
     ...
     • K9   REVERSED 2026-06-08 — Photo Essay flag DROPPED entirely. Workstream I
            NOT a block type. No PE palette item. Surfaced as a toggle on each
            infers the multi-photo div from photo count; no operator flag, no key.
...
(function () {
  ...
  // The authorable block types. Order = palette order. icon: short code shown
  // in the palette chip. gold: banner-ad family styling.
  // NOTE (v1.0.6): Local Weather is now a SELECTABLE BLOCK (singleton), reversing
  // the K10 "weather = switch" decision. UI authoring is live here; the actual
  // strip POSITION in the rendered newsletter still depends on a Scenario 106
  // rebuild (see WEATHER-AS-BLOCK spec handed off). Until that ships, this block
  // records position in the JSON but the legacy render still uses its fixed spot.
  var PALETTE = [
    ...
    { type: 'Local Weather',       icon: 'WX',  gold: false }
  ];
  var VALID_TYPES = PALETTE.map(function (p) { return p.type; });

  // Singleton block types — at most one may exist in the layout. The palette
  // item disables while one is present; add/drop is guarded too.
  var SINGLETON_TYPES = { 'Local Weather': true };

  // Types explicitly NOT allowed in the Default Layout (stripped on load + never
  // offered): Title Masthead (K11), Splash Ad (code-only). Photo Essay dropped
  // entirely per K9 reversal (2026-06-08) — Workstream I infers the multi-photo
  // div from photo count; no operator flag.
  var STRIP_TYPES = {
    'Title Masthead': true, 'Splash Ad': true, 'Splash': true,
    'Photo Essay': true
  };

  // Default switches panel — now EMPTY (Local Weather graduated to a block in
  // v1.0.6). Framework retained for future non-block toggles; panel hidden when
  // empty.
  var SWITCHES = [];
  ...
  var S = {
    mounted: false,
    root: null,
    blocks: [],        // [{type}] in render order (position is implicit)
  ...
  // Per-title brand from the TITLE collection (multi-tenant). Empty-safe.
  function titleLogoUrl() { return (window.TA_CONFIG && window.TA_CONFIG.titleLogoUrl) || ''; }
  function titleColor()   {
    var c = (window.TA_CONFIG && window.TA_CONFIG.titleColor) || '';
    c = c.trim();
    if (!c) return '';
    if (c[0] !== '#') c = '#' + c;            // tolerate "142e2e" or "#142e2e"
  ...
  function hydrateFromStored() {
    ...
    // MIGRATION (v1.0.6): legacy layouts stored weather as switches.localWeather.
    // If a legacy true is present and no Local Weather block already exists,
    // append one so existing layouts keep their weather. Append at end (its
    // legacy render position was fixed anyway; operator can reorder).
    var hasWeatherBlock = S.blocks.some(function (b) { return b.type === 'Local Weather'; });
    if (parsed && parsed.switches && parsed.switches.localWeather && !hasWeatherBlock) {
      S.blocks.push({ type: 'Local Weather' });
    }
  ...
  function blockExists(type) { return S.blocks.some(function (b) { return b.type === type; }); }

  function render() {
    if (!S.root) return;
    var paletteRows = PALETTE.map(function (p) {
      var disabled = SINGLETON_TYPES[p.type] && blockExists(p.type);
      return '<div class="tdl-pl-item' + (disabled ? ' tdl-pl-item--disabled' : '') + '"' +
               (disabled ? '' : ' draggable="true"') +
               ' data-add="' + esc(p.type) + '"' +
               (disabled ? ' aria-disabled="true" title="Already in layout — only one allowed"' : '') + '>' +
    ...
    // apply the title's brand color as a CSS variable on the root so the banner
    // + tile accents pick it up; falls back to corporate teal if unset.
    var brand = titleColor();
    if (brand) S.root.style.setProperty('--tdl-brand', brand);
    else S.root.style.removeProperty('--tdl-brand');

    S.root.innerHTML =

      // v1.0.16: page head now emitted by the shared component
      //   (ix-header-v1.0.1.js). Was a bespoke .tdl-pagehead copy with
      //   its own icon/title/sub markup. Subtitle (live TITLE name) is
      //   handled inside IxHeader. The gold in-content banner
      //   (.tdl-fixed-hdr) is NOT part of this and is untouched.
      ((window.IxHeader && IxHeader.render)
        ? IxHeader.render({ icon: '\uD83D\uDCD0', title: 'Default Newsletter Layout' })
        : '<div class="ix-hdr"><div class="ix-hdr-left">' +
          '<div class="ix-hdr-icon">\uD83D\uDCD0</div>' +
          '<div><h3>Default Newsletter Layout</h3>' +
          '<div class="ix-hdr-sub">' + esc(titleName() || '') + '</div></div>' +
          '</div><div class="ix-hdr-right"></div></div>'
      ) +
      ...
      '<div class="tdl-grid">' +
        ...
        '<div class="tdl-stack-wrap">' +
          ...
          (S.legacyMasthead
            ...
            : '') +
          // HARDCODED masthead band (HC-TDL-MAST): fixed WLN-specific banner.
          // Styles are INLINE so it renders regardless of which CSS version the
          // head links (JS/CSS version drift kept hiding it). 65% width, 80px,
          // #142e2e, white Fraunces, top corners 5px.
          '<div class="tdl-fixed-mast" style="width:65%;height:80px;margin:0 auto 12px;background:#142e2e !important;color:#fff !important;border-radius:5px 5px 0 0;display:flex;align-items:center;justify-content:center;font-family:\'Fraunces\',Georgia,serif;font-weight:600;font-size:26px;letter-spacing:-.01em;">Wyckoff Living Now</div>' +
          // HARDCODED fixed Header (HC-TDL-HDR): always slot #1, non-draggable,
          // non-deletable chrome (like the masthead) — NOT a palette block.
          // Horizontal row: No. | Title | Date. Inline !important throughout so
          // the page's stylesheets can't override (forces transparent children
          // to kill the page's white-box rule). Placeholder copy is fixed text.
          '<div class="tdl-fixed-hdr" style="width:65%;margin:-12px auto 14px;padding:16px 26px;background:#f4a127 !important;color:#1A1A1A !important;border-radius:0 0 5px 5px;display:flex;flex-direction:row;align-items:center;gap:18px;font-family:\'Fraunces\',Georgia,serif;">' +
            '<div style="background:transparent !important;flex:1;text-align:left;font-weight:700;font-size:14px;letter-spacing:.06em;text-transform:uppercase;white-space:nowrap;">No. 000</div>' +
            '<div style="background:transparent !important;flex:1;text-align:center;font-weight:600;font-size:22px;line-height:1.1;white-space:nowrap;">Sample Title</div>' +
            '<div style="background:transparent !important;flex:1;text-align:right;font-size:14px;white-space:nowrap;">January 1, 2005</div>' +
          '</div>' +
        ...
        '<aside class="tdl-rail">' +
          ...
          '</div>' +
          (SWITCHES.length
            ? '<div class="tdl-switches"><h4>Default Switches</h4>' + switchRows + '</div>'
            : '') +
        '</aside>' +
      '</div>' +
  ...
  function wireStack() {
    ...
    $$('[data-tdl-stack] .tdl-act').forEach(function (b) {
      b.addEventListener('click', function (e) {
        ...
        if (act === 'dup') {
          var dupType = S.blocks[idx].type;
          if (singletonBlocked(dupType)) return;   // can't duplicate a singleton
          var copy = { type: dupType };
          S.blocks.splice(idx + 1, 0, copy);
          renderStack(); recomputeDirty();
          toast('Duplicated <b>' + esc(dupType) + '</b>.');
        } else if (act === 'del') {
          var t = S.blocks[idx].type;
          S.blocks.splice(idx, 1);
          // full render if singleton (palette item must re-enable), else stack only
          if (SINGLETON_TYPES[t]) render(); else renderStack();
          recomputeDirty();
  ...
  // Reject a second instance of a singleton type (Local Weather). Returns true
  // if the add should be blocked.
  function singletonBlocked(type) {
    if (SINGLETON_TYPES[type] && blockExists(type)) {
      toast('<b>' + esc(type) + '</b> can only be added once.', true);
      return true;
    }
    return false;
  }
  ...
  function handleDrop(e, insertAt) {
    ...
    if (addType) {
      ...
      if (singletonBlocked(addType)) return;
      var nb = { type: addType };
      S.blocks.splice(insertAt, 0, nb);
      // full render if singleton (palette disabled-state must update), else stack only
      if (SINGLETON_TYPES[addType]) render(); else renderStack();
      recomputeDirty();
  ...
  function addBlock(type) {
    ...
    if (singletonBlocked(type)) return;
    var nb = { type: type };
    S.blocks.push(nb);
    if (SINGLETON_TYPES[type]) render(); else renderStack();
    recomputeDirty();
  ...
  function mount() {
    ...
    render();
    S.mounted = true;
    console.log('[tdl] Default Layout tab mounted \u00b7 v1.0.15 \u00b7 blocks:', S.blocks.length,
