// ============================================================
// ta-studio-v1.4.11.js
//
// v1.4.10 (T-A header-row standardization — Row 1 of 3):
//   • .ix-hdr-sub now renders the live TITLE name (e.g. "Wyckoff
//     Living NOW") instead of static "Pick an Article → …" copy.
//     Source: .pubplan-slot-wrapper[data-titleadmin-name], the
//     same already-Webflow-bound node PubPlan reads. Falls back
//     to the old copy if that node hasn't painted.
//   • Version badge REMOVED from the .ix-hdr row. It was also a
//     bug: hardcoded 'v1.4.1' on a v1.4.9 file, eight patches
//     stale. Version now renders in the ⓘ info popover from a
//     new STD_VERSION constant — one edit point, can't drift.
//     This also frees .ix-hdr-right for a per-tab "+ Add a …"
//     button on tabs that need one (Studio does not).
//   Pairs with title-admin-page-design v1.4.16 (Row 2: cream
//   rail + Fraunces on .std-subtabs) and ta-intake-manager
//   v3.0.4 (Row 3: tint + DM Mono pill tabs on .bdl-l1).
//   No behavior outside the header row touched.
//
// v1.4.9 (Intake = default subtab — Jeff, May 26):
//   The Studio tab is now the page default, so its first paint
//   should be the intake workspace, not Components. Three
//   touchpoints, no logic changes:
//     1. S.panel initial value 'components' → 'input'.
//     2. Subtab strip: the `active` class moves from the
//        Components button to the Intake button.
//     3. Panel bodies: the `active` class moves from the
//        components panel div to the input panel div.
//   Init-order verified safe: ta-intake-manager's installMount
//   retries until the panel exists and calls fetchAndRender()
//   itself when the panel is active at mount; activation is
//   MutationObserver-based (no dispatch race). Components now
//   mounts on first click via std:panel:components, unchanged.
//
// v1.4.8 (Ad Reformat / Generate split):
//   - The "Generate" tab (panel key `generator`) RELABELED to
//     "Ad Reformat". Key unchanged, so the Reformatter that
//     self-mounts into #std-generator-mount is untouched.
//   - NEW top-level tab "Generate" (panel key `adgen`): panel body
//     with #std-adgen-mount; setPanel dispatches std:panel:adgen on
//     activation. Houses the from-scratch ad creator (mounts itself,
//     same pattern as Converter/Reformatter). Info-popover entry added.
//   Tab strip order: Intake · Components · Assemble · Transcribe ·
//   Convert · Ad Reformat · Generate · Create.
//
// ta-studio-v1.4.7.js
//
// v1.4.7 (tab-label rename — display only):
//   Subtab labels switched to natural verb forms:
//     Assembler → Assemble, Transcriber → Transcribe,
//     Converter → Convert, Generator → Generate.
//   Components + Intake unchanged. ONLY the visible <span> text
//   (subtab strip) and the info-popover `title:` fields changed;
//   every data-std-panel key, dispatch event, mount id, and route
//   is untouched — so nothing downstream (Converter tools, the
//   Generator/Reformatter mount, dispatch listeners) is affected.
//   Also corrected the runtime mount-log version stamp, which had
//   lagged at v1.4.3 through v1.4.6.
//
// ta-studio-v1.4.6.js
//
// v1.4.6 (remove Channels strip — Intake panel now empty shell):
//   1. Channels strip (Drive/Email/Form pills) removed from the
//      Intake panel. ta-intake-manager owns all rendering now.
//
// v1.4.5 (Intake rename + monitor block removal):
//   1. Inputs tab RENAMED to "Intake" (display label only — the
//      internal panel key stays 'input' so data-std-panel
//      selectors, panel mounting, and GA history are unaffected).
//      Changed: tab button <span>, TAB_INFO.input.title/desc.
//   2. DELETED the entire Inputs/Intake monitor block — Section 2
//      (Drive Folder Health count badges + Scenario C note) AND
//      Section 3 ("Monitor only" / S14 parallel-run banner). The
//      counts were not reliably monitoring anything and the banner
//      pointed file management at the old Components/Uploads path,
//      which now contradicts the management UI in ta-bundles. Only
//      the Channels strip (Section 1) remains in the panel chrome.
//      loadInputsFolderCounts / Scenario C call left in place but
//      no longer invoked from renderInputsPanel (dead-path safe;
//      removed call site only). See HC note in changelog.
//   3. No change to bundle/loose-file list rendering — that lives
//      in ta-bundles (v1.3.0+).
//
// ── prior history ──
// ta-studio-v1.4.3.js
// INBXIFY TA Studio — Phase 4 / S14 Step 2 (Inputs monitor panel)
// PDF triage banner + modal at Studio chrome level.
// Inputs tab: monitor-only surface (channel pills, Drive folder
// health, sub-tab count badges). No file list. Default tab = Components.
//
// Mounts into #studio-mount on the T-A page.
//
// ──────────────────────────────────────────────────────────
// v1.4.3 (ⓘ info popover — lands on ALL content panels):
//   FIX. In v1.4.2 the heading ⓘ only appeared on Components and
//   Assembler. Two root causes: (1) attachHeadingInfo's heading
//   selector matched only .cmp-title / .asm-title — it missed
//   Converter's .cm-conv-title, Generator's .std-placeholder-title,
//   and Transcriber's .sct-transcribed / .sct-generated h2s;
//   (2) sibling-owned panels (Components / Transcriber / Converter)
//   mount their heading ASYNCHRONOUSLY after the activation event,
//   so the old 12×60ms (720ms) poll lost the race on slow mounts.
//   Now: expanded INFO_HEADING_SELECTOR covers every panel's real
//   heading class, AND a per-panel MutationObserver attaches the ⓘ
//   whenever the heading paints — however late — then self-
//   disconnects (10s safety stop). INFO_SKIP = { input, create }:
//   Inputs renders no heading element (renderInputsPanel jumps
//   straight to .inp-section), and Create is an action launcher,
//   not a [data-std-panel-body] panel — both intentionally skipped.
//   The init-path attachHeadingInfo('assembler') call is retained
//   (Assembler-first load bypasses setPanel). Version stamps
//   (filename, mount log) normalized to v1.4.3.
//
// ──────────────────────────────────────────────────────────
// v1.4.2 (ⓘ info popover on panel headings — initial build):
//   Added a superscript ⓘ to Studio panel headings, opening a
//   popover with the tab description + the Make scenarios that tab
//   fires (TAB_INFO registry: input / components / assembler /
//   transcriber / converter / generator). injectInfoStyles() +
//   openInfoPopover() + attachHeadingInfo(). KNOWN LIMITATION
//   (fixed in v1.4.3): injection only reliably landed on
//   Components / Assembler due to selector + async-mount-timing
//   gaps. Create launcher unchanged from v1.4.1.
//
// ──────────────────────────────────────────────────────────
// v1.4.1 (Create dropdown — all four asset types wired):
//   The Create menu's Ad / RE Listing / Event items were gated with
//   a "SOON" tag and routed to a "coming soon" toast — only Article
//   opened the ASF. Now that ta-asf-v1.2.3 ships all four variants,
//   every item dispatches window.InbxASF.open({ mode:'create',
//   assetType }). handleCreateType maps the menu key to the ASF
//   assetType — note re → 'realestate' (the ASF rejects bare 're').
//   SOON tags removed; the three items now render active like Article.
//   Version stamps (filename, UI badge, mount log, ASF-launch log
//   prefixes) normalized to v1.4.1.
//
// ──────────────────────────────────────────────────────────
// v1.3.9 (Create dropdown position fix):
//   The dropdown was anchored to the right edge of .std-tabs-wrap
//   via inline right:14px — but on actual layout the wrap container
//   spans full width, so the menu rendered far right of the page
//   instead of below the Create tab. Fix: drop the inline right/top
//   offsets, compute position from the Create button's bounding
//   rect at click time and set absolute top/left dynamically.
//
// ──────────────────────────────────────────────────────────
// v1.3.8 (Create tab — ASF launcher):
//   New 7th tab "Create" on the right side of the strip, visually
//   distinct (gold pill). Clicking the tab does NOT switch panels —
//   instead it reveals a small dropdown menu with four options:
//     • Article  → window.InbxASF.open({ mode:'create', assetType:'article' })
//     • Ad       → window.InbxASF.open({ mode:'create', assetType:'ad' })
//     • RE       → window.InbxASF.open({ mode:'create', assetType:'realestate' })
//     • Event    → window.InbxASF.open({ mode:'create', assetType:'event' })
//   (All four wired in v1.4.1; previously Ad/RE/Event were "coming soon".)
//   Clicking outside the dropdown dismisses it. Clicking the tab
//   again toggles. ASF must be loaded on the page for Article to
//   work; if window.InbxASF isn't defined, a toast surfaces.
//
//   Hyperlink-existing-articles-to-readonly-ASF ships separately
//   (Studio v1.3.9 + ASF v1.0.11 — TD-180).
//
// ──────────────────────────────────────────────────────────
// v1.3.6 (Item #3 — Save-to-CMS redirect to launching sub-tab):
//
//   Problem: ABE save via Studio Assembler triggered a reload but
//   the post-reload restore intermittently landed the user on the
//   PubPlan outer tab instead of Studio>Assembler. Separately, ABE
//   save via the Transcriber (UP-owned legacy surface) never reloaded
//   at all — and worse, was hitting Scenario F's updateArticleBody
//   route which doesn't write Path B's post-body-html field.
//
//   Fix (this version):
//
//   1. openBodyEditor() accepts an optional opts.returnPanel string.
//      Passes it through to studioReloadAfterCMSWrite so the restore
//      lands on the launching panel (Assembler reads S.panel which
//      is 'assembler' at click time; future launchers pass their own).
//
//   2. studioReloadAfterCMSWrite() accepts opts.returnPanel which
//      overrides what gets written to STUDIO_RESTORE_KEY.panel.
//
//   3. STUDIO_RESTORE_KEY payload gains explicit outerTab: 'studio'.
//      Future surfaces (PubPlan, Content Library) that adopt this
//      reload-after-write pattern read outerTab to decide which
//      outer tab to activate. Currently unused on read path
//      (always Studio for now).
//
//   4. studioActivateOuterTab() rewritten as a retry-poller. Tries
//      every 50ms up to 1500ms, bails on first successful click.
//      Kills the body.js race where the .tab-button proxy click
//      ran before body.js wired the handler post-reload.
//
//   5. Stale-payload guard bumped 30s → 120s. Reload-after-save
//      should never exceed 30s, but the cost of bumping is zero
//      and false-discards silently bring back the bug.
//
//   6. Export window.InbxStudioBodyEditor = openBodyEditor so the
//      UP-owned Transcriber surface (and future cross-IIFE callers)
//      can route through Studio's single save path. UP v1.0.12
//      delegates editArticleBody() through this export, killing the
//      Scenario F updateArticleBody call. Result: ONE body-save path
//      for both Assembler and Transcriber. Path B's post-body-html
//      now writes for Transcriber-edited articles automatically.
//
//   Companion change: uploads-processor v1.0.11 → v1.0.12.
//   No Make changes in this delivery. Scenario F's updateArticleBody
//   route stays installed but goes silent — can be deleted later.
//
//   TD-176 (MEDIA detach gap) NOT addressed here — still deferred.
//
// ──────────────────────────────────────────────────────────
// v1.3.5 (S14 Step 2 — Inputs monitor panel):
//
//   1. DEFAULT TAB flips: assembler → components.
//      Rationale: publisher should triage Components (keep the
//      folder empty for next upload batch) before assembling.
//      S.panel initial value, tab strip .active class, and init
//      logic all updated. Session-restore continues to override
//      this default when a CMS-write reload is in progress.
//
//   2. INPUTS TAB — MONITOR-ONLY SURFACE:
//      Replaces the S13 placeholder with a live monitoring panel.
//      Three sections:
//        a. Channel pill strip (Drive, Email, Form) — identical
//           visual pattern to UP's channel strip. Pills are
//           informational-only at this stage (no sub-tab switch).
//        b. Drive folder health widget — four sub-tab badges
//           (Ready, Assigned, Archived, Hidden) showing file
//           counts from scenarioCUrl?action=folderCounts.
//           Loads on first Inputs tab activation, not on mount,
//           to avoid extra Make ops when the publisher isn't on
//           this tab. Manual ↻ refresh button (same pattern as
//           PDF triage banner).
//        c. Monitoring note: explains this is a read-only view;
//           full intake management (assignment modal, file list)
//           lives in Components and UP during parallel-run.
//      No file list rendered here. Decision logged in close-out.
//
//   3. GA4 TD-167: fires 'legacy_uploads_processor_open' event
//      on FIRST Inputs tab activation per page load. One-shot
//      flag INP_GA4_FIRED prevents re-firing on tab revisit.
//      Uses window.gtag if present; silently skips if not.
//      Rationale: this telemetry measures how many publishers
//      are still reaching for the UP entry point after the
//      Studio Inputs tab exists. Fires here because Inputs is
//      where that traffic will route once S14 Step 2 ships.
//
//   4. DRIVE FOLDER COUNTS for Inputs panel read from
//      scenarioCUrl?action=folderCounts (same endpoint UP uses
//      for its own folder count polling). Reads: uploadFolder,
//      preprocessing, ready, assigned, archived, hidden keys
//      from the response. STD_MONITOR extended with ready,
//      assigned, archived, hidden fields. The existing
//      STD_MONITOR.preprocessing field (used by the PDF triage
//      banner) is NOT touched.
//
//   5. UX RULES (platform-wide, enforced here):
//      - Gold border (#C4A35A) on any select/dropdown holding a
//        pending (non-default) selection: class .ix-select--dirty.
//      - Cancel text link: appears alongside any edit-initiated
//        state; reverts to original. Pattern already established
//        in Assembler + Components; documented here as the
//        platform rule for all future surfaces.
//      - Inputs monitor has no editable selects in v1.3.5 (read-
//        only surface), so the gold-border rule has no immediate
//        application here. Logged for S14 Step 2 full intake.
//
//   6. OPEN QUESTION — Cloudflare DOCX conversion interface:
//      The Converter tab uses docx-to-html.jeff-2cd.workers.dev.
//      The interface contract (POST body shape, response shape,
//      error handling, auth header if any) has not been finalized.
//      This is not blocking v1.3.5 — Converter tab is unchanged.
//      Must be resolved before any Converter enhancement work.
//      Logged as OQ-001 in the hardcoding log artifact.
//
//   No CMS schema changes. No Make scenario changes. No changes
//   to Components, Assembler, Transcriber, Converter, or Generator.
//   Only additive: Inputs panel build + default-tab flip.
//
//   Webflow head deploy:
//     SWAP: ta-studio-v1.3.4.js → ta-studio-v1.3.5.js
//     All other files unchanged.
//
// ──────────────────────────────────────────────────────────
// v1.3.4 (S14 Step 1 minor bump — filename only):
//   No changelog logged; file is functionally identical to
//   v1.3.3. Filename incremented for deploy tracking.
//
// ──────────────────────────────────────────────────────────
// v1.3.3 (S14 Step 1 follow-up — manual banner refresh):
//
//   1. Banner gains a ↻ refresh button between the count
//      text and the Triage button. Click re-fires
//      loadStudioPdfCount() to update the count without
//      a page reload.
//
//   2. Why manual instead of auto-poll: a 30s timer would
//      add background traffic for every publisher whose
//      Studio is open. Manual refresh keeps the publisher
//      in control — they refresh when they expect a new
//      PDF to have landed (e.g. they just dropped one in
//      Drive). No background polling = no extra Make ops.
//
//   3. UX: while the refresh fetch is in-flight, the
//      banner gets a .is-refreshing class. The ↻ icon
//      animates (slow 1s spin) until the fetch resolves.
//      No layout change; same compact pill height.
//
//   4. Implementation: T_REFRESHING flag, click handler
//      on [data-std-triage-refresh], and a thin wrapper
//      around loadStudioPdfCount() that toggles the flag.
//      No new endpoints. No CSS module changes — the
//      .is-refreshing rule is small enough to live inside
//      title-admin-page-design or be added to ix-modals
//      v1.0.2 later. For now, inline animation via
//      style attribute on the icon span.
//
//   No CMS schema changes. No Make scenario changes.
//   No ix-modals CSS changes (still v1.0.1). Modal UX,
//   queue logic — all unchanged from v1.3.2.
//
// ──────────────────────────────────────────────────────────
// v1.3.2 (S14 Step 1 follow-up — banner/modal source-of-truth):
//
//   1. Banner count now reads from scenarioPdfQueueUrl (the
//      same Sheet-backed webhook the modal already calls),
//      replacing the prior dependency on
//      scenarioCUrl?action=folderCounts (Drive folder count).
//
//   2. Why: in v1.3.1 the banner counted Drive folder files
//      while the modal counted Sheet rows. When Scenario B
//      hadn't yet written a Sheet row for a Drive file, banner
//      said 3 but modal showed 1. The user saw two
//      contradictory truths.
//
//      v1.3.2 collapses both surfaces to the same source.
//      They can never disagree. If the Sheet has 3 rows, both
//      banner and modal say 3.
//
//   3. Trade-off (logged TD-175): the banner no longer
//      surfaces orphaned PDFs that landed in Drive but never
//      got conditioned. If Scenario B fails on a file, that
//      file sits in Drive un-triaged forever and the banner
//      never alerts on it. Reconciliation work belongs in
//      S14 Step 2 (Inputs migration) where the broader intake
//      audit happens.
//
//   4. Implementation: loadStudioFolderCounts() renamed to
//      loadStudioPdfCount(). One fetch instead of two.
//      Endpoint flips from scenarioCUrl to scenarioPdfQueueUrl.
//      Response shape: array (or {files:[…]}) — count =
//      response.length. STD_MONITOR.preprocessing still
//      populated with the count so the existing render
//      pipeline is unchanged.
//
//      STD_MONITOR.uploadFolder removed — was unused by
//      Studio (UP still tracks its own MONITOR.uploadFolder
//      via its own polling, unaffected).
//
//   5. UP polling unchanged. uploads-processor v1.0.11 keeps
//      reading the folderCounts endpoint for its own banner
//      (TD-174 — duplicate code resolves at S15). Studio and
//      UP remain decoupled.
//
//   No CMS schema changes. No Make scenario changes. No
//   ix-modals CSS changes (still v1.0.1). Modal UX, queue
//   logic, success states — all unchanged from v1.3.1.
//
// ──────────────────────────────────────────────────────────
// v1.3.1 (S14 Step 1 — banner-as-attention primitive):
//
//   1. PDF triage banner mounts in Studio chrome header row,
//      right-justified beside the title block. Visible across
//      all 6 Studio tabs whenever STD_MONITOR.preprocessing > 0.
//      Uses ix-modal-banner--compact variant from
//      ix-modals-v1.0.1.css (loads alongside this file).
//
//   2. Triage modal lifted from uploads-processor v1.0.11.
//      Same per-row dropdown UX (Convert / Transcribe /
//      Archive / Hide), same batch-save model, same sequential
//      apply queue (Convert → Archive → Hide → Transcribe).
//      Modal mounts to <body> as #ix-triage-modal-root and
//      uses ix-overlay/ix-modal/ix-modal-row primitives.
//
//   3. Folder-counts polling lives here now. Studio fetches
//      CFG.scenarioCUrl?action=folderCounts on mount, then
//      again after every triage queue completion. UP-side
//      polling (uploads-processor v1.0.11) continues to fire
//      independently — same endpoint, idempotent. The two
//      banners never coexist visually (UP banner is inside
//      Content Processor tab, Studio banner is in Studio
//      chrome). Duplicate triage code in UP is logged as
//      TD-174 — resolves at S15 UP decommission.
//
//   4. Banner gating: STD_MONITOR.preprocessing > 0. Pulse
//      animation (.is-fresh class) runs once on count climb
//      from 0 → >0; steady-state >0 doesn't pulse. After the
//      user triages and count returns to 0, the next time a
//      PDF arrives, the pulse fires once again.
//
//   5. Transcribe action from inside the modal: closes modal
//      first (clean exit), then surfaces a notice on whatever
//      surface the Transcriber lives on. Locked decision —
//      Transcribe runs LAST in the queue order so the modal
//      close-then-context-switch is the cleanest UX.
//
//   6. New CFG getter: CFG.scenarioPdfQueueUrl. Same key as
//      uploads-processor v1.0.11 reads — both surfaces share
//      the same Make webhook endpoint.
//
//   7. Architectural primitives (Master Roadmap v1.6.1 §10):
//      - Banner-as-attention (this ship establishes the
//        pattern; Vision §10.1 documents forward composition)
//      - Tenant-config-via-Data-Store (consumed via the Make
//        scenario behind scenarioPdfQueueUrl, which resolves
//        per-tenant Sheet IDs through Data Store record 83122)
//
//   No CMS schema changes. No changes to Components delegation
//   (still dispatches std:panel:components — the v1.3.0
//   architecture stays intact). No changes to Assembler or any
//   other tab. Only additive: new banner mount in header, new
//   modal infrastructure, new polling.
//
// ──────────────────────────────────────────────────────────
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // ── v1.4.10: single source of truth for the version string.
    //    Previously the header badge hardcoded 'v1.4.1' and drifted
    //    eight patches out of date. Bump THIS on every version bump;
    //    the ⓘ popover reads it. ──
    var STD_VERSION = '1.4.11';

    // ── v1.4.10: live TITLE name for the .ix-hdr subheader.
    //    Reads the same already-Webflow-bound hidden element PubPlan
    //    uses. Code Embeds cannot resolve {{wf}} tokens themselves,
    //    so reading the bound DOM node is the proven path — do not
    //    replace this with a direct {{wf}} interpolation.
    //    Falls back to the original static copy so the header is
    //    never blank if the slot wrapper hasn't painted. ──
    function getTitleName() {
      var el = document.querySelector('.pubplan-slot-wrapper[data-titleadmin-name]');
      var name = el ? (el.dataset.titleadminName || '') : '';
      return name || 'Pick an Article \u2192 edit inline \u2192 save to Webflow';
    }
    ...
    // ── State ──
    var S = {
      panel: 'input',         // input | components | assembler — v1.4.9: default flipped components→input (Intake first)
    ...
    // ── Shell markup ──
    // v1.4.10 (Row 1 standardization):
    //   • .ix-hdr-sub now shows the live TITLE name instead of the
    //     static "Pick an Article → …" copy. Source is the same DOM
    //     attribute PubPlan reads (.pubplan-slot-wrapper
    //     [data-titleadmin-name]) — Code Embeds can't resolve {{wf}}
    //     tokens directly, so reading the already-bound hidden element
    //     is the proven path (see HC-TDL-MAST/HDR). Falls back to the
    //     old copy if the element isn't present.
    //   • The <span class="ix-badge">v1.4.1</span> is GONE from the
    //     header row — it was also stale (hardcoded 1.4.1 on a 1.4.9
    //     file). Version now renders inside the ⓘ info popover, read
    //     from the STD_VERSION constant so it can't drift again. This
    //     frees ix-hdr-right for a per-tab "+ Add a …" action button
    //     where a tab needs one (Studio does not).
    mount.innerHTML =
      '<div class="std-root">' +
        // v1.4.11: header now emitted by the shared component
        //   (ix-header-v1.0.1.js). Was a hand-written copy of the same
        //   markup — a second source of truth that would drift from
        //   every other tab. Subtitle (live TITLE name) is handled
        //   inside IxHeader; local getTitleName() is now only a
        //   fallback. Triage banner slot passes through as an action
        //   so renderTriageBanner() still finds #std-triage-banner.
        ((window.IxHeader && IxHeader.render)
          ? IxHeader.render({
              icon:    '\u29BE',
              title:   'Studio',
              actions: ['<div id="std-triage-banner" class="std-triage-banner-slot"></div>']
            })
          : '<div class="ix-hdr"><div class="ix-hdr-left">' +
            '<div class="ix-hdr-icon">\u29BE</div>' +
            '<div><h3>Studio</h3><div class="ix-hdr-sub">' + esc(getTitleName()) + '</div></div>' +
            '</div><div class="ix-hdr-right">' +
            '<div id="std-triage-banner" class="std-triage-banner-slot"></div>' +
            '</div></div>'
        ) +
        ...
        // ix-buttons v1.0.1/1.0.2). Default active = 'components'
        // (v1.3.5 flip: was 'assembler'). Text-only labels
        // — icons removed at the strip level (Q-G design).
        // v1.3.8: wrapped in position:relative container so the
        // Create dropdown (.std-create-menu) anchors below the strip.
        '<div class="std-tabs-wrap" style="position:relative;">' +
        '<div class="std-subtabs">' +
          '<button class="ix-btn ix-btn--tab active" data-std-panel="input">' +   // v1.4.9 default
            '<span>Intake</span>' +
          '</button>' +
          '<button class="ix-btn ix-btn--tab" data-std-panel="components">' +
            '<span>Components</span>' +
          '</button>' +
          '<button class="ix-btn ix-btn--tab" data-std-panel="assembler">' +
            '<span>Assemble</span>' +
          '</button>' +
          ...
          '<button class="ix-btn ix-btn--tab" data-std-panel="generator">' +
            '<span>Ad Reformat</span>' +
          '</button>' +
          '<button class="ix-btn ix-btn--tab" data-std-panel="adgen">' +
            '<span>Generate</span>' +
          '</button>' +
          // v1.3.8: Create tab — visually distinct (gold pill, pushed
          // right via margin-left:auto). Click does NOT switch panels;
          // it toggles the create dropdown below via data-std-create.
          '<button class="ix-btn ix-btn--tab std-create-tab" data-std-create="toggle"' +
            ' style="margin-left:auto;background:#C4A35A;color:#1A3A3A;font-weight:600;border-color:#C4A35A;">' +
            '<span>Create</span>' +
          '</button>' +
        '</div>' +
        // v1.3.8: Create dropdown — hidden by default, shown when
        // std-create-tab is clicked. Position is set dynamically from
        // the Create button's bounding rect in the click handler (v1.3.9 fix).
        '<div class="std-create-menu" data-std-create-menu' +
          ' style="display:none;position:absolute;z-index:200;' +
                  'background:#fff;border:1px solid #E8E2D0;border-radius:6px;' +
                  'box-shadow:0 8px 24px rgba(26,58,58,0.18);min-width:200px;' +
                  'padding:6px 0;font-family:\'DM Sans\',sans-serif;">' +
          '<button class="std-create-item" data-std-create-type="article"' +
            ' style="display:block;width:100%;padding:10px 16px;border:0;background:transparent;' +
                    'text-align:left;font-size:14px;color:#1A3A3A;cursor:pointer;font-weight:500;">' +
            'Article' +
          '</button>' +
          '<button class="std-create-item" data-std-create-type="ad"' +
            ' style="display:block;width:100%;padding:10px 16px;border:0;background:transparent;' +
                    'text-align:left;font-size:14px;color:#1A3A3A;cursor:pointer;font-weight:500;">' +
            'Ad' +
          '</button>' +
          '<button class="std-create-item" data-std-create-type="re"' +
            ' style="display:block;width:100%;padding:10px 16px;border:0;background:transparent;' +
                    'text-align:left;font-size:14px;color:#1A3A3A;cursor:pointer;font-weight:500;">' +
            'RE Listing' +
          '</button>' +
          '<button class="std-create-item" data-std-create-type="event"' +
            ' style="display:block;width:100%;padding:10px 16px;border:0;background:transparent;' +
                    'text-align:left;font-size:14px;color:#1A3A3A;cursor:pointer;font-weight:500;">' +
            'Event' +
          '</button>' +
        '</div>' +
        '</div>' +  // close .std-tabs-wrap (v1.3.8)
        // ── Tab 1: Inputs (S14 Step 2 — monitor-only surface) ──
        // Channel pill strip + Drive folder health/counts + sub-tab badges.
        // No file list. Full intake management lives in Components + UP.
        // renderInputsPanel() populates this on first activation.
        '<div class="std-panel active" data-std-panel-body="input">' +   // v1.4.9 default
          '<div id="std-inputs-monitor"></div>' +
        '</div>' +
        // ── Tab 2: Components (default active — v1.3.5 flip) ──
        '<div class="std-panel" data-std-panel-body="components">' +
          '<div class="cmp-root" id="cmp-root"></div>' +
        '</div>' +
        // ── Tab 3: Assembler ──
        '<div class="std-panel" data-std-panel-body="assembler">' +
        ...
        '<div class="std-panel" data-std-panel-body="converter">' +
          '<div id="std-converter-mount"></div>' +
        '</div>' +
        // ── Tab 6: Ad Reformat (generator key; reformatter self-mounts
        //    into #std-generator-mount which it creates in this panel) ──
        '<div class="std-panel" data-std-panel-body="generator">' +
          '<div class="std-placeholder">' +
            '<div class="std-placeholder-icon">\u2728</div>' +
            '<div class="std-placeholder-title">Ad Reformat</div>' +
            '<div class="std-placeholder-sub">Drop an existing ad; rebuild it at newsletter sizes.</div>' +
          '</div>' +
        '</div>' +
        // ── Tab 7: Generate (from-scratch ad creator; adgen tool
        //    mounts into #std-adgen-mount on std:panel:adgen) v1.4.8 ──
        '<div class="std-panel" data-std-panel-body="adgen">' +
          '<div id="std-adgen-mount"></div>' +
        '</div>' +
      '</div>';

    // ── Sub-tab wiring ──
    mount.addEventListener('click', function (e) {
      // v1.3.8: Create tab toggle (does NOT switch panels)
      // v1.3.9: position dropdown dynamically from button rect
      var createTabBtn = e.target.closest('[data-std-create="toggle"]');
      if (createTabBtn) {
        e.stopPropagation();
        var menu = mount.querySelector('[data-std-create-menu]');
        if (menu) {
          var shown = menu.style.display !== 'none';
          if (shown) {
            menu.style.display = 'none';
          } else {
            // Compute position from Create button's bounding rect.
            // offsetParent is .std-tabs-wrap (the positioned ancestor).
            var btnRect = createTabBtn.getBoundingClientRect();
            var parent = menu.offsetParent || createTabBtn.parentElement;
            var parentRect = parent.getBoundingClientRect();
            menu.style.top  = (btnRect.bottom - parentRect.top + 4) + 'px';
            // Anchor menu's RIGHT edge to button's RIGHT edge so the
            // menu drops down flush-right under the button.
      ...
      // v1.3.8: Create-type item click
      var createItem = e.target.closest('[data-std-create-type]');
      if (createItem) {
        e.stopPropagation();
        var type = createItem.getAttribute('data-std-create-type');
        var menu2 = mount.querySelector('[data-std-create-menu]');
    ...
    // v1.3.8: click-outside dismisses the create dropdown
    document.addEventListener('click', function (e) {
      // v1.4.2: also dismiss info popover on outside click
      if (infoOpenKey) {
        var pop = document.getElementById('std-info-popover');
        if (pop && !pop.contains(e.target) && !e.target.closest('[data-std-info-key]')) {
          closeInfoPopover();
        }
      }
      var menu = mount.querySelector('[data-std-create-menu]');
      if (!menu || menu.style.display === 'none') return;
      if (e.target.closest('[data-std-create-menu]') || e.target.closest('[data-std-create="toggle"]')) return;
      menu.style.display = 'none';
    });

    // v1.4.2: Escape closes the info popover
    document.addEventListener('keydown', function (e) {
      if (e.key === 'Escape' && infoOpenKey) closeInfoPopover();
    });

    // v1.4.1: route every create-type pick to the ASF. The menu key
    // maps to the ASF assetType — re → 'realestate' (ta-asf rejects
    // the bare 're' key and would silently fall back to Article).
    function handleCreateType(type) {
      if (!window.InbxASF || typeof window.InbxASF.open !== 'function') {
        showStudioToast('ASF not loaded on this page — cannot create');
        console.warn('[ta-studio v1.4.1] window.InbxASF is not available');
        return;
      }
      var assetTypeByKey = { article: 'article', ad: 'ad', re: 'realestate', event: 'event' };
      var assetType = assetTypeByKey[type] || 'article';
      try {
        window.InbxASF.open({ mode: 'create', assetType: assetType });
      } catch (err) {
        console.error('[ta-studio v1.4.1] InbxASF.open failed:', err);
        showStudioToast('Could not open ASF — see console');
      }
    }

    // ── v1.4.2: Tab info registry + heading ⓘ popover ──
    // Each panel gets a small superscript ⓘ next to its heading.
    // Click opens a popover describing the tab + its Make scenarios.
    // Registry is the single edit point — add/update entries here.
    var TAB_INFO = {
      input: {
        title: 'Intake',
        desc: 'Incoming content for this Title, grouped into Bundles and Loose Files. Multi-select to archive used or orphaned items; archived items can be restored. Channel health (Drive, Email, Form) shown above the lists.',
        scenarios: [
          { name: 'Scenario I — Bundle/Media Reader', route: 'listBundlesAndMedia', webhook: 'TA_CONFIG.scenarioI', notes: 'Reads bundles + loose MEDIA rows for this Title. Fires on first activation and on manual refresh.' },
          { name: 'Scenario K — Attach / Status Write', route: 'mode: attach / archive', webhook: 'TA_CONFIG.makeAttachFiles', notes: 'Patches MEDIA.status (Available / In Use / Archived) on bulk archive and restore.' }
        ]
      },
      components: {
        title: 'Components',
        desc: 'Every MEDIA row for this Title — images, ads, text blocks ready to attach to assets. Filter, search, attach to Article / Ad / Event / RE, or open in ASF.',
        scenarios: [
          { name: 'Scenario K — Attach Files', route: 'mode: attach', webhook: 'TA_CONFIG.makeAttachFiles', notes: 'Fires on Attach, select type + item, then Save. Patches MEDIA back-ref and asset media-items. All four asset types.' },
          { name: 'Scenario B — File Conditioner', route: 'createMediaFromBrowserUpload', webhook: 'TA_CONFIG.makeConditioner', notes: 'Fires on Save to Media Library from Converter.' }
        ]
      },
      assembler: {
        title: 'Assemble',
        desc: 'Build a newsletter article record: fill metadata, attach MEDIA to image slots, write the body in the RTE, and save to CMS. One article at a time.',
        scenarios: [
          { name: 'Scenario G — Studio Read-Write', route: 'readArticle / saveArticle / updateArticleBody', webhook: 'TA_CONFIG.makeStudio', notes: 'Read on load. Save on Save to CMS. Body-write from the RTE save path.' },
          { name: 'Scenario K — Attach Files', route: 'mode: attach', webhook: 'TA_CONFIG.makeAttachFiles', notes: 'Fires when attaching a MEDIA item to Main or Interior image slots.' }
        ]
      },
      transcriber: {
        title: 'Transcribe',
        desc: 'Manually enter article content by transcribing from a source document or image. Simpler form than Assembler. Saves via the same body-write path.',
        scenarios: [
          { name: 'Scenario E — Screenshot Transcriber', route: 'transcribe', webhook: 'TA_CONFIG.makeTranscriber', notes: 'OCR/transcription of an uploaded screenshot into article body text.' },
          { name: 'Scenario G — Studio Read-Write', route: 'updateArticleBody', webhook: 'TA_CONFIG.makeStudio', notes: 'Body-save route via InbxStudioBodyEditor.' }
        ]
      },
      converter: {
        title: 'Convert',
        desc: 'Convert uploaded files before they enter the MEDIA pipeline. Tools: Reduce (resize/compress images) and SVG to JPEG. Output can be downloaded or saved to the Media Library.',
        scenarios: [
          { name: 'Scenario B — File Conditioner', route: 'createMediaFromBrowserUpload', webhook: 'TA_CONFIG.makeConditioner', notes: 'Fires only when Save to Media Library is chosen. Browser-only otherwise.' }
        ]
      },
      generator: {
        title: 'Ad Reformat',
        desc: 'Drop an existing advertiser ad; the Reformatter reads it apart (brand colors, logo, image regions, text) and rebuilds it at newsletter sizes (700\u00d7235 banner).',
        scenarios: [
          { name: 'Worker — ix-adreformat', route: 'extract', webhook: 'ix-adreformat.jeff-2cd.workers.dev', notes: 'Claude vision decomposes the ad into reusable parts.' },
          { name: 'Scenario B + 104', route: 'createMedia + createAsset', webhook: 'TA_CONFIG.makeConditioner / makeCreateAssets', notes: 'Saves the finished banner as a customer-assigned AD.' }
        ]
      },
      adgen: {
        title: 'Generate',
        desc: 'Build an advertiser ad from scratch \u2014 blank canvas, brand kit (logo, colors, fonts), add text and elements \u2014 for advertisers with no existing ad to reformat.',
        scenarios: [
          { name: 'Scenario B + 104', route: 'createMedia + createAsset', webhook: 'TA_CONFIG.makeConditioner / makeCreateAssets', notes: 'Saves the finished ad as a customer-assigned AD (same chain as Ad Reformat).' }
        ]
      }
    };

    var infoStylesInjected = false;
    function injectInfoStyles() {
      if (infoStylesInjected || document.getElementById('std-tab-info-styles')) { infoStylesInjected = true; return; }
      var st = document.createElement('style');
      st.id = 'std-tab-info-styles';
      st.textContent = [
        '.std-heading-info {',
        '  display:inline-block; margin-left:5px; cursor:pointer;',
        '  color:#5b7fff !important; font-size:0.5em; font-weight:600;',
        '  vertical-align:super; line-height:0; user-select:none;',
        '  position:relative; top:-0.2em;',
        '}',
        '.std-heading-info:hover { color:#4060dd !important; }',
        '.std-info-popover {',
        '  position:fixed; width:360px; max-width:calc(100vw - 24px);',
        '  background:#ffffff !important; border:1px solid #d4cfbf;',
        '  border-top:3px solid #5b7fff; border-radius:8px;',
        '  box-shadow:0 10px 32px rgba(26,58,58,0.18); z-index:10200;',
        "  font-family:'DM Sans',system-ui,sans-serif; color:#1a3a3a; overflow:hidden;",
        '}',
        '.std-info-head {',
        '  display:flex; align-items:center; justify-content:space-between;',
        '  padding:10px 14px 8px; border-bottom:1px solid #e8e4d8;',
        '}',
        '.std-info-title { font-weight:700; font-size:13px; color:#1a3a3a; }',
        '.std-info-close {',
        '  background:#f4f1ea !important; border:1px solid #c4bfb0; border-radius:50%;',
        '  width:24px; height:24px; font-size:15px; color:#1a3a3a !important;',
        '  cursor:pointer; line-height:1; padding:0; display:flex;',
        '  align-items:center; justify-content:center;',
        '}',
        '.std-info-close:hover { background:#ece8df !important; }',
        '.std-info-desc { padding:10px 14px; font-size:12px; line-height:1.55; color:#3a4a3a; border-bottom:1px solid #e8e4d8; }',
        '.std-info-scenarios-label {',
        '  padding:8px 14px 4px; font-family:\'DM Mono\',ui-monospace,monospace;',
        '  font-size:9px; letter-spacing:0.07em; text-transform:uppercase; color:#8a8a7a; font-weight:600;',
        '}',
        '.std-info-scenario { padding:8px 14px 10px; border-top:1px solid #f0ece4; }',
        '.std-info-scenario:first-of-type { border-top:none; }',
        '.std-info-scenario-name { font-size:12px; font-weight:600; color:#1a3a3a; margin-bottom:3px; }',
        '.std-info-scenario-route { font-family:\'DM Mono\',ui-monospace,monospace; font-size:10px; color:#5b7fff; margin-bottom:3px; }',
        '.std-info-scenario-webhook { font-family:\'DM Mono\',ui-monospace,monospace; font-size:9px; color:#8a8a7a; word-break:break-all; margin-bottom:4px; }',
        '.std-info-scenario-notes { font-size:11px; color:#5a6a5a; line-height:1.45; }'
      ].join('\n');
      document.head.appendChild(st);
      infoStylesInjected = true;
    }

    var infoOpenKey = null;
    function closeInfoPopover() {
      var ex = document.getElementById('std-info-popover');
      if (ex && ex.parentNode) ex.parentNode.removeChild(ex);
      infoOpenKey = null;
    }
    function openInfoPopover(key, anchor) {
      closeInfoPopover();
      var info = TAB_INFO[key];
      if (!info) return;
      infoOpenKey = key;
      var sc = '';
      if (!info.scenarios.length) {
        sc = '<div class="std-info-scenario" style="font-style:italic;color:#8a8a7a;">No Make scenarios — browser-only tab.</div>';
      } else {
        info.scenarios.forEach(function (s) {
          sc += '<div class="std-info-scenario">' +
            '<div class="std-info-scenario-name">' + s.name + '</div>' +
            '<div class="std-info-scenario-route">' + s.route + '</div>' +
            '<div class="std-info-scenario-webhook">' + s.webhook + '</div>' +
            (s.notes ? '<div class="std-info-scenario-notes">' + s.notes + '</div>' : '') +
          '</div>';
        });
      }
      var pop = document.createElement('div');
      pop.id = 'std-info-popover';
      pop.className = 'std-info-popover';
      pop.innerHTML =
        '<div class="std-info-head">' +
          '<span class="std-info-title">' + info.title + '</span>' +
          // v1.4.10: version lives here now (was a stale hardcoded
          // ix-badge in the .ix-hdr row). Reads STD_VERSION.
          '<span class="ix-badge" style="margin-left:8px;">v' + STD_VERSION + '</span>' +
          '<button class="std-info-close" id="std-info-close">\u00d7</button>' +
        '</div>' +
        '<div class="std-info-desc">' + info.desc + '</div>' +
        '<div class="std-info-scenarios-label">Make Scenarios</div>' +
        sc;
      ...
      pop.querySelector('#std-info-close').addEventListener('click', function (e) {
        e.stopPropagation(); closeInfoPopover();
      });
    }

    // Inject the ⓘ next to the active panel's heading.
    // Panels mount async (sibling files), so retry briefly until the
    // heading exists. Idempotent — never doubles up.
    // v1.4.3: panels whose ⓘ is intentionally skipped.
    //   input  — renderInputsPanel() has no heading element (jumps
    //            straight to .inp-section); nothing to host the ⓘ.
    //   create — action launcher, not a [data-std-panel-body] panel.
    var INFO_SKIP = { input: true, create: true };

    // v1.4.3: every panel's real heading class, confirmed from live
    // sibling files. The v1.4.2 list missed .cm-conv-title (Converter),
    // .std-placeholder-title (Generator), and the .sct-* h2s
    // (Transcriber), which is why only Components/Assembler showed it.
    var INFO_HEADING_SELECTOR = [
      '.cmp-title',              // Components (ta-components-tab)
      '.asm-title',              // Assembler (ta-studio renderAssembler)
      '.cm-conv-title',          // Converter (ta-converter)
      '.sct-transcribed',        // Transcriber (ta-page-body)
      '.sct-generated',          // Transcriber alt heading
      '.std-placeholder-title',  // Generator (placeholder)
      '.std-panel-title', 'h1', 'h2'
    ].join(', ');

    // Track one observer per panel body so we don't stack duplicates.
    var infoObservers = {};

    function _injectHeadingSup(heading, name) {
      if (!heading || heading.querySelector('.std-heading-info')) return false;
      var sup = document.createElement('sup');
      sup.className = 'std-heading-info';
      sup.setAttribute('data-std-info-key', name);
      sup.setAttribute('title', 'About this tab');
      sup.textContent = '\u24d8';
      sup.addEventListener('click', function (e) {
        e.stopPropagation();
        if (infoOpenKey === name) closeInfoPopover();
        else openInfoPopover(name, sup);
      });
      heading.appendChild(sup);
      return true;
    }

    // v1.4.3: attach the heading ⓘ for a panel. Sibling-owned panels
    // (Components / Transcriber / Converter) mount their heading
    // ASYNCHRONOUSLY after we dispatch the activation event, so the
    // old 12×60ms (720ms) poll lost the race on slow mounts. We now
    // (1) try immediately, then (2) leave a MutationObserver watching
    // the panel body so the ⓘ lands whenever the heading paints —
    // however late. Observer self-disconnects once attached.
    function attachHeadingInfo(name) {
      if (INFO_SKIP[name]) return;
      injectInfoStyles();
      var body = mount.querySelector('[data-std-panel-body="' + name + '"]');
      if (!body) return;

      // Immediate attempt (covers synchronous panels like Assembler).
      var heading = body.querySelector(INFO_HEADING_SELECTOR);
      if (heading && _injectHeadingSup(heading, name)) return;
      if (heading) return; // already had the ⓘ

      // Async path: watch the body until the heading appears.
      if (infoObservers[name]) { infoObservers[name].disconnect(); }
      var obs = new MutationObserver(function () {
        var h = body.querySelector(INFO_HEADING_SELECTOR);
        if (h) {
          _injectHeadingSup(h, name);
          obs.disconnect();
          infoObservers[name] = null;
        }
      });
      obs.observe(body, { childList: true, subtree: true });
      infoObservers[name] = obs;
      // Safety stop: disconnect after 10s so a panel that never
      // renders a heading doesn't leave a live observer around.
      setTimeout(function () {
        if (infoObservers[name] === obs) { obs.disconnect(); infoObservers[name] = null; }
      }, 10000);
    }
    ...
    function setPanel(name) {
      ...
      // themselves into the panel container.
      // v1.3.5: Inputs dispatches renderInputsPanel() on first
      // activation; Generator remains a placeholder.
      if (name === 'assembler') {
        renderAssembler();
        ...
        // v1.3.5: monitor-only panel. Renders on first activation;
        // folder counts load lazily (not on Studio mount).
        document.body.classList.remove('studio-editing');
        renderInputsPanel();
        ...
        // v1.4.8: from-scratch ad generator. ta-adreformat (or the
        // adgen tool) listens for this and mounts into #std-adgen-mount.
        try { window.dispatchEvent(new CustomEvent('std:panel:adgen')); }
        catch (e) { /* noop */ }
        document.body.classList.remove('studio-editing');
      } else {
        // generator / any other placeholder.
        document.body.classList.remove('studio-editing');
      }
      // v1.4.2: close any open info popover on panel switch, then
      // (re-)attach the heading ⓘ for the newly active panel.
      closeInfoPopover();
      attachHeadingInfo(name);
    }
    ...
    function openBodyEditor(articleItemId, articleTitle, opts) {
      // v1.3.6: accept opts.returnPanel — the Studio sub-tab the user
      // should land on after Save-to-CMS reloads the page. If omitted,
      // we read S.panel (current Studio sub-tab) which is correct for
      // the Assembler-launch case. Cross-IIFE callers (e.g. UP's
      // Transcriber via window.InbxStudioBodyEditor) pass their own
      // panel name (e.g. 'transcriber') because S.panel at click time
      // is the panel that owns the rendered UI, which == their panel.
      // The override path exists so future callers that don't sit
      // inside Studio's panel chrome can still target a specific
      // restore destination.
      opts = opts || {};
      var returnPanel = opts.returnPanel || S.panel || 'assembler';

      if (!window.InbxRTE || typeof window.InbxRTE.openFullscreen !== 'function') {
        // v1.3.7: dropped stale "ta-rte-v1.1.10.js" version reference
        // from the message. Future ta-rte version bumps no longer
        // require updating this string. The real check is whether
        // window.InbxRTE.openFullscreen exists — usually missing
        // because the ta-rte <script src> reference in the page head
        // points at a filename that no longer exists in the repo, OR
        // because ta-rte's IIFE threw before reaching the public API
        // assignment (see browser console for the actual error).
        console.error('[TA-STUDIO] window.InbxRTE.openFullscreen not available. Check that the ta-rte script tag in the T-A page head points at a real file in the GitHub repo, and that the file loads without errors (red entries in the Console).');
        alert('Body editor is not loaded. Check the ta-rte script reference in the T-A page head, and look for ta-rte errors in the browser console.');
        return;
      }
      if (!articleItemId) {
      ...
      console.log('[TA-STUDIO v1.3.6] openBodyEditor:', { articleItemId: articleItemId, returnPanel: returnPanel });
          ...
          onSave: function (editedHTML) {
            saveArticleBody(articleItemId, editedHTML)
              .then(function () {
                ...
                // affordance.
                // v1.3.6: forward returnPanel so the post-reload restore
                // lands on the panel that launched the editor (Assembler
                // for in-Studio launches, Transcriber for UP launches).
                studioReloadAfterCMSWrite({
                  bannerLabel: '\u2713 Body saved \u2014 refreshing\u2026',
    ...
    // v1.3.6: Cross-IIFE export. UP's editArticleBody (v1.0.12+) delegates
    // here so the Transcriber surface gets the same save → close → reload
    // → restore behavior as the Assembler, AND so the body-save hits
    // Scenario G (which writes post-body + post-body-html per Path B
    // Phase 1) instead of Scenario F. Contract:
    //   window.InbxStudioBodyEditor({ articleItemId, articleTitle, returnPanel })
    // returnPanel: Studio sub-tab name to land on after reload (e.g.
    //   'assembler', 'transcriber'). Accepts any S.panel value generically.
    //   Falls back to current S.panel if omitted.
    //
    // v1.4.0: NEW local-mode path. Used by ASF create flow where no
    // article record exists yet, so neither the Scenario G fetch (to
    // load existing body) nor the Scenario G save (to update existing
    // body) is appropriate. Contract for local mode:
    //   window.InbxStudioBodyEditor({
    //     mode: 'local',
    //     articleTitle: '...',
    //     initialBody: '<existing local HTML or empty string>',
    //     onSave: function(html) { /* caller stores html locally */ },
    //     onClose: function() { /* optional */ }
    //   })
    // In local mode the editor:
    //   - does NOT fetch from Scenario G (caller provides initialBody)
    //   - does NOT POST to Scenario G on save (caller's onSave callback
    //     receives the HTML and handles persistence)
    //   - does NOT reload the page (just closes the RTE modal so the
    //     caller's underlying surface — ASF create form — remains
    //     visible with its in-memory state intact)
    window.InbxStudioBodyEditor = function (params) {
      params = params || {};
      if (params.mode === 'local') {
        return openBodyEditorLocal({
          articleTitle: params.articleTitle || '',
          initialBody:  params.initialBody  || '',
          onSave:       typeof params.onSave  === 'function' ? params.onSave  : function () {},
          onClose:      typeof params.onClose === 'function' ? params.onClose : function () {}
        });
      }
      return openBodyEditor(params.articleItemId, params.articleTitle, {
        returnPanel: params.returnPanel
      });
    };

    // v1.4.0: local-mode body editor. Sibling to openBodyEditor but with
    // server I/O elided. Same defensive cleanup (kill stale RTE instance,
    // strip orphaned overlays) so we can't leave the page in a weird
    // state. Uses the same window.InbxRTE.openFullscreen widget — the
    // RTE component is already callback-driven, so "local mode" is just
    // openBodyEditor with the server calls removed.
    function openBodyEditorLocal(params) {
      if (!window.InbxRTE || typeof window.InbxRTE.openFullscreen !== 'function') {
        console.error('[TA-STUDIO v1.4.1] openBodyEditorLocal: window.InbxRTE.openFullscreen not available.');
        alert('Body editor is not loaded. Check the ta-rte script reference in the T-A page head, and look for ta-rte errors in the browser console.');
        return;
      }
      console.log('[TA-STUDIO v1.4.1] openBodyEditorLocal:', { initialBodyLen: (params.initialBody || '').length });

      // Defensive cleanup — same as openBodyEditor.
      if (window._studioRteInstance &&
          typeof window._studioRteInstance.closeFullscreen === 'function') {
        try { window._studioRteInstance.closeFullscreen(); } catch (e) {}
      }
      ...
      window._studioRteInstance = window.InbxRTE.openFullscreen({
        articleItemId:  null,                              // no record yet
        articleTitle:   params.articleTitle || 'New Article',
        articleViewUrl: undefined,                         // article doesn't exist on the public site yet
        initialHTML:    params.initialBody || '',
        mode:           'edit',                            // RTE's internal mode — 'edit' = normal authoring
        onSave: function (editedHTML) {
          // No server call, no reload. Just close the RTE and forward
          // the HTML to the caller. The caller (ASF create mode) writes
          // it into its in-memory article state, where it'll travel
          // out with the createAsset payload on Save Draft.
          if (window._studioRteInstance &&
              typeof window._studioRteInstance.closeFullscreen === 'function') {
            window._studioRteInstance.closeFullscreen();
          }
          window._studioRteInstance = null;
          try { params.onSave(editedHTML); } catch (e) {
            console.error('[TA-STUDIO v1.4.1] local-mode onSave threw:', e);
          }
        },
        onClose: function () {
          window._studioRteInstance = null;
          try { params.onClose(); } catch (e) {}
        }
      });
    }
    ...
    function studioReloadAfterCMSWrite(opts) {
      ...
      // v1.3.6: payload now includes explicit outerTab field so
      // future surfaces (PubPlan, Content Library) adopting this
      // reload-after-write pattern can target their own outer tab
      // on restore. Currently always 'studio' since this function
      // is only called from Studio. Also accepts opts.returnPanel
      // to override the persisted Studio sub-tab — used by ABE save
      // so the restore lands on the launching panel (Assembler /
      // Transcriber) rather than whatever the current S.panel is.
      try {
        var restorePayload = {
          outerTab: 'studio',
          panel: opts.returnPanel || S.panel || 'assembler',
    ...
    function studioRestoreFromSession() {
      ...
      // Stale-payload guard: if the persisted state is older than 120s,
      ...
      // v1.3.6: bumped 30s → 120s. Reload-after-save should never realistically
      // exceed 30s, but slow Make webhook responses or CDN hiccups can push
      // past that and silently discard the restore — bringing back the
      // "lands on PubPlan after save" symptom. Bumping is zero-cost.
      ...
      // v1.3.6: studioActivateOuterTab is now a retry-poller; no longer
      // needs the setTimeout(0) defer dance because the poller handles
      // body.js wire-up timing internally.
      studioActivateOuterTab();
    }

    // v1.2.5 / v1.3.6: outer-tab activation matching body.js's proven pattern.
    ...
    //
    // v1.3.6 change — retry-poller. Previously this was a single-shot
    // best-effort attempt called via setTimeout(0). When body.js's
    // DOMContentLoaded handler had not yet wired the .tab-button click
    // handlers (a race on slow page loads, post-reload scenarios), the
    // proxy click was a no-op and the user landed on the page's default
    // Webflow outer tab — PubPlan — instead of Studio. This was Item #3's
    // root cause for the Assembler-launched ABE save path.
    //
    // Now: try every 50ms up to 1500ms, bail on first successful click.
    // The proxy / .w-tab-link / legacy selectors are tried in order
    // per-attempt — whichever wires up first wins.
    //
    // Activation order (per attempt):
    //   1. Find .tab-button whose text content is "Studio" — click it.
    //      body.js handles the proxy to .w-tab-link.
    //   2. Fallback: .w-tab-link direct click (Webflow-native).
    //   3. Legacy fallbacks (data-ta-tab, etc.).
    //
    ...
    function studioActivateOuterTab() {
      ...
      function tryActivate() {
        ...
        for (var i = 0; i < tabBtns.length; i++) {
          if (isStudioLabel(tabBtns[i])) {
            try { tabBtns[i].click(); return 'tab-button'; } catch (e) {}
          }
        }
      ...
      function attempt() {
        ...
        if (hit) {
          ...
          console.log('[TA-STUDIO v1.3.6] outer tab activated via ' + hit + ' in ' + elapsed + 'ms');
          return;
        }
        if (Date.now() - startTs >= TIMEOUT_MS) {
          console.warn('[TA-STUDIO v1.3.6] studioActivateOuterTab timed out after ' + TIMEOUT_MS + 'ms — body.js may not have wired tab handlers, or page lacks expected tab DOM. User will need to click Studio manually.');
          return;
        }
    ...
    // ═══════════════════════════════════════════
    // INPUTS MONITOR PANEL  (v1.3.5 / S14 Step 2)
    // ═══════════════════════════════════════════
    // Monitor-only surface for Tab 1. Shows:
    //   1. Channel pill strip (Drive, Email, Form) — informational
    //   2. Drive folder health: 4 sub-tab count badges
    //      (Ready, Assigned, Archived, Hidden) from folderCounts
    //   3. Monitoring context note
    //
    // No file list rendered here. Full intake management (assignment
    // modal, file actions) remains in Components + UP during the
    // parallel-run window. Full migration at S14 Step 2 close.
    //
    // UX platform rules (v1.3.5 — apply to all future surfaces):
    //   - .ix-select--dirty: 2px gold (#C4A35A) border on any select
    //     holding a pending (non-default) selection
    //   - Cancel text link: every edit-initiated state gets a small
    //     "cancel" link that reverts to original state
    //   This panel has no editable selects in v1.3.5 (read-only).
    //   Rules are enforced on the Assembler and Components surfaces
    //   where they already exist; listed here as the platform contract.
    //
    // HC-011: Channel labels ('Drive', 'Email', 'Form') are hardcoded.
    //   Future: read from TA_CONFIG.inputChannels if that key is added.
    //   Intentional — channel vocabulary is foundational and unlikely
    //   to change without a schema-level decision.
    // ────────────────────────────────────────────────────────────

    var INP_LOADED = false;       // has the panel been rendered at least once?
    var INP_GA4_FIRED = false;    // one-shot GA4 TD-167 flag
    var INP_COUNTS_LOADING = false;
    var INP_COUNTS_REFRESHING = false;
    var INP_COUNTS = {            // Drive folder counts from folderCounts endpoint
      uploadFolder: 0,
      preprocessing: 0,
      ready: 0,
      assigned: 0,
    ...
    var INP_COUNTS_ERR = null;
    var INP_COUNTS_LOADED = false; // true after first successful fetch

    // Channel definitions — HC-011
    var INP_CHANNELS = [
    ...
    // Sub-tab badge config (Drive folder buckets)
    // Keys match folderCounts endpoint response keys.
    var INP_DRIVE_TABS = [
    ...
    function renderInputsPanel() {
      // ── GA4 TD-167: fire once on first Inputs activation ──
      // Measures publishers reaching the UP entry point after Studio
      // Inputs tab exists. One-shot per page load.
      if (!INP_GA4_FIRED) {
        INP_GA4_FIRED = true;
        try {
          if (typeof window.gtag === 'function') {
            window.gtag('event', 'legacy_uploads_processor_open', {
              event_category: 'studio',
              event_label: 'inputs_tab_first_activation',
              non_interaction: false
            });
          }
        } catch (e) { /* silently skip if gtag unavailable */ }
      }

      var el = document.getElementById('std-inputs-monitor');
      if (!el) return;

      // Build the panel HTML
      var channelPills = INP_CHANNELS.map(function(ch) {
        return '<span class="inp-channel-pill">' + _esc2(ch.icon) + ' ' + _esc2(ch.label) + '</span>';
      }).join('');

      var driveTabBadges = INP_DRIVE_TABS.map(function(t) {
        var count = INP_COUNTS[t.key];
        var badgeClass = 'inp-drive-badge' + (count > 0 ? ' inp-drive-badge--has-items' : '');
        var countStr = INP_COUNTS_LOADED
          ? String(count)
          : (INP_COUNTS_LOADING ? '…' : '–');
        return '<div class="inp-drive-tab">' +
          '<span class="inp-drive-tab-label">' + _esc2(t.label) + '</span>' +
          '<span class="' + badgeClass + '">' + countStr + '</span>' +
        '</div>';
      }).join('');

      var refreshBtnAttr = INP_COUNTS_REFRESHING ? ' disabled' : '';
      var refreshSpinStyle = INP_COUNTS_REFRESHING
        ? ' style="display:inline-block;animation:inbx-spin 1s linear infinite;"'
        : '';

      var errHtml = INP_COUNTS_ERR
        ? '<div class="inp-error-note">' + _esc2(INP_COUNTS_ERR) + '</div>'
        : '';

      var loadingClass = INP_COUNTS_LOADING ? ' inp-counts--loading' : '';

      el.innerHTML =
        '<div class="inp-root">' +
          // v1.4.6: Channels strip (Section 1) removed. The Intake panel
          // is now an empty shell; ta-intake-manager mounts its own
          // #intk-root here and owns all rendering (subtabs, bundles,
          // loose files, archive/restore). v1.4.5 removed Sections 2+3
          // (Drive Folder Health + Monitor-only banner); v1.4.6 removes
          // the last remnant.
        '</div>';

      INP_LOADED = true;

      // v1.4.6: channel pills + folder-count load no longer invoked.
      // INP_CHANNELS / loadInputsFolderCounts / Scenario C path remain
      // defined below but unused — left to avoid touching unrelated
      // config; safe to fully delete in a later cleanup pass.
    }

    function loadInputsFolderCounts() {
      var CFG = _cfg();
      if (!CFG.scenarioCUrl) {
        INP_COUNTS_ERR = 'scenarioCUrl not configured — folder counts unavailable.';
        var el = document.getElementById('std-inputs-monitor');
        if (el && INP_LOADED) renderInputsPanel();
        return;
      }
      INP_COUNTS_LOADING = true;
      INP_COUNTS_ERR = null;
      var el = document.getElementById('std-inputs-monitor');
      if (el && INP_LOADED) renderInputsPanel();

      var qs = new URLSearchParams({ titleSlug: CFG.titleSlug, action: 'folderCounts' });
      fetch(CFG.scenarioCUrl + '?' + qs.toString())
        .then(function(res) {
          if (!res.ok) throw new Error('folderCounts returned ' + res.status);
          return res.json();
        })
        .then(function(data) {
          if (data && typeof data === 'object') {
            INP_COUNTS.uploadFolder  = Number(data.uploadFolder  || 0);
            INP_COUNTS.preprocessing = Number(data.preprocessing || 0);
            INP_COUNTS.ready         = Number(data.ready         || 0);
            INP_COUNTS.assigned      = Number(data.assigned      || 0);
            INP_COUNTS.archived      = Number(data.archived      || 0);
            INP_COUNTS.hidden        = Number(data.hidden        || 0);
          }
          INP_COUNTS_LOADED = true;
          INP_COUNTS_LOADING = false;
          INP_COUNTS_REFRESHING = false;
          var el2 = document.getElementById('std-inputs-monitor');
          if (el2 && INP_LOADED) renderInputsPanel();
        })
        .catch(function(err) {
          console.error('[Studio Inputs] folderCounts error:', err);
          INP_COUNTS_ERR = 'Could not load folder counts — ' + (err.message || 'network error');
          INP_COUNTS_LOADING = false;
          INP_COUNTS_REFRESHING = false;
          var el2 = document.getElementById('std-inputs-monitor');
          if (el2 && INP_LOADED) renderInputsPanel();
    ...
    function triggerInputsRefresh() {
      if (INP_COUNTS_REFRESHING || INP_COUNTS_LOADING) return;
      INP_COUNTS_REFRESHING = true;
      var el = document.getElementById('std-inputs-monitor');
      if (el && INP_LOADED) renderInputsPanel();
      loadInputsFolderCounts();
    }

    // ── Inputs panel click delegation ──
    document.addEventListener('click', function(e) {
      if (e.target.closest && e.target.closest('[data-inp-refresh]')) {
        e.preventDefault();
        e.stopPropagation();
        triggerInputsRefresh();
      }
    });

    // ═══════════════════════════════════════════
    // END INPUTS MONITOR PANEL
    // ═══════════════════════════════════════════


    // ═══════════════════════════════════════════
    // PDF TRIAGE BANNER + MODAL  (v1.3.1 / S14 Step 1)
    // ═══════════════════════════════════════════
    // Cross-tab attention surface for PDFs awaiting decision.
    // Banner sits in Studio chrome header, right-justified.
    // Modal opens on banner click, lists PDF_PENDING files
    // from scenarioPdfQueueUrl, runs per-row picks through a
    // sequential apply queue.
    //
    // Architecture: see Master Roadmap v1.6.1 §10.1 (banner-as-
    // attention) and §10.2 (tenant-config-via-Data-Store).
    //
    // Companion code: uploads-processor v1.0.11 carries a
    // duplicate of this infrastructure (TD-174). Both poll the
    // same scenarioCUrl?action=folderCounts endpoint; both
    // render a banner gated on STD_MONITOR.preprocessing > 0.
    // The two banners never coexist visually (UP banner is
    // inside Content Processor tab, Studio banner is in Studio
    // chrome). State stays in sync via shared polling.

    // ── CFG getters (Studio-local mirror of UP's CFG) ──
    function _cfg() {
      var W = window.TA_CONFIG || {};
      return {
        scenarioCUrl:        W.scenarioC || '',
        scenarioPdfQueueUrl: W.scenarioPdfQueueUrl || '',
        makeConditioner:     W.makeConditioner || '',
    ...
    // ── State ──
    var STD_MONITOR = { uploadFolder: 0, preprocessing: 0 };
    var STD_TRANSCRIBE_PDFS = []; // post-queue handoff

    var T_OPEN = false;
    var T_LOADING = false;
    var T_ERR = null;
    var T_FILES = [];
    var T_PICKS = {};   // keyed by fileId → 'convert' | 'transcribe' | 'archive' | 'hide'
    var T_STATUS = {};  // keyed by fileId → null | 'running' | 'done' | 'failed'
    var T_RUNNING = false;
    var T_REFRESHING = false; // v1.3.3: banner refresh in-flight
    var T_BANNER_FRESH = false;
    var _T_PRIOR_PREPROCESSING = 0;
    ...
    function renderTriageBanner() {
      var el = document.getElementById('std-triage-banner');
      if (!el) return;
      var n = STD_MONITOR.preprocessing || 0;
      if (n <= 0 && !T_REFRESHING) {
        el.innerHTML = '';
        _T_PRIOR_PREPROCESSING = 0;
        return;
      }
      var fresh = (_T_PRIOR_PREPROCESSING === 0 && n > 0) || T_BANNER_FRESH;
      T_BANNER_FRESH = false;
      _T_PRIOR_PREPROCESSING = n;
      var freshCls = fresh ? ' is-fresh' : '';
      // v1.3.3: refresh button between count and Triage button.
      // Spin animation applied inline via style when refreshing
      // (avoids needing a CSS module bump for this small detail).
      var spinStyle = T_REFRESHING
        ? ' style="display:inline-block;animation:ix-spin 1s linear infinite;"'
        : '';
      var refreshDis = T_REFRESHING ? ' disabled' : '';
      el.innerHTML =
        '<div class="ix-modal-banner ix-modal-banner--alert ix-modal-banner--compact' + freshCls + '">' +
          '<span class="ix-modal-banner-icon">\u{1F4D5}</span>' +
          '<span class="ix-modal-banner-text"><strong>' + n + ' PDF' + (n === 1 ? '' : 's') + ' awaiting</strong></span>' +
          '<button class="ix-btn ix-btn--ghost ix-btn--icon" data-std-triage-refresh' + refreshDis + ' title="Refresh count">' +
            '<span' + spinStyle + '>\u21BB</span>' +
          '</button>' +
          '<button class="ix-btn ix-btn--primary" data-std-triage-open' + refreshDis + '>Triage \u2192</button>' +
        '</div>';
      // v1.3.3: ensure the @keyframes ix-spin rule exists in the
      // document. Inject once on first banner render. Living here
      // (not in CSS module) keeps v1.3.3 a JS-only ship.
      _ensureSpinKeyframes();
    }

    var _spinKeyframesInjected = false;
    function _ensureSpinKeyframes() {
      if (_spinKeyframesInjected) return;
      var style = document.createElement('style');
      style.setAttribute('data-injected-by', 'ta-studio-v1.3.3-spin');
      style.textContent = '@keyframes ix-spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }';
    ...
    function closeTriageModal() {
      ...
      var root = document.getElementById('ix-triage-modal-root');
      if (root) root.remove();
    ...
    function loadTriagePdfs() {
      ...
      if (!CFG.scenarioPdfQueueUrl) {
        T_LOADING = false;
        T_ERR = 'scenarioPdfQueueUrl not configured in window.TA_CONFIG. The triage modal needs a Make webhook returning files where status = PDF_PENDING.';
        renderTriageModal();
        return;
      }
      var qs = new URLSearchParams({ titleSlug: CFG.titleSlug });
      fetch(CFG.scenarioPdfQueueUrl + '?' + qs.toString())
        .then(function(res) { if (!res.ok) throw new Error(res.status); return res.json(); })
        .then(function(data) {
          var arr = Array.isArray(data)
            ? data
            : (Array.isArray(data && data.files) ? data.files : []);
          T_FILES = arr.map(function(f) {
            return {
              fileId:  f.fileId || '',
          ...
          }).filter(function(f) { return f.fileId; });
          T_LOADING = false;
          T_ERR = null;
          renderTriageModal();
        })
        .catch(function(err) {
          console.error('[Studio Triage] Load error:', err);
          T_LOADING = false;
    ...
    function renderTriageModal() {
      ...
      if (T_LOADING) {
        ...
        body = '<div class="ix-modal-empty"><div class="ix-modal-empty-icon">\u26A0</div><div class="ix-modal-empty-text">' + _esc(T_ERR) + '</div></div>';
        ...
        body = T_FILES.map(buildTriageRowHTML).join('');
      }
      var decided = Object.keys(T_PICKS).filter(function(k) { return T_PICKS[k]; }).length;
      var total = T_FILES.length;
      var canApply = !T_RUNNING && !T_LOADING && !T_ERR && decided > 0;
      var lockCls = T_RUNNING ? ' is-locked' : '';
      var closeDis = T_RUNNING ? ' disabled' : '';
      var cancelDis = T_RUNNING ? ' disabled' : '';
      var applyDis = canApply ? '' : ' disabled';
      var applyLabel = T_RUNNING ? 'Applying\u2026' : (decided ? 'Apply Triage (' + decided + ')' : 'Apply Triage');
      var html =
        '<div class="ix-overlay" data-std-triage-overlay>' +
          '<div class="ix-modal' + lockCls + '">' +
            ...
            '<div class="ix-modal-head">' +
              ...
              '</div>' +
              '<button class="ix-btn ix-btn--ghost ix-btn--icon" data-std-triage-close' + closeDis + ' title="Close">\u2715</button>' +
            '</div>' +
            '<div class="ix-modal-body">' + body + '</div>' +
            '<div class="ix-modal-footer">' +
              '<button class="ix-btn ix-btn--ghost" data-std-triage-cancel-all' + cancelDis + '>\u2715 Cancel All</button>' +
              '<div class="ix-modal-footer-right">' +
                '<span class="ix-modal-footer-info">' + decided + ' of ' + (total || 0) + ' decided</span>' +
                '<button class="ix-btn ix-btn--primary" data-std-triage-apply' + applyDis + '>' + applyLabel + '</button>' +
              '</div>' +
            '</div>' +
          '</div>' +
        '</div>';
    ...
    function buildTriageRowHTML(f) {
      var pick = T_PICKS[f.fileId] || '';
      var status = T_STATUS[f.fileId] || null;
      var dirtyCls = pick && !status ? ' has-pending' : '';
      var stateCls = status === 'running' ? ' is-running'
                   : status === 'done' ? ' is-done'
                   : status === 'failed' ? ' is-failed' : '';
      var sizeStr = _fmtSize(f.size);
      var dateStr = f.arrived ? f.arrived.substring(0, 10) : '';
      var controls;
      if (status === 'running' || status === 'done' || status === 'failed') {
        var label = status === 'running' ? 'Running\u2026' : status === 'done' ? 'Done \u2713' : 'Failed \u2715';
        var actionLabel = pick === 'convert'    ? 'Convert to JPEG'
                        : pick === 'transcribe' ? 'Transcribe & Summarize'
                        : pick === 'archive'    ? 'Archive'
                        : pick === 'hide'       ? 'Hide' : pick;
        controls = '<span class="ix-modal-row-status is-' + status + '">' + _esc(actionLabel) + ' \u00B7 ' + label + '</span>';
      } else {
        function opt(val, lbl) { return '<option value="' + val + '"' + (pick === val ? ' selected' : '') + '>' + lbl + '</option>'; }
        var select = '<select class="ix-modal-row-select" data-std-triage-pick="' + _esc(f.fileId) + '"' + (T_RUNNING ? ' disabled' : '') + '>' +
          ...
          '</select>';
        var revert = pick
          ? '<button class="ix-btn ix-btn--ghost" data-std-triage-revert="' + _esc(f.fileId) + '" title="Revert this row">\u21B6 revert</button>'
          : '';
        controls = select + revert;
      }
      return '<div class="ix-modal-row' + dirtyCls + stateCls + '" data-std-triage-row="' + _esc(f.fileId) + '">' +
        '<span class="ix-modal-row-icon">\u{1F4D5}</span>' +
        '<div class="ix-modal-row-info">' +
          '<div class="ix-modal-row-title">' + _esc(f.name) + '</div>' +
          '<div class="ix-modal-row-meta"><span>PDF</span><span>' + sizeStr + '</span>' + (dateStr ? '<span>' + _esc(dateStr) + '</span>' : '') + '</div>' +
        '</div>' +
        '<div class="ix-modal-row-controls">' + controls + '</div>' +
      '</div>';
    }

    var _triageClickHandler = null;
    var _triageChangeHandler = null;
    function bindTriageModalEvents() {
      ...
      _triageClickHandler = function(e) {
        if (e.target.matches && e.target.matches('[data-std-triage-overlay]') && !T_RUNNING) { closeTriageModal(); return; }
        if (e.target.closest('[data-std-triage-close]'))      { if (!T_RUNNING) closeTriageModal(); return; }
        if (e.target.closest('[data-std-triage-cancel-all]')) { if (!T_RUNNING) closeTriageModal(); return; }
        if (e.target.closest('[data-std-triage-apply]'))      { if (!T_RUNNING) runTriageQueue(); return; }
        var revertBtn = e.target.closest('[data-std-triage-revert]');
        if (revertBtn) { handleTriageRowRevert(revertBtn.getAttribute('data-std-triage-revert')); return; }
      };
      _triageChangeHandler = function(e) {
        var sel = e.target.closest && e.target.closest('[data-std-triage-pick]');
        if (sel) handleTriageRowChange(sel.getAttribute('data-std-triage-pick'), sel.value);
      };
    ...
    function runTriageQueue() {
      var ids = Object.keys(T_PICKS).filter(function(k) { return T_PICKS[k]; });
      if (!ids.length) return;
      var order = ['convert', 'archive', 'hide', 'transcribe'];
      var queue = [];
      order.forEach(function(action) {
        ids.forEach(function(id) { if (T_PICKS[id] === action) queue.push({ id: id, action: action }); });
      });
      if (!queue.length) return;
      T_RUNNING = true;
      queue.forEach(function(q) { T_STATUS[q.id] = null; });
      renderTriageModal();
    ...
    function runTriageQueueStep(queue, i) {
      ...
      var step = queue[i];
      var file = T_FILES.find(function(f) { return f.fileId === step.id; });
      if (!file) { runTriageQueueStep(queue, i + 1); return; }
      T_STATUS[step.id] = 'running';
      renderTriageModal();
      triageRunOne(file, step.action)
        .then(function() { T_STATUS[step.id] = 'done'; })
        .catch(function(err) { console.error('[Studio Triage] step failed:', file.name, step.action, err); T_STATUS[step.id] = 'failed'; })
        .then(function() {
          renderTriageModal();
          setTimeout(function() { runTriageQueueStep(queue, i + 1); }, 250);
        });
    }
    ...
    function triageRunOne(file, action) {
      ...
      if (action === 'convert') {
        ...
        return fetch(CFG.makeConditioner, {
          ...
          body: JSON.stringify({
            titleSlug: CFG.titleSlug, fileId: file.fileId, fileName: file.name,
            mimeType: file.mime, action: 'convert_to_jpeg',
          }),
      ...
      if (action === 'archive' || action === 'hide') {
        ...
        var newStatus = action === 'archive' ? 'ARCHIVED' : 'HIDDEN';
        var qs = new URLSearchParams({
          titleSlug: CFG.titleSlug, action: 'updateStatus',
          fileId: file.fileId, newStatus: newStatus,
        });
        return fetch(CFG.scenarioCUrl + '?' + qs.toString())
          .then(function(res) { if (!res.ok) throw new Error(res.status); });
      }
      if (action === 'transcribe') {
        STD_TRANSCRIBE_PDFS.push(file.name);
    ...
    function runTriageQueueDone(queue) {
      T_RUNNING = false;
      var hasTranscribe = queue.some(function(q) { return q.action === 'transcribe' && T_STATUS[q.id] === 'done'; });
      loadStudioFolderCounts();
      if (hasTranscribe) {
        closeTriageModal();
        try { setPanel('transcriber'); } catch (e) { /* setPanel may not exist if Studio not yet inited */ }
        try { window.STD_TRANSCRIBE_PDFS = STD_TRANSCRIBE_PDFS.slice(); } catch (e) {}
        console.log('[Studio Triage] Queue complete \u2014 switched to Transcriber for:', STD_TRANSCRIBE_PDFS.join(', '));
      } else {
        var ok = queue.filter(function(q) { return T_STATUS[q.id] === 'done'; }).length;
        var failed = queue.filter(function(q) { return T_STATUS[q.id] === 'failed'; }).length;
        console.log('[Studio Triage] Queue complete: ' + ok + ' ok, ' + failed + ' failed');
        queue.forEach(function(q) { if (T_STATUS[q.id] === 'done') delete T_PICKS[q.id]; });
        renderTriageModal();
      }
    }

    // ── PDF count polling (v1.3.2: source-of-truth aligned with modal) ──
    // Reads scenarioPdfQueueUrl (Sheet rows where status=PDF_PENDING)
    // instead of scenarioCUrl?action=folderCounts (Drive folder count).
    // Banner and modal now share one source — they can never disagree.
    // See file header v1.3.2 changelog and TD-175 in Master Roadmap.
    function loadStudioPdfCount() {
      var CFG = _cfg();
      if (!CFG.scenarioPdfQueueUrl) {
        // No webhook configured — banner stays hidden. Studio still works.
        STD_MONITOR.preprocessing = 0;
        renderTriageBanner();
        return;
      }
      var qs = new URLSearchParams({ titleSlug: CFG.titleSlug });
      fetch(CFG.scenarioPdfQueueUrl + '?' + qs.toString())
        .then(function(res) { if (!res.ok) throw new Error('PDF queue returned ' + res.status); return res.json(); })
        .then(function(data) {
          var arr = Array.isArray(data)
            ? data
            : (Array.isArray(data && data.files) ? data.files : []);
          STD_MONITOR.preprocessing = arr.length;
          renderTriageBanner();
        })
        .catch(function(err) { console.error('[Studio Triage] PDF count error:', err); });
    }

    // Backwards-compat alias — runTriageQueueDone still calls
    // loadStudioFolderCounts() by its old name. Map it to the new fn.
    var loadStudioFolderCounts = loadStudioPdfCount;

    // ── Banner click delegation ──
    document.addEventListener('click', function(e) {
      // v1.3.3: refresh button — re-fire the count fetch.
      if (e.target.closest && e.target.closest('[data-std-triage-refresh]')) {
        e.preventDefault();
        e.stopPropagation();
        triggerBannerRefresh();
        return;
      }
      if (e.target.closest && e.target.closest('[data-std-triage-open]')) {
        openTriageModal();
      }
    });

    // v1.3.3: thin wrapper around loadStudioPdfCount() that
    // toggles T_REFRESHING so the banner can show a spin
    // animation while the fetch is in flight. The underlying
    // load function already calls renderTriageBanner() on
    // resolve, so we just need to render once before fetch
    // (to show the spin) and let the existing path render
    // again after.
    function triggerBannerRefresh() {
      if (T_REFRESHING) return;
      T_REFRESHING = true;
      renderTriageBanner();
      var CFG = _cfg();
      if (!CFG.scenarioPdfQueueUrl) {
      ...
      fetch(CFG.scenarioPdfQueueUrl + '?' + qs.toString())
        ...
        .catch(function(err) {
          console.error('[Studio Triage] Refresh error:', err);
          T_REFRESHING = false;
          renderTriageBanner();
        });
    }
    ...
    // v1.3.5: default active tab is 'components'. renderAssembler() is
    // no longer called unconditionally on mount — only call it if the
    // session-restore put us on Assembler (or there's no restore data
    // and the user will click to Assembler). Components is activated
    // via setPanel() which dispatches std:panel:components.
    if (S.panel === 'assembler') {
      renderAssembler();
      // v1.4.2: assembler init bypasses setPanel, so attach ⓘ directly.
      closeInfoPopover();
      attachHeadingInfo('assembler');
    } else {
      setPanel(S.panel);
    }
    // v1.3.2: kick off PDF-count polling for the triage banner.
    // Initial fire on Studio mount; banner appears if PDFs are
    // already pending. Source: scenarioPdfQueueUrl (same as modal).
    loadStudioPdfCount();
    console.log('\u29BE Studio v' + STD_VERSION + ' mounted \u2014 header Row 1 standardized: .ix-hdr-sub shows live TITLE name, version badge moved from the header row into the \u24D8 info popover (reads STD_VERSION, no longer drifts). Intake default subtab.');
  });
