/* ============================================================
   ...
   Companion stylesheet: ta-asf-v1.1.1.css (Trix inline styling added)
   Reference spec:       v1.1 ASF layout — Chunk 2 of 4
   Studio dependency:    ta-studio v1.3.6+  (popout body editor still
                         used in EDIT mode; create mode uses inline Trix)
   RTE dependency:       Trix (already loaded on T-A page for Transcriber).
                         Assumes window.Trix is available; falls back to
                         the legacy popout button if not.
   ...
   v1.1.1 — CHUNK 2 · Inline body editor for create mode
     ...
     What ships in Chunk 2:
       ...
       • Trix lifecycle:
           - Mount created once per ASF open via initInlineTrix()
             called from a post-render hook (after S.overlay
             innerHTML is set).
           ...
           - trix-change event → updates BOTH S.article.bodyHtml
             AND S.dirtyFields.bodyHtml (so create-payload picks
             up the change AND dirty styling fires).
     ...
     Dependencies:
       • Trix library must be loaded on the page (already true
         for the T-A page since Transcriber uses it). If
         window.Trix is undefined, renderMainBody() falls back
         to the legacy "Edit body" button + console warning.
         No silent failure.
...
(function () {
  ...
  function renderMainBody() {
    if (S.mode === 'create' && typeof window.Trix !== 'undefined') {
      return renderMainBodyInline();
    }
    if (S.mode === 'create' && typeof window.Trix === 'undefined') {
      warn('renderMainBody: Trix is not loaded; falling back to legacy popout button. ' +
           'Load the Trix library on the T-A page (already required by the Transcriber).');
    }
  ...
  function handleAction(action, el, e) {
    switch (action) {
      ...
      // v1.1.1 (Chunk 2) — inline image picker entry point.
      // Chunk 3 will swap the sidebar to picker mode. For now,
      // surface a toast so the path is wired end-to-end.
      case 'open-inline-image-picker':
        toast('Inline image picker — wires up in Chunk 3', 'info');
        return;
  ...
  log('mounted · v' + VERSION + ' · ASF v2 layout — Chunk 2 of 4 (inline Trix body editor in create mode; sidebar picker swap arrives in Chunk 3).');
})();
