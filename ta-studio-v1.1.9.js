...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
      ...
      '<div class="std-root">' +
        '<div class="ix-hdr">' +
          '<div class="ix-hdr-left">' +
            ...
            '</div>' +
          '</div>' +
          '<span class="ix-badge">v1.1.9</span>' +
        '</div>' +
    ...
    function cmpReadItems() {
      ...
      Array.prototype.forEach.call(wraps, function (el) {
        ...
        items.push({
          ...
          role:          (d.componentRole || '').trim(),   // label — hash is resolved via ROLE_HASHES map
    ...
    //
    // ROLE_HASHES: mapping from role label to Webflow option hash.
    // Enabled roles for 6B = the keys. Other 9 roles fall through to
    // the stub toast (Session 8 work). Schema-stable — Webflow
    // Component Role field options are locked project-wide.
    // See memory #29 / #30 for full hash inventory.
    // HC-009 tech-debt log.
    var ROLE_HASHES = {
      'Article Body HTML': '6dcac8d1a82394a5a3a1105b9619fce7',
      'Main Image':        '3861d7e072587ffbeae7b5d535184746',
      'Interior Image':    'db93c6ccb8d4fb2d6cafe1af156ed32a'
    };
    ...
    function cmpHandleAttach(mediaId) {
      ...
      // Role gating — roles not in ROLE_HASHES get a dated stub toast.
      if (!ROLE_HASHES[it.role]) {
    ...
    function cmpFireAttach(mediaId, articleId, it) {
      ...
      // v1.1.9: in-JS hash lookup instead of DOM attribute read.
      var roleHash = ROLE_HASHES[it.role];
      if (!roleHash) {
        // Should never reach here — cmpHandleAttach already gated on this.
        console.error('[TA-STUDIO v1.1.9] No ROLE_HASHES entry for role:', it.role);
      ...
      .catch(function (err) {
        console.error('[TA-STUDIO v1.1.9] Scenario G Route 4 failed:', err);
    ...
    // ── Init ──
    renderAssembler();
    console.log('\u29BE Studio v1.1.9 mounted \u2014 Components tab (read + attach + archive + hard-delete)');
  });
