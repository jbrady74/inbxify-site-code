/* ════════════════════════════════════════════════════════════════
   ta-cadence-board-v0.5.2.js
   INBXIFY T-A — Cadence Board · SKELETON
   Companion: ta-cadence-board-v0.5.2.css (matched pair — load both)

   ── v0.5.2 — CONTROLS COLLAPSE TO ICONS ──
...
(function () {
  ...
  function metaOf(m) {
    var fd = m.fieldData || {}, out = [], k = mediaTypeOf(fd);
    var sz = sizeOf(fd); if (sz) out.push(esc(sz));
    if (k === 'image') {
      var d = dimsOf(fd);
      if (d) out.push(d.low
        ? '<span class="ixcb-lo">' + d.w + '\u00d7' + d.h + ' \u00b7 low-res</span>'
        : d.w + '\u00d7' + d.h);
    }
    if (k === 'text') {
      var html = String(fd['html-content'] || '');
      if (html) {
        var words = html.replace(/<[^>]*>/g, ' ').trim().split(/\s+/).length;
        if (words > 1) out.push(words + ' words');
      }
    }
...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  function bundleRow(bun) {
    ...
    var mix = Object.keys(counts).map(function (L) { return counts[L] + ' ' + L; }).join(' \u00b7 ');
    ...
    var out =
      '<div class="ixcb-r ixcb-r-fold' + (allOn ? ' ixcb-r-sel' : '') + (openB ? ' ixcb-open' : '') +
        ...
        '<span class="ixcb-n"><span class="ixcb-n-t">' + esc(bun.label) + '</span>' +
          '<span class="ixcb-n-m">' + bun.items.length +
            (bun.items.length === 1 ? ' file \u00b7 ' : ' files \u00b7 ') + esc(mix) + '</span></span>' +
      '</div>';
    if (openB) {
      ...
      out +=
        '<div class="ixcb-g' + (shut ? ' ixcb-shut' : '') + '">' +
          '<button type="button" class="ixcb-g-h" data-ixcb="group" data-id="' + esc(g.key) + '">' +
            ...
            (i === 0 ? '<span class="ix-needs-you">NEWEST</span>' : '') +
  ...
  function colBody(col) {
    if (col.id === 'components') return statusChips() + '<div class="ixcb-lane">' + componentsBody() + '</div>';
    ...
    try {
      window.localStorage.setItem(VIEW_KEY, JSON.stringify({
        ...
        /* q and typeFilter are deliberately NOT persisted — see the
           v0.5.2 note. A question asked last session must not silently
           filter this one. */
      }));
    } catch (e) {}
  }
