// ============================================================
// ta-studio-v1.2.8.js
// INBXIFY TA Studio — Phase 4 / TD-160 / S11.5b Wave 1
// Migrated all button class emits to additive ix-btn--*.
//
// Mounts into #studio-mount on the T-A page.
//
// ──────────────────────────────────────────────────────────
// v1.2.8 (Session 11.5b Wave 1 — TD-160 button system migration):
//
//   Additive class migration. Every <button> and link-button emit
//   now carries BOTH the legacy class AND the new ix-btn ix-btn--*
//   classes per the migration table in TD-160-Button-Audit.docx.
//   The ix-buttons-v1.0.0.css module wins via specificity + !important
//   so visual rendering is determined by the new variants.
//   Legacy classes preserved through migration so:
//     (a) any consumer still emitting old class names keeps working
//     (b) S11.5c can sweep them out after all four files migrated
//
//   Migrated class emit sites (15 total):
//     - asm-back-btn × 2          → ix-btn ix-btn--ghost
//     - asm-cancel-link           → ix-btn ix-btn--ghost
//     - asm-reset-btn             → ix-btn ix-btn--secondary
//     - asm-save-btn              → ix-btn ix-btn--primary
//     - asm-link-btn × 2 (Replace, Pick Cancel) → ix-btn ix-btn--ghost
//     - cmp-btn cmp-btn--primary × 4 (Main/Interior attach,
//                Components attach, picker confirm) → ix-btn ix-btn--primary
//     - cmp-btn (default — Archive) → ix-btn ix-btn--secondary
//     - cmp-clear-all             → ix-btn ix-btn--ghost
//     - cmp-empty-clear           → ix-btn ix-btn--ghost
//     - cmp-attach-cancel         → ix-btn ix-btn--ghost
//     - cmp-drawer-close          → ix-btn ix-btn--ghost ix-btn--icon
//
//   Dynamic class strings (attachBtnCls, confirmCls) updated to
//   always include ix-btn ix-btn--primary. Native [disabled] attribute
//   handles the disabled visual state via the module's :disabled rules.
//
//   Companion file: ta-studio-components-v1.0.8.css strips the
//   v1.0.7 .cmp-btn--primary specificity-bump !important armor,
//   no longer needed since the module wins the cascade.
//
//   Webflow head deploy:
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderOpenAssembler(root) {
      ...
      if (!a) {
        root.innerHTML = '<div class="asm-results-empty">Article not found. It may have been removed, or its CMS data hasn\u2019t loaded yet.</div>' +
                        '<div style="text-align:center;margin-top:12px"><button class="asm-back-btn ix-btn ix-btn--ghost" id="asm-back">\u2190 Back to picker</button></div>';
      ...
      h += '<button class="asm-back-btn ix-btn ix-btn--ghost" data-asm-back-btn>\u2190 Back to picker</button>';
    ...
    function renderSaveBar(dirtyCount) {
      ...
      h += '<a class="asm-cancel-link ix-btn ix-btn--ghost" data-asm-cancel href="javascript:void(0)">\u2715 cancel \u2014 close without saving</a>';
      ...
      h += '<button class="asm-reset-btn ix-btn ix-btn--secondary" data-asm-reset' + (dirtyCount === 0 ? ' disabled' : '') + '>\u21BB Reset</button>';
      h += '<button class="asm-save-btn ix-btn ix-btn--primary" data-asm-save' + (dirtyCount === 0 ? ' disabled' : '') + '>';
    ...
    function renderCol2(a) {
      ...
      if (imgUrl) {
        ...
        if (!pickerOpenForMain) {
          h += '<div class="asm-img-slot-actions">' +
                 '<button type="button" class="asm-link-btn ix-btn ix-btn--ghost" data-asm-attach-open="mainImage">Replace</button>' +
               '</div>';
        }
      } else {
        ...
        if (!pickerOpenForMain) {
          h += '<div class="asm-img-slot-actions">' +
                 '<button type="button" class="cmp-btn cmp-btn--primary ix-btn ix-btn--primary" data-asm-attach-open="mainImage">Attach Main Image</button>' +
               '</div>';
        }
      }
      // Editable alt text
      ...
      if (!pickerOpenForInterior) {
        h += '<div class="asm-img-slot-actions">' +
               '<button type="button" class="cmp-btn cmp-btn--primary ix-btn ix-btn--primary" data-asm-attach-open="interiorImage">+ Attach Interior Image</button>' +
             '</div>';
      }
      if (pickerOpenForInterior) {
    ...
    function cmpRenderFilterBar() {
      ...
      html += '<button type="button" class="cmp-clear-all ix-btn ix-btn--ghost" data-cmp-clear-all>Clear all</button>';
        ...
        if (empty) {
          ...
          empty.innerHTML =
            '<div class="cmp-empty-inner">' +
              ...
              '<button type="button" class="cmp-empty-clear ix-btn ix-btn--ghost" data-cmp-clear-all>Clear all filters</button>' +
            '</div>';
        }
        return;
      }
    ...
    function cmpRenderCardDefault(it) {
      ...
      var attachBtnCls = isAttached ? 'cmp-btn cmp-btn--disabled ix-btn ix-btn--primary' : 'cmp-btn cmp-btn--primary ix-btn ix-btn--primary';
      ...
      return '' +
        '<div class="cmp-card' + (isArchived ? ' cmp-card--archived' : '') + '" data-cmp-card="' + esc(it.mediaId) + '">' +
          ...
          '<div class="cmp-card-actions">' +
            ...
            '<button type="button" class="cmp-btn ix-btn ix-btn--secondary" data-cmp-archive>Archive</button>' +
        ...
        '<div class="cmp-card cmp-card--editing' + (isArchived ? ' cmp-card--archived' : '') + (hasFailed ? ' cmp-card--attach-failed' : '') + '" data-cmp-card="' + esc(it.mediaId) + '">' +
          ...
          '<div class="cmp-attach-edit">' +
            ...
            '<div class="cmp-attach-actions">' +
              '<button type="button" class="cmp-btn cmp-btn--primary ix-btn ix-btn--primary" data-cmp-attach-confirm' + attachBtnDisabled + '>' + esc(attachBtnLabel) + '</button>' +
              '<button type="button" class="cmp-attach-cancel ix-btn ix-btn--ghost" data-cmp-attach-cancel>Cancel</button>' +
            '</div>' +
          '</div>' +
        '</div>';
    }
    ...
    function renderAsmPicker(article, slot) {
      ...
      var confirmCls = 'cmp-btn cmp-btn--primary ix-btn ix-btn--primary' + (canConfirm ? '' : ' cmp-btn--disabled');
      var confirmDis = canConfirm ? '' : ' disabled';

      h += '<div class="asm-pick-footer">';
      h +=   '<button type="button" class="asm-link-btn ix-btn ix-btn--ghost" data-asm-pick-cancel>Cancel</button>';
    ...
    function cmpRenderDrawer() {
      ...
      drawer.innerHTML =
        '<div class="cmp-drawer-header">' +
          ...
          '<button type="button" class="cmp-drawer-close ix-btn ix-btn--ghost ix-btn--icon" aria-label="Close">\u2715</button>' +
        '</div>' +
        preview +
    ...
    console.log('\u29BE Studio v1.2.8 mounted \u2014 Components tab (read + attach + archive) + Assembler field-level Attach (Main Image, Interior Images) + auto-reload + state restore + Mode 1 dual-write payload + RTE article-link + defensive parse + TD-160 button system (additive ix-btn migration)');
  });
