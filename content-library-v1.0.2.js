// ============================================================
// content-library-v1.0.2.js
// INBXIFY Content Library — T-A Page Tab
// Mounts into #content-library-mount on ContentLibrary tab
// Config from window.TA_CONFIG
//
// v1.0.2: Fix selector — wrapper class IS the data-item element,
//         not a parent. Removed descendant space from all
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function readCmsItem(el, fallbackCollection) {
    var productId = (el.dataset.productId || '').trim() || null;
    // Resolve collection and mnlsAbbr from product if available
    var resolvedCollection = fallbackCollection;
    var mnlsAbbr = '';
    if (productId) {
      // Products may not be loaded yet at read time, so also store
      // productId for deferred resolution in init()
    }
    return {
      ...
      mnlsAbbr: (el.dataset.abbr || '').trim().toUpperCase() || '',
