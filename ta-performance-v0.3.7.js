...
(function () {
  ...
  // Accept BOTH shapes: the spec envelope ({v,sources:{...}}) written by
  // the ingest, AND a raw single-report parser output pasted directly
  // (source:'nexstar', delivered, links, items...). Raw gets auto-wrapped
  // so hand-tests never require manual JSON surgery.
  function normalizeStats(j) {
    if (!j || typeof j !== 'object') return j;
    if (j.sources) return j;                        // already spec shape
    if (j.source === 'nexstar' && (j.links || j.items)) {
      return { v: 1, sources: { nexstar: [j] }, settings: { nexstarMultiplier: 1.0 } };
    }
    return j;                                       // unknown shape → shape? pill
  }
  ...
  function readNewsletters() {
    ...
    els.forEach(function (el) {
      ...
      var stats = null;
      // JSON div may be INSIDE the wrapper or its SIBLING within the
      // Collection Item (live layout 2026-07-30 is sibling). Check both.
      ...
      // Live bindings (confirmed 2026-07-30): data-nl-id, data-nl-slug,
      // data-nl-issue-no, data-nl-date, data-nl-status, data-nl-name,
      // data-perf-updated. Generic fallbacks retained for portability.
  ...
  function rowNl(nl, idx) {
    ...
    // Name field is authoritative (multi-tenant: never manufacture a
    // title-prefixed label). Constructed fallback only if name empty.
...
/* End of ta-performance-v0.3.7.js */
