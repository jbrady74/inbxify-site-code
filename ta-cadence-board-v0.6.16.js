...
(function () {
  ...
  var S = {
    ...
    slateFilter: 'all',  // 'all' | 'unbuilt' | 'unassigned' | 'notfornl'
...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  /* ── Next bar ──
     Gold strip under the app bar. Derived, never typed: the issue and
     page count come from the manifest itself, the last-built line
     from the newest plan the board already harvested. */
  function nextBar() {
    ...
    var last = null;
    for (var j = 0; j < S.plans.length; j++) {
      var p = S.plans[j];
      if (!last || (p.issueNo || '') > (last.issueNo || '')) last = p;
    }

    var t = n ? (src || 'Print issue') : 'No manifest';
    var sub = n
      ? (pages ? pages + 'pp \u00b7 ' : '') + n + ' item' + (n === 1 ? '' : 's') + ' read'
      : 'Run the parse to read a print issue.';
    var lastTxt = last
      ? 'Last built ' + (last.name || last.issueLabel || '\u2014')
      : 'No issues built yet';

    return '<div class="ixcb-nextbar">' +
             '<span class="ixcb-nb-lbl">Next</span>' +
             '<span class="ixcb-nb-t">' + esc(t) + '</span>' +
             '<span class="ixcb-nb-s">' + esc(sub) + ' \u00b7 ' + esc(lastTxt) + '</span>' +
             '<span class="ixcb-nb-sp"></span>' +
             '<button type="button" class="ix-btn ix-btn--primary ixcb-nb-btn" disabled ' +
               'title="Arrives with the write scenario">Assign issues</button>' +
           '</div>';
  }

  /* ── Hero: cover + setup gate ──
     THE NUMBERING IS DERIVED. The mockup's own comment is explicit —
     nothing about it is hardcoded, it runs on from the last send. So
     the next issue number is read from the highest plan the board
     harvested, and when there is none the gate says so instead of
     inventing a starting point. */
  function nextIssueNo() {
    var hi = 0;
    for (var i = 0; i < S.plans.length; i++) {
      var v = parseInt(S.plans[i].issueNo || '', 10);
  ...
  function heroHtml() {
    ...
    var opts = function (list, sel) {
      ...
      for (var i = 0; i < list.length; i++) {
        o += '<option value="' + esc(list[i][0]) + '"' +
             (list[i][0] === sel ? ' selected' : '') + '>' + esc(list[i][1]) + '</option>';
      }
      return o;
    };
    ...
    return '<div class="ixcb-hero">' +
      '<div class="ixcb-cover">' +
        '<div class="ixcb-cover-ph">Cover</div>' +
      ...
      '<div class="ixcb-setup">' +
        '<div class="ixcb-gate">' +
          '<div class="ixcb-ref">' +
            (S.plans.length
              ? 'Last built <b>' + esc(lastPlanLabel()) + '</b>'
              : 'No previous issue on record') +
          '</div>' +
          '<h3 class="ixcb-gate-h">' +
            (nxt ? 'Set up issues from WLN-' + nxt : 'Set up issues') + '</h3>' +
          '<div class="ixcb-ctrls">' +
            '<label>Split into</label>' +
            '<select class="ix-picker-input" disabled>' +
              opts([['2','2 issues'],['1','1 issue'],['3','3 issues']], '2') + '</select>' +
            '<label>Editions each</label>' +
            '<select class="ix-picker-input" disabled>' +
              opts([['2','2 (A/B)'],['1','1'],['3','3 (A\u2013C)'],['5','5 (A\u2013E)']], '2') + '</select>' +
            '<label>First send</label>' +
            '<input type="date" class="ix-picker-input" disabled>' +
            '<select class="ix-picker-input" disabled>' +
              opts([['7','weekly'],['14','every 2 weeks']], '7') + '</select>' +
            '<span class="ixcb-ctrls-sp"></span>' +
            '<button type="button" class="ix-btn ix-btn--primary" disabled ' +
              'title="Arrives with PubPlan multi-create">Generate</button>' +
          '</div>' +
          '<div class="ixcb-gate-ro">' +
            (nxt
              ? 'Read-only. Numbering runs on from the last published plan \u2014 it is never typed.'
              : 'No published PUBLICATION PLAN carries an issue number, so the run-on cannot be ' +
                'derived. Bind data-issue-number on pubplan-item.') +
          '</div>' +
        '</div>' +
      '</div>' +
    '</div>';
  ...
  function lastPlanLabel() {
    var last = null;
    for (var i = 0; i < S.plans.length; i++) {
      var p = S.plans[i];
      if (!last || (p.issueNo || '') > (last.issueNo || '')) last = p;
    }
    return last ? (last.name || last.issueLabel || '\u2014') : '\u2014';
  }
