...
(function () {
  ...
  var SYSTEM_PROMPT = [
    ...
    '- If the body already contains <h2> headers throughout, return an empty array.',
    '',
    'ALTERED RULES:',
    '- "altered" is a JSON array of plain strings. The only valid values are "title" and "subtitle".',
    '- Include a field name ONLY if you shortened that field from a longer SOURCE METADATA TABLE value.',
    '- If you shortened nothing, return an empty array.',
    '- Correct shapes: [] · ["subtitle"] · ["title","subtitle"]',
    '- Never put objects, explanations, or nested arrays inside "altered" — plain strings only.'
  ].join('\n');
  ...
  function insertHeaders(html, headers) {
    var out = { html: html, inserted: 0, missed: [] };
    if (!Array.isArray(headers) || !headers.length) return out;
    ...
    for (var h = 0; h < headers.length && added < MAX_HEADERS; h++) {
      ...
      if (idx === -1) { out.missed.push(text); continue; }
      ...
      if (blocks[idx].tag === 'h2' || (prev && String(prev.tagName).toLowerCase() === 'h2')) continue;
    ...
    if (added) { out.html = root.innerHTML; out.inserted = added; }
    return out;
  }
  ...
  function fromBody(bodyHtml, sourceMeta) {
    ...
    return postMessages(payload).then(function (raw) {
      ...
      var ins  = insertHeaders(html, (out && out.headers) || []);
      var body = ins.inserted ? ins.html : String(bodyHtml == null ? '' : bodyHtml);
      if (dbg()) {
        console.log('[InbxGenerate] headers inserted: ' + ins.inserted +
  ...
  window.InbxGenerate = { fromBody: fromBody, _model: ANTHROPIC_MODEL, _version: '1.1.0' };
