...
<div class="cm-root">
  ...
  function archiveSelected() {
    ...
    // Optimistic UI — move to ARCHIVED immediately
    sel.forEach(f => {
      f.status = 'ARCHIVED';
      f.selected = false;
      f.elementType = '';
    ...
    if (CFG.makeArchive) {
      fetch(CFG.makeArchive, {
        ...
        if (!res.ok) console.error('[CP] Archive webhook returned', res.status);
        else console.log('[CP] Archived', fileIds.length, 'files');
      }).catch(err => {
        console.error('[CP] Archive error:', err);
      });
    } else {
      console.warn('[CP] No makeArchive URL configured — local status updated only');
    }
  }
