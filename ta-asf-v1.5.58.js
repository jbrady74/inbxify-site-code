...
(function () {
  ...
  var VERSION = '1.5.58';   // v1.5.31 — was stuck at 1.5.26 through .27-.30
  ...
  var CFG = {
    ...
    sectionFields: {
      ...
      gallery:     ['galleryMediaIds', 'galleryHeroMediaId']
    },
  ...
  var ARTICLE_HYDRATE_MAP = {
    ...
    // module 40 already appends to). galleryHeroMediaId is deliberately
    // NOT mapped: no ARTICLES slug is confirmed for it, so the verifier
    // reports it unverifiable rather than pretending it persisted.
  ...
  function hydrateArticle(articleId) {
    ...
    /* ── v1.5.58 · an article renders MORE THAN ONCE on the page ────────
       Measured: 200 .articles-wrapper elements for 95 unique article ids,
       and only 8 carried .article-flag-photo-essay against 8 photo
       essays. So each article has ~2 instances and only ONE of them
       carries the flag sentinels and the full field set.

       querySelector returns the FIRST match. When that happened to be the
       stripped instance, every flag hydrated OFF — a Photo Essay opened
       as "Std". Worse, clicking "Std" to correct it was a no-op:
       handleTypeChange returns early when the clicked type already equals
       S.articleType, so photoEssay was never set false and the CMS kept
       its old value. The operator could not fix it by hand.

       Now: collect EVERY instance. Use the richest one as the primary
       (the one carrying the body RTE, else the most data attributes),
       and read flags across ALL instances — Webflow omits a sentinel
       entirely when its switch is off, so "present in any instance"
    ...
    var el = els[0], bestScore = -1, ei;
    for (ei = 0; ei < els.length; ei++) {
      var cand  = els[ei];
      var score = 0;
      if (cand.querySelector('.article-body-source')) score += 100;
      try { score += Object.keys(cand.dataset || {}).length; } catch (e) {}
      score += cand.querySelectorAll('[class*="article-flag-"]').length * 10;
      if (score > bestScore) { bestScore = score; el = cand; }
    }
    if (els.length > 1) {
      log('hydrateArticle: ' + els.length + ' wrapper instances for ' + articleId +
          ' \u2014 using the richest (score ' + bestScore + ')');
    }
    var d = el.dataset || {};

    // Rich Text body — cannot bind to data-attribute in Webflow, so
    // we read innerHTML from the .article-body-source RTE element.
    // v1.5.58 — fall back across instances if the primary lacks the RTE.
    var bodyEl = el.querySelector('.article-body-source');
    if (!bodyEl) {
      for (var bi = 0; bi < els.length && !bodyEl; bi++) {
        bodyEl = els[bi].querySelector('.article-body-source');
      }
    }
    ...
    function flagOn(cls) {
      // v1.5.58 — scan every instance, not just the primary. A sentinel
      // present in ANY instance means the switch is on.
      for (var k = 0; k < els.length; k++) {
        var n = els[k].querySelector('.' + cls);
        if (!n) continue;
        if (String(n.className || '').indexOf('w-condition-invisible') > -1) continue;
        try { if (n.style && n.style.display === 'none') continue; } catch (e) {}
        return true;
      }
      return false;
    }
    ...
    // flags. Before this, S.articleType always opened as 'standard'
    // regardless of what the article actually was.
    S.articleType = photoEssay ? 'photo-essay' : (videoArticle ? 'video' : 'standard');
  ...
  function applyGalleryHero(mediaId) {
    ...
    var m = null;
    for (var i = 0; i < all.length; i++) {
      if (all[i].mediaId === mediaId) { m = all[i]; break; }
    }
    if (!m) {
      warn('applyGalleryHero: MEDIA not found in DOM', mediaId);
      toast('That image is no longer in MEDIA', 'error');
      return false;
    }
    if (!setMainImageFromMedia(m)) return false;
    setFieldValue('galleryHeroMediaId', mediaId);
    return true;
  }

  function clearGalleryHero() {
    setFieldValue('galleryHeroMediaId', '');
    if (S.article) {
  ...
  function autofillGalleryFromBundle() {
    ...
    // The rail's auto-assigned Main becomes the hero if it's in the
    // set; otherwise the first bundle image. applyGalleryHero routes
    // through setMainImageFromMedia, so main-image and its derived
    // transforms stay consistent either way.
    var mainId = (S.article && S.article.mainImageMediaId) || '';
    var heroId = (mainId && picked.indexOf(mainId) !== -1) ? mainId : picked[0];
    applyGalleryHero(heroId);
    return true;
  }
  ...
  function renderSideS8Gallery() {
    ...
    var heroId = (S.article && S.article.galleryHeroMediaId) || '';
    var all   = hydrateAllMedia();
    var byId  = {};
        ...
        '<div class="asf-gal-tile' + (isHero ? ' is-hero' : '') + (m ? '' : ' is-missing') + '"' +
          ...
          '<div class="asf-gal-tools">' +
            ...
            '<button type="button" class="asf-gal-btn asf-gal-btn--hero' + (isHero ? ' on' : '') + '"' +
              ...
              ' title="' + (isHero ? 'Hero image (also the newsletter / card image)' : 'Make this the hero') + '">' +
              (isHero ? '\u2605' : '\u2606') +
            '</button>' +
            '<button type="button" class="asf-gal-btn asf-gal-btn--move"' +
              ' data-asf-action="gallery-move" data-asf-gal-id="' + esc(id) + '"' +
  ...
  function handleAction(action, el, e) {
    switch (action) {
      ...
      case 'picker-confirm-gallery': {
        ...
        // First image in becomes the hero by default — the operator
        // can restar any tile. Avoids the "no hero" state on the
        // common path without ever silently reassigning an existing one.
        if (!S.article.galleryHeroMediaId && cgNext.length) {
          applyGalleryHero(cgNext[0]);
        }
        S.inlinePicker.selectedIds = [];
        S.inlinePicker.open        = false;
        render();
      ...
      case 'gallery-remove': {
        ...
        // Removing the hero clears the main image rather than silently
        // promoting a neighbour — a hero reassignment must be a
        // deliberate act, since it also changes the OG image.
        if ((S.article.galleryHeroMediaId || '') === grId) {
          clearGalleryHero();
          if (grNext.length) {
            toast('Hero removed \u2014 star another image', 'info');
          }
        }
        render();
        return;
      ...
      case 'gallery-set-hero': {
        ...
        if (!ghId) return;
        if ((S.article.galleryHeroMediaId || '') === ghId) return;   // already hero
  ...
  var GALLERY_PAYLOAD_KEYS = { galleryMediaIds: 1, galleryHeroMediaId: 1 };
  ...
  function commitCreateArticle() {
    ...
    var payload = {
      ...
      fields: {
        ...
        galleryHeroMediaId: (S.article && S.article.galleryHeroMediaId) || '',
