/* ════════════════════════════════════════════════════════════════
   ta-cadence-board-v0.6.21.js
   INBXIFY T-A — Cadence Board · SKELETON
   Companion: ta-cadence-board-v0.6.21.css — INDEPENDENTLY VERSIONED.
...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  function openIssues() {
    ...
    for (var i = 0; i < S.plans.length; i++) {
      var p = S.plans[i];
      var key = p.issueNo || p.name;
      if (!key || seen[key]) continue;
      seen[key] = 1;
      out.push({ key: key, label: p.issueLabel || p.name || key });
    }
  ...
  function lastSentLabel() {
    var p = lastSentPlan();
    return p ? (p.issueLabel || p.name || '\u2014') : null;
  }
  ...
  function editionsOf(no) {
    ...
    for (var i = 0; i < S.plans.length; i++) {
      ...
      var lbl = (S.plans[i].issueLabel || S.plans[i].name || '');
      var m = /(\d+)\s*([A-Z])\b/.exec(lbl);
      if (m && out.indexOf(m[2]) < 0) out.push(m[2]);
    }
    return out.sort();
  }
  ...
  function heroHtml() {
    ...
    var hiLbl    = hi ? (hi.issueLabel || hi.name || '') : '';
    ...
    return '<div class="ixcb-hero">' +
      ...
      '<div class="ixcb-setup">' +
        '<div class="ixcb-gate">' +
          ...
          '</div>' +
          '<h3 class="ixcb-gate-h">Set up issues</h3>' +
