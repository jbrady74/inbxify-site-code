...
document.addEventListener('DOMContentLoaded', function () {
  ...
  style.textContent = [
    ...
    '.cl-status-row{display:flex;align-items:center;gap:6px;padding:8px 0;margin-bottom:0;flex-wrap:wrap}',
    ...
    '.cl-modal-overlay{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(26,58,58,.35);z-index:10000;display:flex;align-items:flex-start;justify-content:center;padding-top:80px}',
    '.cl-modal{background:#fff;border:2px solid #c4a35a;border-radius:6px;width:520px;max-width:95vw;max-height:80vh;overflow-y:auto;font-family:"DM Sans",sans-serif;color:#1a3a3a}',
    '.cl-modal-bar{height:4px;background:linear-gradient(90deg,#1a3a3a,#c4a35a);border-radius:4px 4px 0 0}',
    ...
    '.cl-modal-close{font-family:"DM Mono",monospace;font-size:10px;color:#c0392b;background:none;border:none;cursor:pointer;padding:2px}',
    '.cl-modal-close:hover{opacity:.7}',
  ...
  function render() {
    ...
    if (S.viewingId) {
      ...
      if (vItem) {
        ...
        h += '<div class="cl-modal-overlay" onclick="_clCloseView()">';
        h += '<div class="cl-modal" onclick="event.stopPropagation()">';
        h += '<div class="cl-modal-bar"></div>';
        h += '<div class="cl-modal-body">';

        // Header
        h += '<div class="cl-modal-hdr"><div>';
        h += '<div class="cl-modal-title">' + esc(vItem.name) + '</div>';
        h += '<div class="cl-modal-meta">';
        h += '<span style="display:inline-flex;align-items:center;gap:4px"><span class="cl-dot ' + vDotClass + '"></span> ' + esc(vItem.status) + '</span>';
        if (vMnls) h += '<span>' + esc(vMnls.abbr) + '</span>';
        if (vItem.productName) h += '<span>' + esc(vItem.productName) + '</span>';
        h += '</div></div>';
        h += '<button class="cl-modal-close" onclick="_clCloseView()">close</button>';
        h += '</div>';

        // Info grid
        h += '<div class="cl-modal-grid">';
        h += '<div><div class="cl-modal-label">Customer</div><div class="cl-modal-val">' + (vItem.customerName ? esc(vItem.customerName) : '<span style="color:#c4c0b0">-</span>') + '</div></div>';
        h += '<div><div class="cl-modal-label">Assignment</div><div style="font-family:\'DM Mono\',monospace;font-size:10px;font-weight:500">';
        if (vItem.pubplanName && vItem.slot) {
          h += esc(vItem.pubplanName) + ' | ' + esc(vItem.slot);
        } else {
          h += '<span style="color:#c4a35a">available</span>';
        }
        h += '</div></div>';
        h += '<div><div class="cl-modal-label">Section</div><div class="cl-modal-val">' + (vItem.mnlsName || (vMnls ? esc(vMnls.label) : '-')) + '</div></div>';
        h += '<div><div class="cl-modal-label">Revenue type</div><div style="font-family:\'DM Mono\',monospace;font-size:10px">' + (vItem.productName ? esc(vItem.productName) : '-') + '</div></div>';
        h += '</div>';

        // Elements section (placeholder — no element data on DOM yet)
        h += '<div class="cl-modal-section">Elements</div>';
        if (vItem.assetCount > 0) {
          h += '<div style="font-family:\'DM Mono\',monospace;font-size:10px;color:#8a8a7a;margin-bottom:8px">' + vItem.assetCount + ' file' + (vItem.assetCount > 1 ? 's' : '') + ' attached</div>';
        } else {
          h += '<div style="font-family:\'DM Mono\',monospace;font-size:10px;color:#a0a090;margin-bottom:8px">No elements attached</div>';
        }
        ...
        if (vItem.assetCount > 0) h += '<span style="font-family:\'DM Mono\',monospace;font-size:9px;color:#a0a090">' + vItem.assetCount + ' element' + (vItem.assetCount > 1 ? 's' : '') + '</span>';
        h += '</div></div>';

        h += '</div></div></div>'; // modal-body, modal, overlay
      }
    }
