...
document.addEventListener('DOMContentLoaded', function () {
  ...
  style.textContent = [
    ...
    '.cl-tabrow{display:flex;align-items:flex-end;justify-content:space-between;gap:20px;margin-bottom:16px}',
    '.cl-tabrow .cl-mtabs.std-subtabs{margin-bottom:0;flex:0 1 auto}',
    ...
    '.cl-seg button.on{background:#142e2e;color:#fff;font-weight:600}',
  ...
  function render() {
    ...
    function mtab(key, label, ct) {
      var on = S.mainTab === key;
      return '<button class="ix-btn ix-btn--tab cl-mtab" data-active="' + (on ? 'true' : 'false') + '" onclick="_clSetMain(\'' + key + '\')">' +
             esc(label) + '<span class="tc">' + ct + '</span></button>';
    }
    h += mtab('articles', 'Articles', artCt);
    h += mtab('ads', 'Ads', adCt);
    h += mtab('events', 'Events', evCt);
    h += mtab('realestate', 'Real Estate', reCt);
    h += '</div>';
