...
document.addEventListener('DOMContentLoaded', function () {
  ...
  style.textContent = [
    ...
    '.cl-thead{display:grid;grid-template-columns:minmax(180px,1.4fr) 140px 200px 60px;gap:0;border-bottom:2px solid #1a3a3a;padding:6px 0;align-items:end}',
    ...
    '.cl-th-sub{font-size:8px;color:#a0a090;letter-spacing:.03em;text-transform:none;margin-top:1px}',
    ...
    '.cl-card-main{display:grid;grid-template-columns:minmax(180px,1.4fr) 140px 200px 60px;gap:0;align-items:center;padding:11px 0;cursor:pointer}',
    ...
    '.cl-cust{font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
    ...
    '.cl-assign-mnls{font-size:11px;color:#5a6a5a}',
    '.cl-assign-slot{font-family:"DM Mono",monospace;font-size:10px;padding:1px 5px;border-radius:2px;background:#1a3a3a;color:#f0edd8;display:inline-block}',
  ...
  function render() {
    ...
    h += '</div><span class="cm-badge">v1.0.16</span></div>';
    ...
    h += '<div class="cl-th"><div>Assignment</div><div class="cl-th-sub">Newsletter / Section / Slot</div></div>';
    ...
    if (!items.length) {
      ...
      items.forEach(function (item) {
        ...
        if (item.pubplanName && item.slot) {
          ...
          h += '<div class="cl-assign-issue">' + esc(item.pubplanName) + (assignDate ? ' &nbsp;|&nbsp; ' + esc(assignDate) : '') + '</div>';
          var mnlsLabel = item.mnlsName || (mnls ? mnls.label : '');
          h += '<div style="display:flex;gap:6px;align-items:center;margin-top:2px">';
          if (mnlsLabel) h += '<span class="cl-assign-mnls">' + esc(mnlsLabel) + '</span>';
          h += '<span class="cl-assign-slot">' + esc(item.slot) + '</span>';
          h += '</div>';
        } else {
        ...
        // Col 4: Edit button
        h += '<div class="cl-cell"><button class="cl-btn" onclick="event.stopPropagation();_clStartEdit(\'' + item.id + '\')">' + (isAssigned ? 'View' : 'Edit') + '</button></div>';
        ...
        if (isEd) {
          ...
          PUBPLANS.forEach(function (pp) {
            // Planning Status hashes (Webflow Option field):
            //   In Progress = cbad0e93deea996b0769d71d808d59c5
            //   Locked      = e13edebeb1ba9d07a0f7f000786e0586
            if (pp.planningStatus && pp.planningStatus !== 'cbad0e93deea996b0769d71d808d59c5') return;
