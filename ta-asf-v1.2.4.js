/* ============================================================
   ...
   Companion stylesheet: ta-asf-v1.2.4.css
   Reference spec:       v1.2 ASF — assetType branching; Article +
                         Real Estate + Event + Ad variants
...
(function () {
  ...
  function commitCreateRE() {
    ...
    var payload = {
      ...
      fields: {
        ...
        // Property image (generic main-image keys)
        mainImageMediaId:   a.mainImageMediaId || '',
        mainImageSrc:       a.mainImageSrc     || '',
      ...
      return '' +
        '<div class="asf-field asf-body-field" style="margin-top:8px;">' +
          '<div class="asf-field-label-row">' +
            '<label class="asf-field-label">Event description <span class="req">*</span>' +
  ...
  function renderEVMain() {
    return '' +
      ...
      '<div class="asf-re-grid">' +
        renderField({
          label: 'Location', field: 'eventLocation', req: true,
  ...
  function commitCreateEV() {
    ...
    var payload = {
      ...
      fields: {
        ...
        availabilityStatus: curVal('availabilityStatus') || 'Available',
        // Event image (hero) — generic main-image keys
        mainImageMediaId:   a.mainImageMediaId || '',
        mainImageSrc:       a.mainImageSrc     || '',
        mainImageAlt:       curVal('mainImageAlt') || '',
        // Social / OG image (promo) — OG-semantic names
        ogImageMediaId:     a.ogImageMediaId || '',
        ogImageSrc:         a.ogImageSrc     || '',
        // Refs
        customerId:         a.customerId   || '',
        customerName:       a.customerName || '',
        newsletterId:       a.newsletterId   || '',
  ...
  function blankADS() {
    ...
    return {
      ...
      customerId:         '',
      customerName:       '',
      // Customer-sourced redirect default (Business Website). ASF does
      // not yet hydrate Customer detail; surfaced only if a caller
      // prefills it, else the sidebar shows a "from Customer record"
      // placeholder.
      customerWebsite:    '',
  ...
  function renderADSMain() {
      ...
      '<div class="asf-ads-section">' +
        ...
        '<div class="asf-cust-profile-ref">' +
          '<div class="asf-cust-profile-ref-head">' +
            ...
            '</button>' +
          '</div>' +
          '<div class="asf-ads-note">The full profile \u2014 tagline, description, address, contact, socials, services, logo \u2014 lives on the Customer record (read-only here). Only the three creative fields below are part of the ad.</div>' +
        '</div>' +
        renderField({
    ...
    var defaultArtAd =
      '<div class="asf-ads-refblock">' +
        ...
        '</div>' +
        '<div class="asf-ads-note">Customer \u00b7 <code>Default ART AD \u2013 GET</code> (ULC link) \u2014 the advertiser\u2019s standing default creative, used if no other ad exists. Read-only reference.</div>' +
      '</div>';

    return '' +
      '<div class="asf-re-headline">' +
        renderField({
          label: 'Ad name', field: 'adName', req: true,
          value: curValRead('adName'),
          placeholder: 'Advertiser name'
        }) +
      '</div>' +
      '<div class="asf-fallback-hint" style="margin:-2px 0 12px;">Convention: same as the Customer name \u2014 auto-filled when you pick the advertiser below.</div>' +
      ...
      banner + sidebar + theFind + lbp + defaultArtAd;
  }
  ...
  // AD create commit → Scenario 104 (assetType:'ad'). Banner via the
  // generic main-image keys, Sidebar via the og-image keys — Scenario
  // 104's ad route maps mainImage* → banner-ad-700x200 and ogImage* →
  // sidebar-ad-480x400 (route built in S15.22 / TD-196). Per-section
  // product derived by Scenario 104 from section presence (HC-AD-1).
  function commitCreateADS() {
    ...
    var payload = {
      ...
      fields: {
        ...
        customerId:         a.customerId   || '',
        customerName:       a.customerName || '',
        // Banner (generic main-image keys → banner-ad-700x200)
        mainImageMediaId:   a.mainImageMediaId || '',
        mainImageSrc:       a.mainImageSrc     || '',
        mainImageAlt:       curVal('mainImageAlt') || '',
        bannerAdLink:       curVal('bannerAdLink'),
        // Sidebar (generic og-image keys → sidebar-ad-480x400)
        ogImageMediaId:     a.ogImageMediaId || '',
        ogImageSrc:         a.ogImageSrc     || '',
  ...
  function resolvePrefilledMediaIds(ids) {
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      if (mt.indexOf('text') !== -1) {
        ...
        if (m.imageUrl) {
          if (imgSlot === 0)      { resolved.mainImageSrc = m.imageUrl; resolved.mainImageMediaId = ids[i]; }
          else if (imgSlot === 1) { resolved.ogImageSrc   = m.imageUrl; resolved.ogImageMediaId   = ids[i]; }
          imgSlot++; matched++;
        }
      }
    }
  ...
  function openSidebarPickerForMode(mode) {
    if (!S.inlinePicker) {
      S.inlinePicker = { open: true, mode: mode, tab: 'available', expandedBundles: {}, search: '' };
    } else {
  ...
  function setMainImageFromMedia(media) {
    ...
    deriveDirtySections();
    log('main image set via sidebar picker', { mediaId: media.mediaId, url: media.imageUrl });
    return true;
  }
  ...
  function setOgImageFromMedia(media) {
    ...
    S.article.ogImageSrc     = media.imageUrl;
    ...
    S.dirtyFields.ogImageSrc     = { from: priorOg, to: media.imageUrl };
    S.dirtyFields.ogImageMediaId = { from: '', to: media.mediaId || '' };

    deriveDirtySections();
    log('OG image set via sidebar picker', { mediaId: media.mediaId, url: media.imageUrl });
    return true;
  }
  ...
  function fireUpdateRef(item, refType) {
    ...
    if (S.mode === 'create') {
      ...
      // v1.2.4 — Ad name follows the customer (it can't be a dynamic
      // field). Seed adName from the picked customer if the operator
      // hasn't typed one yet; stays fully editable afterward.
      if (refType === 'customer' && S.assetType === 'ad' && !S.article.adName) {
        S.article.adName = item.name || '';
        if (S.article.adName) {
          S.dirtyFields = S.dirtyFields || {};
          S.dirtyFields.adName = {
            from: (S.originalValues && 'adName' in S.originalValues ? S.originalValues.adName : ''),
            to:   S.article.adName
          };
        }
      }
  ...
  function publicOpen(params) {
    ...
    // RE/EV/ADS variants are create-only for now (no edit hydration yet).
    if (assetType !== 'article' && mode === 'edit') {
      warn('open(): edit mode not yet supported for assetType ' + assetType + ' — forcing create');
      mode = 'create';
    }
