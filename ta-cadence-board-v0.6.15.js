/* ════════════════════════════════════════════════════════════════
   ...
   ── THE CHIPS REFLECT DATA WE ACTUALLY HAVE ──

   The mockup's chips were All / No file / Unassigned / Not for NL.
   "No file" needs the manifest matched against the Drive drop by
   filename, and that matching is not built — so rather than ship a
   chip that silently reports nothing, this release carries
   All / Unbuilt / Unassigned / Not for NL. Unbuilt means no
   asset-article / -re / -event / -ad reference yet. "No file"
   returns when the matching does.
...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  function slateRows() {
    ...
    if (!shown) {
      ...
      rows = '<tr><td colspan="6" class="ixcb-sl-none">' +
  ...
  function overviewBody() {
    if (!S.slate.length) return slateDiag();
    ...
    return '<div class="ixcb-sl">' +
      '<div class="ixcb-sl-head">' +
        ...
        '<span class="ixcb-sl-undec">' + undec + ' of ' + S.slate.length + ' undecided</span>' +
      '</div>' +
      '<div class="ixcb-sl-chips">' + chips + '</div>' +
      '<div class="ixcb-sl-ro">Read-only. Assignment and disposition arrive with the write scenario.</div>' +
      ...
      '<div class="ixcb-sl-tw"><table class="ixcb-sl-tbl">' +
        '<thead><tr>' +
          '<th>Pg</th><th>Section</th><th>Title</th><th>Img</th><th>Issues</th><th>Next step</th>' +
  ...
  function renderAll() {
    ...
    if (S.mode === 'overview') {
      ...
      S.root.innerHTML = appBar() + '<div class="ixcb-ovw">' + overviewBody() + '</div>' + trayHtml();
      return;
    }
