/* ============================================================
   ta-bundles-v1.2.0.js
   ============================================================
   INBXIFY · Bundles Workspace · Cut 3 · TD-183 client companion
   to Scenario K v1.1.3 per spec §11.

   Five surgical additions on top of v1.1.7. No refactor of the
   cascade machinery; cascade Steps 1/2/footer and the
   open/close/switch controllers are untouched.
...
(function () {
  ...
  function doCreateOpen(c) {
    ...
    try {
      ...
      setTimeout(function () {
        ...
        S.selectedIds = {};
        renderCascadeAnchor();
        // Don't refetch immediately — ASF hasn't created anything yet.
        // Next panel activation or manual refresh will pick up the new asset.
      }, 400);
    } catch (err) {
  ...
  function renderBundleCard(bundle, lookup) {
    ...
    // v1.2.0 · TD-183.3: status filter chips + filtered row set + empty state.
    var bodyHtml = '';
    if (isExpanded) {
      var scope        = 'bundle:' + bundle.id;
      var filter       = getFilterFor(scope);
      var counts       = countByStatus(bundle.items);
      var filtered     = filterMediaByStatus(bundle.items, filter);
      ...
      bodyHtml = '<div class="bdl-card-body">' + stripHtml + rowsOrEmpty + '</div>';
    }
