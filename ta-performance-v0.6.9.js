...
(function () {
    ...
    nexCampaigns(stats).forEach(function (camp) {
      ...
      keys.forEach(function (k, idx) {
        ...
        var rec = out[k] || (out[k] = {
          ...
          slug: it.slug || '', utmContent: it.utmContent || '', domain: it.domain || '',
  ...
  var TIPS = {
    ...
    placements: 'Total ad slots filled. Higher than Issues when an advertiser runs more than one slot in the same issue.',
  ...
  // Tip text rides on the attribute; a single body-level node does the
  // rendering. Nothing to clip, nothing to escape.
  function iCircle(tip) { return '<span class="perf-i" data-tip="' + esc(tip) + '">i</span>'; }
  ...
  function tipNode() {
    ...
    TIPNODE.className = 'perf-tipfloat';
  ...
  function showTip(el) {
    ...
    var r = el.getBoundingClientRect(), b = n.getBoundingClientRect();
    var left = r.left + r.width / 2 - b.width / 2;
    left = Math.max(8, Math.min(left, window.innerWidth - b.width - 8));
    var top = r.top - b.height - 8;
    if (top < 8) top = r.bottom + 8;          // flip below when tight above
  ...
  // Delegated at document level so it covers the tab, the drill view and
  // every modal without per-surface wiring.
  if (!window.__ixPerfTipBound) {
    window.__ixPerfTipBound = true;
    document.addEventListener('mouseover', function (e) {
      var el = e.target.closest && e.target.closest('.perf-i');
      if (el) showTip(el);
    });
    document.addEventListener('mouseout', function (e) {
      if (e.target.closest && e.target.closest('.perf-i')) hideTip();
    });
  ...
  function rowNl(nl, idx) {
    ...
    return '<tr class="perf-row' + (wait ? ' perf-row--wait' : '') + '" data-perf-nl="' + idx + '">'
      ...
      + '<td class="num">' + (nl.stats ? factor(nl.stats).toFixed(2) : dash()) + '</td>'
      + '</tr>';
  }

  /* ── by advertiser + drill-in (unchanged) ─────────────── */

  // Slot family from the real slot code — BA-2 -> BA, EV-1 -> EV. Falls
  // back to the item type when a placement carries no slot code.
  var KIND_LABEL = { ad: 'Banner', find: 'The Find', event: 'Event', article: 'Sponsored article', realestate: 'Real estate', external: 'Link', other: 'Other' };

  var TYPE_FAMILY = { ad: 'BA', find: 'TF', event: 'EV', article: 'FA', realestate: 'RE', external: 'LINK', other: 'OTHER' };

  // Fixed ceiling so the column can never force a wrap. Full value stays
  // available on the title attribute.
  ...
  function slotFamily(rec) {
    var m = /^([A-Za-z]{2,4})-?\d*/.exec((rec.utmContent || '').trim());
    if (m && m[1]) return m[1].toUpperCase();
    return TYPE_FAMILY[rec.type] || 'OTHER';
  }

  // ── HC-PERF-004 · manual attribution (TEMPORARY) ──────────────────────
  // For links the email HTML never tagged with utm_term. Key is the bare
  // domain, value is the customer NAME exactly as it reads in CUSTOMERS.
  // `kind` is optional and only relabels the placement in reports.
  // This exists because the events block emits untagged sponsor banners.
  // Once Scenario 106 (or its replacement) writes utm_term on those links,
  // delete the entry — a real utm_term always takes precedence over this.
  var LINK_ATTRIBUTION = {
    'patonlawfirm.com': { customer: 'Paton Law Firm', kind: 'event' }
  };

  function attributionFor(rec) {
    if (rec.customerId) return null;                 // tagged wins, always
    var d = (rec.domain || '').replace(/^www\./, '').toLowerCase();
    return d ? (LINK_ATTRIBUTION[d] || null) : null;
  }

  // Name -> id, resolved against whatever CUSTOMERS holds at render time.
  function customerIdByName(name) {
    var want = String(name || '').trim().toLowerCase();
    if (!want) return '';
    var keys = Object.keys(CUSTOMERS);
    for (var i = 0; i < keys.length; i++) {
      if (String(CUSTOMERS[keys[i]].name || '').trim().toLowerCase() === want) return keys[i];
    }
    return '';
  }
  ...
  function advIndex() {
    ...
    NLS.forEach(function (nl) {
      ...
      Object.keys(items).forEach(function (k) {
        var r = items[k];
        // A customerId IS the sponsorship signal. Any placement type can
        // carry one — banner, event, sponsored article, real estate, Find.
        // House links never do.
        // Fill a blank customerId from the manual map before the guard.
        var man = attributionFor(r);
        if (man) {
          var mid = customerIdByName(man.customer);
          if (mid) { r.customerId = mid; r.manualAttr = true; if (man.kind) r.type = man.kind; }
        }
        if (!r.customerId || r.type === 'house') return;
  ...
  function viewDrill(custId) {
    ...
    if (a) {
      ...
      issueRows = a.rows.map(function (row) {
        ...
        var title = (r.type === 'article') ? articleTitle(r) : '';
        var linkTotal = 0;
        nexCampaigns(nl.stats).forEach(function (c) { (c.links || []).forEach(function () { linkTotal++; }); });
        return '<tr><td>' + esc(nl.name || (nl.issue != null ? '#' + nl.issue : nl.id)) + '</td>'
          + '<td>' + dShort(nl.date) + '</td>'
          + '<td><span class="perf-type">' + esc(r.utmContent || slotFamily(r)) + '</span></td>'
          + '<td>' + esc(KIND_LABEL[r.type] || r.type)
          + (r.manualAttr ? ' <span class="perf-manual" title="Attributed manually \u2014 this link carries no utm_term">M</span>' : '') + '</td>'
          + '<td class="perf-title-cell"' + (title ? ' title="' + esc(title) + '"' : '') + '>'
          + (title ? esc(truncate(title, 40)) : dash()) + '</td>'
    ...
    return '<span class="perf-back" data-perf-back>\u2190 All advertisers</span>'
      ...
      + tot('Issues', a ? Object.keys(a.issues).length : 0)
      + tot('Placements ' + iCircle(TIPS.placements), a ? a.rows.length : 0)
      ...
      + '<thead><tr><th>Issue</th><th>Date</th><th>Slot</th><th>Kind</th><th>Title</th>'
      ...
      + '<tbody>' + (issueRows || '<tr><td colspan="9">' + dash('no ingested placements') + '</td></tr>') + '</tbody></table>'
  ...
  // Last-resort: set Webflow's own tab classes directly. No handler
  // involved, so this works even if Webflow never wired the link or
  // something re-asserted the default pane after us.
  function forceWTab(key) {
    var links = document.querySelectorAll('[data-w-tab].w-tab-link, a[data-w-tab]');
    var panes = document.querySelectorAll('.w-tab-pane[data-w-tab]');
    if (!links.length || !panes.length) return false;
    var hit = false;
    [].slice.call(links).forEach(function (l) {
      var on = l.getAttribute('data-w-tab') === key;
      l.classList.toggle('w--current', on);
      l.setAttribute('aria-selected', on ? 'true' : 'false');
      if (on) hit = true;
    });
    [].slice.call(panes).forEach(function (p) {
      var on = p.getAttribute('data-w-tab') === key;
  ...
  function restoreWTab() {
    ...
    (function attempt() {
      if (i >= delays.length) {
        tabLog('click route failed, forcing classes');
        forceWTab(key);
        return;
      }
      var wait = delays[i++];
  ...
  function render() {
    ...
    view.innerHTML = '<div class="ta-perf__card perf-nl-card"><table>'
      ...
      + '<th class="num">CTR</th><th class="num">CTOR</th>'
      + '<th class="num">Factor</th></tr></thead><tbody>'
      + (NLS.map(rowNl).join('') || '<tr><td colspan="14">' + dash('no newsletters found \u2014 is the newsletter-wrapper list on the page?') + '</td></tr>')
      + '</tbody></table></div>';
  }
