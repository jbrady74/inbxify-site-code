/* ════════════════════════════════════════════════════════════════
   ...
   first four plans harvested — which offered a tick-box for WLN-117,
   an issue that shipped in July. v0.6.22 narrowed that to plans at or
   above the gate's Start at, which was still the wrong question:
   Start at is about the plans being CREATED, not the ones this board
   already covers. Under it, 119A, 119B and 120A were invisible on the
   September board even though that is exactly where they live.
   ...
   Considered and rejected:
     ...
     · SLATE.publication-plan, working backwards from assignment. That
       ...
       show nothing until it was finished — the surface would be empty
       exactly when it is most needed.
     · source. A filename identifies a PDF, not a working period, and
       "June-August 2026" spans three of them.

   WHEN THE BOARD IS UNTITLED there is no label to match, so nothing
   can be a member and the strip says so — rather than falling back to
   a proximity guess and quietly showing the wrong issues again.
  ...
  function harvestPlans() {
    ...
    Array.prototype.forEach.call(items, function (item) {
      ...
      var rec = {
        ...
        /* v0.6.23 — which cadence board this plan belongs to. */
        cadence:    cadence,
        slots:      slots
      };
...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  /* Plans belonging to THIS cadence board. Membership is
     PUBLICATION PLAN . cadence-label matched against the board's
     title — one field, one string, no inference.

     Matching is case- and whitespace-insensitive because the label is
     typed by hand in two places and "September 2026" should not miss
     "september 2026 ". Nothing beyond that: a fuzzy match would be a
  ...
  function boardPlans() {
    ...
    if (!key) return [];              /* untitled board owns nothing */
    var out = [];
    for (var i = 0; i < S.plans.length; i++) {
  ...
  /* Editions and the assignment columns render these. Sorted by issue
     number then edition letter, so 119A precedes 119B precedes 120A. */
  function openIssues() {
