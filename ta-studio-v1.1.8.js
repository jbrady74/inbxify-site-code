// ============================================================
// ta-studio-v1.1.8.js
// INBXIFY TA Studio — Components tab Attach UI + archive/delete
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    //                             9 roles get a stub toast. Opens State B
    //                             for the card. Only one card at a time —
    ...
    //                             re-renders grid. Does not clear the
    ...
    function cmpHandleAttach(mediaId) {
      ...
      // Role gating — non-enabled roles get a dated stub toast.
    ...
    function cmpHandleAttachConfirm(mediaId) {
      ...
      if (!it) return;
      if (S.cmp.attachEdit.mediaId !== mediaId) return;
      if (!S.cmp.attachEdit.articleId) return; // button should be disabled but belt-and-braces
    ...
    function cmpFireAttach(mediaId, articleId, it) {
      ...
      // server-side. Hash lives on .media-wrapper[data-component-role-hash]
      // (added in v1.1.8 deploy prereqs). Defensive fallback reads direct
      // from DOM in case readComponents() didn't grab it.
      ...
      if (!roleHash) {
        ...
        cmpRenderGrid();
        showStudioToast('Configuration error \u2014 missing role hash. Check Webflow Designer binding.');
        return;
      }
      ...
      .then(function (body) {
        // Strict success check (v1.1.7.2 pattern): only treat as ok if
        // the server explicitly confirmed with {ok:true}. Empty bodies
        // or non-JSON responses fall through as !ok.
        var ok = false;
        var role = '';
        try {
        ...
        // If drawer is open on this item, re-render it so "Attached to" line updates.
      ...
      .catch(function (err) {
        ...
        S.cmp.attachInFlight = null;
        cmpRenderGrid(); // brings Attach button back to enabled
