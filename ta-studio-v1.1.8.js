// ============================================================
// ta-studio-v1.1.8.js
// INBXIFY TA Studio — Components tab (read + attach) + archive/delete
...
//     fetch. On success (200 + role in response), card flips to
//     State C (Attached) with attached-Article title rendered
//     under role line, IDs exposed on card, button disabled +
//     relabeled "Attached". Toast top-right, 4s duration:
...
//   - Failure handling: non-200 response or network error
//     leaves card in State B with enabled button, red toast
//     "Attach failed — please retry".
//
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var S = {
      ...
      // Components tab state (v1.1.7, extended in v1.1.8)
      cmp: {
        ...
        // v1.1.8 Attach UI state. Only one card can be in editing state
        // at a time; clicking Attach on another card auto-cancels.
        attachEdit: {
          mediaId: null,        // mediaId of the card currently in State B, or null
          publishStatus: 'Draft', // 'Draft' | 'Live' | 'All' — defaults to Draft per D3/Q3
      ...
      '<div class="std-root">' +
        '<div class="ix-hdr">' +
          '<div class="ix-hdr-left">' +
            ...
            '</div>' +
          '</div>' +
          '<span class="ix-badge">v1.1.8</span>' +
        '</div>' +
          ...
          onSave: function (editedHTML) {
            saveArticleBody(articleItemId, editedHTML)
              .then(function () {
                ...
                window._studioRteInstance = null;
                // v1.1.4: Show a brief success toast so the user knows
                // the save landed, not that the modal just disappeared.
                showStudioToast('\u2713 Body saved to Webflow');
              })
              .catch(function (err) {
    ...
    function cmpReadItems() {
      ...
      Array.prototype.forEach.call(wraps, function (el) {
        ...
        items.push({
          ...
          role:          (d.componentRole || '').trim(),   // label
          componentRoleHash: (d.componentRoleHash || '').trim(), // v1.1.8: option hash for Route 4
    ...
    function cmpRender(root) {
      root.innerHTML =
        ...
        '</div>' +
        '<div class="cmp-drawer-backdrop" id="cmp-drawer-backdrop" style="display:none"></div>' +
        '<aside class="cmp-drawer" id="cmp-drawer" aria-hidden="true"></aside>';
    ...
    function cmpBindEvents(root) {
      ...
      root.addEventListener('click', function (e) {
        ...
        if (t.closest('[data-cmp-clear-all]')) {
          S.cmp.filters = {
            status: 'Available', role: 'All', mediaType: 'All',
        ...
        // v1.1.8: Except when the card is in State B editing — don't open drawer
        // and don't swallow the click. Lets form controls (Status filter, picker)
        // work normally.
        if (S.cmp.attachEdit.mediaId === mediaId) return;
        cmpOpenDrawer(mediaId);
      });

      // v1.1.8: ESC closes State B editing (matches Cancel)
      document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape' && S.cmp.attachEdit.mediaId) {
          cmpHandleAttachCancel();
        }
      });
      ...
      if (backdrop) backdrop.addEventListener('click', cmpCloseDrawer);
      document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape' && S.cmp.drawerOpenId) cmpCloseDrawer();
      });
    }
    ...
    // The States A and C markup lives in cmpRenderCardDefault.
    // State B lives in cmpRenderCardEditing.
    ...
    // — the difference is just the action-row button label + state and
    // a small "Attached to" line + IDs surfaced beneath role.
    function cmpRenderCardDefault(it) {
      ...
      // v1.1.8: State C surface "Attached to" line + Article ID + Media ID (TD-124)
      var attachedLine = '';
      if (isAttached && it.articleId) {
        var articleTitle = cmpLookupArticleTitle(it.articleId) || '(unknown Article)';
        attachedLine = '<div class="cmp-card-attached">' +
                         '\u2192 ' + esc(articleTitle) +
                       '</div>';
      }
    ...
    // State B (Editing): same card chrome as State A but action row replaced
    ...
    function cmpRenderCardEditing(it) {
      ...
      // Status filter for Article picker — 3 options, Draft default
      ...
      // Article picker — filtered by status, sorted most-recently-created first
      ...
      var attachBtnDisabled = (!ae.articleId || isFiring) ? ' disabled' : '';
      var attachBtnLabel = isFiring ? 'ATTACHING\u2026' : 'Attach';

      return '' +
        '<div class="cmp-card cmp-card--editing' + (isArchived ? ' cmp-card--archived' : '') + '" data-cmp-card="' + esc(it.mediaId) + '">' +
          thumb +
          '<div class="cmp-card-body">' +
            '<div class="cmp-card-name">' + esc(it.name || '(Untitled)') + '</div>' +
            '<div class="cmp-card-meta">' +
    ...
    function cmpReadArticlesForPicker(publishStatus) {
      ...
      out.sort(function (a, b) { return b.created - a.created; }); // desc — most recent first
      return out;
    }
    ...
    //                             Role-gates to the 3 enabled roles; other
    //                             9 roles get a stub toast (D5). Opens
    //                             State B for the card. Only one card at
    //                             at a time — auto-cancels any other card
    //                             currently in State B.
    //   cmpHandleAttachCancel   — Cancel text link OR ESC key. Reverts
    //                             attachEdit state to defaults and
    //                             re-renders grid (card flips back to A).
    //   cmpHandleAttachConfirm  — clicking the Attach button inside State B
    //                             with a selected Article. Fires Route 4.
    //
    // Enabled roles for 6B. Matches the three Route 4 branches built in
    // Scenario G Checkpoint B.
    var CMP_ATTACH_ENABLED_ROLES = ['Article Body HTML', 'Main Image', 'Interior Image'];

    function cmpHandleAttach(mediaId) {
      var it = cmpFindItem(mediaId);
      if (!it) return;

      // Safety: Attached cards shouldn't reach this path (button is
      ...
      // Role gating (D5 — dated stub toast)
      if (CMP_ATTACH_ENABLED_ROLES.indexOf(it.role) === -1) {
        showStudioToast('Field-level attach for ' + (it.role || 'this role') + ' ships in Session 8.');
        return;
      }

      // Open State B for this card. If another card is already in State B,
      // it auto-cancels (attachEdit.mediaId moves to this one).
      S.cmp.attachEdit = {
        mediaId: mediaId,
        publishStatus: 'Draft',   // D2: default Draft
        articleId: null
      };
      cmpRenderGrid();
    }
    ...
    function cmpHandleAttachCancel() {
      ...
      S.cmp.attachEdit = { mediaId: null, publishStatus: 'Draft', articleId: null };
      cmpRenderGrid();
    }
    ...
    function cmpHandleAttachConfirm(mediaId) {
      ...
      if (!it) return;
      if (S.cmp.attachEdit.mediaId !== mediaId) return;
      if (!S.cmp.attachEdit.articleId) return; // guard — button should be disabled but belt-and-braces
    ...
    function cmpFireAttach(mediaId, articleId, it) {
      ...
      // Resolve the Component Role hash. JS filters on role labels client-side
      // (HC-009), but Route 4's inner Router expects the option-field hash
      // server-side. The hash lives on the .media-wrapper data attribute
      // data-component-role-hash (added by Webflow Designer binding in v1.1.8
      // deploy prereqs). Fallback: pull from the item if we stored it at
      // readComponents() time.
      var roleHash = it.componentRoleHash;
      if (!roleHash) {
        // Defensive lookup direct from DOM — in case readComponents didn't grab it
        var wrapper = document.querySelector('.media-wrapper[data-media-id="' + mediaId.replace(/"/g, '') + '"]');
        if (wrapper) roleHash = wrapper.dataset.componentRoleHash || '';
      }
      if (!roleHash) {
        console.error('[TA-STUDIO v1.1.8] No component-role-hash for MEDIA', mediaId, 'role', it.role);
        S.cmp.attachInFlight = null;
        cmpRenderGrid();
        showStudioToast('Configuration error — missing role hash. Contact support.');
        return;
      }
      ...
      .then(function (body) {
        ...
        } catch (e) { /* plain text — treat as success */ }
        if (!ok) throw new Error('Server rejected the request');
        ...
        showStudioToast('\u2713 Attached to ' + articleTitle, { duration: 4000 });

        // If drawer is open on this item, re-render it so the "Attached to" line updates.
        if (S.cmp.drawerOpenId === mediaId) cmpRenderDrawer();
        cmpRenderGrid();
      })
      .catch(function (err) {
        console.error('[TA-STUDIO v1.1.8] Scenario G Route 4 failed:', err);
        S.cmp.attachInFlight = null;
        cmpRenderGrid(); // re-render brings Attach button back to enabled
        showStudioToast('Attach failed \u2014 please retry.');
      });
    }
    ...
    function cmpFireDelete(mediaId, mode, it) {
      ...
      .then(function (body) {
        ...
        // Tolerate plain text "Accepted" too.
        var ok = true;
        try {
          var j = JSON.parse(body);
          if (j && j.ok === false) ok = false;
        } catch (e) { /* plain text response — treat as success */ }
        ...
        if (mode === 'archive') {
          ...
          showStudioToast('Component archived.');
          ...
          showStudioToast('Component deleted.');
          cmpRenderGrid();
        }
      })
      .catch(function (err) {
    ...
    function cmpCloseDrawer() {
      ...
      var backdrop = document.getElementById('cmp-drawer-backdrop');
      if (drawer) {
        drawer.classList.remove('cmp-drawer--open');
    ...
    // ── Init ──
    renderAssembler();
    console.log('\u29BE Studio v1.1.8 mounted \u2014 Components tab (read + attach + archive + hard-delete)');
  });
