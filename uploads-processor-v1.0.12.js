// ============================================================
// uploads-processor-v1.0.12.js
// INBXIFY Uploads Processor — System 2 UI
// Mounts into #content-manager-mount on T-A page
// Config from window.TA_CONFIG
//
// v1.0.12: TRANSCRIBER ABE SAVE PATH UNIFIED WITH STUDIO ASSEMBLER
//          (Item #3 — Save-to-CMS redirect to launching sub-tab)
//
//          editArticleBody() — the function exported as
//          window.InbxEditBody that the Transcriber surface calls
//          to open the article body editor — now delegates to
//          window.InbxStudioBodyEditor (added in ta-studio v1.3.6+).
//          Studio owns the save flow: Scenario G via saveArticleBody,
//          close RTE on success, reload via studioReloadAfterCMSWrite,
//          session-restore brings the user back to the Transcriber
//          sub-tab.
//
//          Two bugs fixed by this single delegation:
//
//          A) Path B field gap. Pre-v1.0.12, UP saved via Scenario F's
//             updateArticleBody route, which writes ONLY the legacy
//             Rich Text post-body field. Path B (added morning of
//             2026-05-13) added post-body-html as a Plain Text field
//             rendered via Webflow HTML Embed — but Scenario F was
//             never extended to write it. Transcriber-edited articles
//             were rendering through the legacy Rich Text widget,
//             losing all the Path B benefits (image rehosting, attr
//             stripping, blanket CSS overrides on size/alignment).
//             Now: Transcriber-saved articles route through Scenario G
//             (Module #12) which writes BOTH post-body and post-body-html.
//             Path B fidelity preserved end-to-end.
//
//          B) No reload, no redirect. Pre-v1.0.12, UP's onSave just
//             fired the fetch and walked away. The RTE stayed open
//             after save; no closeFullscreen, no banner, no reload,
//             no session restore. User had to click Close manually
//             and might land anywhere. Now: matches Assembler behavior
//             exactly — save → close RTE → "Body saved — refreshing"
//             banner → reload → restore to Studio>Transcriber sub-tab.
//
//          Local fetchArticleBody (line 387 area, Scenario F's
//          getArticleBody) is NOT removed — may be called elsewhere
//          and not worth disturbing in this surgical fix. The save
//          path is what mattered.
//
//          Make scenarios untouched in this delivery. Scenario F's
//          updateArticleBody route stays installed but stops being
//          called. Can be deleted server-side later when confidence
//          is high. Scenario F's getArticleBody route still called
//          by UP's own (unrelated) read paths.
//
//          Companion: ta-studio-v1.3.6.js (required — exports
//          window.InbxStudioBodyEditor). If Studio is missing or
//          not yet loaded at click time, editArticleBody falls back
//          with a console error + alert. Defensive only — the script
//          load order on the T-A page guarantees Studio is loaded
...
<div class="cm-root">
  ...
  // ══════════════════════════════════════════════
  //
  // v1.0.12: Body editor flow now delegates to Studio's unified
  // openBodyEditor via window.InbxStudioBodyEditor (added in
  // ta-studio v1.3.6). Studio owns the fetch (Scenario G Route 1),
  // RTE wiring, save (Scenario G Route 3 — writes BOTH post-body AND
  // post-body-html for Path B), close-on-success, reload banner, and
  // session-restore-to-launching-panel.
  //
  // The 'transcriber' returnPanel value tells Studio's
  // studioReloadAfterCMSWrite to persist Transcriber as the
  // post-reload restore target. After reload, Studio's mount-time
  // studioRestoreFromSession reads this and brings the user back to
  // Studio > Transcriber sub-tab (where they were when they clicked
  // edit body).
  //
  // Fallback: if Studio isn't loaded for some reason (e.g. script load
  // order changes, defensive mount-order failure), surface a clear
  // error rather than silently no-op'ing. The T-A page's <head> block
  // currently loads ta-studio after uploads-processor, and Studio's
  // window export happens at DOMContentLoaded — by the time a user
  // click triggers this function, Studio is up.
  //
  // The pre-v1.0.12 implementation (Scenario F fetch + inline save,
  // no closeFullscreen, no reload) is intentionally deleted. Do NOT
  // restore it — it bypasses Path B's post-body-html field and
  // strands the user inside the open RTE post-save.
  async function editArticleBody(articleItemId, articleTitle) {
    if (!articleItemId) {
      console.error('[CP v1.0.12] editArticleBody called without articleItemId');
      return;
    }
    if (typeof window.InbxStudioBodyEditor !== 'function') {
      console.error('[CP v1.0.12] window.InbxStudioBodyEditor not available. Is ta-studio-v1.3.6.js (or later) loaded BEFORE uploads-processor on this page?');
      alert('Body editor is not available. Studio module may not be loaded — check the browser console.');
      return;
    }
    console.log('[CP v1.0.12] Delegating body editor to Studio for:', articleItemId, '(returnPanel: transcriber)');
    return window.InbxStudioBodyEditor({
      articleItemId: articleItemId,
      articleTitle: articleTitle || '',
      returnPanel: 'transcriber'
    });
  }
  ...
  loadFolderCounts();
  console.log('\u{1F4C2} Uploads Processor v1.0.12 mounted \u2014 editArticleBody now delegates to window.InbxStudioBodyEditor (Studio v1.3.6+) with returnPanel: transcriber. Transcriber body saves now route through Scenario G (writes post-body + post-body-html, Path B fidelity) and trigger studioReloadAfterCMSWrite for restore-to-Transcriber post-reload. Scenario F updateArticleBody route no longer called.');
});
