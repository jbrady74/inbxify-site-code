/* ============================================================
   pubplan-sequencer-v1.0.2.js
   INBXIFY — MNA Workstream C / K-3: PubPlan loose-typing pass

   CHANGELOG
     v1.0.2 (K-3, from v1.0.1):
       - LOOSE ROSTER: FALLBACK_TYPES is now the 9 loose authoring types
         (Greeting, Article, Lead Banner Ad, Banner Ad, The Find, LBP,
         Real Estate Section, Events Section, Local Weather). The live
         roster still comes from [data-blocktype-list]; this is only the
         safety net, and it now degrades to the CORRECT loose set instead
         of the old strict-11.
       - STAGE ON WRITES (per-row): when the mount declares planning
         context (data-pp-stage="planning" on the root), every SAVE item
         carries stage:"planning" + stageHash. Reorder items never carry
         stage (a reorder can't change stage). Backwards-compatible:
         absent attr → stage omitted entirely → legacy writes byte-identical.
       - A-LOCK (editing-state UX): the gold dirty border on a changed
         <select> is confirmed present (pps-changed-field). No regression.
       NOTE: at stage=planning the row stores the LOOSE label string +
       planning hash. Strict block-type hash resolution (Article→FA|TS,
       Banner ordering, Local Weather→LW) happens at PROMOTE (K-6), not
       here. K-3 never needs the strict hashes.
   ...
   WRITE PATH (v1.0.2 — LIVE)
     ...
     reorder items: { id, position }                    ← never carries stage
     save items:    { id|null, blockType, blockTypeHash, position,
                      planningNote, showAsSponsored, assetType, assetId,
                      stage?, stageHash? }               ← stage only when
                      the mount declares data-pp-stage="planning"
   ...
   │    data-pp-stage="planning" makes every SAVE write the stage    │
   │    field (K-3). OMIT the attr on legacy strict PubPlans → no    │
   │    stage written, writes stay byte-identical to v1.0.1.        │
   ...
   LOAD ORDER (Webflow <head>)
     ...
     5. (this JS self-loads pubplan-sequencer-v1.0.2.css)
...
(function () {
  ...
  // K-3 stage hashes (NL-BLOCKS `stage` Option field). PubPlan rows are
  // always "planning"; Promote (K-6) flips them to "locked". The locked
  // hash is here for completeness only — the sequencer never writes it.
  var STAGE_HASH = {
    planning: '225372aa2fc87422924a58ed48447ca3',
    locked:   '3b847dba134c80679c811f5975f2235f'
  };

  // K-3 loose roster (9). The live roster is read from [data-blocktype-list];
  // this is ONLY the safety net when that wrapper is absent/unrendered. It now
  // degrades to the correct loose set (was the strict-11 in v1.0.1). Hashes are
  // blank here on purpose — at stage=planning the loose label string is what
  // gets written; strict hashes resolve at Promote.
  var FALLBACK_TYPES = [
    'Greeting', 'Article', 'Lead Banner Ad', 'Banner Ad', 'The Find',
    'LBP', 'Real Estate Section', 'Events Section', 'Local Weather'
  ];
  ...
  function demoBlocks() {
    // Loose-9 labels (K-3). Demo only — shown when no [data-nlblocks-wrapper]
    // is wired. Mirrors a plausible PubPlan in planning stage.
    var seed = [
      ['nlb_01', 'Greeting', 'Welcome — Issue 113', '', 1.0, false],
      ['nlb_02', 'Article', 'Meet the Ambrose Family', '', 2.0, false],
      ['nlb_03', 'Article', null, 'Profile on the new river park reopening', 3.0, false],
      ['nlb_04', 'Lead Banner Ad', 'Valley Dental — 700x200', '', 4.0, true],
      ['nlb_05', 'Banner Ad', 'Riverside Realty — 700x200', '', 5.0, true],
      ['nlb_06', 'Local Weather', 'Wyckoff 7-day', '', 6.0, false],
      ['nlb_07', 'Events Section', null, 'Fall street fair — confirm date w/ chamber', 7.0, false],
      ['nlb_08', 'The Find', 'Bergen Eats — local picks', '', 8.0, false]
    ];
  ...
  function Sequencer(root) {
    ...
    // K-3: stage context. data-pp-stage="planning" → PubPlan authoring; emit
    // stage on saves. Absent (legacy strict PubPlans) → stage omitted, writes
    // stay byte-identical to v1.0.1. Only "planning" is honored as a writer
    // context; anything else (or absent) means "don't write stage".
    this.stage = (root.getAttribute('data-pp-stage') || '').trim().toLowerCase();
    this.emitStage = this.stage === 'planning';
  ...
  Sequencer.prototype.buildPayload = function () {
    ...
    var saves = dirty.filter(function (b) { return self.fieldChanged(b); }).map(function (b) {
      var item = {
        ...
        blockType: b.type,                     // matches {{20.blockType}} — LOOSE label at planning stage
        blockTypeHash: b.typeHash || '',       // matches {{20.blockTypeHash}} — blank at planning (resolved @ promote)
      ...
      // K-3: stage rides on the row, saves only. Reorders never carry stage.
      if (self.emitStage) {
        item.stage = self.stage;               // "planning" — label for the DOM/legibility
        item.stageHash = STAGE_HASH[self.stage] || ''; // hash for the Make router (Option field)
      }
      return item;
    });
  ...
  function diagnose(root) {
    ...
    var stage = (root && (root.getAttribute('data-pp-stage') || '').trim().toLowerCase()) || '';
    ...
    console.info('pp-stage attr        :', stage || '(absent → stage NOT written, legacy-compat)');
    console.info('stage on writes      :', stage === 'planning' ? 'YES (planning + hash on saves)' : 'NO');
    console.info('NL-BLOCKS wrapper    :', wrap ? 'found' : 'MISSING (will demo)');
    console.info('NL-BLOCKS rows       :', rows);
    console.info('block-type roster    :', roster ? 'found' : 'MISSING (FALLBACK_TYPES = loose-9, hashes blank)');
