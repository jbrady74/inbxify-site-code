/* ============================================================================
   ta-default-layout-v1.0.7.js
   INBXIFY · T-A Studio · Default Layout tab  (Workstream K-2)

   v1.0.7 — per-title branding (multi-tenant): renders the title's real masthead
            logo (TA_CONFIG.titleLogoUrl ← TITLE.title-masthead-ulc-link) at the
            top of the newsletter column, and applies the title's primary color
            (TA_CONFIG.titleColor ← TITLE.primary-color) as --tdl-brand for tile
            accents. Both empty-safe (fall back to default styling).
...
(function () {
  ...
  function render() {
    ...
    // apply the title's brand color as a CSS variable on the root so tiles +
    // accents pick it up; falls back to the file's default accent if unset.
    var brand = titleColor();
    if (brand) S.root.style.setProperty('--tdl-brand', brand);
    else S.root.style.removeProperty('--tdl-brand');

    var logo = titleLogoUrl();
    var mastheadHtml = logo
      ? '<div class="tdl-masthead"><img class="tdl-masthead-img" src="' + esc(logo) + '" alt="' + esc(titleName()) + ' masthead" /></div>'
      : '';

    S.root.innerHTML =

      // ── Page header: title masthead logo (real newsletter masthead) when the
      //    title has one, then the section heading + rule. ──
      mastheadHtml +
  ...
  function mount() {
    ...
    render();
    S.mounted = true;
    console.log('[tdl] Default Layout tab mounted \u00b7 v1.0.7 \u00b7 blocks:', S.blocks.length,
