...
(function () {
  ...
  window.submitSection = async function(section) {
    ...
    if (section === 'gr') {
      ...
      if(currentTfMode==='txa'){for(var i=1;i<=5;i++){var s=state['TXA-'+i];if(s&&s.dirty)slots.push({'hidden-pubplan-id':pubplanId,'hidden-slot-label':s.sc,'hidden-titleadmin-id':s.titleadminId||'','hidden-section-code':s.secC,'hidden-category-group':s.section,'hidden-customer-id':s.custId,'hidden-customer-name':s.custNm,'hidden-category-id':tfCategoryId,'hidden-category-name':tfCategoryNm});}}
