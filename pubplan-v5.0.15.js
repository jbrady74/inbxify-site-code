...
//   FIX #22: Category-only save — Save TF Changes activates when
//            ANY category is selected (not just when different from
//            original). TF-CAT payload fires when tfCategoryId exists.
...
(function () {
  ...
  window.handleTfCategoryChange = function(sel) {
    ...
    var doIt = function() {
      ...
      // FIX #22: Any category selection activates save (not just different-from-original)
  ...
  function updateTfProgress(){
    ...
    updateSaveButton('tf',pc>0||!!tfCategoryId);
  }
  ...
  window.submitSection = async function(section) {
    ...
    if (section === 'gr') {
      ...
      // FIX #22: Category-level POST fires when any category is selected
      if (tfCategoryId) {
