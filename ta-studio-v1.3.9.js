...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function handleCreateType(type) {
      if (type === 'article') {
        if (!window.InbxASF || typeof window.InbxASF.open !== 'function') {
          ...
          console.warn('[ta-studio v1.3.8] window.InbxASF is not available');
          return;
        }
        try {
    ...
    loadStudioPdfCount();
    console.log('\u29BE Studio v1.3.6 mounted \u2014 Item #3 fix: launching-panel restore (returnPanel param), hardened outer-tab retry-poller (50ms x 30 attempts), explicit outerTab in restore payload, stale guard 30s\u2192120s, window.InbxStudioBodyEditor exported for UP delegation. Inputs monitor panel retained from v1.3.5. PDF triage banner retained. Companion: uploads-processor-v1.0.12.js.');
  });
