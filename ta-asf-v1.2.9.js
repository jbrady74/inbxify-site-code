/* ============================================================
   ...
   Companion stylesheet: ta-asf-v1.2.9.css
...
(function () {
  ...
  function render() {
    ...
    initLbpTrix();     // v1.2.3 — no-op unless LBP rich-text editor is in DOM  initAiAssist();    // v1.2.9 — no-op unless create-mode Article + InbxGenerate present
  }
  ...
  function renderAiAssistBar() {
    ...
    if (a.busy) {
      ...
      actions = '<span class="asf-ai-spin"></span>';
    } else if (a.done) {
      title = 'Auto-filled from body';
      sub   = 'Filled empty fields + added section headers. Existing text untouched.';
      actions =
        (a.bodyUndo != null ? '<button type="button" class="asf-ai-btn asf-ai-btn--ghost" data-asf-ai="undo">Undo headers</button>' : '') +
        '<button type="button" class="asf-ai-btn asf-ai-btn--ghost" data-asf-ai="regen">\u21bb Regenerate</button>';
    } else {
      title = 'AI assist';
      ...
      actions = '<button type="button" class="asf-ai-btn asf-ai-btn--gold" data-asf-ai="gen"' +
                (bodyLen ? '' : ' disabled') + '>\u2728 Generate from body</button>';
    }
    return '' +
      '<div class="asf-ai-bar' + (a.busy ? ' busy' : '') + '">' +
  ...
  function initAiAssist() {
    if (!aiAssistEligible()) return;
    var a = aiAssistState();
    var bar = qs(S.overlay, '.asf-ai-bar');
    if (bar && !bar._asfAiBound) {
      bar._asfAiBound = true;
      bar.addEventListener('click', function (e) {
        var btn = closestEl(e.target, '[data-asf-ai]');
        if (!btn || isDisabled(btn)) return;
        var act = btn.getAttribute('data-asf-ai');
        if (act === 'gen')   aiGenerate({ overwrite:false });
        if (act === 'regen') aiGenerate({ overwrite:true });
        if (act === 'undo')  aiUndoBody();
      });
    }
