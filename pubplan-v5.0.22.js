...
(function () {
  ...
  function renderGr() {
    ...
    if (isEd) {
      ...
      ssRow = '<div class="gr-ss-row" style="display:flex;align-items:flex-start;gap:8px;padding:6px 10px 8px 54px;border-top:1px dashed rgba(196,163,90,0.3);"><div class="pc" style="flex:1;"><span class="ppt-col-label">Super Short Greeting</span><input type="text" class="pi" data-field="grSS" maxlength="'+GR_LIMITS.grSS+'" value="'+(s.grSS||'').replace(/"/g,'&quot;')+'" placeholder="140 char max...">'+charCountHtml(GR_LIMITS.grSS,s.grSS)+'</div></div>';
    } else if (hasC) {
      ...
      ssRow = '<div class="gr-ss-row" style="display:flex;align-items:flex-start;gap:8px;padding:4px 10px 6px 54px;border-top:1px dashed rgba(196,163,90,0.3);"><div class="pc" style="flex:1;"><span class="ppt-col-label">Super Short</span><span class="pcv">'+(s.grSS||'—')+'</span></div></div>';
    } else {
    ...
    var html = '<div id="tile-gr-1"><div class="ptr grr'+(isEd?' hp':'')+'" id="tile-gr-1-row"><div class="psi sgr">GR-1</div>'+fh+'<div class="pac">'+ah+'</div></div>'+ssRow+'</div>';
