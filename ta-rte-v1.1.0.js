...
(function () {
  ...
  class InbxRTE {
    ...
    swapPlaceholderForImage(placeholderId, opts) {
      // Trix wraps content attachments inside its own <figure data-trix-attachment>.
      // Find by our marker on the inner <figure class="rte-upload-placeholder">.
      const inner = this.mount.querySelector(
        `figure.rte-upload-placeholder[data-rte-upload-id="${placeholderId}"]`
      );
      if (!inner) {
    ...
    injectStyles() {
      ...
      style.textContent = `
        ...
        .rte-img-upload-btn span { pointer-events:none; }
