...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function loadAssets(tabKey) {
    ...
    return fetchAssetPage(tabKey, off).then(function (resp) {
      ...
      // asset ids referenced by an NL-BLOCKS row FOR THIS TITLE (title match
      // client-side per multi-tenant rule — Webflow can't ref-filter server-side).
      // WRAPPED: status enrichment must NEVER abort the asset load. If anything
      // here throws (missing nlblocks, PUBPLANS not ready), assets still render;
      // we just skip the durable-status decoration this pass.
      try {
        var nlb = (resp.nlblocks && Array.isArray(resp.nlblocks)) ? resp.nlblocks
                : (resp.nlblocks && resp.nlblocks.items && Array.isArray(resp.nlblocks.items)) ? resp.nlblocks.items
                : [];
        if (nlb.length) {
        ...
        console.warn('[ASSET-LIBRARY] Piece 2 status enrichment skipped:', e && e.message);
      }
  ...
  function init() {
    ...
    console.log('[CONTENT-LIBRARY v1.0.50] Init:', {
