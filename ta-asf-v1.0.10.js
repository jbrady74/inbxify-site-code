...
(function () {
  ...
  function renderSectionNarrative() {
    var a = S.article || {};
    var lim = CFG.limits;
    // v1.0.5: editing state drives renderTextarea mode + grid balance
    var isEditing = S.editingSection === 'narrative';
    var readonly = !isEditing;
    return '' +
  ...
  function commitCreateArticle() {
    ...
    var payload = {
      ...
      fields: {
        ...
        slug:              a.slug,
        subtitle:          a.subtitle,
        bannerStatement:   a.bannerStatement,
        teaser:            a.teaser,
        shortSummary:      a.shortSummary,
        printIssueSource:  a.printIssueSource,
        ctaButton:         a.ctaButton,
        ctaText:           a.ctaText,
        ctaUrl:            a.ctaUrl,
        writerName:        a.writerName,
        writerTitle:       a.writerTitle,
        cowriterName:      a.cowriterName,
        cowriterTitle:     a.cowriterTitle,
        photographer:      a.photographer,
        showPhotoCredits:  a.showPhotoCredits,
        subTitleNA:        a.subTitleNA,
        bannerStatementNA: a.bannerStatementNA,
        bodyComplete:      a.bodyComplete
        // body html intentionally NOT sent — body editing happens
        // in edit mode after first save
      }
    };
    log('createArticle payload', payload);

    if (!url) {
      toast('makeStudio URL not configured — see console for payload', 'error');
      console.log('[ASF v' + VERSION + '] would POST createArticle →', payload);
      return;
    }
    ...
    fetch(url, {
      ...
      .then(function (body) {
        ...
        if (!j || j.ok !== true) {
          throw new Error('Scenario G did not confirm createArticle — likely the Make route is not yet built. Build it as a new Scenario G route that accepts { action:"createArticle", fields:{...} } and returns { ok:true, articleItemId:"<new>" } after the Webflow record is created.');
        }
        var newId = j.articleItemId || j.articleId || null;
        if (!newId) {
          throw new Error('createArticle succeeded but no articleItemId returned in response');
        }
        ...
        snapshotOriginals();
        render();
        toast('Article created — drafted as "' + (a.name || 'Untitled') + '"', 'success');
      })
      .catch(function (err) {
        S.saving = false;
        setSaveButtonState('idle');
        warn('createArticle failed', err);
  ...
  log('mounted · v' + VERSION + ' · Create mode enabled — open({mode:"create"}) starts a blank article. Save Draft in create mode posts createArticle to Scenario G; needs Make route built. Publish disabled until first save.');
})();
