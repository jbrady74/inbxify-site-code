/* ============================================================
   ...
   v1.0.1 changes (S9 Part A — schema migration only):
     - readMediaItems(): rewritten for post-S6 MEDIA schema
         was: data-item-id / data-name / data-uc-uuid / data-pipeline-status / data-article-ref
         now: data-media-id / data-media-name / data-media-type /
     ...
     - Insert flow: <img src> uses bare data-image-url (no JS-side transform)
                   data-media-id attribute on <img> = Webflow MEDIA Item ID (for body-save reconciliation)
     - Removed: imageWidths config, width selector UI, buildUcUrl helper
     - Caption input retained
     - Fixed duplicate destroy() method (TD-137); single canonical destroy()
     - openFullscreen / init contracts UNCHANGED — Studio v1.2.6 and Transcriber callers continue to work without modification

   Out of scope for v1.0.1 (deferred to A.3 / Part B):
...
(function () {
  ...
  /* ── Read MEDIA items from hidden collection list ──
     Schema (post-S6, confirmed via Webflow Designer audit on Test Account Weekly):
       data-media-id        ← (Self) MEDIA Item ID  (Webflow Item ID — used as body-save reconciliation marker)
       data-media-name      ← Name
       data-media-type      ← Media Type            (option NAME, e.g. "Image" or "Text")
       data-component-role  ← Component Role        (option NAME, e.g. "Interior Image" or "Article Body HTML")
       ...
       data-image-url       ← Image URL             (full Uploadcare URL, already conditioned by Scenario B)

     Multi-tenant scope is enforced server-side by the Collection List's TITLE-ADMIN filter
     (Designer-confirmed: media-source filters on TITLE-ADMIN = Current TITLES-ADMIN).
     No JS-side T-A filter is required.
  ── */
  function readMediaItems(selector) {
    ...
    // Each MEDIA record renders as a .w-dyn-item. Inside the item is a child div
    // (named "media-wrapper" in the page DOM) carrying all data-* attributes.
    // Use querySelector('[data-media-id]') for resilience against future structural changes.
    const items = wrapper.querySelectorAll('.w-dyn-item');
    const media = [];
    items.forEach(el => {
      const host = el.querySelector('[data-media-id]') || el;
      const id            = host.getAttribute('data-media-id')        || '';
      const name          = host.getAttribute('data-media-name')      || '';
      const mediaType     = host.getAttribute('data-media-type')      || '';
      const componentRole = host.getAttribute('data-component-role')  || '';
      const status        = host.getAttribute('data-status')          || '';
      const articleId     = host.getAttribute('data-article-id')      || '';
      const imageUrl      = host.getAttribute('data-image-url')       || '';
      if (id) {
