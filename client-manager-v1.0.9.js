...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window.cmUploadcare = function (slug) {
    if (window.uploadcare && window.uploadcare.openPanel) {
      // Uploadcare v3.x — openPanel
      uploadcare.openPanel(null, {
        publicKey: window.UPLOADCARE_PUBLIC_KEY || '',
        imagesOnly: true,
        multiple: false
      }).done(function (file) {
        file.done(function (info) { cmSetVal(slug, info.cdnUrl); });
      });
    } else if (window.uploadcare && window.uploadcare.Widget) {
      // Uploadcare v3.x Widget approach — create temp input
      var inp = document.createElement('input');
      inp.type = 'hidden';
      inp.setAttribute('role', 'uploadcare-uploader');
      inp.setAttribute('data-images-only', '');
