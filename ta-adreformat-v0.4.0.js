...
(function () {
  ...
  function openEditor() {
    ...
    ed.innerHTML =
      ...
      '<b>Reset</b> restores the original.</div>' +
  ...
  function composeBanner(cb) {
    ...
    S.layers.forEach(function (L) {
      ...
      if (L.type === 'img' && L.canvas) {
        ...
        ctx.font = (L.weight || 400) + ' ' + ((L.size || 13) * sx) + 'px "DM Sans", sans-serif';
        ctx.textBaseline = 'middle';
        // simple single-line clip to the box width
        ctx.save();
        ctx.beginPath(); ctx.rect(x, y, w, h); ctx.clip();
        ctx.fillText(L.value || '', x + 2 * sx, y + h / 2);
        ctx.restore();
      }
    });
