/* ============================================================
   ta-asset-workbench-v0.3.1.js
   INBXIFY TA Studio — Asset Workbench (detail/management surface)
   Companion stylesheet: ta-asset-workbench-v0.3.1.css

   ────────────────────────────────────────────────────────────
   v0.3.1 — normalizeMedia tolerates Make aggregator fieldData shape;
...
(function () {
  ...
  function hydrateAsset(assetType, assetId) {
    ...
    function normalizeMedia(arr) {
      return (arr || []).map(function (m) {
        // Tolerate two shapes:
        //  (a) flat:           { mediaId, name, role, src }
        //  (b) Make aggregator:{ id, fieldData: { name, 'component-role', 'image-url' } }
        var fd = m.fieldData || m.fielddata || {};
        return {
          mediaId: m.mediaId || m.id || '',
          name:    m.name || fd.name || '',
          role:    m.role || m.componentRole || m['component-role'] ||
                   fd['component-role'] || fd.componentRole || '',
  ...
  // Edit → relaunch the correct ASF variant for this asset type (no 2nd
  // editor). NOTE: ASF currently only hydrates EDIT mode for 'article';
  // for ad/event/realestate ASF will force CREATE mode until ASF edit-
  // hydration ships for those types. We still pass the real assetType +
  // id so the moment ASF supports it, this works unchanged.
  function launchEdit() {
    ...
    publicClose();
    window.InbxASF.open({ assetType: type, articleId: id, assetId: id });
  }
