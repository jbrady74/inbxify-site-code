/* ============================================================
   ...
   v1.1.1 changes (S11 testing fixes):
     - swapPlaceholderForImage(): find placeholder by class only
       (was: class + data-rte-upload-id). Trix wraps content
       attachments in its own <figure data-trix-attachment="..."> and
       JSON-serializes the content payload, which mangles data-* attrs
       on the inner figure. The class survives intact. Since the
       upload modal is modal — only one upload runs at a time per RTE
       instance — disambiguation by ID is unnecessary. Same fix
       applied to removeUploadPlaceholder().
     - .rte-img-upload-btn span: explicit color:white !important to
       override Trix's .trix-button cascade that was bleeding the
       text color toward gray on first paint.
...
(function () {
  ...
  class InbxRTE {
    ...
    swapPlaceholderForImage(placeholderId, opts) {
      // S11 NOTE: Trix wraps our content attachment inside its own
      // <figure data-trix-attachment="..."> and JSON-serializes the
      // content payload, which mangles data-* attributes on the inner
      // figure (e.g. data-rte-upload-id="..." becomes
      // data-rte-upload-id="\&quot;...\&quot;"). The CLASS survives
      // intact. Since the modal is modal — only one upload runs at a
      // time per RTE instance — we don't need to disambiguate by ID;
      // we just find "the" pending placeholder by class.
      const inner = this.mount.querySelector('figure.rte-upload-placeholder');
      if (!inner) {
        // Placeholder wasn't found; fall back to inserting a fresh figure
        // at the current cursor — better than losing the upload.
        console.warn('[RTE inline upload] placeholder not found; inserting at cursor');
        this.insertCompletedInlineImage(opts);
        return;
      }
      // Build the replacement figure markup
      inner.classList.remove('rte-upload-placeholder');
      inner.removeAttribute('data-rte-upload-id');
      inner.innerHTML =
        `<img src="${esc(opts.url)}" alt="${esc(opts.altText || '')}" ` +
          `data-media-id="${esc(opts.mediaItemId)}" ` +
          `data-component-role="${esc(opts.componentRole || '')}">`;

      // Trix mirrors editor DOM to its hidden input on the next change tick.
      // Force a sync by triggering a no-op edit.
      try {
        if (this.trixEditor && this.trixEditor.element) {
          this.trixEditor.element.dispatchEvent(new Event('trix-change', { bubbles: true }));
        }
      } catch (e) { /* noop */ }
    }
    ...
    removeUploadPlaceholder(placeholderId) {
      // Find the placeholder by class only (see swapPlaceholderForImage
      // for the rationale on why we don't disambiguate by ID).
      const inner = this.mount.querySelector('figure.rte-upload-placeholder');
      if (!inner) return;
      // Find Trix's outer attachment wrapper if present
      const trixAttachment = inner.closest('[data-trix-attachment]');
      const target = trixAttachment || inner;
    ...
    injectStyles() {
      ...
      style.textContent = `
        ...
        .rte-img-upload-btn { display:inline-flex !important; align-items:center !important; gap:4px !important; padding:0 12px !important; font-family:'DM Mono',monospace !important; font-size:10px !important; font-weight:600 !important; color:white !important; background:${TEAL} !important; border:1px solid ${TEAL} !important; border-radius:4px !important; margin-left:4px !important; height:30px !important; cursor:pointer !important; letter-spacing:0.02em; }
        .rte-img-upload-btn:hover { background:#0f2929 !important; border-color:#0f2929 !important; }
        .rte-img-upload-btn:disabled { opacity:0.5 !important; cursor:not-allowed !important; }
        .rte-img-upload-btn span { pointer-events:none; color:white !important; }
