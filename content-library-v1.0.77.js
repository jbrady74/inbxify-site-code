...
document.addEventListener('DOMContentLoaded', function () {
  ...
  style.textContent = [
    ...
    /* v1.0.77 multi-placement pills. Wrap is allowed but capped at two
       + a counter, so the column cannot be pushed wide by a reused
       asset. Colours are taken from the existing assign cell, not
       invented — this is the same information, counted. */
    '.cl-pp-pills{display:flex;flex-wrap:wrap;gap:4px;align-items:center}',
    '.cl-pp-pill{display:inline-block;padding:1px 7px;border-radius:9px;font-size:11px;line-height:1.6;'
      + 'font-weight:600;white-space:nowrap;background:#eef3fb;color:#0c447c;border:1px solid #cfe0f4}',
    '.cl-pp-more{background:transparent;color:#5b6b7f;border-color:#d8dee6;cursor:default}',
  ...
  function render() {
    ...
    if (!items.length) {
      ...
      items.forEach(function (item) {
        ...
        // v1.0.77: an asset can sit in several PubPlans at once. This
        // column showed ONE and there was no signal the others existed.
        var pl = item.placements || [];
        if (pl.length > 1) {
          var shownPl = pl.slice(0, 2), restPl = pl.length - shownPl.length;
          h += '<div class="cl-pp-pills">';
          shownPl.forEach(function (r) {
            h += '<span class="cl-pp-pill" title="' + esc(r.label + (r.date ? ' \u00b7 ' + r.date : '')) + '">' +
                 esc(r.label) + '</span>';
          });
          if (restPl > 0) {
            // Every label in the tooltip — the +N must be answerable
            // without leaving the row.
            var restNames = pl.slice(2).map(function (r) { return r.label; }).join(', ');
            h += '<span class="cl-pp-pill cl-pp-more" title="' + esc(restNames) + '">+' + restPl + '</span>';
          }
          h += '</div>';
        } else if (item.pubplanName || item.pubplanId) {
          var assignPp = PUBPLANS.find(function (p) { return p.id === item.pubplanId; });
          var ppLabel = item.pubplanName || (assignPp && assignPp.label) || 'Assigned';
          h += '<div class="cl-assign-issue">' + esc(ppLabel) + '</div>';
          if (assignPp && assignPp.date) h += '<div class="cl-assign-mnls">' + esc(assignPp.date) + '</div>';
