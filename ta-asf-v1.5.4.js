...
(function () {
  ...
  var ARTICLE_HYDRATE_MAP = {
    // identity / text — inverse of the updateAsset write map (build-spec)
    name:             'name',
    slug:             'slug',
    subtitle:         'sub-title',
    ...
    writerTitle:      ['writer-title', 'writertitle'],
    ...
    // body — read for display continuity; NOT re-written via updateAsset
    bodyHtml:         ['post-body-html', 'post-body'],
    // main image — URL-only render
    mainImageSrc:     'main-image',
    // refs (ids)
  ...
  function publicOpen(params) {
    ...
    if (mode === 'edit' && assetType !== 'article') {
      // v1.5.4: article reverted to DOM hydrate (webhook article path
      // returned no fieldData → blank form). ad/event/RE stay on webhook.
      // Seed the blank shape immediately so applyPrefill has a target,
      // then hydrate from the webhook and prefill.
