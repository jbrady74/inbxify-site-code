/* ============================================================
   ...
   v1.0.18 — subsForMajor() no longer trusts REF_DATA.subs alone —
             that DOM-derived list is capped at Webflow's 100-item
             List Items ceiling, and with 145+ subs across the real
             taxonomy most never render into the page at all. Selecting
             a Major now fires a live fetch (fetchSubcategoriesForMajor)
             against the 112_IBX_getBusinessSubcategories Make webhook,
             which paginates the full collection Make-side and returns
             both pages; the client filters by parent-category-bc and
             caches per major-id (SUBCAT_CACHE) so repeat selections in
             the same session don't re-fetch. Falls back to the old
             DOM-capped list if TA_CONFIG.getSubcategoriesUrl isn't set
             or the fetch fails, so nothing breaks if unconfigured.
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var DEFAULT_GROUPS = [
    ...
    { id: 'relationships', label: 'Relationships', fields: [
      { s: 'publisher', l: 'Publisher', t: 'readonly' },
      { s: 'titles-admin', l: 'Titles-Admin', t: 'readonly' },
      { s: 'directories', l: 'Directories', t: 'ref-select', src: 'directories' },
      { s: 'directory-tags', l: 'Directory Tags', t: 'text' },
      { s: 'major-category', l: 'Major Category', t: 'ref-select', src: 'majors' },
      { s: 'business-category', l: 'Business Category', t: 'ref-cascade', src: 'subs', parentField: 'major-category' },
      { s: 'filtering-categories', l: 'Filtering Categories', t: 'text' },
      { s: 'parent-category-name', l: 'Parent Category Name', t: 'auto', from: 'major-category', prop: 'name' },
      { s: 'parent-category-id', l: 'Parent Category ID', t: 'auto', from: 'major-category', prop: 'id' }
    ]},
    ...
    { id: 'archives', label: 'Archives', fields: [
      { s: 'archive-assignment', l: 'Newsletter Archive', t: 'ref-select', src: 'archives' }
    ]},
  ...
  function subsForMajor(majorId) {
    ...
    // subs never make it into REF_DATA.subs at all. So: check a live cache
    // first (populated on-demand by fetchSubcategoriesForMajor below); fall
    // back to the DOM-derived list only if the live fetch hasn't run yet or
    // failed, so something still shows rather than a hard blank.
    if (SUBCAT_CACHE[majorId]) return SUBCAT_CACHE[majorId];
    return REF_DATA.subs.filter(function (s) { return s.parentId === majorId; });
  }

  // v1.0.18: per-major subcategory cache, populated live from the
  // 112_IBX_getBusinessSubcategories webhook (paginates past Webflow's
  // 100-item List Items ceiling on the Make side, then this filters by
  // parent-category-bc client-side — see build notes in that scenario).
  var SUBCAT_CACHE = {};
  var SUBCAT_LOADING = {}; // majorId -> true while a fetch is in flight

  function fetchSubcategoriesForMajor(majorId) {
    if (!majorId || SUBCAT_CACHE[majorId] || SUBCAT_LOADING[majorId]) return;
    if (!CFG.subcatUrl) {
      console.warn('[CLIENT MGR] Subcategory endpoint not configured (TA_CONFIG.getSubcategoriesUrl) — falling back to DOM-capped list.');
      return;
    }
    SUBCAT_LOADING[majorId] = true;
    if (MODAL) renderModal(); // shows "Loading…" in the cascade dropdown
    ...
    fetch(CFG.subcatUrl, {
      ...
      .then(function (data) {
        if (!data || !data.ok) throw new Error((data && data.error) || 'Subcategory fetch failed');
        var combined = [].concat(data.page1 || [], data.page2 || []);
        var subs = combined
          .filter(function (it) {
            var fd = it.fieldData || {};
            return fd['parent-category-bc'] === majorId;
          })
          .map(function (it) {
            var fd = it.fieldData || {};
            return { id: it.id, name: fd.name || '', parentId: fd['parent-category-bc'] };
          })
          .sort(function (a, b) { return a.name.localeCompare(b.name); });
        SUBCAT_CACHE[majorId] = subs;
      })
      .catch(function (err) {
        console.warn('[CLIENT MGR] Subcategory fetch failed, using DOM-capped fallback:', err && err.message);
      })
      .finally(function () {
        delete SUBCAT_LOADING[majorId];
  ...
  function renderModal() {
    ...
    if (isAdd && CFG.extractClientUrl) {
      ...
      // Logo candidate picker
      if (M.scrapeImages && M.scrapeImages.length) {
        h += '<div class="cm-logo-pick"><div class="cm-logo-pick-lbl">Pick a logo (sets Logo Link):</div><div class="cm-logo-strip">';
        M.scrapeImages.forEach(function (src, i) {
          var sel = (M.edit['logo-link'] === src) ? ' cm-logo-sel' : '';
          h += '<button class="cm-logo-thumb' + sel + '" title="' + esc(src) + '" onclick="cmPickLogo(' + i + ')">' +
               '<img src="' + esc(src) + '" alt="logo candidate ' + (i + 1) + '" loading="lazy" ' +
               'onerror="this.closest(\'.cm-logo-thumb\').style.display=\'none\'"></button>';
        });
        h += '</div></div>';
      }
      h += '</div>';
    ...
    GROUPS.forEach(function (g) {
      ...
      g.fields.forEach(function (f) {
        ...
        if (f.t === 'auto') {
          ...
          M.edit[f.s] = autoVal;
          var autoHas = autoVal !== '';
          h += '<div class="cm-gi' + (autoVal !== origVal ? ' cm-dirty' : '') + '">';
        ...
        h += '<div class="cm-gi' + (isDirty ? ' cm-dirty' : '') + '" data-cm-field="' + esc(f.s) + '">';
        h += '<div class="cm-gi-d ' + dot + '"></div>';
        h += '<span class="cm-gi-l">' + f.l + '</span>';
        ...
        if (isEditing) {
          ...
          if (f.t === 'upload') {
            ...
            var isLoadingSubs = parentVal && SUBCAT_LOADING[parentVal];
      ...
      .then(function (data) {
        ...
        if (majorGuess && String(majorGuess).trim() !== '') {
          ...
          if (matched) {
            ...
            fetchSubcategoriesForMajor(matched.id);
          }
        }
  ...
  window.cmPickLogo = function (i) {
    ...
    if (!src) return;
    // Toggle: click selected logo again to clear.
    MODAL.edit['logo-link'] = (MODAL.edit['logo-link'] === src) ? '' : src;
    renderModal();
  };
  ...
  window.cmSetVal = function (slug, val, noRender) {
    ...
    if (slug === 'major-category') {
      ...
      if (val) fetchSubcategoriesForMajor(val);
    }
  ...
  function getCss() {
    return [
      ...
      '.cm-logo-thumb{width:54px;height:54px;padding:3px;border:1.5px solid #d8d3c4;border-radius:5px;background:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;overflow:hidden;transition:border-color .12s}',
