...
//
// v1.0.1 CHANGES:
//   - FA/TS: article title as main name, category as detail line
//   - TF: customer name as main display name
//   - TF/LBP: detect mode from products-wrapper sectionCode,
//     show 1 tile + spacer to match 5-tile height
//
...
(function () {
  ...
  function getSlotDisplayName(sec, slotData, catData) {
    ...
    if (sec.code === 'tf') {
      // Customer name is the main display for TF
      return (slotData && slotData.customerName) || '';
    }
    if (sec.code === 'ba' || sec.code === 'sb' || sec.code === 're') {
      return (slotData && slotData.customerName) || '';
    }
    if (sec.code === 'ev') {
      return (slotData && slotData.eventId) ? 'Event assigned' : '';
    }
    return '';
  }
  ...
  function getSlotDetail(sec, slotData, catData) {
    if (!slotData && !catData) return '';
    if (sec.code === 'fa' || sec.code === 'ts') {
      // Category label as detail line under the article title
  ...
  function renderIssueColumn(issue, isFirst) {
    ...
    SECTIONS.forEach(function (sec) {
      ...
      var tfMode = null;
      var tfSlotCount = sec.count;
      if (sec.code === 'tf') {
        tfMode = detectTfMode(issue);
        if (tfMode.mode === 'lbp') tfSlotCount = 1;
        else if (tfMode.mode === 'none') tfSlotCount = 0;
      }

      h += '<div class="pp-section-hdr ' + sec.hdrClass + '"><span class="pp-sh-bar"></span>' + sec.label + '</div>';

      // TF with no category — show empty placeholder at full height
      if (sec.code === 'tf' && tfMode.mode === 'none') {
        h += '<div class="pp-tf-empty-block"><span class="pp-slot-cat empty-cat">No category set</span></div>';
        h += '<div class="pp-section-gap"></div>';
        return;
      }

      var renderCount = (sec.code === 'tf') ? tfSlotCount : sec.count;
      ...
      for (var i = 1; i <= renderCount; i++) {
        ...
        // For FA/TS: article title is main, category is detail
        // For TF/BA/etc: customer name is main
        var displayName = getSlotDisplayName(sec, contentData, catData);

        // Fallback: if no article title but has category, show category as display
        if (!displayName && catData && catData.catLabel) {
          displayName = catData.catLabel;
        }

        var detailLine = getSlotDetail(sec, contentData, catData);
        ...
        // LBP mode: show "LBP" instead of slot number
        var slotLabel = (sec.code === 'tf' && tfMode.mode === 'lbp') ? 'LBP' : String(i);
      ...
      // LBP mode: pad remaining 4 slots worth of height
      if (sec.code === 'tf' && tfMode.mode === 'lbp') {
        // 4 missing slots × (44px height + 3px margin) = 188px
        h += '<div class="pp-tf-lbp-spacer"></div>';
      }
