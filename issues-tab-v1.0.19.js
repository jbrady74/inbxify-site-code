...
//
//   HC-039 (NEW) — PROMOTE_NEXT_STATUS_ID is hardcoded client-side
//     so there is something local to diff the echo against. It
//     mirrors the option ID already hardcoded in Scenario 107's
//     Update module. Single-tenant safe: publishing-status is a
//     platform-level option set on NEWSLETTERS, shared by every
//     publisher, not a per-title value. Retire when the status
//     option set moves into TA_CONFIG.optionIds.
//
...
(function () {
  ...
  // ── Promote target status (NEW v1.0.19 — HC-039) ──
  // NEWSLETTERS > Publishing Status > "Publish Next as Current".
  // Confirmed against the live Webflow option set 2026-09-05.
  // This is NOT sent to Make — Scenario 107 sets the status itself.
  // It exists so the client has a known-good value to diff the
  // echoed write against. If the option is ever renamed or
  // recreated in Webflow, this ID changes and every promote will
  // start toasting red with a named mismatch — which is the
  // correct, loud failure.
  var PROMOTE_NEXT_STATUS_ID = '9e7e2a932044010bc7c50840a54c45ba';
  ...
  function promoteToNext(btn) {
    ...
    IxToast.submit({
      ...
      sent: { 'publishing-status': PROMOTE_NEXT_STATUS_ID },
