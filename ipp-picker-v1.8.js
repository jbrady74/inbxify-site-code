/* ============================================================
   ipp-picker-v1.8.js — Picker view (+ define-order fix for registerDropdown)
   CHANGELOG
...
(function () {
  ...
  function wireTileDropdowns(){
    if(!IPP.registerDropdown || !IPP.dropdown){
      // Either dependency not ready yet. mount() is guarded by
      // data-picker-mounted (never re-runs), so self-retry rather than
      // bail. Covers both load-order (ipp-dropdown) AND in-file define
      // order (registerDropdown). ~50ms × up to 40 = ~2s.
      if(wireTileDropdowns._tries==null) wireTileDropdowns._tries=0;
      if(wireTileDropdowns._tries++ < 40){ setTimeout(wireTileDropdowns, 50); return; }
      console.warn('[ipp-picker] dropdown deps never ready (registerDropdown='+!!IPP.registerDropdown+' dropdown='+!!IPP.dropdown+') — tiles unwired'); return;
    }
    ...
    console.info('[ipp-picker] tile dropdowns wired ('+document.querySelectorAll('[data-dd-wired]').length+' buttons)');
  }
  ...
  function mount(){
    ...
    console.info('[ipp-picker] mounted (v1.8 · editState + tile dropdowns · define+load-order robust · save stubbed Path 1)');
  }
