...
(function () {
  ...
  function stitchNow(run) {
    ...
    postWorker('/stitch', { reads: run.reads, titleName: cfg().titleName || '' }).then(function (j) {
      if (!live(run)) return;
      ...
      if (!live(run)) return;
      run.phase = 'stitchfail'; run.error = String(e.message || e); renderPz();
    });
  }
