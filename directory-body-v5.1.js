/* ============================================
   ...
   File: directory-body-v5.1.js
   GitHub: jbrady74/inbxify-site-code
   jsDelivr: https://cdn.jsdelivr.net/gh/jbrady74/inbxify-site-code@latest/directory-body-v5.js
...
(function () {
  ...
  /* --- Exposed for pill click integration (task #4) — re-exposed after patching below --- */
  ...
  /* ===========================================
     7. MOBILE/TABLET ICON BAR
     Replaces full filter bar at ≤991px.
     Search + Filter icons, expand panels float
     over cards. Syncs with filter state.
     =========================================== */
  var iDOM = {};
  ...
  function iInjectIconBar() {
    ...
    // Main container
    iDOM.bar = D.createElement('div');
    iDOM.bar.className = 'dir-icon-bar';

    iDOM.pill = D.createElement('div');
    iDOM.pill.className = 'dir-icon-pill';

    // Search icon button
    iDOM.searchBtn = D.createElement('button');
    iDOM.searchBtn.className = 'dir-icon-btn';
    iDOM.searchBtn.type = 'button';
    iDOM.searchBtn.setAttribute('aria-label', 'Search');
    iDOM.searchBtn.innerHTML = SEARCH_SVG;

    // Divider
    var divider = D.createElement('div');
    divider.className = 'dir-icon-divider';

    // Filter icon button
    iDOM.filterBtn = D.createElement('button');
    iDOM.filterBtn.className = 'dir-icon-btn';
    iDOM.filterBtn.type = 'button';
    iDOM.filterBtn.setAttribute('aria-label', 'Filter');
    iDOM.filterBtn.innerHTML = FILTER_SVG;

    // Filter badge dot
    iDOM.badge = D.createElement('div');
    iDOM.badge.className = 'dir-filter-badge';
    iDOM.filterBtn.appendChild(iDOM.badge);

    // Active filter pill
    iDOM.activePill = D.createElement('span');
    iDOM.activePill.className = 'dir-active-pill';

    // Reset button
    iDOM.iconReset = D.createElement('a');
    iDOM.iconReset.className = 'dir-icon-reset';
    iDOM.iconReset.href = '#';
    iDOM.iconReset.textContent = 'RESET';
    iDOM.iconReset.addEventListener('click', function (e) {
      e.preventDefault();
      fResetFilters();
      iSyncState();
      iCloseAll();
    ...
    // Result count
    iDOM.iconCount = D.createElement('span');
    iDOM.iconCount.className = 'dir-icon-count';
    iDOM.iconCount.innerHTML = '<span>' + filterState.allCards.length + '</span> <span>results</span>';

    // Assemble pill
    iDOM.pill.appendChild(iDOM.searchBtn);
    iDOM.pill.appendChild(divider);
    iDOM.pill.appendChild(iDOM.filterBtn);
    iDOM.pill.appendChild(iDOM.activePill);
    iDOM.pill.appendChild(iDOM.iconReset);
    iDOM.pill.appendChild(iDOM.iconCount);

    // --- Search expand panel ---
    iDOM.searchPanel = D.createElement('div');
    iDOM.searchPanel.className = 'dir-expand-panel';
    var searchDiv = D.createElement('div');
    searchDiv.className = 'dir-panel-search';
    iDOM.mobileSearchInput = D.createElement('input');
    iDOM.mobileSearchInput.type = 'text';
    iDOM.mobileSearchInput.placeholder = 'Search the Directory';
    searchDiv.appendChild(iDOM.mobileSearchInput);
    iDOM.searchPanel.appendChild(searchDiv);

    // --- Filter expand panel ---
    iDOM.filterPanel = D.createElement('div');
    iDOM.filterPanel.className = 'dir-expand-panel';
    var filterDiv = D.createElement('div');
    filterDiv.className = 'dir-panel-filters';

    // Clone the category and subcategory wrappers into the filter panel
    if (fDOM.catWrap && fDOM.subWrap) {
      iDOM.mobileCatWrap = fDOM.catWrap.cloneNode(true);
      iDOM.mobileSubWrap = fDOM.subWrap.cloneNode(true);
      filterDiv.appendChild(iDOM.mobileCatWrap);
      filterDiv.appendChild(iDOM.mobileSubWrap);
    }
    iDOM.filterPanel.appendChild(filterDiv);

    // Assemble bar
    iDOM.bar.appendChild(iDOM.pill);
    iDOM.bar.appendChild(iDOM.searchPanel);
    iDOM.bar.appendChild(iDOM.filterPanel);
  ...
  function iBindEvents() {
    // Toggle search panel
    iDOM.searchBtn.addEventListener('click', function (e) {
      e.preventDefault(); e.stopPropagation();
      var wasOpen = iDOM.searchPanel.classList.contains('is-open');
      iCloseAll();
      if (!wasOpen) {
        iDOM.searchPanel.classList.add('is-open');
        iDOM.searchBtn.classList.add('is-active');
        setTimeout(function () { iDOM.mobileSearchInput.focus(); }, 50);
      }
    });

    // Toggle filter panel
    iDOM.filterBtn.addEventListener('click', function (e) {
      e.preventDefault(); e.stopPropagation();
      var wasOpen = iDOM.filterPanel.classList.contains('is-open');
      iCloseAll();
      if (!wasOpen) {
        iDOM.filterPanel.classList.add('is-open');
        iDOM.filterBtn.classList.add('is-active');
        // Rebuild mobile dropdown options
        iRebuildMobileDropdowns();
      }
    });

    // Sync mobile search input with Jetboost search
    var desktopSearch = Q('.jetboost-list-search-input-19pb');
    if (desktopSearch) {
      iDOM.mobileSearchInput.addEventListener('input', function () {
        desktopSearch.value = iDOM.mobileSearchInput.value;
        desktopSearch.dispatchEvent(new Event('input', { bubbles: true }));
        setTimeout(function () { iSyncCount(); }, 300);
      });
    }

    // Close panels on outside click
    D.addEventListener('click', function (e) {
      if (!e.target.closest('.dir-icon-bar')) {
        iCloseAll();
      }
    });

    // Mobile dropdown delegation
    if (iDOM.mobileCatWrap) {
      iDOM.mobileCatWrap.querySelector('.dir-filter-select')?.addEventListener('click', function (e) {
        e.preventDefault(); e.stopPropagation();
        var panel = iDOM.mobileCatWrap.querySelector('.dir-filter-panel');
        var wasOpen = iDOM.mobileCatWrap.classList.contains('is-open');
        iDOM.mobileCatWrap.classList.remove('is-open');
        iDOM.mobileSubWrap?.classList.remove('is-open');
        if (!wasOpen) {
          iDOM.mobileCatWrap.classList.add('is-open');
          iRebuildMobileDropdowns();
        }
      });

      iDOM.mobileCatWrap.querySelector('.dir-filter-panel')?.addEventListener('click', function (e) {
        var item = e.target.closest('.dir-filter-item');
        if (!item) return;
        fSelectCategory(item.dataset.value);
        iDOM.mobileCatWrap.classList.remove('is-open');
    ...
    if (iDOM.mobileSubWrap) {
      iDOM.mobileSubWrap.querySelector('.dir-filter-select')?.addEventListener('click', function (e) {
        e.preventDefault(); e.stopPropagation();
        var wasOpen = iDOM.mobileSubWrap.classList.contains('is-open');
        iDOM.mobileCatWrap?.classList.remove('is-open');
        iDOM.mobileSubWrap.classList.remove('is-open');
        if (!wasOpen) {
          iDOM.mobileSubWrap.classList.add('is-open');
          iRebuildMobileDropdowns();
        }
      });

      iDOM.mobileSubWrap.querySelector('.dir-filter-panel')?.addEventListener('click', function (e) {
        var item = e.target.closest('.dir-filter-item');
        if (!item) return;
        fSelectSubcategory(item.dataset.value);
        iDOM.mobileSubWrap.classList.remove('is-open');
  ...
  function iRebuildMobileDropdowns() {
    ...
    // Rebuild category panel
    var catPanel = iDOM.mobileCatWrap.querySelector('.dir-filter-panel');
    if (catPanel) {
      catPanel.innerHTML = '';
      var sorted = Object.keys(filterState.catMap).sort(function (a, b) { return a.localeCompare(b); });
      catPanel.appendChild(fMakeItem('All Categories', fTotalCount(), filterState.activeCat === ''));
      sorted.forEach(function (cat) {
        catPanel.appendChild(fMakeItem(cat, filterState.catMap[cat].count, filterState.activeCat === cat));
      });
    }
    ...
    if (catBtn) {
      catBtn.textContent = filterState.activeCat || 'Category';
      if (filterState.activeCat) {
        catBtn.classList.remove('is-placeholder');
        iDOM.mobileCatWrap.classList.add('is-filtered');
      } else {
        catBtn.classList.add('is-placeholder');
        iDOM.mobileCatWrap.classList.remove('is-filtered');
      }
    }

    // Rebuild subcategory panel
    ...
    if (subPanel) {
      ...
      subSorted.forEach(function (sub) {
        subPanel.appendChild(fMakeItem(sub, subs[sub], filterState.activeSub === sub));
      });
    }
    ...
    if (subBtn) {
      subBtn.textContent = filterState.activeSub || 'Subcategory';
      if (filterState.activeSub) {
        subBtn.classList.remove('is-placeholder');
        iDOM.mobileSubWrap.classList.add('is-filtered');
      } else {
        subBtn.classList.add('is-placeholder');
        iDOM.mobileSubWrap.classList.remove('is-filtered');
      }
    }
  }
  ...
  function iSyncState() {
    ...
    // Badge
    if (hasFilter) {
      iDOM.filterBtn.classList.add('has-filter');
    } else {
      iDOM.filterBtn.classList.remove('has-filter');
    }

    // Active pill
    var pillText = filterState.activeSub || filterState.activeCat || '';
    if (pillText) {
      iDOM.activePill.textContent = pillText;
      iDOM.activePill.classList.add('is-visible');
    } else {
      iDOM.activePill.classList.remove('is-visible');
    }

    // Reset
    if (hasFilter) {
      iDOM.iconReset.classList.add('is-visible');
    } else {
      iDOM.iconReset.classList.remove('is-visible');
    }
  ...
  function iSyncCount() {
    ...
    filterState.allCards.forEach(function (c) {
      var wrapper = c.el.closest('.w-dyn-item') || c.el;
      if (getComputedStyle(wrapper).display !== 'none') v++;
    });
    iDOM.iconCount.innerHTML = '<span>' + v + '</span> <span>results</span>';
  }

  // Patch fUpdateResultCount to also sync icon bar
  var origFUpdateResultCount = fUpdateResultCount;
  fUpdateResultCount = function (count) {
    origFUpdateResultCount(count);
    if (iDOM.iconCount) {
      iDOM.iconCount.innerHTML = '<span>' + count + '</span> <span>results</span>';
    }
  };

  // Patch fSelectCategory/fSelectSubcategory/fResetFilters to sync icon bar
  ...
  fResetFilters = function () {
    origReset();
    iSyncState();
    if (iDOM.mobileSearchInput) iDOM.mobileSearchInput.value = '';
  };

  // Re-expose patched functions
  ...
  function fCompleteInit() {
    ...
    console.log('[DirFilter] v5 ready —', filterState.allCards.length, 'cards,', Object.keys(filterState.catMap).length, 'categories');
  }
...
/* ===========================================
     8. CARD PILL CLICK → FILTER
     Clicking .card-category on any card filters
     to that parent category. Clicking
     .card-subcategory filters to parent + sub.
     Scrolls to top of card list. Works at all
     breakpoints. Uses the existing filter system.
     =========================================== */
  function bindPillClicks() {
    D.addEventListener('click', function (e) {
      var catEl = e.target.closest('.card-category');
      var subEl = e.target.closest('.card-subcategory');
      if (!catEl && !subEl) return;
      e.preventDefault();
      e.stopPropagation();
      var card = e.target.closest('.directory-card');
      if (!card) return;
      var cat = (card.dataset.category || '').trim();
      var sub = (card.dataset.subcategory || '').trim();
      if (!cat) return;
      if (subEl && sub) {
        fSelectCategory(cat);
        fSelectSubcategory(sub);
      } else {
        fSelectCategory(cat);
      }
      iSyncState();
      if (iDOM.bar) iRebuildMobileDropdowns();
      var scrollTarget = Q('.filter-bar') || Q('.directory-grid-wrapper') || Q('[class*="jetboost-list-wrapper"]');
      if (scrollTarget) {
