/* ============================================================
   ta-asset-workbench-v0.8.8.js
   INBXIFY TA Studio — Asset Workbench (detail/management surface)
   Companion stylesheet: ta-asset-workbench-v0.8.6.css  (UNCHANGED —
                         keep the existing head link)
   Shared stylesheet:    ix-internal-notes-v1_0_0.css
                         (self-loaded AND belongs in the Webflow head)

   ────────────────────────────────────────────────────────────
   v0.8.8 — stop GUESSING whether the article is published. Ask.

     WHAT v0.8.7 GOT WRONG. It gated the iframe on hasBeenPublished(),
     which read lastPublished / publishedOn off the hydrated asset.
     Scenario 120 does not return those fields — the sidebar's own
     Published Date renders "—" for exactly the same reason. So every
     article, published or not, failed the check and got the draft
     fallback with a message that was simply false.

     THE FIX IS TO STOP INFERRING. The page itself is the authority on
     whether the page exists. v0.8.8 always frames it and probes the
     proxy in parallel; a 404 swaps the pane to the standalone render.
     Evidence about a state we just observed, instead of a guess from
     a field that is not even in the payload.

     hasBeenPublished() is GONE rather than repaired. Any publish
     signal derived from the hydrate would have the same defect, and
     leaving a half-trusted helper in place invites its reuse.

     WHY A PROBE AND NOT iframe.onerror. A cross-origin iframe fires
     onload for a 404 body just as happily as for a real page; there
     is no readable status. The proxy is same-origin to fetch(), so a
     HEAD-style GET returns a status we can actually read.

   ────────────────────────────────────────────────────────────
   v0.8.7 — ARTICLE preview is the real page again.

     WHY THIS REVERSES v0.4.3. That version replaced the article
     iframe with renderArticleStandalone() and BAKED the article
     template's typography into .awb-article-render in the companion
     stylesheet. Its own changelog flagged the cost: the modal has to
     be edited in lockstep every time the article template changes.
     The requirement is the opposite — change the template, the modal
     follows, no code edit — and only the real page can deliver that,
     because element ORDER is hand-coded in the standalone render and
     a shared stylesheet would not carry it.

     THE 404 THAT KILLED IT WAS NOT ARTICLE. The Asset View Renderer
     initiative existed to remove "the 404 class caused by non-article
     asset types having no destination pages" — ads and real estate.
     Article has a real page at /articles-blog-posts/{slug} and was
     swept along with them. TD-215 had already solved the actual
     framing blocker (Webflow's frame-ancestors CSP) with the preview
     proxy, which is still here and still working for ad.

     WHAT CHANGED. renderRenderPane no longer short-circuits article
     to the standalone render. It falls through to the iframe pane,
     same as ad, through buildPreviewUrl().

     DRAFTS. An article that has never been published has no page to
     frame. renderArticleStandalone() is KEPT for exactly that case
     and is now a fallback, not the default: no publish timestamp →
     standalone render, with a note saying why. Published articles —
     the normal case — never touch the baked CSS again.

     PADDING. The article no longer sits inside .awb-render--standalone
     (32/24/64) or .awb-article-render (40/48/52). The iframe fills the
     pane edge to edge under the URL bar, which is what was asked for.
     No CSS change; those rules now apply only to the draft fallback,
     event and real estate.

   ────────────────────────────────────────────────────────────
   v0.8.6 — SELF-LOAD 404 FIX. ensureStylesLoaded() was still
     deliberately requesting the UNDERSCORED companion CSS name
     (a v0.8.4 workaround, back when that version's CSS really was
     committed with underscores). v0.8.5's CSS was committed with
     dots again, so the underscore guess fired a permanent, harmless
     -but-noisy 404 on every load. Reverted to dots; confirmed
     directly against the repo (v0.8.5.css → 200, v0_8_5.css → 404).
...
(function () {
  ...
  function ensureStylesLoaded() {
    ...
    // v0.8.4 FIX (now reverted, v0.8.6): repo filename for v0.8.4 was
    // committed with underscores (v0_8_4.css), so this deliberately
    // requested the underscored form. As of v0.8.5 the companion CSS
    // is committed with DOTS again (matches every other file's
    // convention + the <head> <link>), so the underscore guess was
    // firing a permanent 404 for a file that doesn't exist — confirmed
    // directly against the repo (v0.8.5.css = 200, v0_8_5.css = 404).
    // Back to dots. If a future bump gets committed with underscores
    // again, fix the COMMIT, not this line — don't reintroduce the
    // mangle-guess workaround.
    if (!SCRIPT_SRC) return;
    var wants = [
      'ta-asset-workbench-v' + VERSION + '.css',
      IX_NOTES_CSS
    ];
  ...
  // v0.8.8 — does the published page actually exist? ASK IT.
  // The hydrate carries no publish signal (Scenario 120 returns
  // fieldData only — no lastPublished, which is also why the sidebar's
  // Published Date shows "—"). Rather than infer from a field that is
  // not there, frame the page immediately and probe the proxy at the
  // same time. Only a 404 swaps the pane to the local render.
  //
  // The iframe cannot report this itself: a cross-origin frame fires
  // onload for a 404 body exactly as it does for a real page. The
  // proxy is fetch-reachable, so its status is readable.
  var _probeToken = 0;

  function scheduleArticleProbe(previewUrl) {
    var token = ++_probeToken;
    setTimeout(function () { probeArticlePage(previewUrl, token); }, 0);
  }

  function probeArticlePage(previewUrl, token) {
    fetch(previewUrl, { method: 'GET' })
      .then(function (r) {
        // A stale probe from a previously-opened asset must never
        // overwrite the pane of the one now on screen.
        if (token !== _probeToken) return;
        if (r.status === 404 || r.status === 410) swapToStandalone();
      })
      .catch(function () {
        // Network/proxy failure is NOT proof the article is a draft.
        // Leave the iframe alone — it may still render fine.
        log('article page probe failed; leaving iframe in place');
      });
  }

  function swapToStandalone() {
    var pane = document.querySelector('[data-awb-article-pane="1"]');
    if (!pane) return;
    pane.className = 'awb-render awb-render--standalone';
    pane.innerHTML =
      '<div class="awb-render-note awb-render-note--draft">' +
        'This article has no published page yet — showing a local ' +
        'render. Publish it to preview the real page.' +
      '</div>' +
      renderArticleStandalone(S.asset || {});
    log('article page 404 — swapped to standalone render');
  }
  ...
  function renderRenderPane(url, previewUrl) {
    // v0.8.8 — article always frames the real page. If the page turns
    // out not to exist, probeArticlePage() swaps this pane for the
    // standalone render once the proxy answers. No up-front guess.
    if (S.assetType === 'article' && url && previewUrl) {
      scheduleArticleProbe(previewUrl);
    } else if (S.assetType === 'article') {
      // no URL at all — nothing to frame, render locally and say so
      return (
        '<div class="awb-render awb-render--standalone" data-awb-article-pane="1">' +
          '<div class="awb-render-note awb-render-note--draft">' +
            'No published URL for this article yet — showing a local render.' +
          '</div>' +
          renderArticleStandalone(S.asset || {}) +
        '</div>'
      );
    ...
    return (
      '<div class="awb-render"' +
        (S.assetType === 'article' ? ' data-awb-article-pane="1"' : '') + '>' +
  ...
  ensureStylesLoaded();
  log('mounted · v' + VERSION + ' · article+ad iframe pane (proxy) · event/RE standalone · unpublished article detected by proxy probe, not by guessing');
})();
