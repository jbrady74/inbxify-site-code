...
(function () {
  ...
  function buildTfCategoryDropdown(selVal) {
    ...
    els.forEach(function(el) {
      if (['TF','The Find','tf'].indexOf(el.dataset.group) !== -1 && el.dataset.id) {
        cats.push({ id: el.dataset.id, name: el.dataset.name || '(unnamed)', type: el.dataset.type || '' });
      }
    });
    cats.sort(function(a, b) { return a.name.localeCompare(b.name); });
    var h = '<option value="" disabled' + (!selVal ? ' selected' : '') + '>Select category...</option>';
    h += '<option value="lbp"' + (selVal === 'lbp' ? ' selected' : '') + '>Local Business Profile (LBP)</option>';
    if (cats.length) {
      h += '<option disabled>───────────</option>';
      cats.forEach(function(c) {
        h += '<option value="' + c.id + '"' + (c.id === selVal ? ' selected' : '') + '>' + c.name + '</option>';
      });
    }
    return h;
  }
  ...
  window.handleTfCategoryChange = function(sel) {
    ...
    var newCatId = isLbp ? '' : val;
    var newCatNm = isLbp ? '' : (sel.options[sel.selectedIndex] ? sel.options[sel.selectedIndex].textContent : '');
    ...
    var doIt = function() {
      ...
      if (newMode === 'txa') {
        ...
        for (var i = 1; i <= 5; i++) {
          var s = state['TXA-' + i];
          if (s && s.custId) { s.dirty = true; markedAny = true; }
        }
        if (!markedAny && state['TXA-1']) state['TXA-1'].dirty = true;
      } else {
        if (state['LBP-1']) state['LBP-1'].dirty = true;
      }
    ...
    if (hasChanges) {
      showInlineConfirm(sel, 'Switching categories will clear unsaved changes.', doIt, function() {
        sel.value = currentTfMode === 'lbp' ? 'lbp' : tfCategoryId;
      });
    } else {
      doIt();
    }
  ...
  window.submitSection = async function(section) {
    ...
    if (section === 'gr') {
      ...
      else{var s=state['LBP-1'];if(s&&s.dirty)slots.push({'hidden-pubplan-id':pubplanId,'hidden-slot-label':s.sc,'hidden-titleadmin-id':s.titleadminId||'','hidden-section-code':s.secC,'hidden-category-group':s.section,'hidden-customer-id':s.custId,'hidden-customer-name':s.custNm,'hidden-category-id':'','hidden-category-name':'','hidden-tf-mode':'lbp'});}
    }
  ...
  function init() {
    ...
    if (tfCategoryId) {
      ...
      if (catEl) tfCategoryNm = catEl.dataset.name || '';
    }

    // FIX #14: Populate unified dropdown, replace mode toggle buttons
    var tfSel = document.getElementById('tf-category-select');
    if (tfSel) {
      var initVal = currentTfMode === 'lbp' ? 'lbp' : tfCategoryId;
      tfSel.innerHTML = buildTfCategoryDropdown(initVal);
      tfSel.classList.toggle('hs', !!initVal);
      tfSel.addEventListener('change', function() { handleTfCategoryChange(tfSel); });
    }
