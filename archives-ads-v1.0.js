 ...
 * v1.0 — April 4, 2026
 *
 * Pattern: identical to directory-ads-v1.5.js
...
(function () {
  ...
  var LEAD_POSITION = 3;   // After 3rd tile (index 2)
  var INTERVAL = 5;        // Then every 5 tiles
  ...
  function injectAds() {
    ...
    // Remove any previously injected ads (re-entry safety)
    var existing = list.querySelectorAll('.' + AD_CLASS);
    for (var i = 0; i < existing.length; i++) {
      existing[i].remove();
    }

    // Re-query tiles after cleanup
    ...
    // Lead ad after tile at LEAD_POSITION
    insertions.push({
    ...
    // Subsequent ads every INTERVAL tiles after lead position
  ...
  function watchForTiles(listEl) {
    // If tiles already present, inject
