...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
      ...
      '<div class="std-root">' +
        '<div class="ix-hdr">' +
          '<div class="ix-hdr-left">' +
            ...
            '</div>' +
          '</div>' +
          '<span class="ix-badge">v1.2.2</span>' +
        '</div>' +
    ...
    function asmFireAttach(slot, mediaId, articleId, it) {
      ...
      if (!roleHash) {
        console.error('[TA-STUDIO v1.2.2] No componentRole hash for slot:', slot);
      ...
      .catch(function (err) {
        console.error('[TA-STUDIO v1.2.2] Asm Route 4 failed:', err);
    ...
    function getRoleHash(roleLabel) {
      ...
      if (!hash) {
        console.error('[TA-STUDIO v1.2.2] No TA_CONFIG.optionIds.componentRole.' + key + ' configured for role:', roleLabel);
        return '';
      }
      return hash;
    }
    ...
    function getRoleHashForSlot(slotKey) {
      ...
      if (!hash) {
        console.error('[TA-STUDIO v1.2.2] No TA_CONFIG.optionIds.componentRole.' + slotKey + ' configured for slot:', slotKey);
        return '';
      }
      return hash;
    }
    ...
    function cmpFireAttach(mediaId, articleId, it) {
      ...
      if (!roleHash) {
        ...
        console.error('[TA-STUDIO v1.2.2] No componentRole hash for role:', it.role);
        showStudioToast('Configuration error \u2014 unknown role.');
        return;
      }
      ...
      .catch(function (err) {
        console.error('[TA-STUDIO v1.2.2] Scenario G Route 4 failed:', err);
    ...
    // ── Init ──
    renderAssembler();
    console.log('\u29BE Studio v1.2.2 mounted \u2014 Components tab (read + attach + archive + hard-delete) + Assembler field-level Attach (Main Image, Interior Images)');
  });
