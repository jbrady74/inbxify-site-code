...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  /* Issues this board is working on — NOT "the first four plans
     harvested", which is what v0.6.21 returned. That let September's
     manifest offer a tick-box for WLN-117, an issue that shipped in
     July.

     Eligible means: at or above the gate's Start at. Before anything
     is created that is usually empty, and an empty strip is the
     honest answer. */
  function openIssues() {
    var floor = parseInt(gateVal('start', ''), 10);
    if (isNaN(floor)) {
      var hi = highestPlan();
      floor = hi ? planNo(hi) : NaN;
    }
  ...
  function editionsHtml(issues) {
    var cells = '';
    if (!issues.length) {
      cells = '<div class="ixcb-ed-none">No issues yet. Create them above and they appear here.</div>';
    } else {
  ...
  function overviewBody() {
    ...
    var note = issues.length
      ...
      : 'Read from the print PDF. No issues exist yet, so assignment is closed \u2014 create them above to open it.';
