...
(function () {
  ...
  var S = {
    ...
    slateFilter: 'all',  // 'all' | 'nofile' | 'unassigned' | 'notfornl'
...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  function slateMatch(r, f) {
    if (f === 'nofile')     return !r.file;
    if (f === 'unassigned') return !r.plans.length;
    if (f === 'notfornl')   return r.disp === 'not-for-newsletter';
    return true;
  }
  ...
  function slateRows(issues) {
    ...
    for (var i = 0; i < S.slate.length; i++) {
      ...
      shown++;
      var ns = nextStep(r);
      var plans = r.plans.length
        ? r.plans.map(function (p) { return esc(p.label); }).join(' \u00b7 ')
        : '\u2014';

      var asn = '';
      for (var j = 0; j < issues.length; j++) {
        var on = false;
        for (var k = 0; k < r.plans.length; k++) {
          if (r.plans[k].label === issues[j].label || r.plans[k].id === issues[j].key) on = true;
        }
        asn += '<td class="ixcb-sl-asn">' +
                 '<input type="checkbox" disabled' + (on ? ' checked' : '') +
                 ' aria-label="' + esc(r.name + ' in ' + issues[j].label) + '">' +
               '</td>';
      }

      rows +=
        '<tr class="ixcb-sl-row">' +
          '<td class="ixcb-sl-pg">' + (r.page == null ? '\u2014' : r.page) + '</td>' +
          '<td class="ixcb-sl-sec">' + esc(r.section || '\u2014') + '</td>' +
          '<td class="ixcb-sl-ttl">' + esc(r.name) +
            (r.byline ? '<span class="ixcb-sl-by">' + esc(r.byline) + '</span>' : '') +
          '</td>' +
          '<td class="ixcb-sl-img">' + (r.images || '\u2014') + '</td>' +
          '<td class="ixcb-sl-file">' + (r.file ? esc(r.file) : '\u2014') + '</td>' +
          '<td class="ixcb-sl-plans">' + plans + '</td>' +
          '<td class="ixcb-sl-next ixcb-sl-next--' + ns.tone + '">' + esc(ns.t) + '</td>' +
          asn +
        '</tr>';
    }
    if (!shown) {
      /* A filtered list must never read as an empty one. */
      rows = '<tr><td colspan="' + (7 + issues.length) + '" class="ixcb-sl-none">' +
  ...
  function overviewBody() {
    ...
    var ith = '';
    for (var q = 0; q < issues.length; q++) ith += '<th class="ixcb-sl-asn">' + esc(issues[q].label) + '</th>';

    /* The mockup's own note, kept because it is the honest one for
       this state: nothing is an asset yet and assignment is closed
       until issues exist. */
    var note = issues.length
      ? 'Read from the print PDF. Assignment and disposition are read-only until the write scenario lands.'
      : (cadenceKey(cadenceLabel())
          ? 'Read from the print PDF. No plan carries this board\u2019s cadence label yet, so ' +
            'assignment is closed \u2014 label the plans that belong here, or create them above.'
          : 'Read from the print PDF. Name this board in the bar above to open assignment.');
    ...
    return '<div class="ixcb-sl">' +
      ...
      '<div class="ixcb-sl-tw"><table class="ixcb-sl-tbl">' +
        '<thead><tr>' +
          '<th>Pg</th><th>Section</th><th>Title</th><th>Img</th>' +
          '<th title="Filename matching against the Drive drop is not built yet">Matched file</th>' +
          '<th>Issues</th><th>Next step</th>' + ith +
        '</tr></thead><tbody>' + slateRows(issues) + '</tbody>' +
      '</table></div>' +
    '</div>';
  }
