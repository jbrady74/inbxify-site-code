...
//
//   4. Banner gating: MONITOR.preprocessing > 0. Pulse
...
//
//   7. Architectural primitives (Master Roadmap v1.6 §10):
...
//
//   No CMS schema changes. No changes to existing Studio panels
//   (Components / Assembler / Transcriber / Converter /
//   Generator). Only additive: new banner mount in header,
//   new modal infrastructure, new polling.
//
// ──────────────────────────────────────────────────────────
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function setPanel(name) {
      ...
      // ── 6-tab dispatch (S13 / v1.2.9) ──
      // Assembler and Components have direct render functions
      // owned by ta-studio. Transcriber and Converter are owned
      // by ta-page-body and uploads-processor respectively, so
      // ta-studio dispatches a CustomEvent on activation; those
      // owners listen for the event and (re-)mount themselves
      // into the panel container. Inputs and Generator are
      // placeholders in S13 — no listener needed.
      if (name === 'assembler') {
        renderAssembler();
      } else if (name === 'components') {
        // v1.1.7: re-read .media-wrapper DOM on every activation
        // to catch newly-created MEDIA rows from a republish.
        renderComponents();
    ...
    // ═══════════════════════════════════════════
    // PDF TRIAGE BANNER + MODAL  (v1.2.11 / S14 Step 1)
    // ═══════════════════════════════════════════
    ...
    //
    // Architecture: see Master Roadmap v1.6 §10.1 (banner-as-
    ...
    // render a banner gated on MONITOR.preprocessing > 0. The
    // two banners never coexist visually (UP banner is inside
    // Content Processor tab, Studio banner is in Studio
    ...
    function renderTriageBanner() {
      ...
      // Compact variant: chip-sized pill, fits in the chrome row.
      // Layout: [icon] [count text] [Triage button]
      el.innerHTML =
    ...
    function runTriageQueue() {
      ...
      // Order locked: Convert → Archive → Hide → Transcribe.
      // Transcribe last so context-switch is clean exit.
    ...
    function triageRunOne(file, action) {
      ...
      if (action === 'transcribe') {
        // No webhook fires per-row. Filename accumulates for
        // post-queue handoff. The Transcriber surface (Studio
        // Tab 4 via std:panel:transcriber listener in
        // ta-page-body) reads STD_TRANSCRIBE_PDFS and renders
        // a notice when the user lands there.
    ...
    function runTriageQueueDone(queue) {
      ...
      // Refresh folder counts so banner updates.
      loadStudioFolderCounts();
      if (hasTranscribe) {
        // Clean exit: close modal, switch Studio tab to
        // Transcriber. ta-page-body picks up STD_TRANSCRIBE_PDFS
        // when it mounts the Transcriber surface.
        closeTriageModal();
        // Switch to Transcriber tab. setPanel is defined earlier
        // in this IIFE.
        try { setPanel('transcriber'); } catch (e) { /* setPanel may not exist if Studio not yet inited */ }
        // Expose for ta-page-body to consume.
        ...
        // Clear succeeded picks; failed picks remain so user can retry.
        queue.forEach(function(q) { if (T_STATUS[q.id] === 'done') delete T_PICKS[q.id]; });
        renderTriageModal();
      }
    }

    // ── Folder counts polling ──
    function loadStudioFolderCounts() {
      var CFG = _cfg();
      if (!CFG.scenarioCUrl) return;
      var url = CFG.scenarioCUrl + '?titleSlug=' + encodeURIComponent(CFG.titleSlug) + '&action=folderCounts';
      fetch(url)
        .then(function(res) { if (!res.ok) throw new Error('Folder counts returned ' + res.status); return res.json(); })
        .then(function(data) {
          STD_MONITOR.uploadFolder = data.uploadFolder || 0;
          STD_MONITOR.preprocessing = data.preprocessing || 0;
          renderTriageBanner();
        })
        .catch(function(err) { console.error('[Studio Triage] Folder counts error:', err); });
    }

    // ── Banner click delegation ──
    // Single delegated listener on the Studio root for the
    // banner CTA. Anchored on document so it survives any
    ...
    // v1.2.11: kick off folder-counts polling for the triage banner.
    // Initial fire on Studio mount; banner appears if PDFs are
    // already pending.
    loadStudioFolderCounts();
    console.log('\u29BE Studio v1.2.11 mounted \u2014 6-tab shell + PDF triage banner (S14 Step 1). Banner gates on MONITOR.preprocessing > 0; modal mounts to <body>. Companion module: ix-modals-v1.0.1.css.');
  });
