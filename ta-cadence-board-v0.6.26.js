...
/* D-CB-21 — manual. Each mode is a declarative column arrangement,
  ...
  function slateMatch(r, f) {
    if (f === 'short')      return arrivedCount(r) < (r.images || 0);
  ...
  function heroHtml() {
    var hi       = highestPlan();
    var hiNo     = hi ? planNo(hi) : null;
    var hiLbl    = hi ? planLabel(hi) : '';
    var lastLbl  = lastSentLabel();
    var edGuess  = lastEditionCount();
    var srcName  = S.slate.length ? (S.slate[0].source || '') : '';
    var cover    = S.slate.length ? (S.slate[0].cover || '') : '';

    /* Suggestions, all overridable. Start at the highest issue that
       still has a free edition letter; otherwise the next number. */
    var sugNo  = hiNo != null
      ? (editionsOf(hiNo).length && nextLetterFor(hiNo) !== 'A' ? hiNo : hiNo + 1)
      : '';
    var sugLet = hiNo != null ? nextLetterFor(sugNo) : 'A';

    var vStart = gateVal('start', sugNo === '' ? '' : String(sugNo));
    var vLet   = gateVal('let',   sugLet);
    var vIss   = gateVal('nIss',  '2');
    var vEd    = gateVal('nEd',   edGuess ? String(edGuess) : '2');
    var vCad   = gateVal('cad',   '7');
    var vFst   = gateVal('first', '');

    var dirty = function (k) { return S.gate && S.gate[k] !== undefined && S.gate[k] !== null; };
    var chg   = function (k) { return dirty(k) ? ' ix-picker-input--changed' : ''; };

    var sel = function (key, list, cur, tip) {
      var o = '';
      for (var i = 0; i < list.length; i++) {
        o += '<option value="' + esc(list[i][0]) + '"' +
             (list[i][0] === cur ? ' selected' : '') + '>' + esc(list[i][1]) + '</option>';
      }
      return '<select class="ix-picker-input' + chg(key) + '" data-ixcb="gate" data-k="' + key + '"' +
             (tip ? ' title="' + esc(tip) + '"' : '') + '>' + o + '</select>';
    };

    var letOpts = [];
    for (var L = 0; L < 6; L++) letOpts.push([LETTERS[L], LETTERS[L]]);

    /* The gate shows its working. If the highest plan is a stray test
       row, that is visible here in one glance rather than silently
       skewing every suggestion below it. */
    var prov = hi
      ? 'Suggested from ' + (hiLbl || ('WLN-' + hiNo)) + ', the highest plan on this title.'
      : 'No plan on this title carries an issue number, so nothing can be suggested.';
    ...
    return '<div class="ixcb-hero">' +
      ...
      '</div>' +
      '<div class="ixcb-setup">' +
        '<div class="ixcb-gate">' +
          '<div class="ixcb-ref">' +
            (hiLbl
              ? 'Furthest plan built <b>' + esc(hiLbl) + '</b>'
              : 'No plan on this title carries an issue number') +
          '</div>' +
          '<h3 class="ixcb-gate-h">Set up Publication Plans</h3>' +
          '<div class="ixcb-ctrls">' +
            '<label>Start at</label>' +
            '<input type="text" inputmode="numeric" class="ix-picker-input ixcb-gate-no' + chg('start') +
              '" data-ixcb="gate" data-k="start" value="' + esc(vStart) + '" ' +
              'placeholder="120" aria-label="Starting issue number">' +
            sel('let', letOpts, vLet, 'Editions already on that issue: ' +
                (hiNo != null && editionsOf(vStart === '' ? hiNo : parseInt(vStart, 10)).join(', ') || 'none')) +
            '<span class="ixcb-ctrls-div"></span>' +
            '<label>Split into</label>' +
            sel('nIss', [['1','1 issue'],['2','2 issues'],['3','3 issues']], vIss, '') +
            '<span class="ixcb-ctrls-div"></span>' +
            '<label>Editions each</label>' +
            sel('nEd', [['1','1'],['2','2 (A/B)'],['3','3 (A\u2013C)'],['5','5 (A\u2013E)']], vEd,
                edGuess ? 'Last issue ran ' + edGuess + '.' : '') +
            '<span class="ixcb-ctrls-div"></span>' +
            '<label>First send</label>' +
            '<input type="date" class="ix-picker-input' + chg('first') +
              '" data-ixcb="gate" data-k="first" value="' + esc(vFst) + '">' +
            sel('cad', [['7','weekly'],['14','every 2 weeks']], vCad, '') +
            '<span class="ixcb-ctrls-sp"></span>' +
            (S.gate && Object.keys(S.gate).length
              ? '<button type="button" class="ix-revert" data-ixcb="gate-reset">Cancel</button>'
              : '') +
            '<button type="button" class="ix-btn ix-btn--primary" disabled ' +
              'title="Creating plans arrives with PubPlan multi-create">Create these</button>' +
          '</div>' +
          '<div class="ixcb-gate-ro">' + esc(prov) + ' All editable.</div>' +
        '</div>' +
      '</div>' +
    '</div>';
  }
  ...
  function overviewBody() {
    var issues = openIssues();   /* Editions strip only \u2014 the table no longer uses it. */

    /* Even with no manifest the shell renders. The mockup's frame is
       the design; only the table's contents are missing. */
    if (!S.slate.length) {
      return nextBar() + heroHtml() + editionsHtml(issues) + slateDiag() +
             orphansHtml() + lanesHtml();
    }
  ...
  function overviewHtml() {
    ...
    if (!S.slate.length) return overviewBody();
    return nextBar() + heroHtml() + editionsHtml(issues) + overviewBody() +
           orphansHtml() + lanesHtml();
  }
