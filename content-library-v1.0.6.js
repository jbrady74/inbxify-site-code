...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function saveEdit() {
    ...
    var webhookUrl = (window.TA_CONFIG && window.TA_CONFIG.pubplanWebhook) || '';
    if (!webhookUrl) {
      console.error('[CONTENT-LIBRARY] No pubplanWebhook in TA_CONFIG');
      return;
    }
    ...
    params.set('hidden-slot-label', S.edit.slot || '');
    params.set('hidden-titleadmin-id', (slotData && slotData.titleadminId) || CFG.taItemId || '');
    params.set('hidden-section-code', slotData ? (slotData.code.split('-')[0]) : '');
    ...
    fetch(webhookUrl + '?' + params.toString(), { mode: 'no-cors' })
      .then(function () {
        console.log('[CONTENT-LIBRARY] Webhook sent for', item.name, '→', S.edit.slot);
      })
      .catch(function (err) {
