// ============================================================
// PUBPLAN TILE UI — v5.0.6
// Fixes from v5.0.3,4,5:
...
(function () {
  ...
  function renderFa(sc) {
    ...
    // Sponsor column — clean label, checkbox moves to actions area
    var sponCol = '';
    var noSpHtml = '';
    if (isSponsored) {
      noSpHtml = '<label class="pxl pxl-right"><input type="checkbox" data-checkbox="fa-nospon" data-sc="' + sc + '"' +
        (s.noSponsor ? ' checked' : '') + '></label>';
      ...
      if (s.noSponsor) {
        ...
        sponCol = '<div class="pc"><span class="ppt-col-label">Sponsor</span>' +
          ...
          buildCustomerOptions(s.sponsorId, 'Select sponsor...') + '</select></div>';
      }
    }

    // Actions: No Sponsor checkbox (if sponsored) + edit/cancel + chevron
    var actionHtml = '';
    if (noSpHtml) actionHtml += noSpHtml;
    if (isEditing) {
    ...
    // FIX #4: spon-pending shows gold border without full edit mode
    var tileClass = 'ptr';
    if (isEditing) tileClass += ' hp';
    else if (s.dirty && s.sponDirtyOnly) tileClass += ' spon-pending';

    var tileHtml =
      '<div class="' + tileClass + '" id="tile-' + sc + '">' +
  ...
  function updateFaProgress() {
    updateSlotIndicators('fa', 4, function (code) {
      var s = state[code];
      if (!s || !s.artId || s.dirty) return false;
      // FIX #5: If sponsored category, sponsor must be resolved
      var isSpon = s.catType && s.catType.toLowerCase().indexOf('sponsor') !== -1;
      if (isSpon && !s.sponsorId && !s.noSponsor) return false;
      return true;
    });
    var pendingCount = 0;
    for (var i = 1; i <= 4; i++) {
  ...
  function renderTs(sc) {
    ...
    var noSpHtml = '';
    if (isSponsored) {
      noSpHtml = '<label class="pxl pxl-right"><input type="checkbox" data-checkbox="ts-nospon" data-sc="' + sc + '"' +
        (s.noSponsor ? ' checked' : '') + '></label>';
    ...
    var tsTileClass = 'ptr tsr';
    if (isEditing) tsTileClass += ' hp';
    else if (s.dirty && s.sponDirtyOnly) tsTileClass += ' spon-pending';

    var tileHtml =
      '<div class="' + tsTileClass + '" id="tile-' + sc + '">' +
