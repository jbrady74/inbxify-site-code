...
//           PDF Queue | Ready to Assign | Assigned | Hidden | Transcriber
//         - Add monitor div: Publisher Upload + Pre-processing folder counts
//         - Kill "All" filter — each tab = one status
//         - Transcriber tab: shell container, existing transcriber JS renders inside
...
<div class="cm-root">
  <div class="cm-hdr">
    ...
    <span class="cm-badge">v2.4.0</span>
  </div>
  ...
  function renderFileList() {
    ...
    if (!vis.length) {
      const msgs = { 'pdf-queue':'No PDFs awaiting decision', ready:'No files ready to assign', assigned:'No files assigned yet', hidden:'No hidden files' };
    ...
    el.innerHTML = vis.map(f => {
      ...
      // Badge
      const badge = isAssigned ? 'assigned' : isHidden ? 'hidden' : isPdf ? 'decision' : 'ready';
      const label = isAssigned ? 'Assigned' : isHidden ? 'Hidden' : isPdf ? 'PDF Queue' : 'Ready';
      const uc    = f.uuid ? `<span style="color:#1a3a3a">UC:${f.uuid.substring(0,8)}\u2026</span>` : '';
      const disabled = isAssigned || isHidden || isPdf;
  ...
  function setTab(id) {
    ...
    A = freshA();
    if (id === 'transcriber') {
      // No file load needed for transcriber
      LOADING = false;
      render();
      return;
    }
    const tab = TABS.find(t => t.id === id);
    if (tab && tab.status) loadFiles(tab.status);
    else render();
  }

  function reload() {
    if (TAB === 'transcriber') { render(); return; }
    const tab = TABS.find(t => t.id === TAB);
    if (tab && tab.status) loadFiles(tab.status);
    // Also refresh monitor counts
    loadMonitorCounts();
  }
  ...
  async function loadMonitorCounts() {
    // Publisher Upload Folder: count files via Google Drive API (or a lightweight Scenario)
    // Pre-processing: count AWAITING rows from Sheet via Scenario C
    // For now, these are populated if window.TA_CONFIG provides monitor endpoints
    // or via a combined Scenario C call that returns counts
    try {
      if (CFG.scenarioCUrl) {
        const url = `${CFG.scenarioCUrl}?titleSlug=${encodeURIComponent(CFG.titleSlug)}&status=AWAITING`;
        const res = await fetch(url);
        if (res.ok) {
          const data = await res.json();
          MONITOR.preprocessing = Array.isArray(data) ? data.length : 0;
        }
      }
      // Upload folder count would need a separate endpoint or Drive API call
      // Stubbed for now — will be wired when endpoint exists
    } catch(e) {
      console.warn('[CP] Monitor count fetch error:', e);
    }
    renderMonitor();
  }
  ...
  loadFiles('DONE');
  loadMonitorCounts();
  console.log('\u{1F4C2} Content Processor v2.4.0 mounted');
});
