...
//
// COMPANION CSS:  issues-tab-v1.0.9.css
// REQUIRES:       ix-success-toast-v1.0.0.js (IxToast) above this file
...
(function () {
  ...
  function activeTile(iss) {
    ...
    if (!iss.newsletterId) {
      ...
      h += '<div class="it-note it-note--info">The newsletter already says ' +
           esc(check.got) + '. Set as Next to mark this PubPlan too.</div>';
    }

    if (ui === 'confirm') {
      h += '<div class="it-confirm" role="group" aria-label="Confirm Set as Next">' +
             '<p>Set ' + esc(iss.title) + ' as the next newsletter? This changes the live newsletter status.</p>' +
  ...
  function nextTile(iss) {
    ...
    if (l.url) h += '<a class="it-btn it-btn--ongold"' + linkAttrs(l) + '>Open PubPlan</a>';
  ...
  function render() {
    ...
    mountSibling.insertAdjacentHTML('beforebegin',
      renderHeader() + '<div class="it-overview" id="it-overview-root"></div>');
    renderZones();
  ...
  document.addEventListener('click', function (e) {
    ...
    if (t.hasAttribute('data-it-set')) {
      ...
      S.ui[k] = 'confirm';
      renderZones();
      var c = document.querySelector('[data-it-confirm="' + cssEsc(k) + '"]');
