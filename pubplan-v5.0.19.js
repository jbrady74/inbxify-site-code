...
(function () {
  ...
  // ═══ THE FIND (TF) — UNIFIED CATEGORY DROPDOWN + TXA (5) + LBP (1) ═══
  // FIX #14: Replaces switchTfMode with handleTfCategoryChange
  ...
  function renderTfCategory() {
    ...
    if (showDropdown) {
      var h = '<div class="pc" style="min-width:280px;"><span class="ppt-col-label">The Find Category</span>';
      ...
      if (tfCategoryEditing && hasSaved) {
        h += '<a class="ppt-cancel-lnk visible" id="tf-cat-cancel" style="margin-top:4px;">cancel</a>';
      }
      h += '</div>';
      mount.innerHTML = h;
      ...
      if (cancelBtn) cancelBtn.addEventListener('click', function() {
        ...
        if (tfCategoryId) {
          ...
          if (catEl) {
            ...
            var sc = catEl.dataset.sectionCode || '';
            currentTfMode = (sc === 'lbp') ? 'lbp' : 'txa';
          }
        }
        ...
        if (txa) txa.classList.toggle('is-active', currentTfMode === 'txa');
        if (lbp) lbp.classList.toggle('is-active', currentTfMode === 'lbp');
        renderTfCategory();
        updateTfProgress();
      });
    } else {
      ...
      var h = '<div class="pc" style="min-width:280px;"><span class="ppt-col-label">The Find Category</span>';
      h += '<span class="pcv" style="display:inline-block;margin-top:4px;">' + (tfCategoryNm || '—') + ' <span style="font-size:10px;color:#888;font-weight:400;">(' + modeLabel + ')</span></span>';
      h += '<a class="pei" id="tf-cat-edit" title="Change category" style="margin-left:8px;">✎</a>';
      h += '</div>';
      mount.innerHTML = h;
  ...
  window.handleTfCategoryChange = function(sel) {
    ...
    var hasTxa = [1,2,3,4,5].some(function(i) { var s = state['TXA-'+i]; return s && s.dirty; });
    var hasLbp = state['LBP-1'] && state['LBP-1'].dirty;
    var hasChanges = (currentTfMode === 'txa' && hasTxa) || (currentTfMode === 'lbp' && hasLbp);
    ...
    var doIt = function() {
      ...
      var txa = document.querySelector('.the-find-txa');
      var lbp = document.querySelector('.the-find-lbp');
      if (txa) txa.classList.toggle('is-active', newMode === 'txa');
      if (lbp) lbp.classList.toggle('is-active', newMode === 'lbp');

      var ce = document.getElementById('tf-slot-count');
      if (ce) ce.textContent = newMode === 'txa' ? '5 slots' : '1 slot';
    ...
    if (hasChanges) {
      showInlineConfirm(sel, 'Switching categories will clear unsaved changes.', doIt, function() {
