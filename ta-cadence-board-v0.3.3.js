/* ════════════════════════════════════════════════════════════════
   ta-cadence-board-v0.3.3.js
   INBXIFY T-A — Cadence Board · SKELETON
   Companion: ta-cadence-board-v0.3.3.css (matched pair — load both)

   ── v0.3.3 — CONTROLS COLLAPSE TO ICONS ──
...
(function () {
  ...
  function appBar() {
    return '<div class="ixcb-appbar ix-on-dark">' +
             ...
             '</button>' +
             '<button type="button" class="ix-btn ix-btn--pill' + (S.focus ? ' is-active' : '') + '" ' +
    ...
    try {
      window.localStorage.setItem(VIEW_KEY, JSON.stringify({
        ...
        /* q and typeFilter are deliberately NOT persisted — see the
           v0.3.3 note. A question asked last session must not silently
           filter this one. */
      }));
    } catch (e) {}
  }
