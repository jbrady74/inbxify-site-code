/* ============================================================
   ipp-picker-v1.7.js — Picker view (+ load-order-robust tile dropdown wiring)
   CHANGELOG
...
(function () {
  ...
  function wireTileDropdowns(){
    if(!IPP.registerDropdown){ console.warn('[ipp-picker] registerDropdown missing — cannot wire tiles'); return; }
    if(!IPP.dropdown){
      // ipp-dropdown not loaded YET (load-order independent). Defer and
      // retry briefly instead of bailing permanently — mount() is guarded
      // by data-picker-mounted so it never re-runs, so we must self-retry.
      if(wireTileDropdowns._tries==null) wireTileDropdowns._tries=0;
      if(wireTileDropdowns._tries++ < 40){           // ~2s max (40×50ms)
        setTimeout(wireTileDropdowns, 50); return;
      }
      console.warn('[ipp-picker] ipp-dropdown never loaded — tile dropdowns unavailable'); return;
    }
  ...
  function mount(){
    ...
    console.info('[ipp-picker] mounted (v1.7 · editState + tile dropdowns · load-order-robust · save stubbed Path 1)');
  }
