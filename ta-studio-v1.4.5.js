    ...
    function renderInputsPanel() {
      ...
      el.innerHTML =
        '<div class="inp-root">' +
          ...
          // v1.4.5: Section 2 (Drive Folder Health badges + Scenario C
          // note) and Section 3 (Monitor-only / S14 banner) removed.
          // Counts were not reliably monitoring; banner contradicted
          // the bundle/loose-file management UI below (ta-bundles).

        '</div>';

      INP_LOADED = true;

      // v1.4.5: folder-count load no longer invoked. loadInputsFolderCounts
      // and the Scenario C path remain defined below but are not called
      // from render. Left in place to avoid touching unrelated config; can
      // be fully removed in a later cleanup once Drive tab work is scoped.
    }
