/* ta-asf-v1.7.0.js
   ...
   v1.7.0 is JS only. Do not touch the head CSS link.
...
(function () {
  ...
  function readSections() {
    var pub  = (typeof asfPublisherId === 'function') ? (asfPublisherId() || '') : '';
    var seen = {}, out = [];
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      // A row with no publisher is shown: a missing attribute should not
      // silently empty the picker. A row with one must match.
      if (pub && owner && owner !== pub) continue;
      seen[id] = true;
