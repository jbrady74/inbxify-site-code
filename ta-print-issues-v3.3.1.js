...
(function () {
  ...
  function startRead() {
    ...
    run.source = run.file.name; run.phase = 'reading'; run.done = 0; run.error = '';
    renderPz();

    function live() { return !run.cancelled && P === run; }
    var pdf;
    loadPdfjs().then(function (L) {
      return run.file.arrayBuffer().then(function (buf) { return L.getDocument({ data: buf }).promise; });
    }).then(function (doc) {
      pdf = doc; run.pages = doc.numPages; if (live()) renderPz();
      return pool(run.pages, READ_CONCURRENCY, function (n) {
        return readOnePage(run, pdf, n, READ_RETRIES).then(function (r) {
          run.done++; if (live()) progPz(); return r;
        });
      });
    }).then(function (reads) {
      if (!live()) return null;
      run.phase = 'stitching'; renderPz();
      return postWorker('/stitch', { reads: reads, titleName: cfg().titleName || '' });
    }).then(function (j) {
      if (pdf) { try { pdf.destroy(); } catch (x) {} }
      if (!j || !live()) return;
      ...
      run.items = j.items.map(function (it) {
        ...
        var v = { name: it.name || '', section: it.section || '', byline: it.byline || '',
                  type: TYPE_OPTS.some(function (o) { return o.v === it.type; }) ? it.type : 'other' };
        return {
          orig: v, cur: { name: v.name, section: v.section, byline: v.byline, type: v.type },
  ...
  function reviewHtml() {
    ...
    return '' +
      ...
      '<div class="tpi-pz-tw"><table class="tpi-pz-t">' +
        '<thead><tr>' +
          '<th class="tpi-pz-c--inc"></th><th class="tpi-pz-c--pg">Page</th>' +
          '<th>Name</th><th class="tpi-pz-c--section">Section</th>' +
          '<th class="tpi-pz-c--byline">Byline</th><th class="tpi-pz-c--img">Img</th>' +
          '<th class="tpi-pz-c--type">Type</th><th class="tpi-pz-c--conf">Check</th>' +
        '</tr></thead><tbody>' +
        P.items.map(function (it, i) {
          return '<tr class="' + (it.include ? '' : 'tpi-pz-off') + '">' +
      ...
      '</tbody></table></div>' +
      '<p class="tpi-pz-hint">Untick anything that should not become a row. ' +
        'Ads are not listed. Nothing is written until you create the rows.</p>' +
  ...
  function renderPz() {
    ...
    if (P.phase === 'pick') {
      ...
      body =
        ...
        '<p class="tpi-pz-hint" data-pz-chosen="1"' + (P.file ? '' : ' hidden') + '>Chosen: <b>' +
          esc(P.file ? P.file.name : '') + '</b></p>' +
    ...
    else if (P.phase === 'reading' || P.phase === 'stitching') {
      ...
      foot = '<button type="button" class="ix-revert" data-pz-close="1">Close</button>' +
        (P.items.length ? '<button type="button" class="ix-btn ix-btn--secondary" data-pz-back="1">Back to review</button>' : '');
    }
  ...
  function onPzChange(e) {
    ...
    if (t.hasAttribute('data-pz-file')) {
      ...
      var ch = m && m.querySelector('[data-pz-chosen]');
      if (ch) { ch.hidden = !P.file; ch.innerHTML = P.file ? 'Chosen: <b>' + esc(P.file.name) + '</b>' : ''; }
      var warn = m && m.querySelector('.tpi-pz-warn');
      if (warn) warn.parentNode.removeChild(warn);
      return;
    }
    if (t.hasAttribute('data-pz-inc')) {
      P.items[+t.getAttribute('data-pz-inc')].include = t.checked;
      renderPz();
    }
  }
  ...
  function onPzClick(e) {
    var t = e.target.closest && e.target.closest(
      ...
      '[data-pz-type],[data-pz-reload],[data-pz-back]');
    if (!t) return;
    e.preventDefault();
