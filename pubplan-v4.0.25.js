...
function initBaState() {
  ...
  for (let i = 1; i <= 12; i++) {
    const code = `ba-${i}`;
    if (!state[code]) {
      state[code] = { sc: code, secC: 'ba', slotNum: i, section: 'Banner Ad', custId: '', custNm: '', adId: '', adName: '', dirty: false };
    }
  }
}
...
function initTxaState() {
  ...
  for (let i = 1; i <= 5; i++) {
    const code = `txa-${i}`;
    if (!state[code]) {
      state[code] = { sc: code, secC: 'txa', slotNum: i, section: 'The Find', pubplanId: '', titleadminId: '', custId: '', custNm: '', dirty: false };
    }
  }
}
