/* ta-print-issues-v3.2.1.js
   ════════════════════════════════════════════════════════════════
   INBXIFY T-A — Print Issue Allocator
   Companion: ta-print-issues-v3.2.0.css
...
(function () {
  ...
  /* v3.2.1 — header renamed to "Print Issue Allocator". Label only:
     no logic, no markup, no CSS. Companion ta-print-issues-v3.2.0.css
     is UNCHANGED — do not touch the head CSS link.

     The breadcrumb back-button at ~line 646 still reads
     "‹ Print Issues" and is deliberately NOT changed: it names the
     LIST you return to, not the surface. Flagged for Jeff rather
     than decided here. */
  var VERSION = '3.2.1';
  ...
  function headHtml(right, extra) {
    if (!hasHeader()) {
      return '<div class="tpi-head"><h2 class="tpi-h2">Print Issue Allocator</h2>' +
             (extra || '') + '<span class="tpi-sp"></span>' + (right || '') + '</div>';
    }
    return window.IxHeader.render({
      icon:  '\uD83D\uDCF0',            /* newspaper — the print issue itself */
      /* v3.2.1 — the surface is the Print Issue Allocator. The rail
         group above it is "01 Print Issues" (the object) and the tab
         is "Allocator" (the tool); this header is the surface, so it
         carries both. Keep this string and the no-ix-header fallback
         below in step — a surface with two names is how the two
         revenue-type fields got confusing. */
      title: 'Print Issue Allocator',
      actions: [
        extra || '',
        right  || '',
