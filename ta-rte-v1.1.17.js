/* ============================================================
   ...
   v1.1.17 changes (Two fixes from v1.1.16 testing):

     FIX 1 — Editor WYSIWYG broken (size + alignment not visible).
       ROOT CAUSE: The size/align percentages on the inner
       figure.rte-inserted-image were computed against the wrong
       parent. Trix wraps every attachment in an outer
       <figure data-trix-attachment> that renders as inline-block
       shrink-wrap (only as wide as its content). So
       max-width:75% applied to the inner figure = 75% of the
       shrink-wrapped wrapper width, which is roughly the image's
       natural width — i.e., NOT 75% of the editor.

       v1.1.15 PART B fix added inline styles on the inner but
       never addressed the outer wrapper sizing. v1.1.16 inherited
       the same bug. Live render works because the article page
       doesn't have Trix's outer wrapper — only the inner figure,
       so percentages compute against the article body width
       directly.

       FIX: One CSS rule forces the outer Trix attachment wrapper
       to display:block + width:100% + no border/margin/padding.
       Now the inner's percentage applies to the editor width.

     FIX 2 — Click on image had no response.
       ROOT CAUSE: Bubble-phase delegation on the trix-editor
       element. Trix's internal selection handler runs on click
       events on attachments and calls stopPropagation() on the
       paths that bubble through trix-editor itself, so the
       click never reaches a bubble-phase listener on that
       element.

       FIX: Delegate at the document level on the capture phase.
       Capture phase fires on the way DOWN to the target, before
       any stopPropagation calls in the bubble phase. Handler
       ref tracked on the instance for clean removal in destroy().
       Added a console.log on first attempt so the diagnostic
       path is visible.

     No other behavior changes from v1.1.16. All click-to-edit
     popover logic (Cancel / Remove / Replace / Apply, gold
     dirty border, position flipping) intact.

   v1.1.16 changes (Click-to-Edit Attached Image):

     FEATURE: Inline images in the editor are now click-editable
     without delete + reinsert. Clicking any figure.rte-inserted-image
     opens a small popover anchored below the figure with:
       - Size strip:  [33%] [50%] [75%] [100%]
       - Align strip: [◀] [■] [▶]
       - Cancel link  (close, no changes — pre-edit state preserved)
       - [Remove]     (delete the figure; uses v1.1.2 attachment removal pattern)
       - [Replace]    (closes popover, reopens MEDIA picker in replace mode;
                      selecting a new image swaps src/data-media-id while
                      preserving the current size/align)
       - [Apply]      (writes new data-img-size, data-img-align, and inline
                      style to the figure; enabled only when dirty)

     UX rules followed:
       - Gold border on each toggle strip when its value differs from
         original (platform "dirty select" rule)
       - Cancel link always visible while popover is open (platform
         "cancel on every edit state" rule)
       - Apply button disabled until dirty (no-op clicks prevented)
       - Click outside popover OR Escape → close + revert (acts as Cancel)
       - Popover positioned below the figure; flips above when no room below

     TRIX MODEL TRUTH (v1.1.2 lesson):
       Direct DOM mutations to figures inside <trix-editor> get reverted
       on Trix's next render tick — Trix re-renders from its internal
       Document model. So Apply writes via attachment.setAttribute(
       'content', newHTML), which IS the Trix model. Remove uses the
       same v1.1.2 pattern: getRangeOfAttachment → setSelectedRange →
       deleteInDirection('forward').

       Finding the Trix.Attachment for a clicked figure:
         1. From the inner figure.rte-inserted-image, find ancestor
            <figure data-trix-attachment>
         2. Read its data-trix-id attribute
         3. Match against editor.getDocument().getAttachments() by id
       Cached per click; no persistent registry needed.

     REPLACE PATH:
       Setting this._replacingAttachment = <Trix.Attachment> before
       opening the picker reroutes insertImage(): instead of building
       a new attachment, it generates new figure HTML preserving the
       OLD figure's data-img-size + data-img-align (read from the
       attachment's content via DOMParser), and writes the new content
       to the existing attachment via setAttribute('content', ...).
       Caption is preserved if present in the original.

     New methods on InbxRTE:
       bindImageEditClickHandler()         ── click delegation in editor
       openEditPopover(state)              ── builds + positions popover
       closeEditPopover(revert)            ── tears down popover
       applyEditPopover()                  ── attachment.setAttribute('content', ...)
       removeFromEditPopover()             ── v1.1.2 removal pattern
       replaceFromEditPopover()            ── sets _replacingAttachment, opens picker
       renderEditPopover()                 ── (re)renders the popover markup
       _positionEditPopover()              ── anchor below/above figure
       _findAttachmentForFigure(innerFig)  ── data-trix-id → Trix.Attachment

     New state on InbxRTE instance:
       _editPopoverEl          ── DOM element (in body, absolute-positioned)
       _editState              ── { attachment, innerFig, img:{src,alt,mediaId,componentRole,caption},
                                    originalSize, originalAlign, stagedSize, stagedAlign }
       _replacingAttachment    ── Trix.Attachment | null (replace-mode flag for insertImage())
       _editEscHandler         ── escape-key listener (added on open, removed on close)
       _editClickOutsideHandler ── outside-click listener

     CSS additions (injectStyles):
       .rte-img-edit-popover  + .rte-img-edit-row + .rte-img-edit-actions
       Reuses .rte-img-size-btn and .rte-img-align-btn styling from picker
       so the toggles look identical to the insert-time controls.

     No changes to Make scenarios. No changes to cleanHTMLForWebflow
     (it already preserves data-img-size, data-img-align, inline style,
     and class="rte-inserted-image"). No changes to insertImage()
     signature — only an early-return branch for replace mode.
...
(function () {
  ...
  class InbxRTE {
    constructor(config) {
      ...
      // v1.1.16: click-to-edit popover state
      this._editPopoverEl = null;
      this._editState = null;            // { attachment, innerFig, img:{...}, originalSize, originalAlign, stagedSize, stagedAlign }
      ...
      this._imageEditClickHandler = null; // v1.1.17: document-level capture-phase delegate
    }
    ...
    waitForTrixInit() {
      ...
      el.addEventListener('trix-initialize', () => {
        ...
        // v1.1.16: click-to-edit popover for inserted images
    ...
    togglePicker() {
      ...
      if (this.pickerOpen) {
        ...
        // v1.1.16: if the picker was in replace mode and the user closed it
        // without selecting (e.g. clicked the close button), clear the flag so
        // the next insert is a normal insert.
    ...
    insertImage() {
      ...
      // v1.1.16: Replace-mode branch — when _replacingAttachment is set, this
      // call is a swap, NOT an insert. We preserve the existing attachment's
      // size, align, and caption (read from its current content), updating
      // only the image src + data-media-id + data-component-role.
      if (this._replacingAttachment) {
    ...
    /* ── v1.1.16: Replace path for insertImage() ──
       Reads the current attachment's content to preserve size/align/caption,
       then writes new content with the new image src + data-media-id.
       Trix re-renders the attachment in place.
    ── */
    _performReplace(media) {
      ...
      const _size  = (oldInner && oldInner.getAttribute('data-img-size'))  || 'large';
      const _align = (oldInner && oldInner.getAttribute('data-img-align')) || 'center';
      const _style = buildFigureInlineStyle(_size, _align);
      const oldCaptionEl = oldInner ? oldInner.querySelector('figcaption') : null;
      const caption = oldCaptionEl ? (oldCaptionEl.textContent || '') : '';
      ...
      console.log(`[RTE] Replaced image content: new MEDIA ID ${media.id} | preserved size=${_size} align=${_align}`);
    }

    /* ── v1.1.16: Click-to-Edit Popover for inline images ───────────────
       Wires a click handler on the editor's contenteditable body that
       opens a small popover anchored to the clicked figure.rte-inserted-
       image. Popover provides size/align toggles + Cancel/Remove/Replace/
       Apply. Apply writes via Trix.Attachment.setAttribute('content',...)
       — the v1.1.2 lesson: Trix's model is the single source of truth.

       v1.1.17: handler moved to document level + capture phase. Trix's
       internal click handling stops propagation on bubble paths that
       run through trix-editor, which killed the v1.1.16 bubble-phase
       handler. Capture phase fires on the way DOWN to the target,
       before Trix gets a chance to stop the event.
    ── */
    bindImageEditClickHandler() {
      if (!this.editorEl) return;

      this._imageEditClickHandler = (event) => {
        // Only react to clicks inside our editor
        if (!this.editorEl || !this.editorEl.contains(event.target)) return;
        // Bail on toolbar/action buttons inside the editor
        if (event.target.closest('trix-toolbar, .trix-button, [data-trix-action]')) return;

        const innerFig = event.target.closest('figure.rte-inserted-image');
        if (!innerFig) return;

        console.log('[RTE] click-to-edit fired on figure', innerFig);

        // Don't reopen if popover already open for this figure
        if (this._editState && this._editState.innerFig === innerFig) return;
        // If popover open for a different figure, close it first (no revert
        // needed — pre-edit state is preserved until Apply).
        if (this._editPopoverEl) this.closeEditPopover(/*revert=*/false);

        const attachment = this._findAttachmentForFigure(innerFig);
        if (!attachment) {
          console.warn('[RTE] click-to-edit: no Trix.Attachment matched for figure', innerFig);
          return;
        }

        // Read current state from the figure DOM
        const img = innerFig.querySelector('img');
        const captionEl = innerFig.querySelector('figcaption');
        const state = {
          attachment,
          innerFig,
          img: {
            ...
            mediaId:       img ? (img.getAttribute('data-media-id') || '')    : '',
            componentRole: img ? (img.getAttribute('data-component-role')||''): '',
          ...
          stagedSize:    null, // populated below
          stagedAlign:   null
        };
        state.stagedSize  = state.originalSize;
        state.stagedAlign = state.originalAlign;

        this.openEditPopover(state);
      };

      // Capture phase on document — runs before any bubble-phase
      // stopPropagation calls inside the editor.
      document.addEventListener('click', this._imageEditClickHandler, true);
    }

    _findAttachmentForFigure(innerFig) {
      const trixWrapper = innerFig.closest('figure[data-trix-attachment]');
      if (!trixWrapper) return null;
      const trixId = trixWrapper.getAttribute('data-trix-id');
    ...
    openEditPopover(state) {
      ...
      // Build popover element and append to body (escapes editor scroll)
      ...
      this._editClickOutsideHandler = (e) => {
        ...
        // Clicks on the figure itself should not close (would immediately reopen)
        if (this._editState && this._editState.innerFig.contains(e.target)) return;
        this.closeEditPopover(/*revert=*/true);
      };
      // Use timeout so the click that opened the popover doesn't also close it
      setTimeout(() => {
    ...
    renderEditPopover() {
      ...
      pop.innerHTML = `
        ...
        <div class="rte-img-edit-actions">
          ...
          <div class="rte-img-edit-action-buttons">
            ...
            <button type="button" class="ix-btn ix-btn--primary rte-img-edit-apply" id="${this.id}-img-edit-apply" ${anyDirty ? '' : 'disabled'}>Apply</button>
          </div>
        </div>
      `;
      ...
      pop.querySelectorAll('.rte-img-align-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
          e.preventDefault();
          if (this._editState.stagedSize === 'full') return; // disabled when full-width
    ...
    _positionEditPopover() {
      const pop = this._editPopoverEl;
      const st = this._editState;
      if (!pop || !st || !st.innerFig) return;

      // Use the OUTER Trix attachment wrapper for positioning (the inner figure
      // may have margin:auto and start mid-screen). The wrapper is the
      // visually-anchored element.
      const anchor = st.innerFig.closest('figure[data-trix-attachment]') || st.innerFig;
      ...
      // Default: below the figure, horizontally centered on the figure
      ...
      // Clamp vertically (don't go off the top either)
    ...
    closeEditPopover(revert) {
      // revert is currently a no-op since we never mutate the attachment
      // before Apply — staged state lives in memory only. Kept as a param
      // for clarity at call sites and to match the platform UX rule.
    ...
    applyEditPopover() {
      ...
      const sizeDirty  = st.stagedSize  !== st.originalSize;
      const alignDirty = st.stagedAlign !== st.originalAlign;
      if (!sizeDirty && !alignDirty) { this.closeEditPopover(false); return; }
      ...
      // Trix re-renders the attachment from this content
      ...
      console.log(`[RTE] Edited image: size ${st.originalSize}→${_size}, align ${st.originalAlign}→${_align} | MEDIA ID ${st.img.mediaId}`);
    ...
    removeFromEditPopover() {
      ...
      // v1.1.2 pattern: getRangeOfAttachment → setSelectedRange → deleteInDirection
      const doc = this.trixEditor.getDocument();
      ...
      if (range) {
        ...
        this.updateCounts();
        this.updateDirtyState();
        console.log(`[RTE] Removed image | MEDIA ID ${st.img.mediaId}`);
      }
    ...
    replaceFromEditPopover() {
      ...
      // Flag picker into replace mode; insertImage() will detect and swap
      // content on the existing attachment instead of inserting new.
      this._replacingAttachment = st.attachment;

      // Close the edit popover, open the picker
      this.closeEditPopover(false);
    ...
    destroy() {
      ...
      this.hideUploadModal();
      // v1.1.16: tear down edit popover if open when RTE closes
      if (this._editPopoverEl) {
        this.closeEditPopover(/*revert=*/false);
      }
      // v1.1.17: remove document-level click-to-edit listener
    ...
    injectStyles() {
      ...
      style.textContent = `
        ...
        /* ── v1.1.17 WYSIWYG FIX ──
           Trix wraps every attachment in <figure data-trix-attachment>
           which renders as inline-block shrink-wrap by default. The
           inner figure.rte-inserted-image with max-width:75% computes
           75% of the SHRINK-WRAPPED WRAPPER, not 75% of the editor.
           Force the outer wrapper to be a full-width block so the
           inner's percentage applies to the editor's content width.
           Also strip the border/margin/padding the prior generic
           figure rule added to it, since the inner figure carries
           its own border. ── */
        ...
        /* Suppress the empty attachment__caption that Trix adds as a
           sibling of the inner figure — we have our own figcaption
           inside the inner figure when there's caption text. */
        trix-editor.rte-trix-editor figure[data-trix-attachment] > figcaption.attachment__caption:empty {
        ...
        /* Inline images get a subtle hover hint so users discover click-to-edit */
        trix-editor figure.rte-inserted-image { cursor:pointer; transition:outline-color 0.12s; outline:1.5px solid transparent; outline-offset:2px; }
        trix-editor figure.rte-inserted-image:hover { outline-color:${GOLD}; }
        ...
        .rte-fs-mount .rte-picker-panel { position:fixed; bottom:0; left:0; right:0; z-index:10010; border-radius:8px 8px 0 0; border:1.5px solid ${GOLD}; border-bottom:none; background:white; max-height:50vh; overflow-y:auto; box-shadow:0 -4px 24px rgba(0,0,0,0.12); }
