/* ════════════════════════════════════════════════════════════════
   ta-cadence-board-v0.6.0.js
   INBXIFY T-A — Cadence Board · SKELETON
   Companion: ta-cadence-board-v0.6.0.css (matched pair — load both)

   ── v0.6.0 — CONTROLS COLLAPSE TO ICONS ──
...
(function () {
  ...
  /* v0.6.0 — the meta line is gone. Size, dimensions and word counts
  ...
  function harvestPlans() {
    ...
    Array.prototype.forEach.call(items, function (item) {
      ...
      /* Lock state — same contract issues-tab v1.0.17 reads. A plan's
         status lives on its slot wrapper, not on the item. */
      var status = '';
    ...
    try {
      window.localStorage.setItem(VIEW_KEY, JSON.stringify({
        ...
        /* q and typeFilter are deliberately NOT persisted — see the
           v0.6.0 note. A question asked last session must not silently
           filter this one. */
      }));
    } catch (e) {}
  }
