...
(function () {
  ...
  function hydrateAssetViaWebhook(assetType, assetId) {
    ...
    // v1.5.7: in-flight guard — coalesce concurrent calls for the SAME
    // asset so we don't stampede the webhook (the root cause of rejections).
    var key = assetType + ':' + assetId;
    if (_hydrateInFlight[key]) return _hydrateInFlight[key];

    var promise = fetchJsonWithRetry(url, 1)
      .then(function (body) {
    ...
    // Track + clean up the in-flight promise.
    _hydrateInFlight[key] = promise;
    promise.then(function () { delete _hydrateInFlight[key]; },
                 function () { delete _hydrateInFlight[key]; });
    return promise;
  }

  // v1.5.7: in-flight registry keyed by assetType:assetId.
  var _hydrateInFlight = {};
  ...
  function render() {
    ...
    // ── TEMP TD-217 PROBE — remove after capture ──
    try {
      console.log('[TD-217] render() ENTER',
        'mode=', S.mode,
        'assetType=', S.assetType,
        'editAssetId=', S.editAssetId,
        'article?', !!S.article,
        'article.id=', S.article && S.article.id,
        'media.len=', S.media && S.media.length,
        'rtp=', (function(){ try { return rtpReadyState(); } catch(e){ return 'rtpThrew:'+e.message; } })()
      );
    } catch(e){ console.log('[TD-217] probe1 threw', e); }
    // ── END TEMP TD-217 PROBE ──
    ...
    try {
      ...
      console.log('[TD-217] RENDER THREW \u2192', e && e.message, e && e.stack);
      panel.innerHTML = '<div style="padding:20px;color:#b00">TD-217 render error: ' + (e && e.message) + '</div>';
    }
    // ── END TEMP TD-217 PROBE ──
