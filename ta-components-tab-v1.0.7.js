/* ============================================================
   ta-components-tab-v1.0.7.js
   ============================================================
   INBXIFY · Components Rebuild · Slice 1 · Sub-session A (v1.0.6)

   ─── v1.0.6 (Item #5 part 2 — Card thumb expand icon) ───
   Each component card's image thumb now has a small expand icon
   (top-right corner) that opens the shared ix-lightbox modal
   showing the image at full resolution + name caption.

   Companion: ix-lightbox-v1.0.0.js — MUST be loaded BEFORE
   ta-components-tab on the page. Defines window.InbxLightbox.open
   and the .ix-expand-icon / .ix-expand-icon-host shared classes.

   Markup change:
     In the renderCard() function — when it.imageUrl is truthy,
     the .cmp-thumb container gains the .ix-expand-icon-host class
     and contains a <button class="ix-expand-icon"> with an inline
     SVG magnify icon. NO icon for text-preview cards or empty-state
     cards (per scope lock — image cards only).

   Click handling:
     New data-cmp-card-action="expand-preview" branch in the click
     delegation at root.addEventListener('click'). The handler runs
     BEFORE all other card-action branches (placed first) so that
     e.stopPropagation prevents the default "toggle card selection"
     fallthrough at the bottom of the delegation chain.

   No CSS in this file. The .ix-expand-icon / .ix-expand-icon-host
   styles ship from ix-lightbox-v1.0.0.js. Both the RTE picker
   (ta-rte v1.1.21+) and this surface use the same source of truth.

   No state change, no behavior change to existing flows (selection
   toggle, inline edit, open-drawer, etc.) — purely additive.

   ─── v1.0.5 fix (vs v1.0.4) ───
   Per-tile inline-edit dropdown was unusable. Symptoms:
     • Click "Assign…" → dropdown + Save/Cancel render (correct).
     • Click inside the dropdown → first click silently does
       nothing visible. Second click flashes the customer list
       then collapses, AND the bulk-action bar appears with
       the card now selected. Third click closes everything.
     • No console errors.

   Root cause: event-bubble race. The click delegation at
   §6 (root.addEventListener('click')) checks card-level
   actions in order, then falls through to the DEFAULT
   "toggle card selection" branch when no [data-cmp-card-action]
   matches. A click on the <select> element inside the
   inline-edit panel has no [data-cmp-card-action] ancestor
   between itself and the card, so it bubbles into the
   default toggle. That selects the card → renderBulkBar()
   fires → bulk bar appears. Worse, on Chromium native
   selects, the toggle's renderGrid() rewrites the DOM
   mid-open which collapses the open dropdown.

   Fix: exempt clicks inside .cmp-inline-edit from the
   card-default-toggle. The select, its options, the labels,
   and the panel's own padding all stay click-through-safe
   for the browser's native select handling. Same pattern
   already protects the bulk-edit panel (which is not
   nested inside a card and so doesn't have this problem).

   One-line guard added directly above the default toggle.
   No other changes. Save still toasts "not wired yet
   (Sub-session C)." Cancel still snapshot-reverts.

   ──────────────────────────────────────────────────────────── */

/* ============================================================
   ta-components-tab-v1.0.4.js (history)
   INBXIFY · Components Rebuild · Slice 1 · Sub-session A (v1.0.4)

   ─── v1.0.4 changes (vs v1.0.3) ───
   1. Bulk-edit panel gains a redundant close affordance: a
      square ✕ icon button (ix-btn--ghost ix-btn--icon) sits
      to the right of the Cancel link in both bulk-edit
      sub-panels (assign-customer + clear-customer). Same
      data-cmp-bulk="cancel-bulk-edit" target as the Cancel
      link, so it routes through the existing handler at line
      ~463 — no new wiring needed.

      Why two affordances: Cancel is the explicit reverter,
      the ✕ is a familiar dismiss target for users who scan
      for "close this panel" rather than reading "Cancel".
      Both undo the same way (snapshot revert, no toast).

   2. Carries forward v1.0.3's body-lock removal (drawer side
      panel does not need overflow:hidden on body).

   ─── v1.0.4 also depends on ta-studio-components-v1.0.12.css ───
   Bulkbar surface rewritten in v1.0.12: cream #FAF9F5 with
   3px gold #C4A35A left-border. All .cmp-bulkbar .ix-btn--*
   override rules DELETED. ix-buttons-v1.0.4 now works as
   designed — no inverted text colors, no white-on-teal
   overrides. Surface = "action-pending bar" visual language,
   same as has-pending field treatment.

   ──────────────────────────────────────────────────────────── */

/* ============================================================
   ta-components-tab-v1.0.3.js (history)
   INBXIFY · Components Rebuild · Slice 1 · Sub-session A (v1.0.3)

   ─── v1.0.3 fix (vs v1.0.2) ───
   The body-lock added in v1.0.2 was the wrong call. Side
   drawers don't need to freeze background scroll — that's
   for full-screen modals. Removed document.body.classList
   .add('cmp-detail-locked'). The drawer host's fixed-position
   side panel is enough. The underlying page remains
   interactive (or at least scrollable to find offscreen
   content) while the drawer is open.

   ──────────────────────────────────────────────────────────── */

/* ============================================================
   ta-components-tab-v1.0.2.js (history)
   INBXIFY · Components Rebuild · Slice 1 · Sub-session A (v1.0.2 fixes)

   ─── v1.0.2 fixes (vs v1.0.1) ───
   1. Drawer no longer blows up the underlying page. Body gets
      .cmp-detail-locked class while drawer is open (overflow:
      hidden). Drawer host shrinks to side-panel-only width to
      avoid covering the full viewport.
   2. Bulk toolbar contrast strengthened (was too faint).
   3. Card actions row pinned to card bottom with grid-row layout
      so cards have predictable structure regardless of content.
   4. Title fixed to exactly 2 lines via line-clamp (was inheriting
      v1.0.8 rule but fixed-height was unstable).
   5. Customer line truncates to one line with ellipsis. Container
      uses min-width: 0 so flex truncation actually works.
   6. "change" link replaced with a pencil-icon button positioned
      to the right of the customer name on the same line. Customer
      row uses flex with name growing, button fixed at end.
      Card body font scale reduced 2px overall.

   See v1.0.1 / v1.0.0 history below for full module changelog.

   ──────────────────────────────────────────────────────────── */

/* ============================================================
   ta-components-tab-v1.0.1.js (history)
...
(function () {
  ...
  // v1.0.7: Create a new Article in the ASF seeded from the currently
  // selected components. Reuses the same prefill bridge the Bundles
  // cascade uses — resolvePrefilledMediaIds maps first image -> main,
  // second -> OG, text component -> body HTML. No new pipeline: wires the
  // existing selection model to the existing create endpoint. Selection
  // is left intact on open so a cancel in the ASF doesn't strand the
  // operator. Requires ASF v1.2.3+ (the resolver).
  function bulkCreateArticle() {
    ...
    if (!window.InbxASF || typeof window.InbxASF.open !== 'function') {
      toast('ASF not loaded on this page — cannot create.');
      if (DEBUG) console.warn('[InbxComponentsTab] window.InbxASF unavailable');
      return;
    }
    try {
      window.InbxASF.open({ mode: 'create', assetType: 'article', prefilledMediaIds: ids });
    } catch (err) {
      if (DEBUG) console.error('[InbxComponentsTab] InbxASF.open failed:', err);
  ...
  function bindShellEvents(root) {
    ...
    root.addEventListener('click', function (e) {
      ...
      if (card) {
        // v1.0.6: expand-preview — opens shared lightbox modal showing
        // the thumb at full resolution. Placed FIRST in the action
        // chain so it runs before any other handler (including the
        // default card-toggle fallthrough). Defensive check on
        // window.InbxLightbox so a missing ix-lightbox script doesn't
        // throw — card behavior continues as before.
        if (t.closest('[data-cmp-card-action="expand-preview"]')) {
          e.stopPropagation();
          var mediaId = card.getAttribute('data-cmp-card');
          var it = S.items && S.items.find ? S.items.find(function (x) { return x.mediaId === mediaId; }) : null;
          if (!it || !it.imageUrl) return;
          if (typeof window.InbxLightbox === 'undefined' || typeof window.InbxLightbox.open !== 'function') {
            console.warn('[CMP-TAB v1.0.6] window.InbxLightbox not loaded. Add ix-lightbox-v1.0.0.js (or later) to the page <head> before ta-components-tab.');
            return;
          }
        ...
        // v1.0.5 guard: clicks inside the inline-edit panel
        // (e.g. on the <select>, its options, the field label,
        // or the panel's own padding) must NOT fall through to
        // the card-default-toggle. The default toggle would
        // select the card → render the bulk bar → renderGrid()
        // mid-open would collapse the native dropdown. The
        // inline-edit panel handles its own change events via
        // [data-cmp-edit-field]; nothing useful happens by
        // also toggling card selection. Bail out silently.
        if (t.closest('.cmp-inline-edit')) {
          return;
        }
      ...
      if (t.closest('[data-cmp-bulk="create-article"]')) {
        bulkCreateArticle();
        return;
      }
  ...
  function renderCard(it) {
    ...
    // v1.0.6: imageUrl thumbs gain the .ix-expand-icon-host class
    // (position:relative for the absolute-positioned icon) and embed
    // an .ix-expand-icon button. Click handling lives in the click
    // delegation at root.addEventListener('click'). Text-preview and
    // empty-state thumbs are NOT modified — they have no preview image
    // to expand.
    var thumb;
    if (it.imageUrl) {
      thumb = '<div class="cmp-thumb ix-expand-icon-host">' +
                '<img class="cmp-thumb-img" src="' + escAttr(it.imageUrl) +
                '" alt="' + escAttr(it.name) + '" loading="lazy" />' +
                '<button type="button" class="ix-expand-icon" ' +
                  'data-cmp-card-action="expand-preview" ' +
  ...
  function renderCustomerCell(it) {
    if (S.inlineEdit && S.inlineEdit.mediaId === it.mediaId && S.inlineEdit.field === 'customerId') {
      // covered by renderInlineEditPanel; no inline display needed
      return '';
    }
    ...
    if (!name) {
      return (
        ...
        '<button type="button" class="ix-btn ix-btn--ghost cmp-card-link-btn" ' +
          'data-cmp-card-action="quick-edit-customer" aria-label="Assign customer">' +
          'Assign\u2026' +
        '</button>'
      );
    }
    return (
      '<span class="cmp-card-customer-lbl">Customer</span>' +
      '<span class="cmp-card-customer-val" title="' + escAttr(name) + '">' + escHtml(name) + '</span>' +
      '<button type="button" class="cmp-card-edit-btn" ' +
        'data-cmp-card-action="quick-edit-customer" aria-label="Change customer" title="Change customer">' +
        '\u270E' +  // pencil glyph U+270E
      '</button>'
    );
  }
    ...
    if (S.bulkEdit) {
      ...
      actionPanel =
        '<div class="cmp-bulk-actions">' +
          '<button type="button" class="ix-btn ix-btn--primary" ' +
            'data-cmp-bulk="create-article">Create Article from ' + n + '…</button>' +
    ...
    if (be.action === 'assign-customer') {
      ...
      return (
        '<div class="cmp-bulk-edit">' +
          ...
          '<button type="button" class="ix-btn ix-btn--ghost ix-btn--icon" ' +
            'data-cmp-bulk="cancel-bulk-edit" aria-label="Close bulk edit" ' +
            'title="Close">\u2715</button>' +
        '</div>'
      );
    }
