...
function buildArtOpts(custId, catId, selectedId) {
...
function buildTsArtOpts(catId, selectedId) {
  const wrapper = document.querySelector('.ts-picker-wrapper');
  if (!wrapper) return '<option value="" disabled selected>No articles</option>';
  const items = Array.from(wrapper.querySelectorAll('.ts-picker-item')).filter(el => {
    return !catId || el.dataset.catId === catId;
  }).map(el => ({
    id: el.dataset.articleId || '',
    name: el.dataset.artNm || ''
  })).filter(a => a.id && a.name);
...
function buildCatOpts(group, selectedId) {
  ...
  const validGroups = GROUP_MAP[group] || [group];
  const cats = [];
  prodEls.forEach(el => {
    if (validGroups.includes(el.dataset.group) && el.dataset.id) {
      cats.push({
        id: el.dataset.id,
...
function getCatType(catId) {
  if (!catId) return '';
  const el = document.querySelector(`.products-wrapper[data-id="${catId}"]`);
  return el ? el.dataset.type || '' : '';
}

function getSectionGroup(secCode) {
  const map = { 'fa': 'Feature Article', 'ts': 'Themed Spotlight', 'ba': 'Banner Ad', 'txa': 'The Find', 'lbp': 'The Find', 'gr': 'Greeting', 'em': 'Email' };
  return map[(secCode || '').toLowerCase()] || secCode || '';
}

function initFaState() {
  const slotEls = document.querySelectorAll('.pubplan-slot-wrapper[data-section-code="fa"]');
  slotEls.forEach(el => {
    const code = el.dataset.slotCode?.toUpperCase();
    if (!code) return;
    const faCatId = el.dataset.catId || '';
    state[code] = {
      sc: code, secC: 'fa',
  ...
  for (let i = 1; i <= 4; i++) {
    const code = `fa-${i}`;
    if (!state[code]) {
      state[code] = { sc: code, secC: 'fa', slotNum: i, section: 'Feature Article', catId: '', catNm: '', artId: '', artNm: '', custId: '', custNm: '', sponsorId: '', sponNm: '', artAdId: '', artAdName: '', noSponsor: false, dirty: false };
    }
  }
}
...
function renFa(sc) {
  ...
  if (existing) {
    const drawerOpen = existingDrawer ? existingDrawer.classList.contains('open') : false;
...
// resetFaCat replaced by onFaCatClear
...
window.cancelFaEdit = function(sc) {
  ...
  if (s && orig) { Object.assign(s, orig); s.dirty = false; s.catChanged = false; delete origSt[sc]; }
  renFa(sc);
  updFaProg();
};

function updFaProg() {
  updSlotInd('fa', 4, (code) => !!(state[code] && state[code].artId && !state[code].dirty));
  let pCnt = 0;
...
function initTsState() {
  ...
  for (let i = 1; i <= 4; i++) {
    const code = `ts-${i}`;
    if (!state[code]) {
      state[code] = { sc: code, secC: 'ts', slotNum: i, section: 'Themed Spotlight', catId: '', catNm: '', artId: '', artNm: '', sponsorId: '', sponNm: '', artAdId: '', artAdName: '', noSponsor: false, dirty: false };
    }
  }
}
...
function renTs(sc) {
  ...
  if (!s) return;
  const isEd = s.dirty || !!origSt[sc];
  // TS uses data-type from products-wrapper: Paid, Sponsored, or none
  const isSponsored = s.catType && s.catType.toLowerCase().includes('sponsor');
...
// resetTsCat replaced by onTsCatClear
...
function updTsProg() {
  updSlotInd('ts', 4, (code) => !!(state[code] && state[code].artId && !state[code].dirty));
  let pCnt = 0;
...
function initBaState() {
  ...
  for (let i = 1; i <= 12; i++) {
    const code = `ba-${i}`;
    if (!state[code]) {
      state[code] = { sc: code, secC: 'ba', slotNum: i, section: 'Banner Ad', custId: '', custNm: '', catId: '', adId: '', adName: '', dirty: false };
    }
  }
}
...
function renBa(sc) {
  ...
  const isEd = s.dirty || origSt[sc];
  const d = getBaPickerData(s.slotNum);
  const adThumb = !isEd && s.adId && d.adGet
...
function updBaProg() {
  updSlotInd('ba', 12, (code) => !!(state[code] && state[code].adId && !state[code].dirty));
  let pCnt = 0;
...
function initTxaState() {
  ...
  for (let i = 1; i <= 5; i++) {
    const code = `txa-${i}`;
    if (!state[code]) {
      state[code] = { sc: code, secC: 'txa', slotNum: i, section: 'The Find', pubplanId: '', titleadminId: '', custId: '', custNm: '', catId: '', dirty: false };
    }
  }
}
...
window.submitSection = async function(section) {
  ...
  try {
    // Fire one webhook per dirty slot — use no-cors since Make doesn't return CORS headers.
    // Opaque responses cannot be inspected; treat completion without network error as success.
    await Promise.all(
    ...
    // All fetches completed without throwing — Make received the payloads.
    showToast(`${section.toUpperCase()} saved! Refreshing…`);
    // Brief pause so the toast is visible, then reload to reflect updated CMS state.
    setTimeout(() => {
    ...
    // Network-level failure — Make was not reached.
