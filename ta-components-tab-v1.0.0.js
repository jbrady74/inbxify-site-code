/* ============================================================
   ta-components-tab-v1.0.0.js
   ============================================================
   INBXIFY · Components Rebuild · Slice 1 · Sub-session A
   Components surface for the Studio "Components" tab on the
   Title-Admin page. NEW FILE — replaces the cmp-* code that
   lived inline in ta-studio v1.2.x. Loaded after ta-studio.
...
(function () {
  ...
  function renderShell(root) {
    root.innerHTML =
      '<div class="cmp-root">' +
        ...
        '<div class="cmp-drawer-host" id="cmp-drawer-host"></div>' +
      '</div>';
  ...
  function bindShellEvents(root) {
    ...
    root.addEventListener('click', function (e) {
      ...
      // — Toggle "More filters"
      if (t.closest && t.closest('[data-cmp-action="toggle-secondary"]')) {
        S.secondaryOpen = !S.secondaryOpen;
        renderFilterBar();
        return;
      }
      ...
      if (t.classList && t.classList.contains('cmp-drawer-backdrop')) {
        closeDrawer();
        return;
      }
    });
  ...
  function renderFilterBar() {
    ...
    // Primary: Status, Customer, Source Channel
    var html =
      '<div class="cmp-filter-primary">' +
        ...
        '<div class="cmp-filter-spacer"></div>' +
        '<button type="button" class="cmp-filter-more" data-cmp-action="toggle-secondary">' +
          (S.secondaryOpen ? 'Hide filters' : 'More filters') +
          (countActiveSecondary() > 0
            ? ' <span class="cmp-filter-more-badge">' + countActiveSecondary() + '</span>'
            : '') +
        '</button>' +
        (filtersAreDefault()
          ? ''
          : '<button type="button" class="cmp-filter-reset" data-cmp-action="reset-filters">Reset</button>') +
      '</div>';

    if (S.secondaryOpen) {
      html +=
        '<div class="cmp-filter-secondary">' +
          ...
          filterSelect('hasCustomer', 'Has Customer',
            '<option value="All"' + sel(f.hasCustomer, 'All') + '>All</option>' +
            '<option value="has"' + sel(f.hasCustomer, 'has') + '>Has customer</option>' +
            '<option value="none"' + sel(f.hasCustomer, 'none') + '>No customer</option>',
  ...
  function countActiveSecondary() {
    var n = 0;
    if (S.filters.role          !== DEFAULT_FILTERS.role) n++;
    if (S.filters.mediaType     !== DEFAULT_FILTERS.mediaType) n++;
    if (S.filters.dateRange     !== DEFAULT_FILTERS.dateRange) n++;
    if (S.filters.pdfProvenance !== DEFAULT_FILTERS.pdfProvenance) n++;
    if (S.filters.hasCustomer   !== DEFAULT_FILTERS.hasCustomer) n++;
    return n;
  }
  ...
  function renderCard(it) {
    ...
    var thumb = it.imageUrl
      ? '<img class="cmp-card-thumb" src="' + escAttr(it.imageUrl) + '" alt="" />'
      : '<div class="cmp-card-thumb cmp-card-thumb--placeholder">' + (firstLetter(it.name) || '?') + '</div>';

    var customerName = customerNameById(it.customerId);
    var statusBadge =
      '<span class="cmp-badge cmp-badge--' + statusModifier(it.status) + '">' + escHtml(it.status || '\u2014') + '</span>';

    return (
      '<article class="' + classes.join(' ') + '" data-cmp-card="' + escAttr(it.mediaId) + '">' +
        '<div class="cmp-card-thumb-wrap">' + thumb + '</div>' +
        '<div class="cmp-card-body">' +
          '<div class="cmp-card-row cmp-card-row--head">' +
            '<div class="cmp-card-name" title="' + escAttr(it.name) + '">' + escHtml(it.name || '(unnamed)') + '</div>' +
            statusBadge +
          '</div>' +
          '<div class="cmp-card-row cmp-card-row--meta">' +
            '<span class="cmp-card-meta">' + escHtml(it.role || '\u2014') + '</span>' +
            '<span class="cmp-card-meta">' + escHtml(it.mediaType || '\u2014') + '</span>' +
            (it.sourceChannel ? '<span class="cmp-card-meta">' + escHtml(it.sourceChannel) + '</span>' : '') +
            (it.pdfProvenance ? '<span class="cmp-card-meta cmp-card-meta--pdf">PDF\u00b7' + escHtml(it.pdfProvenance) + '</span>' : '') +
          '</div>' +
          '<div class="cmp-card-row cmp-card-row--customer">' +
  ...
  function renderCustomerCell(it) {
    if (S.inlineEdit && S.inlineEdit.mediaId === it.mediaId && S.inlineEdit.field === 'customerId') {
      ...
      return '<span class="cmp-card-customer-label">Customer:</span> ' +
             '<span class="cmp-card-customer-value">(editing\u2026)</span>';
    }
    var name = customerNameById(it.customerId);
    if (!name) {
      return (
        '<span class="cmp-card-customer-label">Customer:</span> ' +
        '<button type="button" class="cmp-card-link" data-cmp-card-action="quick-edit-customer">' +
          'Assign\u2026' +
        '</button>'
      );
    }
    return (
      '<span class="cmp-card-customer-label">Customer:</span> ' +
      '<span class="cmp-card-customer-value">' + escHtml(name) + '</span> ' +
      '<button type="button" class="cmp-card-link cmp-card-link--quiet" data-cmp-card-action="quick-edit-customer">' +
        'change' +
      '</button>'
    );
  }
  ...
  function renderCardActions(it) {
    return (
      '<div class="cmp-card-actions">' +
        '<button type="button" class="ix-btn ix-btn--xs ix-btn--quiet" data-cmp-card-action="open-drawer">' +
          'Details\u2026' +
        '</button>' +
        '<button type="button" class="ix-btn ix-btn--xs ix-btn--quiet" data-cmp-card-action="quick-edit-status">' +
          'Status\u2026' +
        '</button>' +
      '</div>'
    );
    ...
    return (
      '<div class="cmp-inline-edit">' +
        ...
        '<div class="cmp-inline-actions">' +
          '<button type="button" class="ix-btn ix-btn--xs ix-btn--primary"' +
            (dirty ? '' : ' disabled') +
            ' data-cmp-card-action="save-edit">Save</button>' +
          '<button type="button" class="cmp-card-link cmp-card-link--cancel" ' +
    ...
    if (S.bulkEdit) {
      ...
      actionPanel =
        '<div class="cmp-bulk-actions">' +
          '<button type="button" class="ix-btn ix-btn--sm" data-cmp-bulk-action="assign-customer">' +
            'Assign Customer\u2026' +
          '</button>' +
          '<button type="button" class="ix-btn ix-btn--sm" data-cmp-bulk-action="clear-customer">' +
            'Clear Customer' +
          '</button>' +
        '</div>';
    }
    ...
    host.innerHTML =
      '<div class="cmp-bulk-inner">' +
        '<div class="cmp-bulk-left">' +
          ...
          '<button type="button" class="cmp-card-link" data-cmp-bulk="select-all">Select all visible</button>' +
          '<button type="button" class="cmp-card-link" data-cmp-bulk="clear-selection">Clear</button>' +
        '</div>' +
    ...
    if (be.action === 'assign-customer') {
      ...
      return (
        '<div class="cmp-bulk-edit">' +
          ...
          '</label>' +
          '<button type="button" class="ix-btn ix-btn--sm ix-btn--primary"' +
            (dirty ? '' : ' disabled') + ' data-cmp-bulk-apply>Apply to ' + selectionCount() + '</button>' +
          '<button type="button" class="cmp-card-link cmp-card-link--cancel" ' +
    ...
    if (be.action === 'clear-customer') {
      return (
        '<div class="cmp-bulk-edit">' +
          ...
          '<button type="button" class="ix-btn ix-btn--sm ix-btn--primary" data-cmp-bulk-apply>Clear</button>' +
          '<button type="button" class="cmp-card-link cmp-card-link--cancel" ' +
            'data-cmp-bulk="cancel-bulk-edit">Cancel</button>' +
        '</div>'
      );
  ...
  function renderDrawer() {
    var host = document.getElementById('cmp-drawer-host');
    if (!host) return;
    if (!S.drawerOpenId) {
      host.innerHTML = '';
      host.classList.remove('cmp-drawer-host--open');
      return;
    }
    ...
    host.classList.add('cmp-drawer-host--open');
    host.innerHTML =
      '<div class="cmp-drawer-backdrop" data-cmp-drawer-action="close"></div>' +
      '<aside class="cmp-drawer" role="dialog" aria-modal="true" aria-label="Component details">' +
        '<header class="cmp-drawer-head">' +
          '<div class="cmp-drawer-title">' + escHtml(it.name || '(unnamed)') + '</div>' +
          '<button type="button" class="cmp-drawer-close" data-cmp-drawer-action="close" aria-label="Close">\u00d7</button>' +
        '</header>' +
        '<div class="cmp-drawer-body">' +
          (it.imageUrl
            ? '<img class="cmp-drawer-thumb" src="' + escAttr(it.imageUrl) + '" alt="" />'
            : '') +
          '<dl class="cmp-drawer-dl">' +
            kv('Status',          it.status) +
          ...
          '</dl>' +
          '<div class="cmp-drawer-deferred">' +
            '<div class="cmp-drawer-deferred-icon">\u270E</div>' +
            '<div class="cmp-drawer-deferred-text">Structural edits (Article ref, Component Role) wire up in Sub-session C.</div>' +
          '</div>' +
        '</div>' +
      '</aside>';
  }
  ...
  function kv(label, val) {
    return (
      '<dt class="cmp-drawer-key">' + escHtml(label) + '</dt>' +
      '<dd class="cmp-drawer-val">' + escHtml(val || '\u2014') + '</dd>'
    );
  }
