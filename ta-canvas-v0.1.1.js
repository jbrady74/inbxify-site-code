/* ════════════════════════════════════════════════════════════════
   ...
   Companion: ta-canvas-v0.1.1.css (matched pair — load both)

   v0.1.1 — color-pass companion (CSS carries the visual change):
   • Stage-3 slot rows emit data-sec="<sectionCode>" so the CSS
     can render section-colored slot pills (fa/ts/ba/tf/ev/re/sb).
   • No behavior changes. Complete replacement of v0.1.0 — remove
     the old <script> tag.
  ...
  function renderStage3() {
    ...
    body.innerHTML = slots.length
      ? slots.map(function (sl) {
          ...
          return '' +
            '<div class="ixcv-slot" data-sec="' + esc(sl.sectionCode) + '">' +
