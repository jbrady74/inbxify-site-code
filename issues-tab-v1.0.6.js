...
(function () {
  ...
  function renderModalHtml(context, nextNumber) {
    ...
    if (hasShort) {
      ...
      issueRow =
        '<div class="ppm-field">' +
          '<label class="ppm-label" for="ppm-number">Issue</label>' +
          '<div class="ppm-issue-row">' +
            ...
            '<input type="text" id="ppm-number" class="ppm-input ppm-number-input" ' +
              'data-pp-add-number value="' + esc(String(defaultNumber)) + '" autocomplete="off">' +
          '</div>' +
  ...
  function handleAddPubplanSubmit(context, backdrop) {
    ...
    // v1.0.5: issue identifier is free text (e.g. "114", "114B",
    // "Holiday"). No integer coercion — the raw trimmed string is
    // used verbatim to build the issue name.
    var issueIdRaw = numberInput ? (numberInput.value || '').trim() : '';
    ...
    var issueName = abbreviation + '-' + issueIdRaw;
