...
(function () {
    ...
    return '' +
      '<div class="asf-topbar">' +
        ...
        '<div class="asf-topbar-right">' +
          ...
          '</div>' +
          '<button class="asf-tb-close" type="button" aria-label="Close ASF" data-asf-action="close-overlay">Close</button>' +
        '</div>' +
      '</div>';
  }
  ...
  function handleAction(action, el, e) {
    switch (action) {
      ...
      // Topbar / nav
      case 'nav-studio':
        // Soft handoff — host page decides what to do. Toast for now;
        // Chunk D may route through a Studio public API.
        toast('Studio handoff not yet wired — closing ASF', 'info');
        publicClose();
        return;
      case 'nav-assembler':
        toast('Assembler handoff not yet wired — closing ASF', 'info');
        publicClose();
        return;
  ...
  log('mounted · v' + VERSION + ' · CMS Refs section: 4 fields visually unlocked + labels renamed (Major NL Section, Product Library); 2 system fields stay locked. Edit picker ships in Chunk D.');
})();
