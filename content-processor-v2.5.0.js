...
// v2.5.0: TWO-LEVEL TAB STRUCTURE
...
<div class="cm-root">
  <div class="cm-hdr">
    ...
    <span class="cm-badge">v2.5.0</span>
  </div>
  ...
  function render() {
    ...
    transEl.style.display  = CHANNEL === 'transcriber' ? '' : 'none';
  ...
  function renderTranscriberShell() {
    ...
    // If the transcriber has already mounted, just update the notice
    if (el.dataset.mounted === 'true') {
      const existingNotice = el.querySelector('.cm-transcribe-notice');
      if (existingNotice) existingNotice.remove();
      if (notice) el.insertAdjacentHTML('afterbegin', notice);
      return;
    }
    el.innerHTML = `${notice}<div class="cm-transcriber-shell" id="cm-transcriber-mount"></div>`;
  ...
  loadFiles();
  console.log('\u{1F4C2} Content Processor v2.5.0 mounted');
});
