/* ============================================================
   ...
   v1.0.13 — z-index fix. The active Webflow tab pill (.w--current)
             was poking through the modal overlay because active tabs
             create their own stacking context. Overlay z-index raised
             to near-max, and a body.cm-modal-open flag now drops the
             active tab + tab menu z-index while any modal is open.
             No change to scrape or fields.
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ── Render ──
  function render() {
    // v1.0.13: flag body while a modal is open so CSS can drop the
    // active Webflow tab's z-index below the overlay (the active tab
    // pill was poking through the modal). Cleans up automatically
    // because render() also runs on modal close.
    try { document.body.classList.toggle('cm-modal-open', !!MODAL); } catch (e) {}
      ...
      g.fields.forEach(function (f) {
        ...
        if (isEditing) {
          ...
          if (f.t === 'upload') {
            ...
            h += '<select class="cm-esel" onchange="cmSetVal(\'' + esc(f.s) + '\',this.value)"' + (!parentVal ? ' disabled' : '') + '>';
            if (!parentVal) {
  ...
  window.cmScrapeClient = function () {
    ...
    var input = mount.querySelector('#cm-scrape-url');
  ...
  window.cmToggleEdit = function (slug) {
    ...
    var scrollEl = mount.querySelector('.cm-mb');
    var scrollTop = scrollEl ? scrollEl.scrollTop : 0;
    renderModal();
    var newScrollEl = mount.querySelector('.cm-mb');
  ...
  window.cmSetVal = function (slug, val, noRender) {
    ...
    if (noRender) {
      ...
      var gi = mount.querySelector('[data-cm-field="' + slug + '"]');
      if (gi) {
      ...
      var dcEl = mount.querySelector('.cm-mf-dc');
      if (dcEl) dcEl.textContent = dc > 0 ? dc + ' unsaved change' + (dc > 1 ? 's' : '') : '';
      var svBtn = mount.querySelector('.cm-mf-sv');
      var pubBtn = mount.querySelector('.cm-mf-pub');
  ...
  function getCss() {
    return [
      ...
      // v1.0.13: while the modal is open, knock the active Webflow tab
      // and tab menu out of the way so neither can poke through the
      // overlay (active tabs create their own stacking context).
      'body.cm-modal-open .w-tab-link.w--current,body.cm-modal-open .w--current{z-index:0 !important}',
      'body.cm-modal-open .w-tab-menu,body.cm-modal-open .dashboard-tabs,body.cm-modal-open .tabs-content-9{z-index:auto !important}',
