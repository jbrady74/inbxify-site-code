...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var ATTR_MAP = {
    ...
    'self-this-item-id': 'selfId'
  };

  // Publisher and T-A names from page config (can't bind ref names as data-*)
  var PUBLISHER_NAME = window.TA_CONFIG?.publisherName || 'Bergen Neighbors Media';
  var TA_NAME = window.TA_CONFIG?.titleName || 'Wyckoff Living Now';
  ...
  document.querySelectorAll('.customers-wrapper').forEach(function (el) {
    ...
    ALL_FIELDS.forEach(function (f) {
      var key = f.s;
      // Publisher and T-A are page-level, not per-customer attributes
      if (key === 'publisher') { rec[key] = PUBLISHER_NAME; return; }
  ...
  function renderModal() {
    ...
    GROUPS.forEach(function (g) {
      ...
      g.fields.forEach(function (f) {
        ...
        // Readonly fields — no pencil, no editing
        if (f.t === 'readonly') {
          h += '<div class="cm-gi">';
  ...
  window.cmSetVal = function (slug, val) {
    MODAL.edit[slug] = val;
    // Cascade: when major changes, clear sub and auto-open sub editor
