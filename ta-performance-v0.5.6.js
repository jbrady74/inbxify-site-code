...
(function () {
  ...
  function currentWTab() {
    var el = document.querySelector('.pl-link-tabs.w-tab-link.w--current');
  ...
  function restoreWTab() {
    ...
    if (currentWTab() === key) return;          // already there
    var link = document.querySelector('.pl-link-tabs.w-tab-link[data-w-tab="' + key + '"]');
    if (!link) return;
    // Webflow binds tab handlers on its own init; defer past it.
  ...
  function wire(root) {
    root.addEventListener('click', function (e) {
      ...
      if (rf) {
        ...
        try { sessionStorage.setItem(TAB_KEY, currentWTab()); } catch (e) { /* non-fatal */ }
        location.reload();
        return;
      }
